...
const VALID_HIERARCHY_TYPES: &[&str] = &[
    "account",
    "cost_center",
    "entity",
    "product",
...
pub trait AccountHierarchyRepository: Send + Sync {
    ...
    async fn get_hierarchy_by_code(
        &self,
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<AccountHierarchy>>;
    async fn list_hierarchies(
        &self,
        org_id: Uuid,
        hierarchy_type: Option<&str>,
        is_active: Option<bool>,
    ) -> AtlasResult<Vec<AccountHierarchy>>;
    async fn update_hierarchy_status(
        &self,
        id: Uuid,
        status: &str,
        is_active: bool,
...
/// `PostgreSQL` stub
#[allow(dead_code)]
pub struct PostgresAccountHierarchyRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
impl PostgresAccountHierarchyRepository {
    #[must_use]
...
impl AccountHierarchyRepository for PostgresAccountHierarchyRepository {
    ...
    async fn get_hierarchy(&self, _: Uuid) -> AtlasResult<Option<AccountHierarchy>> {
        Ok(None)
    }
    async fn get_hierarchy_by_code(
        &self,
    ...
    async fn increment_version(&self, _: Uuid) -> AtlasResult<AccountHierarchy> {
        Err(AtlasError::EntityNotFound("Not found".into()))
    }
    async fn add_node(
        &self,
    ...
    async fn get_node(&self, _: Uuid) -> AtlasResult<Option<HierarchyNode>> {
        Ok(None)
    }
    async fn list_nodes(&self, _: Uuid) -> AtlasResult<Vec<HierarchyNode>> {
        Ok(vec![])
    }
    async fn list_children(&self, _: Uuid) -> AtlasResult<Vec<HierarchyNode>> {
        Ok(vec![])
    }
    async fn update_node(
        &self,
    ...
    async fn remove_node(&self, _: Uuid) -> AtlasResult<()> {
        Ok(())
    }
    async fn get_ancestors(&self, _: Uuid) -> AtlasResult<Vec<HierarchyNode>> {
        Ok(vec![])
    }
    async fn get_descendants(&self, _: Uuid) -> AtlasResult<Vec<HierarchyNode>> {
        Ok(vec![])
    }
    async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<HierarchyDashboard> {
...
impl AccountHierarchyEngine {
    pub fn new(repository: Arc<dyn AccountHierarchyRepository>) -> Self {
        Self { repository }
    }
    ...
    pub async fn create_hierarchy(
        ...
        if !VALID_HIERARCHY_TYPES.contains(&hierarchy_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid hierarchy type '{}'. Must be one of: {}",
                hierarchy_type,
                VALID_HIERARCHY_TYPES.join(", ")
            )));
        }
        if let Some(to) = effective_to {
            if let Some(from) = effective_from {
                if to < from {
                    return Err(AtlasError::ValidationFailed(
                        "Effective to must be after effective from".into(),
                    ));
                }
            }
        }
        if self
            .repository
            .get_hierarchy_by_code(org_id, code)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Hierarchy '{code}' already exists"
            )));
        }
    ...
    pub async fn get_hierarchy(&self, id: Uuid) -> AtlasResult<Option<AccountHierarchy>> {
        self.repository.get_hierarchy(id).await
    }
    pub async fn get_hierarchy_by_code(
        &self,
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<AccountHierarchy>> {
        self.repository.get_hierarchy_by_code(org_id, code).await
    }

    pub async fn list_hierarchies(
        &self,
        org_id: Uuid,
        hierarchy_type: Option<&str>,
        is_active: Option<bool>,
        ...
        if let Some(ht) = hierarchy_type {
            if !VALID_HIERARCHY_TYPES.contains(&ht) {
                return Err(AtlasError::ValidationFailed(format!(
                    "Invalid hierarchy type '{ht}'"
                )));
            }
        }
        self.repository
            .list_hierarchies(org_id, hierarchy_type, is_active)
            .await
    }
    ...
    pub async fn activate(&self, id: Uuid) -> AtlasResult<AccountHierarchy> {
        let h = self
            ...
            .get_hierarchy(id)
            .await?
            .ok_or_else(|| AtlasError::EntityNotFound(format!("Hierarchy {id} not found")))?;
        if h.is_active {
            return Err(AtlasError::ValidationFailed("Already active".into()));
        }
        info!("Activating hierarchy {}", h.code);
        self.repository
            .update_hierarchy_status(id, "active", true)
            .await
    }
    ...
    pub async fn deactivate(&self, id: Uuid) -> AtlasResult<AccountHierarchy> {
        ...
        self.repository
            .update_hierarchy_status(id, "inactive", false)
            .await
    }
    ...
    pub async fn add_node(
        ...
        let h = self
            .repository
            .get_hierarchy(hierarchy_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Hierarchy {hierarchy_id} not found"))
            })?;
        if !h.is_active {
            return Err(AtlasError::ValidationFailed(
                "Cannot add nodes to inactive hierarchy".into(),
            ));
        }
        if account_code.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Account code is required".into(),
            ));
        }
        ...
        let level_depth = if let Some(pid) = parent_id {
            let parent = self.repository.get_node(pid).await?.ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Parent node {pid} not found"))
            })?;
            if parent.hierarchy_id != hierarchy_id {
                return Err(AtlasError::ValidationFailed(
                    "Parent must belong to same hierarchy".into(),
                ));
            }
        ...
        if parent_id.is_none() {
            ...
            if existing.iter().any(|n| n.parent_id.is_none()) {
                return Err(AtlasError::ValidationFailed(
                    "Hierarchy already has a root node".into(),
                ));
            }
        }
    ...
    pub async fn get_node(&self, id: Uuid) -> AtlasResult<Option<HierarchyNode>> {
        self.repository.get_node(id).await
    }
    pub async fn list_nodes(&self, hierarchy_id: Uuid) -> AtlasResult<Vec<HierarchyNode>> {
        self.repository.list_nodes(hierarchy_id).await
    }
    pub async fn list_children(&self, parent_id: Uuid) -> AtlasResult<Vec<HierarchyNode>> {
        self.repository.list_children(parent_id).await
    }
    ...
    pub async fn update_node(
        ...
        self.repository
            .update_node(id, account_name, display_order, is_enabled)
            .await
    }
    ...
    pub async fn move_node(
        ...
        if let Some(pid) = new_parent_id {
            ...
            if parent.hierarchy_id != node.hierarchy_id {
                return Err(AtlasError::ValidationFailed(
                    "Cannot move across hierarchies".into(),
                ));
            }
            ...
            if descendants.iter().any(|d| d.id == pid) {
                return Err(AtlasError::ValidationFailed(
                    "Cannot move node under its own descendant".into(),
                ));
            }
        ...
        self.repository
            .move_node(id, new_parent_id, new_display_order, new_depth)
            .await
    }
    ...
    pub async fn remove_node(&self, id: Uuid) -> AtlasResult<()> {
        ...
        if !children.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Cannot remove node with children. Remove children first".into(),
            ));
        }
...
mod tests {
    ...
    impl MockRepo {
        fn new() -> Self {
            Self {
                hierarchies: std::sync::Mutex::new(vec![]),
    ...
    impl AccountHierarchyRepository for MockRepo {
        async fn create_hierarchy(
            ...
            description: Option<&str>,
            hierarchy_type: &str,
            eff_from: Option<chrono::NaiveDate>,
            ...
            let h = AccountHierarchy {
                ...
                hierarchy_type: hierarchy_type.into(),
                version: 1,
                status: "draft".into(),
                is_active: true,
        ...
        async fn get_hierarchy_by_code(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|h| h.organization_id == org_id && h.code == code)
                .cloned())
        }
        async fn list_hierarchies(
            &self,
            ...
            Ok(self
                ...
                .filter(|h| {
                    h.organization_id == org_id
                        && (hierarchy_type.is_none() || h.hierarchy_type == hierarchy_type.unwrap())
                        && (is_active.is_none() || h.is_active == is_active.unwrap())
                })
                .cloned()
                .collect())
        }
        ...
        async fn list_nodes(&self, hierarchy_id: Uuid) -> AtlasResult<Vec<HierarchyNode>> {
            Ok(self
                ...
                .unwrap()
                .iter()
                .filter(|n| n.hierarchy_id == hierarchy_id)
                .cloned()
                .collect())
        }
        async fn list_children(&self, parent_id: Uuid) -> AtlasResult<Vec<HierarchyNode>> {
        ...
        async fn move_node(
            ...
            n.display_order = new_display_order;
            n.level_depth = new_depth;
            n.updated_at = Utc::now();
            Ok(n.clone())
        }
        async fn remove_node(&self, id: Uuid) -> AtlasResult<()> {
        ...
        async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<HierarchyDashboard> {
            ...
            let org_h: Vec<_> = hierarchies
                .iter()
                .filter(|h| h.organization_id == org_id)
                .collect();
            let org_n = nodes
                .iter()
                .filter(|n| org_h.iter().any(|h| h.id == n.hierarchy_id))
                .count() as i32;
            Ok(HierarchyDashboard {
                organization_id: org_id,
                total_hierarchies: org_h.len() as i32,
    ...
    fn eng() -> AccountHierarchyEngine {
        AccountHierarchyEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_create_hierarchy_duplicate() {
        ...
        let org = Uuid::new_v4();
        let _ = e
            .create_hierarchy(org, "COA", "Chart", None, "account", None, None, None)
            .await
            .unwrap();
        assert!(e
            .create_hierarchy(org, "COA", "Chart 2", None, "account", None, None, None)
            .await
            .is_err());
    }
    ...
    async fn test_add_root_node() {
        ...
        let h = e
            ...
            .unwrap();
        let root = e
            .add_node(h.id, org, None, "0000", Some("All Accounts"), "root", 1)
            .await
            .unwrap();
    ...
    async fn test_add_child_node() {
        ...
        let h = e
            ...
            .unwrap();
        let root = e
            .add_node(h.id, org, None, "0000", Some("All"), "root", 1)
            .await
            .unwrap();
        let child = e
            .add_node(
    ...
    async fn test_add_node_empty_code() {
        ...
        let h = e
            ...
            .unwrap();
        assert!(e
            .add_node(h.id, org, None, "", None, "root", 1)
            .await
            .is_err());
    }
    ...
    async fn test_add_node_invalid_type() {
        ...
        let h = e
            ...
            .unwrap();
        assert!(e
            .add_node(h.id, org, None, "0000", None, "invalid", 1)
            .await
            .is_err());
    }
    ...
    async fn test_add_node_inactive_hierarchy() {
        ...
        let _ = e.deactivate(h.id).await.unwrap();
        assert!(e
            .add_node(h.id, org, None, "0000", None, "root", 1)
            .await
            .is_err());
    }
    ...
    async fn test_add_node_duplicate_root() {
        ...
        let _ = e
            ...
            .unwrap();
        assert!(e
            .add_node(h.id, org, None, "0001", None, "root", 2)
            .await
            .is_err());
    }
    ...
    async fn test_remove_node_with_children() {
        ...
        let root = e
            ...
            .unwrap();
        let _ = e
            .add_node(h.id, org, Some(root.id), "1000", None, "summary", 1)
            .await
            .unwrap();
    ...
    async fn test_update_node() {
        ...
        let h = e
            ...
            .unwrap();
        let root = e
            .add_node(h.id, org, None, "0000", Some("Old"), "root", 1)
            .await
            .unwrap();
        let updated = e
            .update_node(root.id, Some("All Accounts"), Some(2), None)
            .await
            .unwrap();
    ...
    async fn test_move_node_circular() {
        ...
        let child = e
            ...
            .unwrap();
        let grandchild = e
            .add_node(h.id, org, Some(child.id), "1100", None, "detail", 1)
            .await
            .unwrap();
    ...
    async fn test_get_ancestors() {
        ...
        let child = e
            ...
            .unwrap();
        let grandchild = e
            .add_node(h.id, org, Some(child.id), "1100", Some("Cash"), "detail", 1)
            .await
            .unwrap();
    ...
    async fn test_list_hierarchies_invalid_type() {
        assert!(eng()
            .list_hierarchies(Uuid::new_v4(), Some("invalid"), None)
            .await
            .is_err());
    }
