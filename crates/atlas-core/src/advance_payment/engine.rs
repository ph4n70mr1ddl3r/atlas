...
use super::{
    AdvanceApplication, AdvancePayment, AdvancePaymentDashboard, AdvancePaymentRepository,
...
#[allow(dead_code)]
const VALID_PAYMENT_METHODS: &[&str] = &["check", "electronic", "wire", "ach", "manual"];
...
impl AdvancePaymentEngine {
    ...
    pub async fn create_advance(
        ...
        if advance_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Advance number is required".into(),
            ));
        }
        ...
        if let Some(exp) = expiration_date {
            if exp < advance_date {
                return Err(AtlasError::ValidationFailed(
                    "Expiration date must be after advance date".into(),
                ));
            }
        }
        if let Some(due) = due_date {
            if due < advance_date {
                return Err(AtlasError::ValidationFailed(
                    "Due date must be on or after advance date".into(),
                ));
            }
        }
        if self
            .repository
            .get_advance_by_number(org_id, advance_number)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Advance number '{advance_number}' already exists"
            )));
        }

        info!(
            "Creating advance payment {} for supplier {} ({})",
            advance_number, supplier_name, supplier_id
        );
    ...
    pub async fn get_advance_by_number(
        &self,
        org_id: Uuid,
        number: &str,
    ...
    pub async fn list_advances(
        ...
        self.repository
            .list_advances(org_id, status, supplier_id)
            .await
    }
    ...
    pub async fn approve_advance(
        ...
        self.repository
            .update_advance_status(advance_id, "approved")
            .await
    }
    ...
    pub async fn pay_advance(
        ...
        if adv.status != "approved" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot pay advance in '{}' status. Must be 'approved'.",
                adv.status
            )));
        }
        ...
        let _updated = self
            .repository
            .update_payment_info(advance_id, payment_reference, paid_by)
            .await?;
        self.repository
            .update_advance_status(advance_id, "paid")
            .await
    }
    ...
    pub async fn cancel_advance(
        ...
        if adv.status != "draft" && adv.status != "approved" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot cancel advance in '{}' status. Only 'draft' or 'approved' allowed.",
                adv.status
            )));
        }
        ...
        self.repository
            .update_advance_status(advance_id, "cancelled")
            .await
    }
    ...
    pub async fn apply_to_invoice(
        ...
        if adv.status != "paid" && adv.status != "partially_applied" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot apply advance in '{}' status. Must be 'paid' or 'partially_applied'.",
                adv.status
            )));
        }
        ...
        info!(
            "Applying advance {} to invoice (amount: {})",
            adv.advance_number, applied_amount
        );
        ...
        let applications = self
            .repository
            .list_applications_by_advance(advance_id)
            .await?;
        let active_apps: Vec<&AdvanceApplication> = applications
            .iter()
            .filter(|a| a.status == "applied")
            .collect();
        let total_applied: f64 = active_apps
            .iter()
    ...
    pub async fn unapply_application(
        ...
        info!(
            "Unapplying advance application for advance {}",
            app.advance_number.as_deref().unwrap_or("?")
        );
        ...
        let applications = self
            .repository
            .list_applications_by_advance(app.advance_id)
            .await?;
    ...
    pub async fn list_applications_by_advance(
        &self,
        advance_id: Uuid,
...
mod tests {
    ...
    impl AdvancePaymentRepository for MockRepo {
        async fn create_advance(
            ...
            let a = AdvancePayment {
                ...
                advance_number: an.into(),
                supplier_id: sid,
                supplier_name: sn.into(),
                supplier_site_id: ssi,
                description: desc.map(Into::into),
                status: "draft".into(),
                currency_code: cc.into(),
                advance_amount: amt.into(),
                applied_amount: "0".into(),
                unapplied_amount: amt.into(),
                exchange_rate: er.map(Into::into),
                payment_method: pm.map(Into::into),
                payment_reference: None,
                prepayment_account_code: pac.map(Into::into),
                liability_account_code: lac.map(Into::into),
                advance_date: ad,
                payment_date: None,
                due_date: dd,
                expiration_date: ed,
        ...
        async fn get_advance_by_number(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|a| a.organization_id == org_id && a.advance_number == num)
                .cloned())
        }
        ...
        async fn update_advance_status(
            &self,
            id: Uuid,
            status: &str,
        ) -> AtlasResult<AdvancePayment> {
        ...
        async fn create_application(
            ...
            let app = AdvanceApplication {
                ...
                organization_id: org_id,
                advance_id: aid,
                advance_number: an.map(Into::into),
                invoice_id: iid,
        ...
        async fn list_applications_by_advance(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .filter(|a| a.advance_id == advance_id)
                .cloned()
                .collect())
        }
    ...
    async fn test_cancel_draft_valid() {
        ...
        let a = e
            ...
            .unwrap();
        let cancelled = e
            .cancel_advance(a.id, Some("No longer needed"))
            .await
            .unwrap();
    ...
    async fn test_list_advances_valid_status() {
        let r = eng()
            .list_advances(Uuid::new_v4(), Some("draft"), None)
            .await;
