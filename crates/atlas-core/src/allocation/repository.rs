...
use atlas_shared::{
    ...
    GlAllocationDashboardSummary, GlAllocationPool, GlAllocationRule, GlAllocationRun,
    GlAllocationRunLine, GlAllocationTargetLine,
};
use sqlx::PgPool;
use sqlx::Row;
use uuid::Uuid;
...
pub trait AllocationRepository: Send + Sync {
    ...
    async fn get_basis_detail_by_id(
        &self,
        id: Uuid,
    ) -> AtlasResult<Option<GlAllocationBasisDetail>>;
    async fn list_basis_details(
        &self,
        basis_id: Uuid,
        period_name: Option<&str>,
    ) -> AtlasResult<Vec<GlAllocationBasisDetail>>;
    async fn update_basis_detail_amount(
        &self,
        id: Uuid,
        basis_amount: &str,
    ) -> AtlasResult<GlAllocationBasisDetail>;
    ...
    async fn get_dashboard_summary(
        &self,
        org_id: Uuid,
    ) -> AtlasResult<GlAllocationDashboardSummary>;
}
...
impl AllocationRepository for PostgresAllocationRepository {
    ...
    async fn create_pool(
        ...
        .bind(source_account_range_from)
        .bind(source_account_range_to)
    ...
    async fn get_pool_by_code(
        ...
        let row = sqlx::query(
            "SELECT * FROM _atlas.gl_allocation_pools WHERE organization_id = $1 AND code = $2",
        )
        .bind(org_id)
        .bind(code)
        .fetch_optional(&self.pool)
    ...
    async fn delete_pool(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        sqlx::query(
            "DELETE FROM _atlas.gl_allocation_pools WHERE organization_id = $1 AND code = $2",
        )
        .bind(org_id)
        .bind(code)
        .execute(&self.pool)
    ...
    async fn get_basis_by_code(
        ...
        let row = sqlx::query(
            "SELECT * FROM _atlas.gl_allocation_bases WHERE organization_id = $1 AND code = $2",
        )
        .bind(org_id)
        .bind(code)
        .fetch_optional(&self.pool)
    ...
    async fn delete_basis(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        sqlx::query(
            "DELETE FROM _atlas.gl_allocation_bases WHERE organization_id = $1 AND code = $2",
        )
        .bind(org_id)
        .bind(code)
        .execute(&self.pool)
    ...
    async fn create_rule(
        ...
        .bind(journal_batch_prefix)
        .bind(round_to_largest)
    ...
    async fn get_rule_by_code(
        ...
        let row = sqlx::query(
            "SELECT * FROM _atlas.gl_allocation_rules WHERE organization_id = $1 AND code = $2",
        )
        .bind(org_id)
        .bind(code)
        .fetch_optional(&self.pool)
    ...
    async fn delete_rule(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        ...
        sqlx::query(
            "DELETE FROM _atlas.gl_allocation_rules WHERE organization_id = $1 AND code = $2",
        )
        .bind(org_id)
        .bind(code)
        .execute(&self.pool)
    ...
    async fn create_target_line(
        ...
        .bind(target_department_name)
        .bind(target_cost_center)
        .bind(target_project_id)
        .bind(target_project_name)
        .bind(target_account_code)
        .bind(target_account_name)
        .bind(fixed_percentage)
        .bind(is_active)
        .fetch_one(&self.pool)
        .await
        .map_err(|e| AtlasError::DatabaseError(e.to_string()))?;
    ...
    async fn get_dashboard_summary(
        ...
        let total_allocated: serde_json::Value = run_row
            .try_get("total_allocated")
