...
use super::{
    ArAgingBucket, ArAgingDashboard, ArAgingDefinition, ArAgingRepository, ArAgingSnapshot,
    ArAgingSnapshotLine, ArAgingSummary, AtlasError, AtlasResult,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
pub struct ArAgingEngine {
    repository: Arc<dyn ArAgingRepository>,
}

impl ArAgingEngine {
    pub fn new(r: Arc<dyn ArAgingRepository>) -> Self {
        Self { repository: r }
    }
    ...
    pub async fn create_definition(
        ...
        if !VALID_AGING_BASES.contains(&aging_basis) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid aging_basis '{aging_basis}'"
            )));
        }
    ...
    pub async fn list_definitions(
        ...
        org_id: Uuid,
        status: Option<&str>,
    ) -> AtlasResult<Vec<ArAgingDefinition>> {
    ...
    pub async fn create_bucket(
        ...
        to_days: Option<i32>,
        display_order: i32,
    ) -> AtlasResult<ArAgingBucket> {
        self.repository
            .get_definition_by_id(def_id)
            .await?
            .ok_or_else(|| AtlasError::EntityNotFound(format!("Definition {def_id} not found")))?;
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
    ...
    pub fn determine_bucket<'a>(
        &self,
        days_past_due: i32,
        buckets: &'a [ArAgingBucket],
    ) -> Option<&'a ArAgingBucket> {
        buckets.iter().find(|b| {
    ...
    pub async fn create_snapshot(
        ...
        info!(
            "Creating AR aging snapshot for definition {} as of {}",
            def.definition_code, as_of_date
        );
        self.repository
            .create_snapshot(org_id, def_id, as_of_date, currency_code, created_by)
            .await
    }
    ...
    pub async fn list_snapshots(
        ...
        org_id: Uuid,
        def_id: Option<Uuid>,
    ) -> AtlasResult<Vec<ArAgingSnapshot>> {
    ...
    pub async fn add_snapshot_line(
        ...
        bucket_name: &str,
        currency_code: &str,
    ) -> AtlasResult<ArAgingSnapshotLine> {
        if customer_number.is_empty() || invoice_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Customer number and invoice number are required".into(),
            ));
        }
    ...
    pub async fn list_snapshot_lines(
        &self,
        snapshot_id: Uuid,
    ) -> AtlasResult<Vec<ArAgingSnapshotLine>> {
...
mod tests {
    ...
    impl ArAgingRepository for MockRepo {
        ...
        async fn get_definition(
            ...
            org_id: Uuid,
            code: &str,
        ) -> AtlasResult<Option<ArAgingDefinition>> {
            Ok(self
                .defs
                .lock()
                .unwrap()
        ...
        async fn delete_definition(&self, _id: Uuid) -> AtlasResult<()> {
            Ok(())
        }
        ...
        async fn create_snapshot_line(
            ...
            let l = ArAgingSnapshotLine {
                id: Uuid::new_v4(),
                organization_id: org_id,
                snapshot_id: sid,
                customer_id: cid,
                customer_number: cn.into(),
        ...
        async fn list_snapshot_lines(&self, sid: Uuid) -> AtlasResult<Vec<ArAgingSnapshotLine>> {
            Ok(self
                .lines
                .lock()
                .unwrap()
        ...
        async fn get_aging_summary(&self, _: Uuid) -> AtlasResult<Vec<ArAgingSummary>> {
            Ok(vec![])
        }
    ...
    fn eng() -> ArAgingEngine {
        ArAgingEngine::new(Arc::new(MockRepo::new()))
    }
