...
const VALID_AMORTIZATION_ADJUSTMENTS: &[&str] = &["catchup", "prospectively", "restatement"];
...
pub trait AssetReclassificationRepository: Send + Sync {
    async fn create(
        ...
        from_depr_expense_account_code: Option<&str>,
        ...
        to_depr_expense_account_code: Option<&str>,
    ...
    async fn get_by_number(
        ...
        org_id: Uuid,
        number: &str,
    ) -> AtlasResult<Option<AssetReclassification>>;
    async fn list(
        &self,
        org_id: Uuid,
        status: Option<&str>,
        asset_id: Option<Uuid>,
    ) -> AtlasResult<Vec<AssetReclassification>>;
...
#[allow(dead_code)]
pub struct PostgresAssetReclassificationRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
impl PostgresAssetReclassificationRepository {
    #[must_use]
...
impl AssetReclassificationRepository for PostgresAssetReclassificationRepository {
    ...
    async fn get(&self, _: Uuid) -> AtlasResult<Option<AssetReclassification>> {
        Ok(None)
    }
    async fn get_by_number(&self, _: Uuid, _: &str) -> AtlasResult<Option<AssetReclassification>> {
        Ok(None)
    }
    async fn list(
        &self,
    ...
    async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<ReclassificationDashboard> {
        Ok(ReclassificationDashboard {
            ...
            pending_reclassifications: 0,
            approved_reclassifications: 0,
            completed_reclassifications: 0,
            rejected_reclassifications: 0,
...
impl AssetReclassificationEngine {
    ...
    pub async fn create(
        ...
        if let Some(adj) = amortization_adjustment {
            if !VALID_AMORTIZATION_ADJUSTMENTS.contains(&adj) {
                return Err(AtlasError::ValidationFailed(format!(
                    ...
                    adj,
                    VALID_AMORTIZATION_ADJUSTMENTS.join(", ")
                )));
            }
        }
        ...
        info!(
            "Creating asset reclassification {} for asset {}",
            number, asset_id
        );
        ...
        self.repository
            .create(
                ...
                from_category_code,
                from_asset_type,
                from_depreciation_method,
                from_useful_life_months,
                None,
                None, // from depr expense account
                None,
                to_category_code,
                to_asset_type,
                to_depreciation_method,
                to_useful_life_months,
                to_asset_account_code,
                None, // to depr expense account
                effective_date,
    ...
    pub async fn approve(&self, id: Uuid, approved_by: Uuid) -> AtlasResult<AssetReclassification> {
        let rc = self.repository.get(id).await?.ok_or_else(|| {
            AtlasError::EntityNotFound(format!("Reclassification {id} not found"))
        })?;

        if rc.status != "pending" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot approve reclassification in '{}' status. Must be 'pending'.",
                rc.status
            )));
        }
    ...
    pub async fn complete(&self, id: Uuid) -> AtlasResult<AssetReclassification> {
        ...
        if rc.status != "approved" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot complete reclassification in '{}' status. Must be 'approved'.",
                rc.status
            )));
        }
    ...
    pub async fn reject(&self, id: Uuid) -> AtlasResult<AssetReclassification> {
        ...
        if rc.status != "pending" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot reject reclassification in '{}' status. Must be 'pending'.",
                rc.status
            )));
        }
    ...
    pub async fn cancel(&self, id: Uuid) -> AtlasResult<AssetReclassification> {
        ...
        if rc.status != "pending" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot cancel reclassification in '{}' status. Must be 'pending'.",
                rc.status
            )));
        }
...
mod tests {
    ...
    impl AssetReclassificationRepository for MockRepo {
        ...
        async fn get_by_number(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|r| r.organization_id == org_id && r.reclassification_number == number)
                .cloned())
        }
        async fn list(
            &self,
        ...
        async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<ReclassificationDashboard> {
            ...
            Ok(ReclassificationDashboard {
                ...
                pending_reclassifications: org_items
                    .iter()
                    .filter(|r| r.status == "pending")
                    .count() as i32,
                approved_reclassifications: org_items
                    .iter()
                    .filter(|r| r.status == "approved")
                    .count() as i32,
                completed_reclassifications: org_items
                    .iter()
                    .filter(|r| r.status == "completed")
                    .count() as i32,
                rejected_reclassifications: org_items
                    .iter()
    ...
    fn eng() -> AssetReclassificationEngine {
        AssetReclassificationEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_create_valid() {
        let rc = eng()
            .create(
                ...
                Some("Laptop"),
                "category_change",
                Some("Reclassified to IT equipment"),
                Some("GEN_EQUIP"),
                None,
                None,
                None,
    ...
    async fn test_list_valid() {
        assert!(eng()
            .list(Uuid::new_v4(), Some("pending"), None)
            .await
            .unwrap()
            .is_empty());
    }
