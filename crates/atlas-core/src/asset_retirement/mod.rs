...
pub trait AssetRetirementRepository: Send + Sync {
    ...
    async fn get_by_number(
        ...
        org_id: Uuid,
        number: &str,
    ) -> AtlasResult<Option<AssetRetirement>>;
    async fn list(
        &self,
        org_id: Uuid,
        status: Option<&str>,
        retirement_type: Option<&str>,
...
impl AssetRetirementRepository for PostgresAssetRetirementRepository {
    async fn create(
        ...
        .bind(gain_loss_account)
        .bind(asset_account)
        .bind(depreciation_account)
        .bind(proceeds_account)
        .bind(removal_cost_account)
        .bind(buyer_name)
        .bind(reason)
        .bind(status)
        .bind(created_by)
    ...
    async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<RetirementDashboard> {
        ...
        let total_proceeds: String = row
            .try_get("total_proceeds")
            .unwrap_or_else(|_| "0".to_string());
        let total_gain: String = row
            .try_get("total_gain")
            .unwrap_or_else(|_| "0".to_string());
        let total_loss: String = row
            .try_get("total_loss")
            .unwrap_or_else(|_| "0".to_string());
...
impl AssetRetirementEngine {
    ...
    pub async fn create(
        ...
        let cost_val: f64 = cost
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid cost value".into()))?;
        let acc_dep: f64 = accumulated_depreciation
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid accumulated depreciation".into()))?;
        let proceeds_val: f64 = proceeds
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid proceeds value".into()))?;
        let removal_val: f64 = removal_cost
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid removal cost".into()))?;
        ...
        if acc_dep < 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Accumulated depreciation cannot be negative".into(),
            ));
        }
        if acc_dep > cost_val {
            return Err(AtlasError::ValidationFailed(
                "Accumulated depreciation cannot exceed cost".into(),
            ));
        }
        if proceeds_val < 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Proceeds cannot be negative".into(),
            ));
        }
        if removal_val < 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Removal cost cannot be negative".into(),
            ));
        }
    ...
    pub async fn approve(&self, id: Uuid, approved_by: Uuid) -> AtlasResult<AssetRetirement> {
        ...
        if ret.status != "pending" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot approve retirement in '{}' status",
                ret.status
            )));
        }
    ...
    pub async fn complete(
        ...
        if ret.status != "approved" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot complete retirement in '{}' status",
                ret.status
            )));
        }
    ...
    pub async fn reverse(&self, id: Uuid) -> AtlasResult<AssetRetirement> {
        ...
        if ret.status != "completed" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot reverse retirement in '{}' status",
                ret.status
            )));
        }
    ...
    pub async fn cancel(&self, id: Uuid) -> AtlasResult<AssetRetirement> {
        ...
        if ret.status != "pending" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot cancel retirement in '{}' status",
                ret.status
            )));
        }
...
mod tests {
    ...
    impl AssetRetirementRepository for MockRepo {
        async fn create(
            ...
            let r = AssetRetirement {
                id: Uuid::new_v4(),
                organization_id: org_id,
                retirement_number: number.into(),
                asset_id,
                asset_number: asset_number.map(Into::into),
                asset_description: asset_description.map(Into::into),
                retirement_type: retirement_type.into(),
                retirement_date,
                cost: cost.into(),
                accumulated_depreciation: accumulated_depreciation.into(),
                net_book_value: net_book_value.into(),
                proceeds: proceeds.into(),
                removal_cost: removal_cost.into(),
                gain_loss_amount: gain_loss_amount.into(),
                gain_loss_account: gain_loss_account.map(Into::into),
                asset_account: asset_account.map(Into::into),
                depreciation_account: depreciation_account.map(Into::into),
                proceeds_account: proceeds_account.map(Into::into),
                removal_cost_account: removal_cost_account.map(Into::into),
                buyer_name: buyer_name.map(Into::into),
        ...
        async fn get_by_number(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|r| r.organization_id == org_id && r.retirement_number == number)
                .cloned())
        }
        async fn list(
            &self,
            ...
            Ok(self
                ...
                .filter(|r| {
                    r.organization_id == org_id
                        ...
                        && (retirement_type.is_none()
                            || r.retirement_type == retirement_type.unwrap())
                })
                .cloned()
                .collect())
        }
        ...
        async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<RetirementDashboard> {
            ...
            let completed: Vec<_> = org_items
                .iter()
                .filter(|r| r.status == "completed")
                .collect();
            let total_proceeds: f64 = completed
                .iter()
                .map(|r| r.proceeds.parse::<f64>().unwrap_or(0.0))
                .sum();
            let gains: f64 = completed
                .iter()
                .map(|r| r.gain_loss_amount.parse::<f64>().unwrap_or(0.0))
                .filter(|v| *v > 0.0)
                .sum();
            let losses: f64 = completed
                .iter()
                .map(|r| r.gain_loss_amount.parse::<f64>().unwrap_or(0.0))
                .filter(|v| *v < 0.0)
                .sum();
            Ok(RetirementDashboard {
                ...
                pending_retirements: org_items.iter().filter(|r| r.status == "pending").count()
                    as i32,
    ...
    fn eng() -> AssetRetirementEngine {
        AssetRetirementEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_list_invalid_type() {
        assert!(eng()
            .list(Uuid::new_v4(), None, Some("invalid"))
            .await
            .is_err());
    }
