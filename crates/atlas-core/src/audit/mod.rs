...
use atlas_shared::{AuditAction, AuditEntry, RecordId, UserId};
    ...
    pub fn compute_changes(
        ...
        for (key, new_val) in new_obj {
            ...
            if !values_equal(old_val, new_val) {
                changes.push(Self::new(key, old_val.cloned(), Some(new_val.clone())));
            }
        }
        ...
        for (key, old_val) in old_obj {
            if !new_obj.contains_key(key) {
                changes.push(Self::new(key, Some(old_val.clone()), None));
            }
        }
