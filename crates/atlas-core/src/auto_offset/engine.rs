...
use super::{
    AtlasResult, AutoOffsetActivity, AutoOffsetDashboard, AutoOffsetGeneration, AutoOffsetLine,
    AutoOffsetRepository, AutoOffsetTemplate, AutoOffsetTemplateLine,
};
...
const VALID_GENERATION_METHODS: &[&str] = &["single_entry", "multi_entry", "net_zero"];
const VALID_SOURCE_TYPES: &[&str] = &["journal_entry", "invoice", "payment", "receipt", "manual"];
#[allow(dead_code)]
const VALID_OFFSET_TYPES: &[&str] = &["due_to", "due_from", "clearing"];
#[allow(dead_code)]
const VALID_GENERATION_STATUSES: &[&str] = &["generated", "posted", "reversed", "cancelled"];
...
impl AutoOffsetEngine {
    ...
    pub async fn create_template(
        ...
        if template_code.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Template code is required".into(),
            ));
        }
        ...
        if let Some(_existing) = self
            .repository
            .get_template_by_code(org_id, template_code)
            .await?
        {
        ...
        info!(
            "Auto Offset Engine: Creating template '{}' ({})",
        ...
        self.repository
            .create_template(
                ...
                default_offset_account_description,
                enable_intra_entity,
                intra_entity_account,
                effective_from,
                effective_to,
                created_by,
            )
    ...
    pub async fn get_template_by_code(
        ...
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<AutoOffsetTemplate>> {
    ...
    ) -> AtlasResult<Vec<AutoOffsetTemplate>> {
    ...
    pub async fn add_template_line(
        ...
        info!(
            "Auto Offset Engine: Adding line {} for segment '{}' to template {}",
        ...
        self.repository
            .add_template_line(
                ...
                due_to_account,
                due_to_account_description,
                due_from_account,
                due_from_account_description,
                clearing_account,
                priority,
            )
            .await
    ...
    pub async fn list_template_lines(
        &self,
        template_id: Uuid,
    ) -> AtlasResult<Vec<AutoOffsetTemplateLine>> {
    ...
    pub async fn generate_offsets(
        ...
        for line in journal_lines {
            ...
            *segment_balances
                .entry(line.balancing_segment_value.clone())
                .or_insert(0.0) += net;
        }
        ...
        for (i, seg_a) in segments.iter().enumerate() {
            for seg_b in segments.iter().skip(i + 1) {
                ...
                if (*bal_a > 0.0 && *bal_b < 0.0) || (*bal_a < 0.0 && *bal_b > 0.0) {
                    let offset_amount = if bal_a.abs() < bal_b.abs() {
                        bal_a.abs()
                    } else {
                        bal_b.abs()
                    };
        ...
        let total_debit: f64 = offset_line_data
            .iter()
            .filter(|(_, _, _, _, amt)| *amt > 0.0)
            .map(|(_, _, _, _, amt)| *amt)
            .sum();
        let total_credit: f64 = offset_line_data
            .iter()
            .filter(|(_, _, _, _, amt)| *amt < 0.0)
        ...
        let generation_number = format!(
            "AO-{}",
            uuid::Uuid::new_v4().to_string()[..8].to_uppercase()
        );

        info!(
            "Auto Offset Engine: Generating {} offset lines for {} ({})",
            offset_line_data.len(),
            generation_number,
            source_type
        );
        ...
        let generation = self
            ...
            .create_generation(
                ...
                generation_date,
                currency_code,
                journal_lines.len() as i32,
                segments_affected,
                offset_line_data.len() as i32,
                total_debit,
                total_credit,
                created_by,
            )
        ...
        // Create offset lines
        for (idx, (from_seg, to_seg, offset_type, account, amount)) in
            offset_line_data.iter().enumerate()
        {
            self.repository
                .add_offset_line(
                    org_id,
    ...
    pub async fn get_generation_by_number(
        &self,
        org_id: Uuid,
        number: &str,
    ) -> AtlasResult<Option<AutoOffsetGeneration>> {
        self.repository
            .get_generation_by_number(org_id, number)
            .await
    }
    ...
    ) -> AtlasResult<Vec<AutoOffsetGeneration>> {
        self.repository
            .list_generations(org_id, status, source_type)
            .await
    }
    ...
    pub async fn post_generation(
        ...
        let updated = self
            .repository
            .update_generation_status(generation_id, "posted", Some(posted_by))
            .await?;
        ...
        self.repository
            .log_activity(
                ...
                None,
                "posted",
                Some("Offset generation posted to GL"),
                Some("generated"),
                Some("posted"),
                Some(posted_by),
                None,
    ...
    pub async fn reverse_generation(
        ...
        let updated = self
            .repository
            .update_generation_status(generation_id, "reversed", Some(reversed_by))
            .await?;
        ...
        self.repository
            .log_activity(
                ...
                None,
                "reversed",
                Some("Offset generation reversed"),
                Some("posted"),
                Some("reversed"),
                Some(reversed_by),
                None,
    ...
    pub async fn cancel_generation(
        ...
        info!(
            "Auto Offset Engine: Cancelling generation {}",
            generation_id
        );
        ...
        let updated = self
            .repository
            .update_generation_status(generation_id, "cancelled", Some(cancelled_by))
            .await?;
        ...
        self.repository
            .log_activity(
                ...
                None,
                "cancelled",
                Some("Offset generation cancelled"),
                Some("generated"),
                Some("cancelled"),
                Some(cancelled_by),
                None,
    ...
    pub async fn list_activities(
        &self,
        generation_id: Uuid,
    ) -> AtlasResult<Vec<AutoOffsetActivity>> {
