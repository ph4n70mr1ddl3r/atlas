...
use atlas_shared::{
    AtlasError, AtlasResult, AutoOffsetActivity, AutoOffsetDashboard, AutoOffsetGeneration,
    AutoOffsetLine, AutoOffsetTemplate, AutoOffsetTemplateLine,
};
use sqlx::PgPool;
use uuid::Uuid;
...
pub trait AutoOffsetRepository: Send + Sync {
    ...
    async fn create_generation(
        ...
        currency_code: &str,
        total_source_lines: i32,
        balancing_segments_affected: i32,
        total_offset_lines: i32,
        total_debit_amount: f64,
    ...
    async fn get_generation_by_number(
        &self,
        org_id: Uuid,
        number: &str,
    ...
    async fn add_offset_line(
        ...
        generation_id: Uuid,
        line_number: i32,
        from_segment_value: &str,
        to_segment_value: &str,
        offset_type: &str,
        account_code: &str,
...
#[allow(dead_code)]
pub struct PostgresAutoOffsetRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
impl PostgresAutoOffsetRepository {
    #[must_use]
...
impl AutoOffsetRepository for PostgresAutoOffsetRepository {
    ...
    async fn get_template(&self, _: Uuid) -> AtlasResult<Option<AutoOffsetTemplate>> {
        Ok(None)
    }
    async fn get_template_by_code(
        &self,
    ...
    async fn update_template_status(&self, _: Uuid, _: bool) -> AtlasResult<AutoOffsetTemplate> {
        Err(AtlasError::EntityNotFound("Mock".into()))
    }
    async fn delete_template(&self, _: Uuid) -> AtlasResult<()> {
        Err(AtlasError::EntityNotFound("Mock".into()))
    }
    async fn add_template_line(
        &self,
    ...
    async fn list_template_lines(&self, _: Uuid) -> AtlasResult<Vec<AutoOffsetTemplateLine>> {
        Ok(vec![])
    }
    async fn delete_template_line(&self, _: Uuid) -> AtlasResult<()> {
        Err(AtlasError::EntityNotFound("Mock".into()))
    }
    async fn create_generation(
        &self,
    ...
    async fn get_generation(&self, _: Uuid) -> AtlasResult<Option<AutoOffsetGeneration>> {
        Ok(None)
    }
    async fn get_generation_by_number(
        &self,
    ...
    async fn list_offset_lines(&self, _: Uuid) -> AtlasResult<Vec<AutoOffsetLine>> {
        Ok(vec![])
    }
    async fn log_activity(
        &self,
    ...
    async fn list_activities(&self, _: Uuid) -> AtlasResult<Vec<AutoOffsetActivity>> {
        Ok(vec![])
    }
