...
pub trait AvailableFundsRepository: Send + Sync {
    ...
    async fn list_checks(
        ...
        result: Option<&str>,
    ) -> AtlasResult<Vec<FundsCheckResult>>;
    async fn get_latest_check(
        &self,
        org_id: Uuid,
        budget_code: &str,
        account_code: &str,
    ) -> AtlasResult<Option<FundsCheckResult>>;
...
#[allow(dead_code)]
pub struct PostgresAvailableFundsRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
impl PostgresAvailableFundsRepository {
    #[must_use]
...
impl AvailableFundsRepository for PostgresAvailableFundsRepository {
    ...
    async fn get_check(&self, _: Uuid) -> AtlasResult<Option<FundsCheckResult>> {
        Ok(None)
    }
    async fn list_checks(
        &self,
    ...
    async fn get_override(&self, _: Uuid) -> AtlasResult<Option<FundsOverride>> {
        Ok(None)
    }
    async fn list_overrides(&self, _: Uuid, _: Option<&str>) -> AtlasResult<Vec<FundsOverride>> {
        Ok(vec![])
    }
    async fn approve_override(
        &self,
    ...
    async fn reject_override(&self, _: Uuid, _: Option<&str>) -> AtlasResult<FundsOverride> {
        Err(AtlasError::EntityNotFound("Not found".into()))
    }
    async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<FundsCheckDashboard> {
...
impl AvailableFundsEngine {
    ...
    pub async fn check_funds(
        ...
        if period_number <= 0 || period_number > 13 {
            return Err(AtlasError::ValidationFailed(
                "Period number must be 1-13".into(),
            ));
        }
        if let Some(tt) = tolerance_type {
            if !VALID_TOLERANCE_LEVELS.contains(&tt) {
                return Err(AtlasError::ValidationFailed(format!(
                    "Invalid tolerance type '{tt}'"
                )));
            }
        }

        let budget: f64 = budget_amount
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid budget amount".into()))?;
        let encumbered: f64 = encumbered_amount
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid encumbered amount".into()))?;
        let actual: f64 = actual_amount
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid actual amount".into()))?;
        let requested: f64 = requested_amount
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid requested amount".into()))?;
        ...
        info!(
            "Funds check {} for budget {} account {}: {} (available={}, requested={}, after={})",
            check_number,
            budget_code,
            account_code,
            check_result,
    ...
    pub async fn list_checks(
        ...
        if let Some(r) = result {
            if !VALID_CHECK_RESULTS.contains(&r) {
                return Err(AtlasError::ValidationFailed(format!(
                    "Invalid check result '{r}'"
                )));
            }
        }
        self.repository
            .list_checks(org_id, budget_code, result)
            .await
    }
    ...
    pub async fn get_latest_check(
        ...
        self.repository
            .get_latest_check(org_id, budget_code, account_code)
            .await
    }
    ...
    pub async fn request_override(
        ...
        if check.check_result != "fail" && check.check_result != "warning" {
            return Err(AtlasError::ValidationFailed(
                "Override only allowed for failed or warning checks".into(),
            ));
        }
        if reason.is_empty() {
            return Err(AtlasError::ValidationFailed(
        ...
        if approval_level.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Approval level is required".into(),
            ));
        }

        let override_number = format!("FOV-{}", Uuid::new_v4().to_string()[..8].to_uppercase());
        info!(
            "Creating funds override {} for check {}",
            override_number, check.check_number
        );
        self.repository
            .create_override(
                org_id,
    ...
    pub async fn approve_override(
        ...
        if ov.status != "pending" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot approve override in '{}' status",
                ov.status
            )));
        }
        info!("Approving funds override {}", ov.override_number);
        self.repository
            .approve_override(id, approved_by, notes)
            .await
    }
    ...
    pub async fn reject_override(
        ...
        if ov.status != "pending" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot reject override in '{}' status",
                ov.status
            )));
        }
...
mod tests {
    ...
    impl MockRepo {
        fn new() -> Self {
            Self {
                checks: std::sync::Mutex::new(vec![]),
                overrides: std::sync::Mutex::new(vec![]),
            }
        }
    }
    ...
    impl AvailableFundsRepository for MockRepo {
        async fn create_check(
            ...
            let c = FundsCheckResult {
                ...
                encumbered_amount: encumbered_amount.into(),
                actual_amount: actual_amount.into(),
                available_amount: available_amount.into(),
                requested_amount: requested_amount.into(),
                result_after: result_after.into(),
                check_result: check_result.into(),
                tolerance_type: tolerance_type.map(Into::into),
                tolerance_value: tolerance_value.map(Into::into),
        ...
        async fn create_override(
            ...
            let o = FundsOverride {
                ...
                organization_id: org_id,
                check_id,
                override_number: override_number.into(),
                reason: reason.into(),
                approval_level: approval_level.into(),
                status: "pending".into(),
                approved_by: None,
                approval_notes: None,
                expires_at: None,
        ...
        async fn list_overrides(
            ...
            Ok(self
                ...
                .iter()
                .filter(|o| {
                    o.organization_id == org_id && (status.is_none() || o.status == status.unwrap())
                })
                .cloned()
                .collect())
        }
        ...
        async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<FundsCheckDashboard> {
            ...
            let org_checks: Vec<_> = checks
                .iter()
                .filter(|c| c.organization_id == org_id)
                .collect();
            let org_overrides: Vec<_> = overrides
                .iter()
                .filter(|o| o.organization_id == org_id)
                .collect();
            let total = org_checks.len() as i32;
            let passed = org_checks
                .iter()
                .filter(|c| c.check_result == "pass")
                .count() as i32;
            let failed = org_checks
                .iter()
                .filter(|c| c.check_result == "fail")
                .count() as i32;
            let pending = org_overrides
                .iter()
            ...
            Ok(FundsCheckDashboard {
                ...
                warning_checks: org_checks
                    .iter()
                    .filter(|c| c.check_result == "warning")
                    .count() as i32,
    ...
    fn eng() -> AvailableFundsEngine {
        AvailableFundsEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_request_override() {
        ...
        let c = e
            .check_funds(
                org, "BUD", "1000", 2026, 5, "5000", "3000", "1000", "5000", None, None, None,
                None, None, None,
            )
            .await
            .unwrap();
        let ov = e
            .request_override(org, c.id, "Emergency procurement", "director", None)
            .await
            .unwrap();
    ...
    async fn test_request_override_pass_check() {
        ...
        let c = e
            .check_funds(
                org, "BUD", "1000", 2026, 5, "100000", "0", "0", "1000", None, None, None, None,
                None, None,
            )
            .await
            .unwrap();
        assert!(e
            .request_override(org, c.id, "reason", "level", None)
            .await
            .is_err());
    }
    ...
    async fn test_request_override_empty_reason() {
        ...
        let c = e
            ...
            .unwrap();
        assert!(e
            .request_override(org, c.id, "", "director", None)
            .await
            .is_err());
    }
    ...
    async fn test_approve_override() {
        ...
        let c = e
            ...
            .unwrap();
        let ov = e
            .request_override(org, c.id, "Emergency", "director", None)
            .await
            .unwrap();
        let approved = e
            .approve_override(ov.id, Uuid::new_v4(), Some("Approved by CFO"))
            .await
            .unwrap();
    ...
    async fn test_reject_override() {
        ...
        let ov = e
            ...
            .unwrap();
        let rejected = e
            .reject_override(ov.id, Some("Insufficient justification"))
            .await
            .unwrap();
    ...
    async fn test_approve_override_not_pending() {
        ...
        let ov = e
            ...
            .unwrap();
        let _ = e
            .approve_override(ov.id, Uuid::new_v4(), None)
            .await
            .unwrap();
        assert!(e
            .approve_override(ov.id, Uuid::new_v4(), None)
    ...
    async fn test_list_checks_invalid_result() {
        assert!(eng()
            .list_checks(Uuid::new_v4(), None, Some("invalid"))
            .await
            .is_err());
    }
    ...
    async fn test_list_overrides_invalid_status() {
        assert!(eng()
            .list_overrides(Uuid::new_v4(), Some("invalid"))
            .await
            .is_err());
    }
    ...
    async fn test_full_workflow() {
        ...
        // Request override
        let ov = e
            .request_override(org, c.id, "Critical need", "vp_finance", None)
            .await
            .unwrap();
        assert_eq!(ov.status, "pending");
        ...
        // Approve override
        let approved = e
            .approve_override(ov.id, Uuid::new_v4(), Some("VP approved"))
            .await
            .unwrap();
        assert_eq!(approved.status, "approved");
