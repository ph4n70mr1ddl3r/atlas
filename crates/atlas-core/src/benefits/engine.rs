...
use atlas_shared::{
    AtlasError, AtlasResult, BenefitsDeduction, BenefitsEnrollment, BenefitsPlan, BenefitsSummary,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
const VALID_ENROLLMENT_TYPES: &[&str] = &["open_enrollment", "new_hire", "life_event", "manual"];
...
const VALID_DEDUCTION_FREQUENCIES: &[&str] = &["per_pay_period", "monthly", "semi_monthly"];
...
impl BenefitsEngine {
    ...
    pub async fn create_plan(
        ...
        info!(
            "Creating benefits plan '{}' ({}) for org {}",
    ...
    pub async fn list_plans(
        ...
        if let Some(pt) = plan_type {
            if !VALID_PLAN_TYPES.contains(&pt) {
                return Err(AtlasError::ValidationFailed(format!(
                    "Invalid plan_type filter '{}'. Must be one of: {}",
                    pt,
                    VALID_PLAN_TYPES.join(", ")
                )));
            }
    ...
    pub async fn enroll(
        ...
        if !VALID_ENROLLMENT_TYPES.contains(&enrollment_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid enrollment_type '{}'. Must be one of: {}",
                enrollment_type,
                VALID_ENROLLMENT_TYPES.join(", ")
            )));
        }
        if !VALID_DEDUCTION_FREQUENCIES.contains(&deduction_frequency) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid deduction_frequency '{}'. Must be one of: {}",
                deduction_frequency,
                VALID_DEDUCTION_FREQUENCIES.join(", ")
            )));
        }
        ...
        // Look up the plan
        let plan = self.get_plan(org_id, plan_code).await?.ok_or_else(|| {
            AtlasError::EntityNotFound(format!("Benefits plan '{plan_code}' not found"))
        })?;

        if !plan.is_active {
            return Err(AtlasError::ValidationFailed(format!(
                "Benefits plan '{plan_code}' is not active"
            )));
        }

        // Check if enrollment is within open enrollment period (for open_enrollment type)
        if enrollment_type == "open_enrollment" {
            if let (Some(oe_start), Some(oe_end)) =
                (plan.open_enrollment_start, plan.open_enrollment_end)
            {
        ...
        if enrollment_type == "life_event" && !plan.allow_life_event_changes {
            return Err(AtlasError::ValidationFailed(format!(
                "Plan '{plan_code}' does not allow mid-year life event changes"
            )));
        }

        // Validate coverage tier exists in the plan
        let tier_found = plan.coverage_tiers.as_array().is_some_and(|tiers| {
            tiers
                .iter()
                .any(|t| t.get("tierCode").and_then(|v| v.as_str()) == Some(coverage_tier))
        });
        ...
        let (employee_cost, employer_cost, total_cost) =
            self.extract_tier_costs(&plan, coverage_tier)?;
        ...
        let existing = self
            .repository
            .get_active_enrollment(org_id, employee_id, plan.id)
            .await?;
        if existing.is_some() {
            return Err(AtlasError::Conflict(format!(
                "Employee {employee_id} already has an active enrollment in plan '{plan_code}'"
            )));
        }
        ...
        info!(
            "Enrolling employee {} in plan '{}' with tier '{}'",
            employee_id, plan_code, coverage_tier
        );
        ...
        if let Some(s) = status {
            if !VALID_ENROLLMENT_STATUSES.contains(&s) {
                return Err(AtlasError::ValidationFailed(format!(
                    ...
                    s,
                    VALID_ENROLLMENT_STATUSES.join(", ")
                )));
            }
        }
        self.repository
            .list_enrollments(org_id, employee_id, plan_id, status)
            .await
    }
    ...
    pub async fn activate_enrollment(
        ...
        let enrollment = self
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Enrollment {enrollment_id} not found"))
            })?;

        if enrollment.status != "pending" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot activate enrollment in '{}' status. Must be 'pending'.",
                enrollment.status
            )));
        }

        info!(
            "Activating enrollment {} by {}",
            enrollment_id, processed_by
        );
        self.repository
            .update_enrollment_status(enrollment_id, "active", Some(processed_by), None)
            .await
    }
    ...
    pub async fn waive_enrollment(&self, enrollment_id: Uuid) -> AtlasResult<BenefitsEnrollment> {
        ...
        if enrollment.status != "pending" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot waive enrollment in '{}' status. Must be 'pending'.",
                enrollment.status
            )));
        }
        ...
        self.repository
            .update_enrollment_status(enrollment_id, "waived", None, None)
            .await
    }
    ...
    pub async fn cancel_enrollment(
        ...
        if enrollment.status != "active" && enrollment.status != "pending" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot cancel enrollment in '{}' status. Must be 'active' or 'pending'.",
                enrollment.status
            )));
        }

        info!(
            "Cancelling enrollment {} reason: {:?}",
            enrollment_id, cancellation_reason
        );
        self.repository
            .cancel_enrollment(enrollment_id, cancellation_reason)
            .await
    }
    ...
    pub async fn suspend_enrollment(&self, enrollment_id: Uuid) -> AtlasResult<BenefitsEnrollment> {
        ...
        if enrollment.status != "active" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot suspend enrollment in '{}' status. Must be 'active'.",
                enrollment.status
            )));
        }
        ...
        self.repository
            .update_enrollment_status(enrollment_id, "suspended", None, None)
            .await
    }
    ...
    pub async fn reactivate_enrollment(
        ...
        if enrollment.status != "suspended" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot reactivate enrollment in '{}' status. Must be 'suspended'.",
                enrollment.status
            )));
        }
        ...
        self.repository
            .update_enrollment_status(enrollment_id, "active", None, None)
            .await
    }
    ...
    pub async fn generate_deductions(
        ...
        info!(
            "Generating benefits deductions for period {} to {} org {}",
            pay_period_start, pay_period_end, org_id
        );

        // Get all active enrollments for this organization
        let enrollments = self
            .repository
            .list_enrollments(org_id, None, None, Some("active"))
            .await?;
        ...
        for enrollment in &enrollments {
            ...
            let effective = enrollment.effective_start_date <= pay_period_end
                && enrollment
                    .effective_end_date
                    .is_none_or(|end| end >= pay_period_start);
            ...
            let deduction = self
                ...
                .create_deduction(
                    ...
                    enrollment.plan_id,
                    enrollment.plan_code.as_deref(),
                    enrollment.plan_name.as_deref(),
                    &enrollment.employee_cost,
                    &enrollment.employer_cost,
                    &enrollment.total_cost,
                    pay_period_start,
                    pay_period_end,
        ...
        info!(
            "Generated {} benefits deductions for org {}",
            deductions.len(),
            org_id
        );
        Ok(deductions)
    ...
    pub async fn list_deductions(
        ...
        self.repository
            .list_deductions(org_id, employee_id, enrollment_id)
            .await
    }
    ...
    pub async fn get_summary(&self, org_id: Uuid) -> AtlasResult<BenefitsSummary> {
        ...
        let enrollments = self
            .repository
            .list_enrollments(org_id, None, None, None)
            .await?;
        ...
        let pending_enrollments =
            enrollments.iter().filter(|e| e.status == "pending").count() as i32;
        ...
        let total_employee_cost: f64 = enrollments
            .iter()
            .filter(|e| e.status == "active")
            .map(|e| e.employee_cost.parse::<f64>().unwrap_or(0.0))
            .sum();
        let total_employer_cost: f64 = enrollments
            .iter()
            .filter(|e| e.status == "active")
    ...
    fn extract_tier_costs(
        &self,
        plan: &BenefitsPlan,
        tier_code: &str,
    ) -> AtlasResult<(String, String, String)> {
        let tiers = plan
            .coverage_tiers
            .as_array()
        ...
        let tier = tiers
            ...
            .ok_or_else(|| {
                AtlasError::ValidationFailed(format!(
                    "Coverage tier '{tier_code}' not found in plan"
                ))
            })?;

        let employee_cost = tier
            .get("employeeCost")
            .and_then(|v| v.as_str())
            .unwrap_or("0.00")
            .to_string();
        let employer_cost = tier
            .get("employerCost")
            .and_then(|v| v.as_str())
            .unwrap_or("0.00")
            .to_string();
