...
use atlas_shared::{
    AtlasError, AtlasResult, BudgetDefinition, BudgetLine, BudgetTransfer, BudgetVarianceLine,
    BudgetVarianceReport, BudgetVersion,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
/// Valid budget types
const VALID_BUDGET_TYPES: &[&str] = &["operating", "capital", "project", "cash_flow"];

/// Valid control levels
const VALID_CONTROL_LEVELS: &[&str] = &["none", "advisory", "absolute"];
...
const _VALID_TRANSFER_STATUSES: &[&str] = &["pending", "approved", "rejected", "cancelled"];
...
impl BudgetEngine {
    ...
    pub async fn create_definition(
        ...
        if !VALID_BUDGET_TYPES.contains(&budget_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid budget_type '{}'. Must be one of: {}",
                budget_type,
                VALID_BUDGET_TYPES.join(", ")
            )));
        }
        if !VALID_CONTROL_LEVELS.contains(&control_level) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid control_level '{}'. Must be one of: {}",
                control_level,
                VALID_CONTROL_LEVELS.join(", ")
            )));
        }

        info!(
            "Creating budget definition '{}' ({}) for org {}",
    ...
    pub async fn get_definition(
        ...
        self.repository
            .get_definition(org_id, &code.to_uppercase())
            .await
    }
    ...
    pub async fn delete_definition(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        info!(
            "Deactivating budget definition '{}' for org {}",
            code, org_id
        );
        self.repository
            .delete_definition(org_id, &code.to_uppercase())
            .await
    }
    ...
    pub async fn create_version(
        ...
        let definition = self
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Budget definition '{budget_code}' not found"))
            })?;
        ...
        let version_number = self
            .repository
            .get_next_version_number(definition.id)
            .await?;

        info!(
            "Creating budget version {} for '{}' (v{})",
            version_number, budget_code, version_number
        );
    ...
    pub async fn get_active_version(
        &self,
        definition_id: Uuid,
    ) -> AtlasResult<Option<BudgetVersion>> {
    ...
    pub async fn submit_version(
        ...
        let version = self
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Budget version {version_id} not found"))
            })?;

        if version.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot submit version in '{}' status. Must be 'draft'.",
                version.status
            )));
        }
        ...
        self.repository
            .update_version_status(version_id, "submitted", Some(submitted_by), None, None)
            .await
    }
    ...
    pub async fn approve_version(
        ...
        if version.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot approve version in '{}' status. Must be 'submitted'.",
                version.status
            )));
        }
        ...
        self.repository
            .update_version_status(version_id, "approved", None, Some(approved_by), None)
            .await
    }
    ...
    pub async fn activate_version(&self, version_id: Uuid) -> AtlasResult<BudgetVersion> {
        ...
        if version.status != "approved" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot activate version in '{}' status. Must be 'approved'.",
                version.status
            )));
        }

        // Deactivate any currently active versions for the same definition
        if let Ok(Some(current_active)) = self
            .repository
            .get_active_version(version.definition_id)
            .await
        {
            if current_active.id != version_id {
                self.repository
                    .update_version_status(current_active.id, "closed", None, None, None)
                    .await?;
            }
        }
        ...
        self.repository
            .update_version_status(version_id, "active", None, None, None)
            .await
    }
    ...
    pub async fn reject_version(
        ...
        if version.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot reject version in '{}' status. Must be 'submitted'.",
                version.status
            )));
        }
        ...
        self.repository
            .update_version_status(version_id, "rejected", None, None, reason)
            .await
    }
    ...
    pub async fn close_version(&self, version_id: Uuid) -> AtlasResult<BudgetVersion> {
        ...
        if version.status != "active" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot close version in '{}' status. Must be 'active'.",
                version.status
            )));
        }
        ...
        self.repository
            .update_version_status(version_id, "closed", None, None, None)
            .await
    }
    ...
    pub async fn add_line(
        ...
        let amount_val: f64 = budget_amount.parse().map_err(|_| {
        ...
        info!(
            "Adding budget line {} to version {} (account: {})",
            line_number, version_id, account_code
        );
    ...
    pub async fn update_line_amount(
        ...
        let amount_val: f64 = new_amount.parse().map_err(|_| {
            AtlasError::ValidationFailed("Budget amount must be a valid number".to_string())
        })?;
        if amount_val < 0.0 {
            return Err(AtlasError::ValidationFailed(
        ...
        let line = self
            .repository
            .update_line_amount(line_id, new_amount)
            .await?;
    ...
    pub async fn create_transfer(
        ...
        let definition = self
            .repository
            .get_definition_by_id(version.definition_id)
            .await?
        ...
        if version.status != "active" {
            return Err(AtlasError::WorkflowError(format!(
                "Can only transfer amounts in an active budget version (current: '{}')",
                version.status
            )));
        }

        let amount_val: f64 = amount.parse().map_err(|_| {
            AtlasError::ValidationFailed("Transfer amount must be a valid number".to_string())
        })?;
        if amount_val <= 0.0 {
            return Err(AtlasError::ValidationFailed(
        ...
        let from_line = self
            ...
            .find_line(
                ...
                from_account_code,
                from_period_name,
                from_department_id.as_ref(),
                from_cost_center,
            )
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!(
                    "Source budget line not found for account '{from_account_code}'"
                ))
            })?;
        ...
        let _to_line = self
            ...
            .find_line(
                ...
                to_account_code,
                to_period_name,
                to_department_id.as_ref(),
                to_cost_center,
            )
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!(
                    "Destination budget line not found for account '{to_account_code}'"
                ))
            })?;

        info!(
            "Creating budget transfer {} for version {}",
    ...
    pub async fn approve_transfer(
        ...
        let transfer = self
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Budget transfer {transfer_id} not found"))
            })?;
        ...
        self.repository
            .update_line_amount(from_line.id, &from_line.budget_amount)
            .await?;
    ...
    pub async fn check_budget_control(
        ...
        let active_version = self
            .get_active_version(definition.id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("No active budget version for '{budget_code}'"))
            })?;
