...
pub trait CashFlowStatementRepository: Send + Sync {
    ...
    async fn get_statement_by_number(
        ...
        org_id: Uuid,
        statement_number: &str,
    ) -> AtlasResult<Option<CashFlowStatement>>;
    async fn list_statements(
        &self,
        org_id: Uuid,
        status: Option<&str>,
        method: Option<&str>,
    ) -> AtlasResult<Vec<CashFlowStatement>>;
    ...
    async fn update_statement_amounts(
        &self,
        id: Uuid,
        opening: &str,
        operating: &str,
...
/// `PostgreSQL` stub
#[allow(dead_code)]
pub struct PostgresCashFlowStatementRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
impl PostgresCashFlowStatementRepository {
    #[must_use]
...
impl CashFlowStatementRepository for PostgresCashFlowStatementRepository {
    ...
    async fn get_statement(&self, _: Uuid) -> AtlasResult<Option<CashFlowStatement>> {
        Ok(None)
    }
    async fn get_statement_by_number(
        &self,
    ...
    async fn list_lines(&self, _: Uuid) -> AtlasResult<Vec<CashFlowStatementLine>> {
        Ok(vec![])
    }
    ...
    async fn get_dashboard(&self, _org_id: Uuid) -> AtlasResult<CashFlowDashboard> {
        Ok(CashFlowDashboard {
            total_statements: 0,
            draft_statements: 0,
            published_statements: 0,
            total_operating: "0".into(),
            total_investing: "0".into(),
            total_financing: "0".into(),
        })
    }
}
...
impl CashFlowStatementEngine {
    pub fn new(repository: Arc<dyn CashFlowStatementRepository>) -> Self {
        Self { repository }
    }
    ...
    pub async fn create_statement(
        ...
        if statement_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Statement number is required".into(),
            ));
        }
            ...
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Statement '{statement_number}' already exists"
            )));
        }
        info!(
            "Creating cash flow statement {} for org {}",
    ...
    pub async fn get_statement(&self, id: Uuid) -> AtlasResult<Option<CashFlowStatement>> {
    ...
    pub async fn list_statements(
        ...
        self.repository
            .list_statements(org_id, status, method)
            .await
    }
    ...
    pub async fn calculate(&self, id: Uuid) -> AtlasResult<CashFlowStatement> {
        ...
        if stmt.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot calculate statement in '{}' status",
                stmt.status
            )));
        }
        ...
        self.repository
            ...
            .await?;
        self.repository
            .update_statement_status(id, "calculated", None)
            .await
    }
    ...
    pub async fn review(&self, id: Uuid, reviewer_id: Uuid) -> AtlasResult<CashFlowStatement> {
        ...
        if stmt.status != "calculated" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot review statement in '{}' status",
                stmt.status
            )));
        }
        self.repository
            .update_statement_status(id, "reviewed", Some(reviewer_id))
            .await
    }
    ...
    pub async fn publish(&self, id: Uuid) -> AtlasResult<CashFlowStatement> {
        ...
        if stmt.status != "reviewed" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot publish statement in '{}' status",
                stmt.status
            )));
        }
        self.repository
            .update_statement_status(id, "published", None)
            .await
    }
    ...
    pub async fn archive(&self, id: Uuid) -> AtlasResult<CashFlowStatement> {
        ...
        self.repository
            .update_statement_status(id, "archived", None)
            .await
    }
    ...
    pub async fn add_line(
        ...
        if !VALID_LINE_CATEGORIES.contains(&category) {
            return Err(AtlasError::ValidationFailed(format!(
                ...
                category,
                VALID_LINE_CATEGORIES.join(", ")
            )));
        }
        if !VALID_LINE_TYPES.contains(&line_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid line type '{line_type}'"
            )));
        }
        if line_number <= 0 {
            return Err(AtlasError::ValidationFailed(
...
mod tests {
    ...
    impl MockRepo {
        fn new() -> Self {
            Self {
                statements: std::sync::Mutex::new(vec![]),
    ...
    impl CashFlowStatementRepository for MockRepo {
        async fn create_statement(
            ...
            let s = CashFlowStatement {
                id: Uuid::new_v4(),
                organization_id: org_id,
                statement_number: num.into(),
                method: method.into(),
                period_type: pt.into(),
                period_start: ps,
                period_end: pe,
                opening_cash_balance: "0".into(),
                operating_cash_flow: "0".into(),
                investing_cash_flow: "0".into(),
                financing_cash_flow: "0".into(),
                net_change_in_cash: "0".into(),
                closing_cash_balance: "0".into(),
                exchange_rate_effect: "0".into(),
                prepared_by: cb,
                reviewed_by: None,
                status: "draft".into(),
        ...
        async fn get_statement_by_number(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|s| s.organization_id == org_id && s.statement_number == num)
                .cloned())
        }
        async fn list_statements(
            &self,
            ...
            Ok(self
                ...
                .filter(|s| {
                    s.organization_id == org_id
                        && (status.is_none() || s.status == status.unwrap())
                        && (method.is_none() || s.method == method.unwrap())
                })
                .cloned()
                .collect())
        }
        ...
        async fn update_statement_amounts(
            ...
            if let Some(s) = all.iter_mut().find(|s| s.id == id) {
                s.opening_cash_balance = open.into();
                s.operating_cash_flow = op.into();
                s.investing_cash_flow = inv.into();
                s.financing_cash_flow = fin.into();
                s.net_change_in_cash = nc.into();
                s.closing_cash_balance = close.into();
                s.exchange_rate_effect = ex.into();
            }
            Ok(())
        }
        async fn add_line(
            ...
            let l = CashFlowStatementLine {
                ...
                line_type: lt.into(),
                amount: amt.into(),
                account_range_from: arf.map(Into::into),
                account_range_to: art.map(Into::into),
                is_non_cash: nc,
                display_order: do_,
                metadata: serde_json::json!({}),
                created_at: Utc::now(),
        ...
        async fn list_lines(&self, sid: Uuid) -> AtlasResult<Vec<CashFlowStatementLine>> {
            Ok(self
                ...
                .unwrap()
                .iter()
                .filter(|l| l.statement_id == sid)
                .cloned()
                .collect())
        }
        async fn remove_line(&self, id: Uuid) -> AtlasResult<()> {
            self.lines.lock().unwrap().retain(|l| l.id != id);
            Ok(())
        }
        async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<CashFlowDashboard> {
            let stmts = self.statements.lock().unwrap();
            let org_s: Vec<_> = stmts
                .iter()
                .filter(|s| s.organization_id == org_id)
                .collect();
            Ok(CashFlowDashboard {
                total_statements: org_s.len() as i32,
                draft_statements: org_s.iter().filter(|s| s.status == "draft").count() as i32,
                published_statements: org_s.iter().filter(|s| s.status == "published").count()
                    as i32,
                total_operating: "0".into(),
                total_investing: "0".into(),
                total_financing: "0".into(),
    ...
    fn eng() -> CashFlowStatementEngine {
        CashFlowStatementEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_add_line_invalid_category() {
        ...
        let s = e
            ...
            .unwrap();
        assert!(e
            .add_line(s.id, 1, "invalid", None, "detail", "100", None, None, false, 1)
            .await
            .is_err());
    }
