...
use super::{
    AtlasError, AtlasResult, CashPosition, CashPositionDashboard, CashPositionRepository,
    CashPositionSummary,
};
use std::sync::Arc;
use tracing::info;
...
impl CashPositionEngine {
    ...
    pub async fn record_position(
        ...
        if hold < 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Hold amount must be non-negative".into(),
            ));
        }
        ...
        if (available - (ledger - hold)).abs() > 0.01 {
            return Err(AtlasError::ValidationFailed(format!(
                "Available balance {} doesn't match ledger({}) - hold({}) = {}",
                available,
                ledger,
                hold,
                ledger - hold
        ...
        info!(
            "Recording cash position for account {:?} on {} (closing: {})",
    ...
    pub async fn get_latest_position(
        ...
        self.repository
            .get_latest_position(org_id, bank_account_id)
            .await
    }
    ...
    pub async fn list_positions(
        ...
        self.repository
            .list_positions(org_id, position_date, currency_code)
            .await
    }
    ...
    pub async fn create_summary(
        ...
        let currency_positions: Vec<&CashPosition> = positions
            .iter()
        ...
        if currency_positions.is_empty() {
            return Err(AtlasError::ValidationFailed(format!(
                "No positions found for currency {currency_code}"
            )));
        }

        let total_opening: f64 = currency_positions
            .iter()
            .map(|p| p.opening_balance.parse::<f64>().unwrap_or(0.0))
            .sum();
        let total_inflows: f64 = currency_positions
            .iter()
            .map(|p| p.total_inflows.parse::<f64>().unwrap_or(0.0))
            .sum();
        let total_outflows: f64 = currency_positions
            .iter()
            .map(|p| p.total_outflows.parse::<f64>().unwrap_or(0.0))
            .sum();
        let total_closing: f64 = currency_positions
            .iter()
            .map(|p| p.closing_balance.parse::<f64>().unwrap_or(0.0))
            .sum();
        let total_ledger: f64 = currency_positions
            .iter()
            .map(|p| p.ledger_balance.parse::<f64>().unwrap_or(0.0))
            .sum();
        let total_available: f64 = currency_positions
            .iter()
            .map(|p| p.available_balance.parse::<f64>().unwrap_or(0.0))
            .sum();
        let total_hold: f64 = currency_positions
            .iter()
            .map(|p| p.hold_amount.parse::<f64>().unwrap_or(0.0))
            .sum();

        let accounts: Vec<serde_json::Value> = currency_positions
            .iter()
            .map(|p| {
                serde_json::json!({
        ...
        info!(
            "Creating cash position summary for {} on {} ({} accounts, total: {})",
            currency_code,
            position_date,
            currency_positions.len(),
            total_closing
        );
    ...
    pub async fn get_latest_summary(
        ...
        self.repository
            .get_latest_summary(org_id, currency_code)
            .await
    }
...
mod tests {
    ...
    impl MockRepo {
        fn new() -> Self {
            MockRepo {
                positions: std::sync::Mutex::new(vec![]),
            }
        }
    }
    ...
    impl CashPositionRepository for MockRepo {
        async fn create_position(
            ...
            let p = CashPosition {
                ...
                bank_account_id: bai,
                bank_account_number: ban.map(Into::into),
                bank_account_name: bnam.map(Into::into),
                currency_code: cc.into(),
                opening_balance: ob.into(),
                total_inflows: ti.into(),
                total_outflows: to_.into(),
                closing_balance: cb.into(),
                ledger_balance: lb.into(),
                available_balance: ab.into(),
                hold_amount: ha.into(),
                position_date: pd,
                source_breakdown: sb,
        ...
        async fn create_summary(
            ...
            Ok(CashPositionSummary {
                ...
                currency_code: cc.into(),
                position_date: pd,
                total_opening_balance: ob.into(),
                total_inflows: ti.into(),
                total_outflows: to_.into(),
                total_closing_balance: cb.into(),
                total_ledger_balance: lb.into(),
                total_available_balance: ab.into(),
                total_hold_amount: ha.into(),
                account_count: ac,
                accounts: accs,
                metadata: serde_json::json!({}),
                created_at: chrono::Utc::now(),
    ...
    fn eng() -> CashPositionEngine {
        CashPositionEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_get_latest_position_not_found() {
        let r = eng()
            .get_latest_position(Uuid::new_v4(), Uuid::new_v4())
            .await
            .unwrap();
        assert!(r.is_none());
    }
    ...
    async fn test_list_positions_empty() {
        let r = eng()
            .list_positions(Uuid::new_v4(), None, None)
            .await
            .unwrap();
        assert!(r.is_empty());
    }
    ...
    async fn test_get_latest_summary_empty() {
        let r = eng()
            .get_latest_summary(Uuid::new_v4(), "USD")
            .await
            .unwrap();
        assert!(r.is_none());
    }
