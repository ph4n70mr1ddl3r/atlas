...
use super::{
    AtlasError, AtlasResult, CashReceipt, CashReceiptDashboard, CashReceiptRepository,
    ReceiptApplication, ReceiptBatch,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
impl CashReceiptEngine {
    ...
    pub async fn create_batch(
        ...
        if !VALID_RECEIPT_METHODS.contains(&receipt_method) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid receipt method '{}'. Must be one of: {}",
                receipt_method,
            ...
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Batch number '{batch_number}' already exists"
            )));
        }
    ...
    pub async fn confirm_batch(&self, batch_id: Uuid) -> AtlasResult<ReceiptBatch> {
        ...
        if batch.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot confirm batch in '{}' status. Must be 'draft'.",
                batch.status
            )));
        }
    ...
    pub async fn close_batch(&self, batch_id: Uuid) -> AtlasResult<ReceiptBatch> {
        ...
        if batch.status != "confirmed" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot close batch in '{}' status. Must be 'confirmed'.",
                batch.status
            )));
        }
        ...
        self.repository
            .update_batch_status(batch_id, "closed")
            .await
    }
    ...
    pub async fn create_receipt(
        ...
        if let Some(bid) = batch_id {
            ...
            if batch.status != "draft" && batch.status != "confirmed" {
                return Err(AtlasError::WorkflowError(format!(
                    "Cannot add receipts to a batch in '{}' status",
                    batch.status
                )));
            }
        }
        ...
        if self
            .repository
            .get_receipt_by_number(org_id, receipt_number)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Receipt number '{receipt_number}' already exists"
            )));
        }

        info!(
            "Creating cash receipt '{}' for customer {}",
    ...
    pub async fn list_receipts(
        ...
        self.repository
            .list_receipts(org_id, batch_id, customer_id, status)
            .await
    }
    ...
    pub async fn identify_receipt(&self, receipt_id: Uuid) -> AtlasResult<CashReceipt> {
        ...
        if receipt.status != "unidentified" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot identify receipt in '{}' status. Must be 'unidentified'.",
                receipt.status
            )));
        }
        ...
        self.repository
            .update_receipt_status(receipt_id, "unapplied")
            .await
    }
    ...
    pub async fn apply_receipt(
        ...
        if receipt.status == "reversed" {
            return Err(AtlasError::WorkflowError(
                "Cannot apply a reversed receipt".into(),
            ));
        }
        if receipt.status == "unidentified" {
            return Err(AtlasError::WorkflowError(
                "Cannot apply an unidentified receipt. Identify it first.".into(),
            ));
        }
        ...
        if apply_amt.is_nan() || apply_amt <= 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Applied amount must be a positive number".into(),
            ));
        }
        ...
        info!(
            "Applying receipt '{}' to invoice {} (amount: {})",
            receipt.receipt_number, invoice_id, applied_amount
        );
        ...
        let total_applied_f = if total_applied_f.abs() < 0.005 {
            0.0
        } else {
            total_applied_f
        };
        ...
        // Avoid -0.00 from floating point
        let unapplied = if unapplied.abs() < 0.005 {
            0.0
        } else {
            unapplied
        };
        ...
        self.repository
            .update_receipt_status(receipt_id, new_status)
            .await?;
    ...
    pub async fn unapply_receipt(
        ...
        let application = self
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Application {application_id} not found"))
            })?;

        if application.status != "applied" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot unapply application in '{}' status. Must be 'applied'.",
                application.status
            )));
        }
        ...
        let receipt = self
            .repository
            .get_receipt(application.receipt_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Receipt {} not found", application.receipt_id))
            })?;

        info!(
            "Unapplying receipt '{}' from invoice {}",
            receipt.receipt_number, application.invoice_id
        );

        let app = self
            .repository
            .update_application_status(application_id, "reversed", reversed_by)
            .await?;
        ...
        self.repository
            .update_receipt_status(receipt.id, new_status)
            .await?;
    ...
    pub async fn list_applications(
        ...
        receipt_id: Uuid,
        status: Option<&str>,
    ) -> AtlasResult<Vec<ReceiptApplication>> {
        if let Some(s) = status {
            if !VALID_APPLICATION_STATUSES.contains(&s) {
                return Err(AtlasError::ValidationFailed(format!(
                    "Invalid status '{}'. Must be one of: {}",
                    s,
                    VALID_APPLICATION_STATUSES.join(", ")
                )));
            }
        }
    ...
    pub async fn reverse_receipt(
        ...
        if reason.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Reversal reason is required".into(),
            ));
        }
        ...
        if receipt.status == "reversed" {
            return Err(AtlasError::WorkflowError(
                "Receipt is already reversed".into(),
            ));
        }

        info!(
            "Reversing receipt '{}' (reason: {})",
            receipt.receipt_number, reason
        );

        // Reverse all active applications
        let applications = self
            .repository
            .list_applications(receipt_id, Some("applied"))
            .await?;
        for app in &applications {
            self.repository
                .update_application_status(app.id, "reversed", None)
                .await?;
        }

        self.repository
            .update_receipt_reversal(receipt_id, reason, None)
            .await?;
        let receipt = self
            .repository
    ...
    async fn recalculate_batch_totals(&self, batch_id: Uuid) -> AtlasResult<()> {
        let receipts = self
            .repository
            .list_receipts(Uuid::nil(), Some(batch_id), None, None)
            .await?;
        let total: f64 = receipts
            .iter()
        ...
        self.repository
            .update_batch_totals(batch_id, &format!("{total:.2}"), count)
            .await
    }
}
...
mod tests {
    ...
    impl CashReceiptRepository for MockRepo {
        ...
        async fn get_batch_by_number(
            ...
            org_id: Uuid,
            num: &str,
        ) -> AtlasResult<Option<ReceiptBatch>> {
            Ok(self
                .batches
                .lock()
                .unwrap()
                .iter()
                .find(|b| b.organization_id == org_id && b.batch_number == num)
                .cloned())
        }
        ...
        async fn create_receipt(
            ...
            let r = CashReceipt {
                ...
                deposit_date,
                clearance_status: Some("available".into()),
        ...
        async fn get_receipt_by_number(
            ...
            org_id: Uuid,
            num: &str,
        ) -> AtlasResult<Option<CashReceipt>> {
            Ok(self
                .receipts
                .lock()
                .unwrap()
                .iter()
                .find(|r| r.organization_id == org_id && r.receipt_number == num)
                .cloned())
        }
        ...
        async fn update_receipt_amounts(
            &self,
            id: Uuid,
            applied: &str,
            unapplied: &str,
        ...
        async fn update_receipt_reversal(
            &self,
            id: Uuid,
            reason: &str,
            _: Option<Uuid>,
        ...
        async fn create_application(
            ...
            let a = ReceiptApplication {
                ...
                applied_amount: applied.into(),
                discount_taken: discount.into(),
                status: "applied".into(),
        ...
        async fn update_application_status(
            ...
            let a = as_.iter_mut().find(|a| a.id == id).ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Application {} not found", id))
            })?;
        ...
        async fn get_dashboard(&self, _: Uuid) -> AtlasResult<CashReceiptDashboard> {
            Ok(CashReceiptDashboard {
                ...
                partially_applied_receipts: 0,
                reversed_receipts: 0,
                total_receipt_amount: "0.00".into(),
                total_applied_amount: "0.00".into(),
    ...
    async fn test_create_batch_empty_number_fails() {
        let r = eng()
            .create_batch(Uuid::new_v4(), "", "Batch", None, "bank", None, "USD", None)
            .await;
        assert!(r.is_err());
    }
    ...
    async fn test_create_batch_duplicate_number_fails() {
        ...
        let e = eng();
        let _ = e
            .create_batch(org, "DUP-001", "Batch 1", None, "bank", None, "USD", None)
            .await;
        let r = e
            .create_batch(org, "DUP-001", "Batch 2", None, "cash", None, "USD", None)
            .await;
    ...
    async fn test_identify_receipt() {
        ...
        // To test identification, we manually set status to "unidentified".
        {
            ...
            repo.update_receipt_status(r.id, "unidentified")
                .await
                .unwrap();
        }
    ...
    async fn test_list_receipts_invalid_status() {
        let r = eng()
            .list_receipts(Uuid::new_v4(), None, None, Some("unknown"))
            .await;
        assert!(r.is_err());
    }
}
