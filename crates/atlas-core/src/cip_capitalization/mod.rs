...
pub trait CipCapitalizationRepository: Send + Sync {
    ...
    async fn get_by_number(
        ...
        org_id: Uuid,
        number: &str,
    ) -> AtlasResult<Option<CipCapitalization>>;
    async fn list(&self, org_id: Uuid, status: Option<&str>)
        -> AtlasResult<Vec<CipCapitalization>>;
    ...
    async fn set_capitalized_asset(
        &self,
        id: Uuid,
        asset_id: Uuid,
        asset_number: &str,
...
#[allow(dead_code)]
pub struct PostgresCipCapitalizationRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
impl PostgresCipCapitalizationRepository {
    #[must_use]
...
impl CipCapitalizationRepository for PostgresCipCapitalizationRepository {
    ...
    async fn get(&self, _: Uuid) -> AtlasResult<Option<CipCapitalization>> {
        Ok(None)
    }
    async fn get_by_number(&self, _: Uuid, _: &str) -> AtlasResult<Option<CipCapitalization>> {
        Ok(None)
    }
    async fn list(&self, _: Uuid, _: Option<&str>) -> AtlasResult<Vec<CipCapitalization>> {
        Ok(vec![])
    }
    async fn update_status(
        &self,
    ...
    async fn mark_posted(&self, _: Uuid, _: Uuid) -> AtlasResult<CipCapitalization> {
        Err(AtlasError::EntityNotFound("Not found".into()))
    }
    async fn add_cost_line(
        &self,
    ...
    async fn list_cost_lines(&self, _: Uuid) -> AtlasResult<Vec<CipCostLine>> {
        Ok(vec![])
    }
    async fn toggle_cost_line(&self, _: Uuid, _: bool) -> AtlasResult<CipCostLine> {
        Err(AtlasError::EntityNotFound("Not found".into()))
    }
    async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<CipCapitalizationDashboard> {
        Ok(CipCapitalizationDashboard {
            ...
            draft_capitalizations: 0,
            capitalized_assets: 0,
            total_capitalized_cost: "0".into(),
            pending_review: 0,
        })
    }
}
...
impl CipCapitalizationEngine {
    ...
    pub async fn create(
        ...
        if let Some(months) = useful_life_months {
            if months <= 0 {
                return Err(AtlasError::ValidationFailed(
                    "Useful life months must be positive".into(),
                ));
            }
        }

        let cost_val: f64 = total_cip_cost
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid CIP cost".into()))?;
        let salvage_val: f64 = salvage_value
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid salvage value".into()))?;

        if cost_val <= 0.0 {
            return Err(AtlasError::ValidationFailed(
                "CIP cost must be positive".into(),
            ));
        }
        if salvage_val < 0.0 {
            return Err(AtlasError::ValidationFailed(
        ...
        if salvage_val >= cost_val {
            return Err(AtlasError::ValidationFailed(
                "Salvage value cannot exceed cost".into(),
            ));
        }
        ...
        info!(
            "Creating CIP capitalization {} for CIP asset {}",
            cap_number, cip_asset_id
        );
        ...
        self.repository
            .create(
                ...
                salvage_value,
                total_cip_cost,
                &capitalized_cost_val.to_string(),
                capitalization_date,
    ...
    pub async fn add_cost_line(
        ...
        let cap = self
            .repository
            .get(capitalization_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Capitalization {capitalization_id} not found"))
            })?;
        if cap.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot add cost lines to '{}' capitalization",
                cap.status
            )));
        }
        let cost: f64 = cost_amount
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid cost amount".into()))?;
        if cost <= 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Cost amount must be positive".into(),
            ));
        }
        self.repository
            .add_cost_line(
    ...
    pub async fn submit(&self, id: Uuid) -> AtlasResult<CipCapitalization> {
        let cap =
            self.repository.get(id).await?.ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Capitalization {id} not found"))
            })?;
        if cap.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
        ...
        info!(
            "Submitting CIP capitalization {}",
            cap.capitalization_number
        );
    ...
    pub async fn approve(&self, id: Uuid, approved_by: Uuid) -> AtlasResult<CipCapitalization> {
        ...
        self.repository
            .update_status(id, "approved", Some(approved_by))
            .await
    }
    ...
    pub async fn capitalize(
        ...
        if cap.status != "approved" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot capitalize from '{}' status",
                cap.status
            )));
        }
        if capitalized_asset_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Capitalized asset number is required".into(),
            ));
        }
        info!(
            "Capitalizing CIP asset as {} -> {}",
            cap.capitalization_number, capitalized_asset_number
        );
        let asset_id = Uuid::new_v4();
        let _ = self
            .repository
            .set_capitalized_asset(id, asset_id, capitalized_asset_number)
            .await?;
        let _ = self
            .repository
            .update_status(id, "capitalized", None)
            .await?;
...
mod tests {
    ...
    impl MockRepo {
        fn new() -> Self {
            Self {
                ...
                cost_lines: std::sync::Mutex::new(vec![]),
            }
        }
    }
    ...
    impl CipCapitalizationRepository for MockRepo {
        async fn create(
            ...
            let c = CipCapitalization {
                id: Uuid::new_v4(),
                organization_id: org_id,
                capitalization_number: number.into(),
                cip_asset_id,
                cip_asset_number: cip_asset_number.map(Into::into),
                cip_asset_name: cip_asset_name.map(Into::into),
                capitalized_asset_id: None,
                capitalized_asset_number: None,
                asset_type: asset_type.into(),
                category_id,
                category_code: category_code.map(Into::into),
                book_id,
                book_code: book_code.map(Into::into),
                depreciation_method: depreciation_method.map(Into::into),
                useful_life_months,
                salvage_value: salvage_value.into(),
                total_cip_cost: total_cip_cost.into(),
                capitalized_cost: capitalized_cost.into(),
                capitalization_date,
                date_placed_in_service,
                location: location.map(Into::into),
                department_id,
                department_name: department_name.map(Into::into),
                asset_account: asset_account.map(Into::into),
                cip_account: cip_account.map(Into::into),
        ...
        async fn get_by_number(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|c| c.organization_id == org_id && c.capitalization_number == number)
                .cloned())
        }
        async fn list(
            &self,
            ...
            Ok(self
                ...
                .iter()
                .filter(|c| {
                    c.organization_id == org_id && (status.is_none() || c.status == status.unwrap())
                })
                .cloned()
                .collect())
        }
        async fn update_status(
            ...
            if let Some(ab) = approved_by {
                c.approved_by = Some(ab);
            }
        ...
        async fn add_cost_line(
            ...
            let line = CipCostLine {
                ...
                source_id,
                source_number: source_number.map(Into::into),
                description: description.map(Into::into),
                cost_amount: cost_amount.into(),
                included,
                metadata: serde_json::json!({}),
                created_at: Utc::now(),
            };
        ...
        async fn list_cost_lines(&self, cap_id: Uuid) -> AtlasResult<Vec<CipCostLine>> {
            Ok(self
                ...
                .unwrap()
                .iter()
                .filter(|l| l.capitalization_id == cap_id)
                .cloned()
                .collect())
        }
        async fn toggle_cost_line(&self, id: Uuid, included: bool) -> AtlasResult<CipCostLine> {
        ...
        async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<CipCapitalizationDashboard> {
            ...
            Ok(CipCapitalizationDashboard {
                ...
                draft_capitalizations: org_items.iter().filter(|c| c.status == "draft").count()
                    as i32,
                capitalized_assets: org_items
                    .iter()
                    .filter(|c| c.status == "capitalized")
                    .count() as i32,
                total_capitalized_cost: org_items
                    .iter()
                    .filter(|c| c.status == "capitalized")
                    .map(|c| c.capitalized_cost.parse::<f64>().unwrap_or(0.0))
                    .sum::<f64>()
                    .to_string(),
    ...
    fn eng() -> CipCapitalizationEngine {
        CipCapitalizationEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_add_cost_line_zero_amount() {
        ...
        let c = e
            ...
            .unwrap();
        assert!(e
            .add_cost_line(c.id, c.organization_id, "ap_invoice", None, None, None, "0")
            .await
            .is_err());
    }
    ...
    async fn test_submit_approve_capitalize() {
        ...
        let capitalized = e
            .capitalize(c.id, "FA-BLDG-001", Some(Uuid::new_v4()))
            .await
            .unwrap();
