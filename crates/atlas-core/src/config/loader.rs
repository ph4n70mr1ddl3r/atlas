...
pub async fn load_from_yaml(path: &Path) -> Result<HashMap<String, ConfigValue>, String> {
    ...
    parse_yaml_value("", &yaml).map_err(|e| format!("Failed to parse config: {e}"))
}
...
pub async fn load_from_json(path: &Path) -> Result<HashMap<String, ConfigValue>, String> {
    ...
    flatten_json("", &json).map_err(|e| format!("Failed to parse config: {e}"))
}
...
mod tests {
    ...
    fn test_load_from_env() {
        ...
        assert!(
            matches!(config.get("db.port"), Some(ConfigValue::Number(n)) if (*n - 5432.0).abs() < f64::EPSILON)
        );
    }
    ...
    fn test_config_loader_merge() {
        let loader = ConfigLoader::new()
            .add_source(ConfigSource::Default, {
                ...
                m.insert(
                    "app.name".to_string(),
                    ConfigValue::String("Atlas".to_string()),
                );
        ...
        assert!(
            matches!(merged.get("app.port"), Some(ConfigValue::Number(n)) if (n - 9000.0).abs() < f64::EPSILON)
        );
    }
    ...
    async fn test_load_from_json() {
        ...
        assert!(
            matches!(config.get("app.port"), Some(ConfigValue::Number(n)) if (n - 8080.0).abs() < f64::EPSILON)
        );
        assert!(matches!(
