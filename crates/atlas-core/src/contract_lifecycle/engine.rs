...
const VALID_DELIVERABLE_TYPES: &[&str] = &["document", "product", "service", "report", "payment"];
...
impl ContractLifecycleEngine {
    ...
    pub async fn create_contract_type(
        ...
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Contract type name is required".into(),
            ));
        }
        ...
        if self
            .repository
            .get_contract_type_by_code(org_id, &code_upper)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Contract type '{code_upper}' already exists"
            )));
        }
    ...
    pub async fn create_clause(
        ...
        if title.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Clause title is required".into(),
            ));
        }
        if body.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Clause body is required".into(),
            ));
        }
        ...
        if self
            .repository
            .get_clause_by_code(org_id, &code_upper)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Clause '{code_upper}' already exists"
            )));
        }
    ...
    pub async fn create_template(
        ...
        if self
            .repository
            .get_template_by_code(org_id, &code_upper)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Template '{code_upper}' already exists"
            )));
        }
    ...
    pub async fn add_template_clause(
        ...
        self.repository
            .add_template_clause(template_id, clause_id, section, display_order, is_required)
            .await
    }

    pub async fn list_template_clauses(
        &self,
        template_id: Uuid,
    ...
    pub async fn create_contract(
        ...
        if title.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Contract title is required".into(),
            ));
        }
        validate_enum(contract_category, VALID_CATEGORIES, "contract_category")?;
        ...
        if total_value.parse::<f64>().is_err() {
            return Err(AtlasError::ValidationFailed(
                "Total value must be a valid number".into(),
            ));
        }

        if self
            .repository
            .get_contract_by_number(org_id, &cn_upper)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Contract '{cn_upper}' already exists"
            )));
        }
    ...
    pub async fn list_contracts(
        ...
        self.repository
            .list_contracts(org_id, status, category)
            .await
    }
    ...
    pub async fn transition_contract(
        ...
        self.repository
            .update_contract_status(id, new_status, user_id)
            .await
    }
    ...
    pub async fn list_contract_parties(
        &self,
        contract_id: Uuid,
    ) -> AtlasResult<Vec<ClmContractParty>> {
        ...
        let original_body = if let Some(cid) = clause_id {
            ...
            if clause.is_locked && clause.body != body {
                return Err(AtlasError::ValidationFailed(format!(
                    "Clause '{}' is locked and cannot be modified",
                    clause.code
                )));
            }
            if clause.body == body {
    ...
    pub async fn list_contract_clauses(
        &self,
        contract_id: Uuid,
    ...
    pub async fn create_milestone(
        ...
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Milestone name is required".into(),
            ));
        }
        if let Some(a) = amount {
            if a.parse::<f64>().is_err() {
                return Err(AtlasError::ValidationFailed(
                    "Milestone amount must be a valid number".into(),
                ));
            }
        }
    ...
    pub async fn complete_milestone(&self, id: Uuid) -> AtlasResult<ClmMilestone> {
        ...
        if milestone.status != "pending" && milestone.status != "in_progress" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot complete milestone in '{}' status",
                milestone.status
            )));
        }
        self.repository
            .update_milestone_status(id, "completed")
            .await
    }
    ...
    pub async fn create_deliverable(
        ...
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Deliverable name is required".into(),
            ));
        }
        if quantity.parse::<f64>().is_err() {
            return Err(AtlasError::ValidationFailed(
                "Quantity must be a valid number".into(),
            ));
        }
        ...
        if let Some(mid) = milestone_id {
            let _m =
                self.repository.get_milestone(mid).await?.ok_or_else(|| {
                    AtlasError::EntityNotFound(format!("Milestone {mid} not found"))
                })?;
        }
    ...
    pub async fn accept_deliverable(
        ...
        if d.status != "submitted" && d.status != "under_review" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot accept deliverable in '{}' status",
                d.status
            )));
        }
        self.repository
            .update_deliverable_status(id, "accepted", accepted_by)
            .await
    }
    ...
    pub async fn reject_deliverable(&self, id: Uuid) -> AtlasResult<ClmDeliverable> {
        ...
        if d.status != "submitted" && d.status != "under_review" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot reject deliverable in '{}' status",
                d.status
            )));
        }
        self.repository
            .update_deliverable_status(id, "rejected", None)
            .await
    }
    ...
    pub async fn create_amendment(
        ...
        if title.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Amendment title is required".into(),
            ));
        }
    ...
    pub async fn approve_amendment(
        ...
        self.repository
            .update_amendment_status(id, "approved", approved_by)
            .await
    }
    ...
    pub async fn reject_amendment(&self, id: Uuid) -> AtlasResult<ClmAmendment> {
        ...
        self.repository
            .update_amendment_status(id, "rejected", None)
            .await
    }
    ...
    pub async fn create_risk(
        ...
        if risk_description.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Risk description is required".into(),
            ));
        }
...
fn validate_transition(from: &str, to: &str) -> AtlasResult<()> {
    ...
    if !allowed.iter().any(|pair| pair[0] == from && pair[1] == to) {
        return Err(AtlasError::InvalidStateTransition(
            from.to_string(),
            to.to_string(),
        ));
    }
