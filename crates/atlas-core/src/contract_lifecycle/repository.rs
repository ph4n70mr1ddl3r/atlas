...
use atlas_shared::{
    AtlasError, AtlasResult, ClmAmendment, ClmClause, ClmContract, ClmContractClause,
    ClmContractParty, ClmContractType, ClmDashboard, ClmDeliverable, ClmMilestone, ClmRisk,
    ClmTemplate, ClmTemplateClause,
};
use sqlx::PgPool;
use sqlx::Row;
use uuid::Uuid;
...
pub trait ContractLifecycleRepository: Send + Sync {
    ...
    async fn get_contract_type_by_code(
        &self,
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<ClmContractType>>;
    ...
    async fn get_template_by_code(
        ...
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<ClmTemplate>>;
    ...
    async fn list_template_clauses(&self, template_id: Uuid)
        -> AtlasResult<Vec<ClmTemplateClause>>;
    ...
    async fn get_contract_by_number(
        ...
        org_id: Uuid,
        number: &str,
    ) -> AtlasResult<Option<ClmContract>>;
    ...
    async fn list_contract_clauses(&self, contract_id: Uuid)
        -> AtlasResult<Vec<ClmContractClause>>;
...
fn row_to_contract_type(row: &sqlx::postgres::PgRow) -> ClmContractType {
    ClmContractType {
        ...
        contract_category: row.try_get("contract_category").unwrap_or_default(),
        default_duration_days: row.try_get("default_duration_days").unwrap_or_default(),
        requires_approval: row.try_get("requires_approval").unwrap_or(true),
        is_auto_renew: row.try_get("is_auto_renew").unwrap_or(false),
        risk_scoring_enabled: row.try_get("risk_scoring_enabled").unwrap_or(false),
        status: row
            .try_get("status")
...
fn row_to_clause(row: &sqlx::postgres::PgRow) -> ClmClause {
    ClmClause {
        ...
        clause_category: row
            .try_get("clause_category")
            .unwrap_or_else(|_| "general".to_string()),
        applicability: row
            .try_get("applicability")
...
fn row_to_template(row: &sqlx::postgres::PgRow) -> ClmTemplate {
    ClmTemplate {
        ...
        name: row.try_get("name").unwrap_or_default(),
        description: row.try_get("description").unwrap_or_default(),
        contract_type_id: row.try_get("contract_type_id").unwrap_or_default(),
        ...
        is_standard: row.try_get("is_standard").unwrap_or(false),
        status: row
            .try_get("status")
            .unwrap_or_else(|_| "active".to_string()),
        metadata: row.try_get("metadata").unwrap_or(serde_json::json!({})),
...
fn row_to_template_clause(row: &sqlx::postgres::PgRow) -> ClmTemplateClause {
    ClmTemplateClause {
        ...
        clause_id: row.try_get("clause_id").unwrap_or_default(),
        section: row.try_get("section").unwrap_or_default(),
...
fn row_to_contract(row: &sqlx::postgres::PgRow) -> ClmContract {
    ClmContract {
        ...
        contract_type_id: row.try_get("contract_type_id").unwrap_or_default(),
        template_id: row.try_get("template_id").unwrap_or_default(),
        contract_category: row
            .try_get("contract_category")
            .unwrap_or_else(|_| "general".to_string()),
        currency: row
            .try_get("currency")
            .unwrap_or_else(|_| "USD".to_string()),
        total_value: row
            .try_get::<String, _>("total_value")
        ...
        risk_score: row.try_get("risk_score").unwrap_or_default(),
        risk_level: row.try_get("risk_level").unwrap_or_default(),
        parent_contract_id: row.try_get("parent_contract_id").unwrap_or_default(),
        renewal_type: row
            .try_get("renewal_type")
            .unwrap_or_else(|_| "none".to_string()),
        auto_renew_months: row.try_get("auto_renew_months").unwrap_or_default(),
        renewal_notice_days: row.try_get("renewal_notice_days").unwrap_or(30),
        metadata: row.try_get("metadata").unwrap_or(serde_json::json!({})),
        created_by: row.try_get("created_by").unwrap_or_default(),
...
fn row_to_party(row: &sqlx::postgres::PgRow) -> ClmContractParty {
    ClmContractParty {
        ...
        party_role: row
            .try_get("party_role")
            .unwrap_or_else(|_| "counterparty".to_string()),
        party_name: row.try_get("party_name").unwrap_or_default(),
        ...
        entity_reference: row.try_get("entity_reference").unwrap_or_default(),
...
fn row_to_contract_clause(row: &sqlx::postgres::PgRow) -> ClmContractClause {
    ClmContractClause {
        ...
        clause_type: row
            ...
            .unwrap_or_else(|_| "standard".to_string()),
        display_order: row.try_get("display_order").unwrap_or(0),
        is_modified: row.try_get("is_modified").unwrap_or(false),
        original_body: row.try_get("original_body").unwrap_or_default(),
        metadata: row.try_get("metadata").unwrap_or(serde_json::json!({})),
        created_at: row.try_get("created_at").unwrap_or(chrono::Utc::now()),
        updated_at: row.try_get("updated_at").unwrap_or(chrono::Utc::now()),
    }
...
fn row_to_milestone(row: &sqlx::postgres::PgRow) -> ClmMilestone {
    ClmMilestone {
        ...
        amount: Some(row.try_get::<String, _>("amount").unwrap_or_default()),
        currency: row
            .try_get("currency")
            .unwrap_or_else(|_| "USD".to_string()),
        status: row
        ...
        responsible_party_id: row.try_get("responsible_party_id").unwrap_or_default(),
        metadata: row.try_get("metadata").unwrap_or(serde_json::json!({})),
        created_at: row.try_get("created_at").unwrap_or(chrono::Utc::now()),
        updated_at: row.try_get("updated_at").unwrap_or(chrono::Utc::now()),
    }
...
fn row_to_deliverable(row: &sqlx::postgres::PgRow) -> ClmDeliverable {
    ClmDeliverable {
        ...
        organization_id: row.try_get("organization_id").unwrap_or_default(),
        contract_id: row.try_get("contract_id").unwrap_or_default(),
        milestone_id: row.try_get("milestone_id").unwrap_or_default(),
        name: row.try_get("name").unwrap_or_default(),
        description: row.try_get("description").unwrap_or_default(),
        deliverable_type: row
            .try_get("deliverable_type")
        ...
        unit_of_measure: row
            ...
            .unwrap_or_else(|_| "each".to_string()),
        due_date: row.try_get("due_date").unwrap_or_default(),
        completed_date: row.try_get("completed_date").unwrap_or_default(),
        acceptance_date: row.try_get("acceptance_date").unwrap_or_default(),
        amount: Some(row.try_get::<String, _>("amount").unwrap_or_default()),
        currency: row
            .try_get("currency")
            .unwrap_or_else(|_| "USD".to_string()),
        status: row
            .try_get("status")
            .unwrap_or_else(|_| "pending".to_string()),
        accepted_by: row.try_get("accepted_by").unwrap_or_default(),
        metadata: row.try_get("metadata").unwrap_or(serde_json::json!({})),
        created_at: row.try_get("created_at").unwrap_or(chrono::Utc::now()),
        updated_at: row.try_get("updated_at").unwrap_or(chrono::Utc::now()),
    }
...
fn row_to_amendment(row: &sqlx::postgres::PgRow) -> ClmAmendment {
    ClmAmendment {
        ...
        organization_id: row.try_get("organization_id").unwrap_or_default(),
        contract_id: row.try_get("contract_id").unwrap_or_default(),
        amendment_number: row.try_get("amendment_number").unwrap_or_default(),
        title: row.try_get("title").unwrap_or_default(),
        description: row.try_get("description").unwrap_or_default(),
        amendment_type: row
            .try_get("amendment_type")
            .unwrap_or_else(|_| "modification".to_string()),
        previous_value: row.try_get("previous_value").unwrap_or_default(),
...
fn row_to_risk(row: &sqlx::postgres::PgRow) -> ClmRisk {
    ClmRisk {
        ...
        organization_id: row.try_get("organization_id").unwrap_or_default(),
        contract_id: row.try_get("contract_id").unwrap_or_default(),
        risk_category: row.try_get("risk_category").unwrap_or_default(),
        risk_description: row.try_get("risk_description").unwrap_or_default(),
        probability: row
            .try_get("probability")
        ...
        impact: row
            .try_get("impact")
            .unwrap_or_else(|_| "medium".to_string()),
        mitigation_strategy: row.try_get("mitigation_strategy").unwrap_or_default(),
        residual_risk: row.try_get("residual_risk").unwrap_or_default(),
        owner_id: row.try_get("owner_id").unwrap_or_default(),
        status: row
            .try_get("status")
            .unwrap_or_else(|_| "identified".to_string()),
        metadata: row.try_get("metadata").unwrap_or(serde_json::json!({})),
        assessed_by: row.try_get("assessed_by").unwrap_or_default(),
        created_at: row.try_get("created_at").unwrap_or(chrono::Utc::now()),
        updated_at: row.try_get("updated_at").unwrap_or(chrono::Utc::now()),
    }
}
...
#[allow(dead_code)]
pub struct PostgresContractLifecycleRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
impl PostgresContractLifecycleRepository {
    #[must_use]
...
fn check_del(r: sqlx::postgres::PgQueryResult, l: &str, id: &str) -> AtlasResult<()> {
    if r.rows_affected() == 0 {
        Err(AtlasError::EntityNotFound(format!("{l} '{id}' not found")))
    } else {
        Ok(())
    }
}
...
impl ContractLifecycleRepository for PostgresContractLifecycleRepository {
    ...
    async fn get_contract_type(&self, id: Uuid) -> AtlasResult<Option<ClmContractType>> {
        Ok(
            sqlx::query("SELECT * FROM _atlas.clm_contract_types WHERE id=$1")
                .bind(id)
                .fetch_optional(&self.pool)
                .await?
                .as_ref()
                .map(row_to_contract_type),
        )
    }
    async fn get_contract_type_by_code(
        &self,
        ...
        Ok(sqlx::query(
            "SELECT * FROM _atlas.clm_contract_types WHERE organization_id=$1 AND code=$2",
        )
        .bind(org_id)
        .bind(code)
        .fetch_optional(&self.pool)
    ...
    async fn list_contract_types(&self, org_id: Uuid) -> AtlasResult<Vec<ClmContractType>> {
        Ok(sqlx::query("SELECT * FROM _atlas.clm_contract_types WHERE organization_id=$1 AND status='active' ORDER BY created_at DESC").bind(org_id).fetch_all(&self.pool).await?.iter().map(row_to_contract_type).collect())
    }
    async fn delete_contract_type(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        check_del(
            sqlx::query(
                "DELETE FROM _atlas.clm_contract_types WHERE organization_id=$1 AND code=$2",
            )
            .bind(org_id)
            .bind(code)
            .execute(&self.pool)
    ...
    async fn get_clause(&self, id: Uuid) -> AtlasResult<Option<ClmClause>> {
        Ok(sqlx::query("SELECT * FROM _atlas.clm_clauses WHERE id=$1")
            .bind(id)
            .fetch_optional(&self.pool)
            .await?
            .as_ref()
    ...
    async fn get_clause_by_code(&self, org_id: Uuid, code: &str) -> AtlasResult<Option<ClmClause>> {
        Ok(
            sqlx::query("SELECT * FROM _atlas.clm_clauses WHERE organization_id=$1 AND code=$2")
                .bind(org_id)
                .bind(code)
                .fetch_optional(&self.pool)
                .await?
    ...
    async fn list_clauses(
        ...
        let rows = if let Some(cat) = category {
            sqlx::query("SELECT * FROM _atlas.clm_clauses WHERE organization_id=$1 AND clause_category=$2 AND status='active' ORDER BY created_at DESC").bind(org_id).bind(cat).fetch_all(&self.pool).await?
        } else {
            sqlx::query("SELECT * FROM _atlas.clm_clauses WHERE organization_id=$1 AND status='active' ORDER BY created_at DESC").bind(org_id).fetch_all(&self.pool).await?
        };
        Ok(rows.iter().map(row_to_clause).collect())
    }
    async fn delete_clause(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        check_del(
            sqlx::query("DELETE FROM _atlas.clm_clauses WHERE organization_id=$1 AND code=$2")
                .bind(org_id)
                .bind(code)
                .execute(&self.pool)
                .await?,
    ...
    async fn get_template(&self, id: Uuid) -> AtlasResult<Option<ClmTemplate>> {
        Ok(
            sqlx::query("SELECT * FROM _atlas.clm_templates WHERE id=$1")
                .bind(id)
                .fetch_optional(&self.pool)
                .await?
                .as_ref()
    ...
    async fn get_template_by_code(
        ...
        Ok(
            sqlx::query("SELECT * FROM _atlas.clm_templates WHERE organization_id=$1 AND code=$2")
                .bind(org_id)
                .bind(code)
                .fetch_optional(&self.pool)
                .await?
    ...
    async fn list_templates(&self, org_id: Uuid) -> AtlasResult<Vec<ClmTemplate>> {
        Ok(sqlx::query("SELECT * FROM _atlas.clm_templates WHERE organization_id=$1 AND status='active' ORDER BY created_at DESC").bind(org_id).fetch_all(&self.pool).await?.iter().map(row_to_template).collect())
    }
    async fn delete_template(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        check_del(
            sqlx::query("DELETE FROM _atlas.clm_templates WHERE organization_id=$1 AND code=$2")
                .bind(org_id)
                .bind(code)
                .execute(&self.pool)
                .await?,
    ...
    async fn list_template_clauses(
        &self,
        template_id: Uuid,
    ) -> AtlasResult<Vec<ClmTemplateClause>> {
        Ok(sqlx::query(
            "SELECT * FROM _atlas.clm_template_clauses WHERE template_id=$1 ORDER BY display_order",
        )
        .bind(template_id)
        .fetch_all(&self.pool)
        .await?
        .iter()
        .map(row_to_template_clause)
        .collect())
    }
    async fn create_contract(
        &self,
    ...
    async fn get_contract(&self, id: Uuid) -> AtlasResult<Option<ClmContract>> {
        Ok(
            sqlx::query("SELECT * FROM _atlas.clm_contracts WHERE id=$1")
                .bind(id)
                .fetch_optional(&self.pool)
                .await?
                .as_ref()
    ...
    async fn get_contract_by_number(
        ...
        Ok(sqlx::query(
            "SELECT * FROM _atlas.clm_contracts WHERE organization_id=$1 AND contract_number=$2",
        )
        .bind(org_id)
        .bind(number)
        .fetch_optional(&self.pool)
    ...
    async fn delete_contract(&self, org_id: Uuid, number: &str) -> AtlasResult<()> {
        check_del(
            sqlx::query(
                "DELETE FROM _atlas.clm_contracts WHERE organization_id=$1 AND contract_number=$2",
            )
            .bind(org_id)
            .bind(number)
            .execute(&self.pool)
    ...
    async fn list_contract_parties(&self, contract_id: Uuid) -> AtlasResult<Vec<ClmContractParty>> {
        Ok(sqlx::query(
            "SELECT * FROM _atlas.clm_contract_parties WHERE contract_id=$1 ORDER BY created_at",
        )
        .bind(contract_id)
        .fetch_all(&self.pool)
        .await?
    ...
    async fn remove_contract_party(&self, id: Uuid) -> AtlasResult<()> {
        check_del(
            sqlx::query("DELETE FROM _atlas.clm_contract_parties WHERE id=$1")
                .bind(id)
                .execute(&self.pool)
                .await?,
            "Party",
    ...
    async fn list_contract_clauses(
        &self,
        contract_id: Uuid,
    ) -> AtlasResult<Vec<ClmContractClause>> {
        Ok(sqlx::query(
            "SELECT * FROM _atlas.clm_contract_clauses WHERE contract_id=$1 ORDER BY display_order",
        )
        .bind(contract_id)
        .fetch_all(&self.pool)
        .await?
        .iter()
        .map(row_to_contract_clause)
        .collect())
    }
    async fn remove_contract_clause(&self, id: Uuid) -> AtlasResult<()> {
        check_del(
            sqlx::query("DELETE FROM _atlas.clm_contract_clauses WHERE id=$1")
                .bind(id)
                .execute(&self.pool)
                .await?,
            "Clause",
    ...
    async fn get_milestone(&self, id: Uuid) -> AtlasResult<Option<ClmMilestone>> {
        Ok(
            sqlx::query("SELECT * FROM _atlas.clm_contract_milestones WHERE id=$1")
                .bind(id)
                .fetch_optional(&self.pool)
                .await?
                .as_ref()
    ...
    async fn list_milestones(&self, contract_id: Uuid) -> AtlasResult<Vec<ClmMilestone>> {
        Ok(sqlx::query("SELECT * FROM _atlas.clm_contract_milestones WHERE contract_id=$1 ORDER BY due_date NULLS LAST").bind(contract_id).fetch_all(&self.pool).await?.iter().map(row_to_milestone).collect())
    }
    async fn update_milestone_status(&self, id: Uuid, status: &str) -> AtlasResult<ClmMilestone> {
    ...
    async fn delete_milestone(&self, id: Uuid) -> AtlasResult<()> {
        check_del(
            sqlx::query("DELETE FROM _atlas.clm_contract_milestones WHERE id=$1")
                .bind(id)
                .execute(&self.pool)
                .await?,
            "Milestone",
    ...
    async fn get_deliverable(&self, id: Uuid) -> AtlasResult<Option<ClmDeliverable>> {
        Ok(
            sqlx::query("SELECT * FROM _atlas.clm_contract_deliverables WHERE id=$1")
                .bind(id)
                .fetch_optional(&self.pool)
                .await?
                .as_ref()
                .map(row_to_deliverable),
        )
    }
    async fn list_deliverables(&self, contract_id: Uuid) -> AtlasResult<Vec<ClmDeliverable>> {
        Ok(sqlx::query("SELECT * FROM _atlas.clm_contract_deliverables WHERE contract_id=$1 ORDER BY due_date NULLS LAST").bind(contract_id).fetch_all(&self.pool).await?.iter().map(row_to_deliverable).collect())
    }
    async fn update_deliverable_status(
        &self,
        id: Uuid,
        ...
        let acceptance = if status == "accepted" {
            Some(chrono::Utc::now().date_naive())
        } else {
            None
        };
    ...
    async fn delete_deliverable(&self, id: Uuid) -> AtlasResult<()> {
        check_del(
            sqlx::query("DELETE FROM _atlas.clm_contract_deliverables WHERE id=$1")
                .bind(id)
                .execute(&self.pool)
                .await?,
            "Deliverable",
    ...
    async fn get_amendment(&self, id: Uuid) -> AtlasResult<Option<ClmAmendment>> {
        Ok(
            sqlx::query("SELECT * FROM _atlas.clm_contract_amendments WHERE id=$1")
                .bind(id)
                .fetch_optional(&self.pool)
                .await?
                .as_ref()
                .map(row_to_amendment),
        )
    }
    async fn list_amendments(&self, contract_id: Uuid) -> AtlasResult<Vec<ClmAmendment>> {
        Ok(sqlx::query("SELECT * FROM _atlas.clm_contract_amendments WHERE contract_id=$1 ORDER BY created_at DESC").bind(contract_id).fetch_all(&self.pool).await?.iter().map(row_to_amendment).collect())
    }
    async fn update_amendment_status(
        &self,
        id: Uuid,
    ...
    async fn delete_amendment(&self, id: Uuid) -> AtlasResult<()> {
        check_del(
            sqlx::query("DELETE FROM _atlas.clm_contract_amendments WHERE id=$1")
                .bind(id)
                .execute(&self.pool)
                .await?,
            "Amendment",
    ...
    async fn list_risks(&self, contract_id: Uuid) -> AtlasResult<Vec<ClmRisk>> {
        Ok(sqlx::query(
            "SELECT * FROM _atlas.clm_contract_risks WHERE contract_id=$1 ORDER BY created_at DESC",
        )
        .bind(contract_id)
        .fetch_all(&self.pool)
        .await?
    ...
    async fn delete_risk(&self, id: Uuid) -> AtlasResult<()> {
        check_del(
            sqlx::query("DELETE FROM _atlas.clm_contract_risks WHERE id=$1")
                .bind(id)
                .execute(&self.pool)
                .await?,
            "Risk",
    ...
    async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<ClmDashboard> {
        let contracts = self
            .list_contracts(org_id, None, None)
            .await
            .unwrap_or_default();
        ...
        let high_risk = contracts
            .iter()
            .filter(|c| {
                c.risk_level.as_deref() == Some("high")
                    || c.risk_level.as_deref() == Some("critical")
            })
            .count() as i32;
        let total_value: f64 = contracts
            .iter()
            .filter_map(|c| c.total_value.parse::<f64>().ok())
            .sum();
        let mut by_status: std::collections::HashMap<String, i32> =
            std::collections::HashMap::new();
        let mut by_category: std::collections::HashMap<String, i32> =
            std::collections::HashMap::new();
        for c in &contracts {
            *by_status.entry(c.status.clone()).or_insert(0) += 1;
            *by_category.entry(c.contract_category.clone()).or_insert(0) += 1;
        }
        let today = chrono::Utc::now().date_naive();
        let thirty = today + chrono::Duration::days(30);
        let expiring = contracts
            .iter()
            .filter(|c| {
                c.end_date
                    .is_some_and(|d| d <= thirty && d >= today && c.status == "active")
            })
            .count() as i32;
        ...
        Ok(ClmDashboard {
            ...
            total_contract_value: format!("{total_value:.2}"),
            contracts_by_category: serde_json::to_value(&by_category)
                .unwrap_or(serde_json::json!({})),
            contracts_by_status: serde_json::to_value(&by_status).unwrap_or(serde_json::json!({})),
            high_risk_contracts: high_risk,
            pending_milestones: 0,
            pending_deliverables: 0,
            pending_amendments: 0,
            recent_contracts: serde_json::json!(recent),
        })
    }
}
