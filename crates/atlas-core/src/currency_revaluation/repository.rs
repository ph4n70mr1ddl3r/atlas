...
use atlas_shared::{
    AtlasError, AtlasResult, CurrencyRevaluationAccount, CurrencyRevaluationDashboardSummary,
    CurrencyRevaluationDefinition, CurrencyRevaluationLine, CurrencyRevaluationRun,
};
use sqlx::PgPool;
use sqlx::Row;
use uuid::Uuid;
...
pub trait CurrencyRevaluationRepository: Send + Sync {
    ...
    async fn create_definition(
        ...
        include_subledger: bool,
        auto_reverse: bool,
        reversal_period_offset: i32,
    ...
    async fn update_definition_active(
        &self,
        id: Uuid,
        is_active: bool,
    ) -> AtlasResult<CurrencyRevaluationDefinition>;
    ...
    async fn list_accounts_by_definition(
        &self,
        definition_id: Uuid,
    ...
    async fn create_run_line(
        ...
        original_exchange_rate: &str,
        original_base_amount: &str,
        revalued_exchange_rate: &str,
        revalued_base_amount: &str,
...
impl CurrencyRevaluationRepository for PostgresCurrencyRevaluationRepository {
    ...
    async fn create_definition(
        ...
        .bind(name)
        .bind(description)
        .bind(revaluation_type)
        .bind(currency_code)
        .bind(rate_type)
        .bind(gain_account_code)
        .bind(loss_account_code)
        .bind(unrealized_gain_account_code)
        .bind(unrealized_loss_account_code)
        .bind(account_range_from)
        .bind(account_range_to)
        .bind(include_subledger)
        .bind(auto_reverse)
        .bind(reversal_period_offset)
        .bind(effective_from)
        .bind(effective_to)
        .bind(created_by)
        .fetch_one(&self.pool)
    ...
    async fn get_definition_by_id(
        ...
        let row =
            sqlx::query("SELECT * FROM _atlas.currency_revaluation_definitions WHERE id = $1")
                .bind(id)
    ...
    async fn create_run_line(
        ...
        .bind(original_exchange_rate)
        .bind(original_base_amount)
        .bind(revalued_exchange_rate)
        .bind(revalued_base_amount)
    ...
    async fn get_dashboard_summary(
        ...
        let total_gain: serde_json::Value = run_row
            .try_get("total_gain")
            .unwrap_or(serde_json::json!("0"));
        let total_loss: serde_json::Value = run_row
            .try_get("total_loss")
            .unwrap_or(serde_json::json!("0"));
