...
use super::{
    AtlasError, AtlasResult, CustomerDeposit, CustomerDepositDashboard, CustomerDepositRepository,
    DepositApplication,
};
use std::sync::Arc;
use tracing::info;
...
impl CustomerDepositEngine {
    ...
    pub async fn create_deposit(
        ...
        if deposit_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Deposit number is required".into(),
            ));
        }
        ...
        if let Some(exp) = expiration_date {
            if exp < deposit_date {
                return Err(AtlasError::ValidationFailed(
                    "Expiration date must be after deposit date".into(),
                ));
            }
        }
        if self
            .repository
            .get_deposit_by_number(org_id, deposit_number)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Deposit number '{deposit_number}' already exists"
            )));
        }

        info!(
            "Creating customer deposit {} for customer {} ({})",
            deposit_number, customer_name, customer_id
        );
    ...
    pub async fn get_deposit_by_number(
        &self,
        org_id: Uuid,
        number: &str,
    ...
    pub async fn list_deposits(
        ...
        self.repository
            .list_deposits(org_id, status, customer_id)
            .await
    }
    ...
    pub async fn receive_deposit(
        ...
        if dep.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot receive deposit in '{}' status. Must be 'draft'.",
                dep.status
            )));
        }

        info!(
            "Recording receipt for customer deposit {}",
            dep.deposit_number
        );
        self.repository
            .update_receipt_info(deposit_id, receipt_reference, received_by)
            .await?;
        self.repository
            .update_deposit_status(deposit_id, "received")
            .await
    }
    ...
    pub async fn refund_deposit(
        ...
        if dep.status != "received" && dep.status != "partially_applied" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot refund deposit in '{}' status.",
                dep.status
            )));
        }
        ...
        if unapplied <= 0.01 {
            return Err(AtlasError::ValidationFailed(
                "No unapplied amount to refund".into(),
            ));
        }

        info!("Refunding customer deposit {}", dep.deposit_number);
        self.repository
            .update_refund_info(deposit_id, refund_reference, refunded_by)
            .await?;
        self.repository
            .update_deposit_status(deposit_id, "refunded")
            .await
    }
    ...
    pub async fn cancel_deposit(
        ...
        if dep.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot cancel deposit in '{}' status. Only 'draft' allowed.",
                dep.status
            )));
        }
        ...
        self.repository
            .update_deposit_status(deposit_id, "cancelled")
            .await
    }
    ...
    pub async fn apply_to_invoice(
        ...
        if dep.status != "received" && dep.status != "partially_applied" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot apply deposit in '{}' status. Must be 'received' or 'partially_applied'.",
                dep.status
            )));
        }
        ...
        if amount > unapplied + 0.01 {
            return Err(AtlasError::ValidationFailed(format!(
                "Applied amount {amount} exceeds unapplied amount {unapplied}"
            )));
        }

        info!(
            "Applying deposit {} to invoice (amount: {})",
            dep.deposit_number, applied_amount
        );
        ...
        let applications = self
            .repository
            .list_applications_by_deposit(deposit_id)
            .await?;
        let total_applied: f64 = applications
            .iter()
    ...
    pub async fn unapply_application(
        &self,
        application_id: Uuid,
        ...
        info!(
            "Unapplying deposit application for deposit {}",
            app.deposit_number.as_deref().unwrap_or("?")
        );

        let updated = self
            .repository
            .update_application_status(application_id, "reversed", reversed_by, reason)
            .await?;

        // Recalculate deposit amounts
        let applications = self
            .repository
            .list_applications_by_deposit(app.deposit_id)
            .await?;
        let total_applied: f64 = applications
            .iter()
            .filter(|a| a.status == "applied")
        ...
        if let Some(dep) = self.repository.get_deposit(app.deposit_id).await? {
            let deposit_amt: f64 = dep.deposit_amount.parse().unwrap_or(0.0);
            let new_unapplied = deposit_amt - total_applied;
            let new_status = if total_applied.abs() < 0.01 {
                "received"
            } else {
                "partially_applied"
            };
    ...
    pub async fn list_applications_by_deposit(
        &self,
        deposit_id: Uuid,
    ...
    pub async fn list_applications_by_invoice(
        &self,
        invoice_id: Uuid,
    ) -> AtlasResult<Vec<DepositApplication>> {
        self.repository
            .list_applications_by_invoice(invoice_id)
            .await
    }
...
mod tests {
    ...
    impl MockRepo {
        fn new() -> Self {
            MockRepo {
                deposits: std::sync::Mutex::new(vec![]),
    ...
    impl CustomerDepositRepository for MockRepo {
        async fn create_deposit(
            ...
            let d = CustomerDeposit {
                ...
                deposit_number: dn.into(),
                customer_id: cid,
                customer_name: cn.into(),
                customer_site_id: csi,
                description: desc.map(Into::into),
                status: "draft".into(),
                currency_code: cc.into(),
                deposit_amount: amt.into(),
                applied_amount: "0".into(),
                unapplied_amount: amt.into(),
                exchange_rate: er.map(Into::into),
                deposit_account_code: dac.map(Into::into),
                receivable_account_code: rac.map(Into::into),
                deposit_date: dd,
                receipt_date: None,
        ...
        async fn get_deposit_by_number(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|d| d.organization_id == org_id && d.deposit_number == num)
                .cloned())
        }
        ...
        async fn create_application(
            ...
            let app = DepositApplication {
                ...
                organization_id: org_id,
                deposit_id: did,
                deposit_number: dn.map(Into::into),
                invoice_id: iid,
                invoice_number: in_.map(Into::into),
        ...
        async fn list_applications_by_deposit(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .filter(|a| a.deposit_id == deposit_id)
                .cloned()
                .collect())
        }
        ...
        async fn update_application_status(
            ...
            let app = apps.iter_mut().find(|a| a.id == id).ok_or_else(|| {
    ...
    fn eng() -> CustomerDepositEngine {
        CustomerDepositEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_receive_deposit_valid() {
        ...
        let d = e
            ...
            .unwrap();
        let received = e
            .receive_deposit(d.id, Some("RCT-001"), None)
            .await
            .unwrap();
    ...
    async fn test_cancel_draft_valid() {
        ...
        let d = e
            ...
            .unwrap();
        let cancelled = e
            .cancel_deposit(d.id, Some("Order cancelled"))
            .await
            .unwrap();
    ...
    async fn test_list_deposits_valid() {
        let r = eng()
            .list_deposits(Uuid::new_v4(), Some("draft"), None)
            .await;
