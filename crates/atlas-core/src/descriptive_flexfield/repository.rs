...
pub trait DescriptiveFlexfieldRepository: Send + Sync {
    ...
    async fn get_context_by_code(
        &self,
        flexfield_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<FlexfieldContext>>;
    async fn update_context_enabled(
        &self,
        id: Uuid,
        is_enabled: bool,
    ) -> AtlasResult<FlexfieldContext>;
    ...
    async fn list_segments_by_context(
        &self,
        context_id: Uuid,
    ) -> AtlasResult<Vec<FlexfieldSegment>>;
    async fn list_segments_by_flexfield(
        &self,
        flexfield_id: Uuid,
    ) -> AtlasResult<Vec<FlexfieldSegment>>;
    async fn get_segment_by_code(
        ...
        context_id: Uuid,
        segment_code: &str,
    ) -> AtlasResult<Option<FlexfieldSegment>>;
    ...
    async fn list_value_set_entries(
        &self,
        value_set_id: Uuid,
        parent_value: Option<&str>,
    ...
    async fn delete_flexfield_data(&self, entity_name: &str, entity_id: Uuid) -> AtlasResult<()>;
...
impl DescriptiveFlexfieldRepository for PostgresDescriptiveFlexfieldRepository {
    ...
    async fn get_flexfield(
        ...
        let row =
            sqlx::query("SELECT * FROM _atlas.dff_flexfields WHERE organization_id=$1 AND code=$2")
                .bind(org_id)
                .bind(code)
    ...
    async fn get_flexfield_by_id(&self, id: Uuid) -> AtlasResult<Option<DescriptiveFlexfield>> {
        let row = sqlx::query("SELECT * FROM _atlas.dff_flexfields WHERE id=$1")
            .bind(id)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| AtlasError::DatabaseError(e.to_string()))?;
    ...
    async fn delete_flexfield(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        sqlx::query("DELETE FROM _atlas.dff_flexfields WHERE organization_id=$1 AND code=$2")
            .bind(org_id)
            .bind(code)
            .execute(&self.pool)
            .await
    ...
    async fn list_contexts(&self, flexfield_id: Uuid) -> AtlasResult<Vec<FlexfieldContext>> {
        let rows =
            sqlx::query("SELECT * FROM _atlas.dff_contexts WHERE flexfield_id=$1 ORDER BY code")
                .bind(flexfield_id)
                .fetch_all(&self.pool)
                .await
                .map_err(|e| AtlasError::DatabaseError(e.to_string()))?;
    ...
    async fn get_context_by_code(
        ...
        let row =
            sqlx::query("SELECT * FROM _atlas.dff_contexts WHERE flexfield_id=$1 AND code=$2")
                .bind(flexfield_id)
                .bind(code)
                .fetch_optional(&self.pool)
                .await
    ...
    async fn create_value_set(
        ...
        .bind(min_length)
        .bind(format_mask)
        .bind(table_validation)
        .bind(independent_values)
        .bind(parent_value_set_code)
        .bind(created_by)
        .fetch_one(&self.pool)
        .await
        .map_err(|e| AtlasError::DatabaseError(e.to_string()))?;
    ...
    async fn get_value_set(
        ...
        let row =
            sqlx::query("SELECT * FROM _atlas.dff_value_sets WHERE organization_id=$1 AND code=$2")
                .bind(org_id)
                .bind(code)
                .fetch_optional(&self.pool)
                .await
    ...
    async fn get_value_set_by_id(&self, id: Uuid) -> AtlasResult<Option<FlexfieldValueSet>> {
        let row = sqlx::query("SELECT * FROM _atlas.dff_value_sets WHERE id=$1")
            .bind(id)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| AtlasError::DatabaseError(e.to_string()))?;
    ...
    async fn delete_value_set(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        sqlx::query("DELETE FROM _atlas.dff_value_sets WHERE organization_id=$1 AND code=$2")
            .bind(org_id)
            .bind(code)
            .execute(&self.pool)
            .await
    ...
    async fn get_flexfield_data(
        ...
        let rows = if context_code.is_some() {
            ...
            sqlx::query("SELECT * FROM _atlas.dff_data WHERE entity_name=$1 AND entity_id=$2")
                .bind(entity_name)
                .bind(entity_id)
                .fetch_all(&self.pool)
                .await
    ...
    async fn delete_flexfield_data(&self, entity_name: &str, entity_id: Uuid) -> AtlasResult<()> {
        sqlx::query("DELETE FROM _atlas.dff_data WHERE entity_name=$1 AND entity_id=$2")
            .bind(entity_name)
            .bind(entity_id)
            .execute(&self.pool)
            .await
