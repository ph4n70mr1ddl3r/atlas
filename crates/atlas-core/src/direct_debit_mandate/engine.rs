...
use super::{
    AtlasError, AtlasResult, DirectDebitDashboard, DirectDebitMandate,
    DirectDebitMandateRepository, MandateCollection,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;

const VALID_MANDATE_TYPES: &[&str] = &[
    "core",
    "b2b",
...
impl DirectDebitMandateEngine {
    ...
    pub async fn create_mandate(
        ...
        if mandate_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Mandate number is required".into(),
            ));
        }
        if !VALID_MANDATE_TYPES.contains(&mandate_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid mandate type '{}'. Must be one of: {}",
                mandate_type,
                VALID_MANDATE_TYPES.join(", ")
            )));
        }
        ...
        if bank_account_number.is_none() || bank_account_number.unwrap().is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Bank account number is required".into(),
            ));
        }
        if bank_code.is_none() || bank_code.unwrap().is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Bank code (BIC/sort code) is required".into(),
            ));
        }
        if let Some(max_amt) = max_collection_amount {
            ...
            if !val.is_nan() && val < 0.0 {
                return Err(AtlasError::ValidationFailed(
                    "Max collection amount cannot be negative".into(),
                ));
            }
        }
        if let Some(exp) = expiry_date {
            if exp <= mandate_date {
                return Err(AtlasError::ValidationFailed(
                    "Expiry date must be after mandate date".into(),
                ));
            }
        }
        ...
        if self
            .repository
            .get_mandate_by_number(org_id, mandate_number)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Mandate number '{mandate_number}' already exists"
            )));
        }

        info!(
            "Creating direct debit mandate '{}' for customer {}",
            mandate_number, customer_id
        );
        self.repository
            .create_mandate(
                org_id,
        ...
        if let Some(s) = status {
            if !VALID_MANDATE_STATUSES.contains(&s) {
                return Err(AtlasError::ValidationFailed(format!(
                    ...
                    s,
                    VALID_MANDATE_STATUSES.join(", ")
                )));
            }
        }
        self.repository
            .list_mandates(org_id, customer_id, status)
            .await
    }
    ...
    pub async fn activate_mandate(&self, mandate_id: Uuid) -> AtlasResult<DirectDebitMandate> {
        ...
        if mandate.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot activate mandate in '{}' status. Must be 'draft'.",
                mandate.status
            )));
        }

        info!(
            "Activating direct debit mandate '{}'",
            mandate.mandate_number
        );
        let today = chrono::Utc::now().date_naive();
        self.repository
            .update_mandate_activation(mandate_id, today)
            .await?;
        self.repository
            .update_mandate_status(mandate_id, "active", None)
            .await
    }
    ...
    pub async fn cancel_mandate(
        ...
        {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot cancel mandate in '{}' status.",
                mandate.status
            )));
        }

        info!(
            "Cancelling direct debit mandate '{}' (reason: {})",
            mandate.mandate_number, reason
        );
        self.repository
            .update_mandate_status(mandate_id, "cancelled", Some(reason))
            .await
    }
    ...
    pub async fn revoke_mandate(
        ...
        {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot revoke mandate in '{}' status.",
                mandate.status
            )));
        }

        info!(
            "Revoking direct debit mandate '{}' (reason: {})",
            mandate.mandate_number, reason
        );
        self.repository
            .update_mandate_status(mandate_id, "revoked", Some(reason))
            .await
    }

    /// Set mandate expiry
    pub async fn expire_mandate(
        &self,
        mandate_id: Uuid,
        ...
        info!(
            "Setting expiry for mandate '{}' to {}",
            mandate.mandate_number, expiry_date
        );
        self.repository
            .update_mandate_expiry(mandate_id, expiry_date)
            .await
    }
    ...
    pub async fn create_collection(
        ...
        if collection_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Collection number is required".into(),
            ));
        }
        if !VALID_COLLECTION_TYPES.contains(&collection_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid collection type '{}'. Must be one of: {}",
                collection_type,
        ...
        if mandate.status != "active" && mandate.status != "used" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot create collection for mandate in '{}' status. Must be 'active' or 'used'.",
                mandate.status
            )));
        }
        ...
        if mandate.next_collection_type.as_deref() == Some("first")
        ...
        info!(
            "Creating collection '{}' for mandate '{}' (amount: {})",
            collection_number, mandate.mandate_number, amount
        );
        self.repository
            .create_collection(
                org_id,
    ...
    pub async fn list_collections(
        ...
        mandate_id: Uuid,
        status: Option<&str>,
    ) -> AtlasResult<Vec<MandateCollection>> {
        if let Some(s) = status {
            if !VALID_COLLECTION_STATUSES.contains(&s) {
                return Err(AtlasError::ValidationFailed(format!(
                    "Invalid status '{}'. Must be one of: {}",
                    s,
                    VALID_COLLECTION_STATUSES.join(", ")
                )));
            }
        }
    ...
    pub async fn submit_collection(&self, collection_id: Uuid) -> AtlasResult<MandateCollection> {
        let collection = self
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Collection {collection_id} not found"))
            })?;

        if collection.status != "pending" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot submit collection in '{}' status. Must be 'pending'.",
                collection.status
            )));
        }

        info!(
            "Submitting collection '{}' to bank",
            collection.collection_number
        );
        let today = chrono::Utc::now().date_naive();
        let c = self
            .repository
            .update_collection_status(collection_id, "submitted", None)
            .await?;
    ...
    pub async fn complete_collection(
        ...
        if collection.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot complete collection in '{}' status. Must be 'submitted'.",
                collection.status
            )));
        }
        ...
        let c = self
            .repository
            .update_collection_status(collection_id, "completed", bank_reference)
            .await?;

        // Update mandate stats
        let today = chrono::Utc::now().date_naive();
        let total_collected = self
            .repository
            .sum_collected_for_mandate(collection.mandate_id)
            .await?;
        let count = self
            .repository
            .count_collections_for_mandate(collection.mandate_id)
            .await?;
        let total_f: f64 = total_collected.parse().unwrap_or(0.0);
        self.repository
            .update_mandate_collection_stats(
        ...
        if let Some(m) = mandate {
            if m.status == "active" {
                self.repository
                    .update_mandate_status(collection.mandate_id, "used", None)
                    .await?;
            }
        }
    ...
    pub async fn fail_collection(
        ...
        if collection.status != "submitted" && collection.status != "pending" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot fail collection in '{}' status. Must be 'pending' or 'submitted'.",
                collection.status
            )));
        }

        info!(
            "Failing collection '{}' (reason: {})",
            collection.collection_number, reason_code
        );
        self.repository
            .update_collection_return(collection_id, reason_code, reason_text)
            .await?;
        self.repository
            .update_collection_status(collection_id, "failed", None)
            .await
    }
    ...
    pub async fn return_collection(
        ...
        if collection.status != "completed" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot return collection in '{}' status. Must be 'completed'.",
                collection.status
            )));
        }

        info!(
            "Returning collection '{}' (reason: {})",
            collection.collection_number, reason_code
        );
        self.repository
            .update_collection_return(collection_id, reason_code, reason_text)
            .await?;
        self.repository
            .update_collection_status(collection_id, "returned", None)
            .await
    }
    ...
    pub async fn reverse_collection(
        ...
        if collection.status != "completed" && collection.status != "returned" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot reverse collection in '{}' status. Must be 'completed' or 'returned'.",
                collection.status
            )));
        }

        info!(
            "Reversing collection '{}' (reason: {})",
            collection.collection_number, reason_code
        );
        self.repository
            .update_collection_return(collection_id, reason_code, reason_text)
            .await?;
        self.repository
            .update_collection_status(collection_id, "reversed", None)
            .await
    }
...
mod tests {
    ...
    impl DirectDebitMandateRepository for MockRepo {
        async fn create_mandate(
            ...
            customer_id: Uuid,
            customer_name: Option<&str>,
            _customer_account_number: Option<&str>,
            mandate_type: &str,
            _bank_account_holder: Option<&str>,
            bank_account_number: Option<&str>,
            bank_account_number_type: Option<&str>,
            bank_code: Option<&str>,
            bank_code_type: Option<&str>,
            _bank_name: Option<&str>,
            _bank_branch: Option<&str>,
            _creditor_scheme_id: Option<&str>,
            _creditor_name: Option<&str>,
            _mandate_reference: Option<&str>,
            mandate_date: chrono::NaiveDate,
            expiry_date: Option<chrono::NaiveDate>,
            max_collection_amount: Option<&str>,
            currency_code: &str,
            _authorization_reference: Option<&str>,
            _authorization_method: Option<&str>,
            _notes: Option<&str>,
        ...
        async fn get_mandate_by_number(
            &self,
            org_id: Uuid,
            num: &str,
        ) -> AtlasResult<Option<DirectDebitMandate>> {
            Ok(self
                .mandates
                .lock()
                .unwrap()
                .iter()
                .find(|m| m.organization_id == org_id && m.mandate_number == num)
                .cloned())
        }
        ...
        async fn update_mandate_status(
            &self,
            id: Uuid,
            status: &str,
            reason: Option<&str>,
        ...
        async fn update_mandate_activation(
            &self,
            id: Uuid,
            activation_date: chrono::NaiveDate,
        ) -> AtlasResult<DirectDebitMandate> {
            let mut ms = self.mandates.lock().unwrap();
            let m = ms
                .iter_mut()
        ...
        async fn update_mandate_collection_stats(
            &self,
            id: Uuid,
            count: i32,
            total: &str,
        ...
        async fn update_mandate_expiry(
            &self,
            id: Uuid,
            expiry_date: chrono::NaiveDate,
        ) -> AtlasResult<DirectDebitMandate> {
        ...
        async fn update_collection_status(
            ...
            let c = cs.iter_mut().find(|c| c.id == id).ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Collection {} not found", id))
            })?;
        ...
        async fn update_collection_return(
            &self,
            id: Uuid,
            reason_code: &str,
            reason_text: Option<&str>,
        ...
        async fn get_dashboard(&self, _org_id: Uuid) -> AtlasResult<DirectDebitDashboard> {
            Ok(DirectDebitDashboard {
                ...
                total_collected_amount: "0.00".into(),
    ...
    async fn test_revoke_mandate() {
        ...
        let m = e
            ...
            .unwrap();
        let m = e
            .revoke_mandate(m.id, "Customer revoked authorization")
            .await
            .unwrap();
    ...
    async fn test_submit_and_complete_collection() {
        ...
        let c = e
            .complete_collection(c.id, Some("BANK-REF-001"))
            .await
            .unwrap();
    ...
    async fn test_full_collection_workflow() {
        ...
        let c1 = e
            .complete_collection(c1.id, Some("BANK-001"))
            .await
            .unwrap();
        ...
        let c2 = e
            .complete_collection(c2.id, Some("BANK-002"))
            .await
            .unwrap();
        ...
        let c2 = e
            .return_collection(c2.id, "R01", Some("Insufficient funds"))
            .await
            .unwrap();
    ...
    async fn test_fail_collection() {
        ...
        let c = e
            .fail_collection(c.id, "R04", Some("Account closed"))
            .await
            .unwrap();
