...
pub trait DirectDebitMandateRepository: Send + Sync {
    ...
    async fn update_mandate_collection_stats(
        ...
        total_collected: &str,
        last_collection_date: Option<chrono::NaiveDate>,
        next_type: &str,
    ) -> AtlasResult<()>;
...
#[allow(dead_code)]
pub struct PostgresDirectDebitMandateRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
impl PostgresDirectDebitMandateRepository {
    #[must_use]
...
impl DirectDebitMandateRepository for PostgresDirectDebitMandateRepository {
    ...
    async fn get_mandate(&self, _: Uuid) -> AtlasResult<Option<DirectDebitMandate>> {
        Ok(None)
    }
    async fn get_mandate_by_number(
        &self,
    ...
    async fn get_collection(&self, _: Uuid) -> AtlasResult<Option<MandateCollection>> {
        Ok(None)
    }
    async fn list_collections(
        &self,
    ...
    async fn sum_collected_for_mandate(&self, _: Uuid) -> AtlasResult<String> {
        Ok("0.00".into())
    }
    async fn count_collections_for_mandate(&self, _: Uuid) -> AtlasResult<i64> {
        Ok(0)
    }
