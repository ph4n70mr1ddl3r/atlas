...
pub trait DocumentSequencingRepository: Send + Sync {
    ...
    async fn get_sequence(&self, org_id: Uuid, code: &str)
    ...
    async fn update_sequence_status(&self, id: Uuid, status: &str)
        -> AtlasResult<DocumentSequence>;
    ...
    async fn increment_sequence_value(
        &self,
        id: Uuid,
        increment_by: i32,
    ) -> AtlasResult<DocumentSequence>;
    ...
    async fn get_audit_by_document(
        &self,
        document_id: Uuid,
...
impl DocumentSequencingRepository for PostgresDocumentSequencingRepository {
    async fn create_sequence(
        ...
        .bind(document_type)
        .bind(initial_value)
        .bind(initial_value - i64::from(increment_by.max(1))) // current_value starts before initial
        .bind(increment_by)
        .bind(max_value)
        .bind(cycle_flag)
        .bind(prefix)
    ...
    async fn get_sequence_by_id(&self, id: Uuid) -> AtlasResult<Option<DocumentSequence>> {
        let row = sqlx::query("SELECT * FROM _atlas.document_sequences WHERE id=$1")
            .bind(id)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| AtlasError::DatabaseError(e.to_string()))?;
    ...
    async fn delete_sequence(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        sqlx::query("DELETE FROM _atlas.document_sequences WHERE organization_id=$1 AND code=$2")
            .bind(org_id)
            .bind(code)
            .execute(&self.pool)
            .await
    ...
    async fn get_assignment(&self, id: Uuid) -> AtlasResult<Option<DocumentSequenceAssignment>> {
        let row = sqlx::query("SELECT * FROM _atlas.document_sequence_assignments WHERE id=$1")
            .bind(id)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| AtlasError::DatabaseError(e.to_string()))?;
    ...
    async fn get_audit_by_document(
        ...
        let row = sqlx::query("SELECT * FROM _atlas.document_sequence_audit WHERE document_id=$1")
            .bind(document_id)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| AtlasError::DatabaseError(e.to_string()))?;
