...
use super::{
    AtlasError, AtlasResult, DunningLetterManagementRepository, DunningLetterRun,
    DunningLetterRunResult, DunningLetterSet, DunningLetterSetLine, DunningProfile,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
impl DunningLetterManagementEngine {
    ...
    pub async fn create_letter_set(
        ...
        if set_name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Letter set name is required".into(),
            ));
        }
        if number_of_levels < 1 {
            return Err(AtlasError::ValidationFailed(
                "Number of levels must be at least 1".into(),
            ));
        }
        if minimum_overdue_days < 0 {
            return Err(AtlasError::ValidationFailed(
                "Minimum overdue days cannot be negative".into(),
            ));
        }
        ...
        if !VALID_AGING_BASIS.contains(&aging_basis) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid aging basis '{}'. Must be one of: {}",
                aging_basis,
                VALID_AGING_BASIS.join(", ")
            )));
        }

        if self
            .repository
            .get_letter_set_by_name(org_id, set_name)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Letter set '{set_name}' already exists"
            )));
        }
        ...
        self.repository
            .create_letter_set(
                ...
                description,
                number_of_levels,
                minimum_overdue_days,
                currency_code,
                include_finance_charges,
                include_unapplied_receipts,
                aging_basis,
                created_by,
            )
            .await
    ...
    pub async fn activate_letter_set(&self, id: Uuid) -> AtlasResult<DunningLetterSet> {
        ...
        if set.status != "draft" && set.status != "inactive" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot activate letter set in '{}' status. Must be 'draft' or 'inactive'.",
                set.status
            )));
        }
    ...
    pub async fn deactivate_letter_set(&self, id: Uuid) -> AtlasResult<DunningLetterSet> {
        ...
        if set.status != "active" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot deactivate letter set in '{}' status. Must be 'active'.",
                set.status
            )));
        }
        ...
        self.repository
            .update_letter_set_status(id, "inactive")
            .await
    }
    ...
    pub async fn add_letter_set_line(
        ...
        if level_name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Level name is required".into(),
            ));
        }
        if level_number < 1 {
            return Err(AtlasError::ValidationFailed(
                "Level number must be at least 1".into(),
            ));
        }
        ...
        info!(
            "Adding level {} to dunning letter set '{}'",
            level_number, set.set_name
        );
        self.repository
            .add_letter_set_line(
                set_id,
    ...
    pub async fn list_letter_set_lines(
        &self,
        set_id: Uuid,
    ) -> AtlasResult<Vec<DunningLetterSetLine>> {
    ...
    pub async fn create_profile(
        ...
        if let Ok(amt) = minimum_overdue_amount.parse::<f64>() {
            if amt < 0.0 {
                return Err(AtlasError::ValidationFailed(
                    "Minimum overdue amount cannot be negative".into(),
                ));
            }
        }
    ...
    pub async fn enable_profile(&self, id: Uuid) -> AtlasResult<DunningProfile> {
        ...
        if profile.dunning_status == "enabled" {
            return Err(AtlasError::WorkflowError(
                "Profile is already enabled".into(),
            ));
        }
        self.repository
            .update_profile_status(id, "enabled", None)
            .await
    }
    ...
    pub async fn disable_profile(
        ...
        self.repository
            .update_profile_status(id, "disabled", reason)
            .await
    }
    ...
    pub async fn hold_profile(&self, id: Uuid, reason: &str) -> AtlasResult<DunningProfile> {
        ...
        self.repository
            .update_profile_status(id, "hold", Some(reason))
            .await
    }
    ...
    pub async fn create_run(
        ...
        if let Some(level) = specific_level {
            if level < 1 {
                return Err(AtlasError::ValidationFailed(
                    "Specific level must be at least 1".into(),
                ));
            }
        }
        ...
        {
            return Err(AtlasError::Conflict(format!(
                "Run number '{run_number}' already exists"
            )));
        }
        ...
        if let Some(ls_id) = letter_set_id {
            let ls = self
                .repository
                .get_letter_set(ls_id)
                .await?
                .ok_or_else(|| {
                    AtlasError::EntityNotFound(format!("Letter set {ls_id} not found"))
                })?;
            if ls.status != "active" {
                return Err(AtlasError::ValidationFailed(
    ...
    pub async fn complete_run(&self, id: Uuid) -> AtlasResult<DunningLetterRun> {
        ...
        if run.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot complete run in '{}' status. Must be 'submitted'.",
                run.status
            )));
        }
    ...
    pub async fn add_run_result(
        ...
        if run.status != "draft" && run.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot add results to run in '{}' status",
                run.status
            )));
        }
        ...
        let total_amount: f64 = all_results
            .iter()
    ...
    pub async fn mark_result_sent(
        ...
        if result.status != "generated" && result.status != "pending" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot mark result as sent from '{}' status",
                result.status
            )));
        }
        ...
        let updated = self
            .repository
            .update_run_result_status(id, "sent", Some(today), delivery_confirmation, None)
            .await?;
        ...
        if let Some(profile_id) = result.profile_id {
            let _ = self
                .repository
                .update_profile_dunning_sent(profile_id, result.dunning_level, today)
                .await;
        }
    ...
    pub async fn mark_result_failed(
        ...
        if result.status != "pending" && result.status != "generated" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot mark result as failed from '{}' status",
                result.status
            )));
        }

        self.repository
            .update_run_result_status(id, "failed", None, None, Some(reason))
            .await
    }
}
