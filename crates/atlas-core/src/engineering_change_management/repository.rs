...
impl EngineeringChangeManagementRepository for PostgresEngineeringChangeManagementRepository {
    ...
    async fn get_change_type(&self, id: Uuid) -> AtlasResult<Option<EngineeringChangeType>> {
        let row = sqlx::query("SELECT * FROM _atlas.engineering_change_types WHERE id = $1")
            .bind(id)
    ...
    async fn create_change(
        ...
        .bind(description)
        .bind(change_reason)
        .bind(change_reason_description)
        .bind(priority)
        .bind(status)
        .bind(revision)
        .bind(assigned_to)
        ...
        .bind(actual_hours)
        .bind(regulatory_impact)
        .bind(safety_impact)
        .bind(validation_required)
        .bind(created_by)
        .fetch_one(&self.pool)
        .await?;
    ...
    async fn get_change(&self, id: Uuid) -> AtlasResult<Option<EngineeringChange>> {
        let row = sqlx::query("SELECT * FROM _atlas.engineering_changes WHERE id = $1")
            .bind(id)
            .fetch_optional(&self.pool)
            .await?;
    ...
    async fn list_changes(
        ...
        let mut query =
            String::from("SELECT * FROM _atlas.engineering_changes WHERE organization_id = $1");
    ...
    async fn get_change_line(&self, id: Uuid) -> AtlasResult<Option<EngineeringChangeLine>> {
        let row = sqlx::query("SELECT * FROM _atlas.engineering_change_lines WHERE id = $1")
            .bind(id)
            .fetch_optional(&self.pool)
            .await?;
    ...
    async fn create_affected_item(
        ...
        .bind(item_name)
        .bind(impact_type)
        .bind(impact_description)
        .bind(current_revision)
        .bind(new_revision)
        .bind(disposition)
        .bind(old_item_status)
        .bind(new_item_status)
        .bind(phase_in_date)
        .bind(phase_out_date)
        .bind(created_by)
        .fetch_one(&self.pool)
    ...
    async fn get_affected_item_by_id(
        ...
        let row =
            sqlx::query("SELECT * FROM _atlas.engineering_change_affected_items WHERE id = $1")
                .bind(id)
                .fetch_optional(&self.pool)
                .await?;
        Ok(row.as_ref().map(row_to_affected_item))
    ...
    async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<EcmDashboard> {
        let row = sqlx::query("SELECT * FROM _atlas.ecm_dashboard WHERE organization_id = $1")
            .bind(org_id)
            .fetch_optional(&self.pool)
            .await?;
        ...
        match row {
            Some(r) => Ok(EcmDashboard {
                total_changes: r
                    .try_get::<Option<i32>, _>("total_changes")
                    .unwrap_or(None)
                    .unwrap_or(0),
                open_changes: r
                    .try_get::<Option<i32>, _>("open_changes")
                    .unwrap_or(None)
                    .unwrap_or(0),
                pending_approval: r
                    .try_get::<Option<i32>, _>("pending_approval")
                    .unwrap_or(None)
                    .unwrap_or(0),
                approved_changes: r
                    .try_get::<Option<i32>, _>("approved_changes")
                    .unwrap_or(None)
                    .unwrap_or(0),
                implemented_changes: r
                    .try_get::<Option<i32>, _>("implemented_changes")
                    .unwrap_or(None)
                    .unwrap_or(0),
                rejected_changes: r
                    .try_get::<Option<i32>, _>("rejected_changes")
                    .unwrap_or(None)
                    .unwrap_or(0),
                ecr_count: r
                    .try_get::<Option<i32>, _>("ecr_count")
                    .unwrap_or(None)
                    .unwrap_or(0),
                eco_count: r
                    .try_get::<Option<i32>, _>("eco_count")
                    .unwrap_or(None)
                    .unwrap_or(0),
                ecn_count: r
                    .try_get::<Option<i32>, _>("ecn_count")
                    .unwrap_or(None)
                    .unwrap_or(0),
                critical_open: r
                    .try_get::<Option<i32>, _>("critical_open")
                    .unwrap_or(None)
                    .unwrap_or(0),
                high_open: r
                    .try_get::<Option<i32>, _>("high_open")
                    .unwrap_or(None)
                    .unwrap_or(0),
                medium_open: r
                    .try_get::<Option<i32>, _>("medium_open")
                    .unwrap_or(None)
                    .unwrap_or(0),
                low_open: r
                    .try_get::<Option<i32>, _>("low_open")
                    .unwrap_or(None)
                    .unwrap_or(0),
                avg_days_to_implement: r
                    .try_get::<Option<f64>, _>("avg_days_to_implement")
                    .unwrap_or(None)
                    .unwrap_or(0.0),
                avg_days_to_approve: r
                    .try_get::<Option<f64>, _>("avg_days_to_approve")
                    .unwrap_or(None)
                    .unwrap_or(0.0),
                total_items_affected: r
                    .try_get::<Option<i32>, _>("total_items_affected")
                    .unwrap_or(None)
                    .unwrap_or(0),
                total_estimated_cost: r
                    .try_get::<Option<f64>, _>("total_estimated_cost")
                    .unwrap_or(None)
                    .unwrap_or(0.0),
                total_actual_cost: r
                    .try_get::<Option<f64>, _>("total_actual_cost")
                    .unwrap_or(None)
                    .unwrap_or(0.0),
                changes_by_reason: r
                    .try_get::<serde_json::Value, _>("changes_by_reason")
                    .unwrap_or(serde_json::json!({})),
                changes_by_status: r
                    .try_get::<serde_json::Value, _>("changes_by_status")
                    .unwrap_or(serde_json::json!({})),
                changes_trend: r
                    .try_get::<serde_json::Value, _>("changes_trend")
                    .unwrap_or(serde_json::json!([])),
            }),
