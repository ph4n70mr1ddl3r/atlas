...
pub trait AssetManagementRepository: Send + Sync {
    ...
    async fn get_location_by_code(
        ...
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<serde_json::Value>>;
    ...
    async fn complete_work_order(
        ...
        id: Uuid,
        actual_cost: &str,
        actual_hours: serde_json::Value,
        downtime_hours: f64,
        resolution_code: &str,
        completion_notes: &str,
        materials: serde_json::Value,
        labor: serde_json::Value,
    ) -> AtlasResult<MaintenanceWorkOrder>;
    ...
    async fn create_pm_schedule(
        ...
        interval_unit: &str,
        meter_type: &str,
        meter_threshold: serde_json::Value,
    ...
    async fn get_pm_schedule(&self, id: Uuid)
        -> AtlasResult<Option<PreventiveMaintenanceSchedule>>;
    async fn get_pm_schedule_by_number(
        &self,
        org_id: Uuid,
        schedule_number: &str,
    ) -> AtlasResult<Option<PreventiveMaintenanceSchedule>>;
    ...
    async fn update_pm_schedule_status(
        &self,
        id: Uuid,
        status: &str,
    ) -> AtlasResult<PreventiveMaintenanceSchedule>;
...
fn row_to_work_order(row: &sqlx::postgres::PgRow) -> MaintenanceWorkOrder {
    MaintenanceWorkOrder {
        ...
        estimated_hours: row
            .try_get("estimated_hours")
            .unwrap_or(Some(serde_json::json!({}))),
        actual_hours: row
            .try_get("actual_hours")
            .unwrap_or(Some(serde_json::json!({}))),
...
fn row_to_pm_schedule(row: &sqlx::postgres::PgRow) -> PreventiveMaintenanceSchedule {
    PreventiveMaintenanceSchedule {
        ...
        meter_threshold: row
            .try_get("meter_threshold")
            .unwrap_or(Some(serde_json::json!({}))),
        work_order_template: row
            .try_get("work_order_template")
            .unwrap_or(Some(serde_json::json!({}))),
...
impl AssetManagementRepository for PostgresAssetManagementRepository {
    ...
    async fn delete_work_order(&self, org_id: Uuid, wo_number: &str) -> AtlasResult<()> {
        ...
        if result.rows_affected() == 0 {
            return Err(AtlasError::EntityNotFound(format!(
                "Work order '{wo_number}' not found"
            )));
        }
        Ok(())
    }
    ...
    async fn get_pm_schedule(
        ...
        let row =
            sqlx::query("SELECT * FROM _atlas.preventive_maintenance_schedules WHERE id = $1")
                .bind(id)
                .fetch_optional(&self.pool)
                .await?;
