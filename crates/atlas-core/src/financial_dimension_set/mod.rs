...
pub trait FinancialDimensionSetRepository: Send + Sync {
    ...
    async fn get_by_code(
        ...
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<FinancialDimensionSet>>;
    async fn list(
        &self,
        org_id: Uuid,
        is_active: Option<bool>,
    ) -> AtlasResult<Vec<FinancialDimensionSet>>;
    async fn update(
        &self,
        id: Uuid,
        name: Option<&str>,
...
#[allow(dead_code)]
pub struct PostgresFinancialDimensionSetRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
impl PostgresFinancialDimensionSetRepository {
    #[must_use]
...
impl FinancialDimensionSetRepository for PostgresFinancialDimensionSetRepository {
    ...
    async fn get(&self, _: Uuid) -> AtlasResult<Option<FinancialDimensionSet>> {
        Ok(None)
    }
    async fn get_by_code(&self, _: Uuid, _: &str) -> AtlasResult<Option<FinancialDimensionSet>> {
        Ok(None)
    }
    async fn list(&self, _: Uuid, _: Option<bool>) -> AtlasResult<Vec<FinancialDimensionSet>> {
        Ok(vec![])
    }
    async fn update(
        &self,
    ...
    async fn deactivate(&self, _: Uuid) -> AtlasResult<FinancialDimensionSet> {
        Err(AtlasError::EntityNotFound("Not found".into()))
    }
    async fn activate(&self, _: Uuid) -> AtlasResult<FinancialDimensionSet> {
        Err(AtlasError::EntityNotFound("Not found".into()))
    }
    async fn delete_set(&self, _: Uuid, _: &str) -> AtlasResult<()> {
        Ok(())
    }
    async fn add_member(
        &self,
    ...
    async fn list_members(&self, _: Uuid) -> AtlasResult<Vec<DimensionSetMember>> {
        Ok(vec![])
    }
    async fn remove_member(&self, _: Uuid) -> AtlasResult<()> {
        Ok(())
    }
    async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<DimensionSetDashboard> {
...
impl FinancialDimensionSetEngine {
    ...
    pub async fn create(
        ...
        if self.repository.get_by_code(org_id, code).await?.is_some() {
            return Err(AtlasError::Conflict(format!(
                "Dimension set '{code}' already exists"
            )));
        }
        info!(
            "Creating financial dimension set '{}' for org {}",
            code, org_id
        );
        self.repository
            .create_set(org_id, code, name, description, created_by)
            .await
    }
    ...
    pub async fn update(
        ...
        let ds =
            self.repository.get(id).await?.ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Dimension set {id} not found"))
            })?;
        if !ds.is_active {
            return Err(AtlasError::ValidationFailed(
                "Cannot update inactive dimension set".into(),
            ));
        }
    ...
    pub async fn delete(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        self.repository
            .get_by_code(org_id, code)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Dimension set '{code}' not found"))
            })?;
    ...
    pub async fn add_member(
        ...
        let ds = self
            .repository
            .get(dimension_set_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Dimension set {dimension_set_id} not found"))
            })?;

        if !ds.is_active {
            return Err(AtlasError::ValidationFailed(
                "Cannot add members to inactive dimension set".into(),
            ));
        }
        if dimension_code.is_empty() || dimension_value_code.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Dimension code and value code are required".into(),
            ));
        }
        ...
        if members.iter().any(|m| {
            m.dimension_code == dimension_code && m.dimension_value_code == dimension_value_code
        }) {
            return Err(AtlasError::Conflict(format!(
        ...
        info!(
            "Adding member {}.{} to dimension set {}",
            dimension_code, dimension_value_code, ds.code
        );
        self.repository
            .add_member(
                org_id,
    ...
    pub async fn list_members(
        &self,
        dimension_set_id: Uuid,
    ) -> AtlasResult<Vec<DimensionSetMember>> {
...
mod tests {
    ...
    impl FinancialDimensionSetRepository for MockRepo {
        ...
        async fn get(&self, id: Uuid) -> AtlasResult<Option<FinancialDimensionSet>> {
            ...
            if let Some(ds) = sets.iter_mut().find(|s| s.id == id) {
                ds.members = self
                    ...
                    .unwrap()
                    .iter()
                    .filter(|m| m.dimension_set_id == id)
                    .cloned()
                    .collect();
                Ok(Some(ds.clone()))
            } else {
        ...
        async fn get_by_code(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|s| s.organization_id == org_id && s.code == code)
                .cloned())
        }
        async fn list(
            &self,
            ...
            Ok(self
                ...
                .filter(|s| {
                    s.organization_id == org_id
                        && (is_active.is_none() || s.is_active == is_active.unwrap())
                })
                .cloned()
                .collect())
        }
        ...
        async fn delete_set(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
            self.sets
                .lock()
                .unwrap()
                .retain(|s| !(s.organization_id == org_id && s.code == code));
            Ok(())
        }
        async fn add_member(
            &self,
            ...
            let m = DimensionSetMember {
                ...
                dimension_set_id,
                dimension_id,
                dimension_code: dimension_code.into(),
                dimension_value_id,
                dimension_value_code: dimension_value_code.into(),
                display_order,
                metadata: serde_json::json!({}),
                created_at: Utc::now(),
            };
        ...
        async fn list_members(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .filter(|m| m.dimension_set_id == dimension_set_id)
                .cloned()
                .collect())
        }
        async fn remove_member(&self, id: Uuid) -> AtlasResult<()> {
    ...
    fn eng() -> FinancialDimensionSetEngine {
        FinancialDimensionSetEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_create_empty_code() {
        assert!(eng()
            .create(Uuid::new_v4(), "", "Name", None, None)
            .await
            .is_err());
    }
    ...
    async fn test_create_empty_name() {
        assert!(eng()
            .create(Uuid::new_v4(), "CODE", "", None, None)
            .await
            .is_err());
    }
    ...
    async fn test_add_member() {
        ...
        let org = Uuid::new_v4();
        let ds = e
            .create(org, "DEPT", "Departments", None, None)
            .await
            .unwrap();
        let member = e
            .add_member(
    ...
    async fn test_add_member_empty_codes() {
        ...
        assert!(e
            .add_member(org, ds.id, Uuid::new_v4(), "", Uuid::new_v4(), "IT", 1)
            .await
            .is_err());
        assert!(e
            .add_member(org, ds.id, Uuid::new_v4(), "DEPT", Uuid::new_v4(), "", 1)
            .await
            .is_err());
    }
    ...
    async fn test_add_duplicate_member() {
        ...
        let _ = e
            .add_member(org, ds.id, dim_id, "DEPT", val_id, "IT", 1)
            .await
            .unwrap();
        assert!(e
            .add_member(org, ds.id, dim_id, "DEPT", val_id, "IT", 2)
            .await
            .is_err());
    }
    ...
    async fn test_remove_member() {
        ...
        let ds = e.create(org, "TEST", "Test", None, None).await.unwrap();
        let m = e
            .add_member(org, ds.id, Uuid::new_v4(), "DEPT", Uuid::new_v4(), "HR", 1)
            .await
            .unwrap();
    ...
    async fn test_add_member_to_inactive() {
        ...
        let _ = e.deactivate(ds.id).await.unwrap();
        assert!(e
            .add_member(org, ds.id, Uuid::new_v4(), "DEPT", Uuid::new_v4(), "IT", 1)
            .await
            .is_err());
    }
    ...
    async fn test_update() {
        ...
        let ds = e.create(org, "TEST", "Test", None, None).await.unwrap();
        let updated = e
            .update(ds.id, Some("Updated"), Some("New desc"))
            .await
            .unwrap();
