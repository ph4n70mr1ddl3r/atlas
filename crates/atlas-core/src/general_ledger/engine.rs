...
use atlas_shared::{
    AtlasError, AtlasResult, GlAccount, GlJournalEntry, GlJournalLine, GlTrialBalance,
    GlTrialBalanceLine,
};
use std::sync::Arc;
use tracing::info;
...
const VALID_NATURAL_BALANCES: &[&str] = &["debit", "credit"];

/// Valid entry types
const VALID_ENTRY_TYPES: &[&str] = &["standard", "adjusting", "closing", "reversing", "budget"];

/// Valid journal statuses
const VALID_JOURNAL_STATUSES: &[&str] = &["draft", "submitted", "posted", "reversed", "error"];
...
impl GeneralLedgerEngine {
    ...
    pub async fn create_account(
            ...
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Account code '{account_code}' already exists"
            )));
        }

        info!(
            "Creating GL account {} ({}) for org {}",
            account_code, account_name, org_id
        );
    ...
    pub async fn get_journal_entry_by_number(
        ...
        self.repository
            .get_journal_entry_by_number(org_id, number)
            .await
    }
    ...
    pub async fn list_journal_entries(
        ...
        self.repository
            .list_journal_entries(org_id, status, entry_type)
            .await
    }
    ...
    pub async fn add_journal_line(
        ...
        let je = self
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Journal entry {journal_entry_id} not found"))
            })?;

        if je.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot add lines to journal entry in '{}' status. Must be 'draft'.",
                je.status
            )));
        }
        ...
        let account = self
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Account code '{account_code}' not found"))
            })?;
        ...
        info!(
            "Adding line {} to journal entry {} ({}: dr={}, cr={})",
        ...
        let line = self
            ...
            .create_journal_line(
                ...
                entered_cr,
                entered_dr,
                entered_cr, // accounted = entered for now (same currency)
                &je.currency_code,
                None,
                None,
                None,
    ...
    pub async fn validate_balance(&self, journal_entry_id: Uuid) -> AtlasResult<bool> {
        let lines = self.repository.list_journal_lines(journal_entry_id).await?;
        let total_dr: f64 = lines
            .iter()
            .map(|l| l.entered_dr.parse::<f64>().unwrap_or(0.0))
            .sum();
        let total_cr: f64 = lines
            .iter()
    ...
    pub async fn post_journal_entry(
        ...
        if je.status != "draft" && je.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot post journal entry in '{}' status. Must be 'draft' or 'submitted'.",
                je.status
            )));
        }
        ...
        self.repository
            .update_journal_status(journal_entry_id, "posted", posted_by, None)
            .await
    }
    ...
    pub async fn reverse_journal_entry(
        ...
        if je.status != "posted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot reverse journal entry in '{}' status. Must be 'posted'.",
                je.status
            )));
        }
        ...
        self.repository
            .update_journal_status(journal_entry_id, "reversed", None, None)
            .await
    }
    ...
    pub async fn generate_trial_balance(
        ...
        info!(
            "Generating trial balance for org {} as of {}",
            org_id, as_of_date
        );
        ...
        for account in &accounts {
            ...
            let (period_debit, period_credit) = self
                .repository
