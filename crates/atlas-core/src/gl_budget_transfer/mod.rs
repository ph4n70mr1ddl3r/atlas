...
pub trait GlBudgetTransferRepository: Send + Sync {
    ...
    async fn get_by_number(
        ...
        org_id: Uuid,
        number: &str,
    ) -> AtlasResult<Option<GlBudgetTransfer>>;
    async fn list(
        &self,
        org_id: Uuid,
        status: Option<&str>,
        transfer_type: Option<&str>,
    ) -> AtlasResult<Vec<GlBudgetTransfer>>;
...
#[allow(dead_code)]
pub struct PostgresGlBudgetTransferRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
impl PostgresGlBudgetTransferRepository {
    #[must_use]
...
impl GlBudgetTransferRepository for PostgresGlBudgetTransferRepository {
    ...
    async fn get(&self, _: Uuid) -> AtlasResult<Option<GlBudgetTransfer>> {
        Ok(None)
    }
    async fn get_by_number(&self, _: Uuid, _: &str) -> AtlasResult<Option<GlBudgetTransfer>> {
        Ok(None)
    }
    async fn list(
        &self,
...
impl GlBudgetTransferEngine {
    ...
    pub async fn create(
        ...
        if !VALID_TRANSFER_TYPES.contains(&transfer_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid transfer type '{}'. Must be one of: {}",
                transfer_type,
                VALID_TRANSFER_TYPES.join(", ")
            )));
        }
        ...
        let amount: f64 = transfer_amount
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid transfer amount".into()))?;
        if amount <= 0.0 {
            return Err(AtlasError::ValidationFailed(
        ...
        if effective_date < transfer_date {
            return Err(AtlasError::ValidationFailed(
                "Effective date cannot be before transfer date".into(),
            ));
        }
        ...
        self.repository
            .create(
                ...
                budget_name,
                transfer_type,
                from_account_combination,
                from_department,
                from_period,
                to_account_combination,
                to_department,
                to_period,
                transfer_amount,
                currency_code,
    ...
    pub async fn submit(&self, id: Uuid) -> AtlasResult<GlBudgetTransfer> {
        let bt =
            self.repository.get(id).await?.ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Budget transfer {id} not found"))
            })?;
        ...
        info!(
            "Submitting budget transfer {} for approval",
            bt.transfer_number
        );
        self.repository
            .update_status(id, "pending_approval", None)
            .await
    }
    ...
    pub async fn approve(&self, id: Uuid, approved_by: Uuid) -> AtlasResult<GlBudgetTransfer> {
        ...
        if bt.status != "pending_approval" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot approve transfer in '{}' status. Must be 'pending_approval'.",
                bt.status
            )));
        }
    ...
    pub async fn complete(&self, id: Uuid) -> AtlasResult<GlBudgetTransfer> {
        ...
        if bt.status != "approved" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot complete transfer in '{}' status. Must be 'approved'.",
                bt.status
            )));
        }
    ...
    pub async fn reject(&self, id: Uuid) -> AtlasResult<GlBudgetTransfer> {
        ...
        if bt.status != "pending_approval" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot reject transfer in '{}' status. Must be 'pending_approval'.",
                bt.status
            )));
        }
...
mod tests {
    ...
    impl GlBudgetTransferRepository for MockRepo {
        async fn create(
            ...
            let bt = GlBudgetTransfer {
                ...
                transfer_date,
                effective_date,
                budget_name: budget_name.map(Into::into),
                transfer_type: transfer_type.into(),
                from_account_combination: from_account_combination.map(Into::into),
                from_department: from_department.map(Into::into),
                from_period: from_period.map(Into::into),
                to_account_combination: to_account_combination.map(Into::into),
                to_department: to_department.map(Into::into),
                to_period: to_period.map(Into::into),
        ...
        async fn get_by_number(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|b| b.organization_id == org_id && b.transfer_number == number)
                .cloned())
        }
        async fn list(
            &self,
        ...
        async fn update_status(
            ...
            if let Some(ab) = approved_by {
                bt.approved_by = Some(ab);
                bt.approved_at = Some(Utc::now());
            }
        ...
        async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<BudgetTransferDashboard> {
            ...
            Ok(BudgetTransferDashboard {
                ...
                pending_transfers: org_items
                    .iter()
                    .filter(|b| b.status == "pending_approval")
                    .count() as i32,
                approved_transfers: org_items.iter().filter(|b| b.status == "approved").count()
                    as i32,
                rejected_transfers: org_items.iter().filter(|b| b.status == "rejected").count()
                    as i32,
                completed_transfers: org_items.iter().filter(|b| b.status == "completed").count()
                    as i32,
                total_transfer_amount: org_items
                    .iter()
                    .map(|b| b.transfer_amount.parse::<f64>().unwrap_or(0.0))
                    .sum::<f64>()
                    .to_string(),
                by_transfer_type: serde_json::json!([]),
            })
    ...
    fn eng() -> GlBudgetTransferEngine {
        GlBudgetTransferEngine::new(Arc::new(MockRepo::new()))
    }
