...
use atlas_shared::{
    AtlasError, AtlasResult, GrantAward, GrantBilling, GrantBudgetLine, GrantComplianceReport,
    GrantDashboardSummary, GrantExpenditure, GrantIndirectCostRate, GrantSponsor,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
#[allow(dead_code)]
const VALID_BILLING_BASES: &[&str] = &["cost", "milestone", "fixed_price", "deliverable"];
...
#[allow(dead_code)]
const VALID_EXPENDITURE_TYPES: &[&str] = &["actual", "commitment", "encumbrance", "adjustment"];

#[allow(dead_code)]
const VALID_EXPENDITURE_STATUSES: &[&str] = &["pending", "approved", "billed", "reversed", "hold"];
...
const VALID_REPORT_TYPES: &[&str] = &[
    "federal_financial_report_sf425",
    "progress_report",
    "invention_report",
    "property_report",
    "closeout_report",
...
#[allow(dead_code)]
const VALID_REPORT_STATUSES: &[&str] = &["draft", "in_review", "approved", "submitted", "rejected"];

#[allow(dead_code)]
const VALID_RATE_TYPES: &[&str] = &["negotiated", "predetermined", "fixed", "provisional"];

#[allow(dead_code)]
const VALID_BASE_TYPES: &[&str] = &[
    "modified_total_direct_costs",
    "total_direct_costs",
    "salaries_and_wages",
];
...
impl GrantManagementEngine {
    ...
    pub async fn create_sponsor(
        ...
        if sponsor_code.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Sponsor code is required".into(),
            ));
        }
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Sponsor name is required".into(),
            ));
        }
        if !VALID_SPONSOR_TYPES.contains(&sponsor_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid sponsor type '{}'. Must be one of: {}",
                sponsor_type,
                VALID_SPONSOR_TYPES.join(", ")
            )));
        }
        ...
        if let Some(limit) = credit_limit {
            let val: f64 = limit.parse().map_err(|_| {
                AtlasError::ValidationFailed("Credit limit must be a valid number".into())
            })?;
            if val < 0.0 {
                return Err(AtlasError::ValidationFailed(
        ...
        info!(
            "Creating grant sponsor {} ({}) for org {}",
            sponsor_code, name, org_id
        );
    ...
    pub async fn create_indirect_cost_rate(
        ...
        let pct: f64 = rate_percentage.parse().map_err(|_| {
            AtlasError::ValidationFailed("Rate percentage must be a valid number".into())
        })?;
        ...
        info!(
            "Creating indirect cost rate {} ({}%) for org {}",
            rate_name, rate_percentage, org_id
        );
    ...
    pub async fn list_indirect_cost_rates(
        ...
        org_id: Uuid,
        active_only: bool,
    ) -> AtlasResult<Vec<GrantIndirectCostRate>> {
        self.repository
            .list_indirect_cost_rates(org_id, active_only)
            .await
    }
    ...
    pub async fn create_award(
        ...
        award_title: &str,
        sponsor_id: Uuid,
        sponsor_award_number: Option<&str>,
        award_type: &str,
        award_purpose: Option<&str>,
        start_date: chrono::NaiveDate,
        end_date: chrono::NaiveDate,
        total_award_amount: &str,
        direct_costs_total: &str,
        indirect_costs_total: &str,
        cost_sharing_total: &str,
        currency_code: &str,
        indirect_cost_rate_id: Option<Uuid>,
        indirect_cost_rate: &str,
        cost_sharing_required: bool,
        cost_sharing_percent: &str,
        principal_investigator_id: Option<Uuid>,
        principal_investigator_name: Option<&str>,
        department_id: Option<Uuid>,
        department_name: Option<&str>,
        project_id: Option<Uuid>,
        cost_center: Option<&str>,
        gl_revenue_account: Option<&str>,
        gl_receivable_account: Option<&str>,
        gl_deferred_account: Option<&str>,
        billing_frequency: &str,
        billing_basis: &str,
        reporting_requirements: Option<&str>,
        ...
        if award_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Award number is required".into(),
            ));
        }
        if award_title.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Award title is required".into(),
            ));
        }
        if !VALID_AWARD_TYPES.contains(&award_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid award type '{}'. Must be one of: {}",
                award_type,
                VALID_AWARD_TYPES.join(", ")
            )));
        }
        ...
        if !VALID_BILLING_BASES.contains(&billing_basis) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid billing basis '{}'. Must be one of: {}",
                billing_basis,
                VALID_BILLING_BASES.join(", ")
            )));
        }

        // Validate numeric fields
        let total: f64 = total_award_amount.parse().map_err(|_| {
            AtlasError::ValidationFailed("Total award amount must be a valid number".into())
        })?;
        if total < 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Total award amount cannot be negative".into(),
            ));
        }
        ...
        info!(
            "Creating grant award {} ({}) for org {}",
            award_number, award_title, org_id
        );
    ...
    pub async fn get_award_by_number(
        &self,
        org_id: Uuid,
        number: &str,
    ) -> AtlasResult<Option<GrantAward>> {
    ...
    pub async fn list_awards(
        ...
        self.repository
            .list_awards(org_id, status, sponsor_id)
            .await
    }
    ...
    pub async fn activate_award(&self, award_id: Uuid) -> AtlasResult<GrantAward> {
        ...
        if award.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot activate award in '{}' status. Must be 'draft'.",
                award.status
            )));
        }

        info!(
            "Activating grant award {} ({})",
            award.award_number, award.award_title
        );
        self.repository
            .update_award_status(award_id, "active", None, None)
            .await
    }
    ...
    pub async fn suspend_award(&self, award_id: Uuid) -> AtlasResult<GrantAward> {
        ...
        if award.status != "active" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot suspend award in '{}' status. Must be 'active'.",
                award.status
            )));
        }
        ...
        self.repository
            .update_award_status(award_id, "suspended", None, None)
            .await
    }
    ...
    pub async fn complete_award(
        ...
        if award.status != "active" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot complete award in '{}' status. Must be 'active'.",
                award.status
            )));
        }
    ...
    pub async fn terminate_award(
        ...
        if award.status != "active" && award.status != "suspended" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot terminate award in '{}' status. Must be 'active' or 'suspended'.",
                award.status
            )));
        }
    ...
    pub async fn create_budget_line(
        ...
        if award.status != "draft" && award.status != "active" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot add budget lines to award in '{}' status.",
                award.status
            )));
        }
        ...
        if !VALID_BUDGET_CATEGORIES.contains(&budget_category) {
            return Err(AtlasError::ValidationFailed(format!(
                ...
                budget_category,
                VALID_BUDGET_CATEGORIES.join(", ")
            )));
        }

        let amount: f64 = budget_amount.parse().map_err(|_| {
            AtlasError::ValidationFailed("Budget amount must be a valid number".into())
        })?;
        if amount < 0.0 {
            return Err(AtlasError::ValidationFailed(
        ...
        info!(
            "Adding budget line ({}) to award {}",
            budget_category, award.award_number
        );
    ...
    pub async fn create_expenditure(
        ...
        expenditure_date: chrono::NaiveDate,
        description: Option<&str>,
        budget_line_id: Option<Uuid>,
        budget_category: Option<&str>,
        amount: &str,
        ...
        gl_debit_account: Option<&str>,
        gl_credit_account: Option<&str>,
        notes: Option<&str>,
        created_by: Option<Uuid>,
        ...
        if award.status != "active" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot record expenditures on award in '{}' status. Must be 'active'.",
                award.status
            )));
        }
        ...
        info!(
            "Creating expenditure {} on award {} (amount: {})",
            expenditure_number, award.award_number, amount
        );
    ...
    pub async fn approve_expenditure(
        ...
        let exp = self
            .repository
            .get_expenditure(expenditure_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Expenditure {expenditure_id} not found"))
            })?;

        if exp.status != "pending" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot approve expenditure in '{}' status. Must be 'pending'.",
                exp.status
            )));
        }
        ...
        self.repository
            .update_expenditure_status(expenditure_id, "approved", approved_by)
            .await
    }
    ...
    pub async fn reverse_expenditure(&self, expenditure_id: Uuid) -> AtlasResult<GrantExpenditure> {
        ...
        if exp.status != "approved" && exp.status != "pending" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot reverse expenditure in '{}' status.",
                exp.status
            )));
        }
        ...
        let result = self
            .repository
            .update_expenditure_status(expenditure_id, "reversed", None)
            .await?;
    ...
    pub async fn list_expenditures(
        ...
        award_id: Uuid,
        status: Option<&str>,
    ) -> AtlasResult<Vec<GrantExpenditure>> {
    ...
    pub async fn create_billing(
        ...
        if award.status != "active" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot create billing for award in '{}' status. Must be 'active'.",
                award.status
            )));
        }

        // Get approved expenditures in the period that haven't been billed yet
        let expenditures = self
            .repository
            .list_expenditures(award_id, Some("approved"))
            .await?;
        let period_expenditures: Vec<_> = expenditures
            .iter()
        ...
        let billing = self
            ...
            .create_billing(
                ...
                period_end,
                Some(due_date),
                &format!("{direct_costs:.2}"),
                &format!("{indirect_costs:.2}"),
                &format!("{cost_sharing:.2}"),
                &format!("{total_amount:.2}"),
                serde_json::json!(exp_ids),
                notes,
                created_by,
            )
            .await?;
        ...
        for exp in &period_expenditures {
            self.repository
                .update_expenditure_status(exp.id, "billed", None)
                .await?;
        }
    ...
    pub async fn submit_billing(
        ...
        self.repository
            .update_billing_status(billing_id, "submitted", submitted_by, None)
            .await
    }
    ...
    pub async fn approve_billing(
        ...
        self.repository
            .update_billing_status(billing_id, "approved", approved_by, None)
            .await
    }
    ...
    pub async fn mark_billing_paid(
        ...
        if billing.status != "approved" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot mark billing as paid in '{}' status. Must be 'approved'.",
                billing.status
            )));
        }
        ...
        let result = self
            .repository
            .update_billing_status(billing_id, "paid", None, payment_reference)
            .await?;
    ...
    pub async fn create_compliance_report(
        ...
        let period_exps: Vec<_> = expenditures
            .iter()
            .filter(|e| {
                e.expenditure_date >= reporting_period_start
                    && e.expenditure_date <= reporting_period_end
            })
            .collect();
        ...
        for bill in &billings {
            if bill.invoice_date >= reporting_period_start
                && bill.invoice_date <= reporting_period_end
            {
        ...
        info!(
            "Creating compliance report ({}) for award {}",
            report_type, award.award_number
        );
    ...
    pub async fn submit_compliance_report(
        ...
        self.repository
            .update_compliance_report_status(report_id, "submitted", submitted_by, None)
            .await
    }
    ...
    pub async fn approve_compliance_report(
        ...
        if report.status != "in_review" && report.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot approve report in '{}' status.",
                report.status
            )));
        }
        ...
        self.repository
            .update_compliance_report_status(report_id, "approved", None, approved_by)
            .await
    }
    ...
    ) -> AtlasResult<Vec<GrantComplianceReport>> {
        self.repository
            .list_compliance_reports(award_id, report_type)
            .await
    }
    ...
    async fn recalculate_award_totals(&self, award_id: Uuid) -> AtlasResult<()> {
        ...
        let billings = self.repository.list_billings(award_id, None).await?;
        let total_billed: f64 = billings
            .iter()
            .filter(|b| b.status != "cancelled")
            .map(|b| b.total_amount.parse::<f64>().unwrap_or(0.0))
            .sum();

        let total_collected: f64 = billings
            .iter()
    ...
    async fn recalculate_budget_line(&self, budget_line_id: Uuid) -> AtlasResult<()> {
        ...
        let bl = self
            .repository
            .get_budget_line(budget_line_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Budget line {budget_line_id} not found"))
            })?;

        let expenditures = self.repository.list_expenditures(bl.award_id, None).await?;
        let line_exps: Vec<_> = expenditures
            .iter()
            .filter(|e| e.budget_line_id == Some(budget_line_id))
            .collect();

        let expended: f64 = line_exps
            .iter()
            .filter(|e| e.status == "approved" || e.status == "billed")
            .map(|e| e.total_amount.parse::<f64>().unwrap_or(0.0))
            .sum();

        let committed: f64 = line_exps
            .iter()
            .filter(|e| e.status == "pending")
            .map(|e| e.total_amount.parse::<f64>().unwrap_or(0.0))
            .sum();

        let billed: f64 = line_exps
            .iter()
