...
pub trait GrantManagementRepository: Send + Sync {
    ...
    async fn list_indirect_cost_rates(
        &self,
        org_id: Uuid,
        active_only: bool,
    ...
    async fn update_award_status(
        ...
        id: Uuid,
        status: &str,
        closeout_date: Option<chrono::NaiveDate>,
        closeout_notes: Option<&str>,
    ) -> AtlasResult<GrantAward>;
    async fn update_award_totals(
        &self,
        id: Uuid,
        total_expenditures: &str,
    ...
    async fn update_budget_line_amounts(
        &self,
        id: Uuid,
        committed: &str,
        expended: &str,
    ...
    async fn create_expenditure(
        ...
        amount: &str,
        indirect_cost_amount: &str,
        total_amount: &str,
        cost_sharing_amount: &str,
    ...
    async fn update_expenditure_status(
        &self,
        id: Uuid,
        status: &str,
    ...
    async fn create_billing(
        ...
        direct_costs_billed: &str,
        indirect_costs_billed: &str,
        cost_sharing_billed: &str,
        total_amount: &str,
        expenditure_ids: serde_json::Value,
        notes: Option<&str>,
        created_by: Option<Uuid>,
    ...
    async fn update_compliance_report_status(
        &self,
        id: Uuid,
        status: &str,
...
impl PostgresGrantManagementRepository {
    ...
    fn row_to_sponsor(&self, row: &sqlx::postgres::PgRow) -> GrantSponsor {
        GrantSponsor {
            ...
            sponsor_code: row.get("sponsor_code"),
            name: row.get("name"),
    ...
    fn row_to_award(&self, row: &sqlx::postgres::PgRow) -> GrantAward {
        GrantAward {
            ...
            closeout_date: row.get("closeout_date"),
            closeout_notes: row.get("closeout_notes"),
            metadata: row.try_get("metadata").unwrap_or(serde_json::json!({})),
            created_by: row.get("created_by"),
            created_at: row.get("created_at"),
            updated_at: row.get("updated_at"),
...
impl GrantManagementRepository for PostgresGrantManagementRepository {
    ...
    async fn get_sponsor(&self, org_id: Uuid, code: &str) -> AtlasResult<Option<GrantSponsor>> {
        let row = sqlx::query(
            "SELECT * FROM _atlas.grant_sponsors WHERE organization_id=$1 AND sponsor_code=$2",
        )
        .bind(org_id)
        .bind(code)
    ...
    async fn delete_sponsor(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        sqlx::query(
            "DELETE FROM _atlas.grant_sponsors WHERE organization_id=$1 AND sponsor_code=$2",
        )
        .bind(org_id)
        .bind(code)
        .execute(&self.pool)
    ...
    async fn create_indirect_cost_rate(
        ...
        Ok(GrantIndirectCostRate {
            ...
            negotiated_by: row.get("negotiated_by"),
    ...
    async fn list_indirect_cost_rates(
        ...
        Ok(rows
            ...
            .map(|r| GrantIndirectCostRate {
                ...
                negotiated_by: r.get("negotiated_by"),
    ...
    async fn get_award_by_number(
        ...
        let row = sqlx::query(
            "SELECT * FROM _atlas.grant_awards WHERE organization_id=$1 AND award_number=$2",
        )
        .bind(org_id)
        .bind(number)
        .fetch_optional(&self.pool)
    ...
    async fn create_expenditure(
        ...
        .bind(org_id)
        .bind(award_id)
        .bind(expenditure_number)
        .bind(expenditure_type)
        .bind(expenditure_date)
        .bind(description)
        .bind(budget_line_id)
        .bind(budget_category)
        .bind(amount)
        .bind(indirect_cost_amount)
        .bind(total_amount)
        .bind(cost_sharing_amount)
        .bind(employee_id)
        .bind(employee_name)
        .bind(vendor_id)
        .bind(vendor_name)
        ...
        .bind(source_entity_number)
        .bind(gl_debit_account)
        .bind(gl_credit_account)
        .bind(status)
        .bind(notes)
        .bind(created_by)
        .fetch_one(&self.pool)
    ...
    async fn get_dashboard_summary(&self, org_id: Uuid) -> AtlasResult<GrantDashboardSummary> {
        ...
        let total_value: serde_json::Value =
            rows.try_get("total_value").unwrap_or(serde_json::json!(0));
        let total_funded: serde_json::Value =
            rows.try_get("total_funded").unwrap_or(serde_json::json!(0));
        let total_exp: serde_json::Value =
            rows.try_get("total_exp").unwrap_or(serde_json::json!(0));
        let total_avail: serde_json::Value =
            rows.try_get("total_avail").unwrap_or(serde_json::json!(0));

        let exp_f64: f64 = total_exp.to_string().parse().unwrap_or(0.0);
        let val_f64: f64 = total_value.to_string().parse().unwrap_or(0.0);
        let utilization = if val_f64 > 0.0 {
            exp_f64 / val_f64 * 100.0
        } else {
            0.0
        };
...
fn row_to_expenditure(row: &sqlx::postgres::PgRow) -> GrantExpenditure {
    GrantExpenditure {
        ...
        organization_id: row.get("organization_id"),
        award_id: row.get("award_id"),
        expenditure_number: row.get("expenditure_number"),
        ...
        indirect_cost_amount: get_num(row, "indirect_cost_amount"),
        total_amount: get_num(row, "total_amount"),
        cost_sharing_amount: get_num(row, "cost_sharing_amount"),
        ...
        gl_debit_account: row.get("gl_debit_account"),
        gl_credit_account: row.get("gl_credit_account"),
        status: row.get("status"),
        approved_by: row.get("approved_by"),
