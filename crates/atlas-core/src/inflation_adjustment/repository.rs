...
use atlas_shared::{
    AtlasResult, InflationAdjustmentLine, InflationAdjustmentRun, InflationDashboardSummary,
    InflationIndex, InflationIndexRate,
};
use sqlx::PgPool;
use uuid::Uuid;
...
fn row_to_run(row: &sqlx::postgres::PgRow) -> InflationAdjustmentRun {
    InflationAdjustmentRun {
        ...
        total_debit_adjustment: row
            .try_get("total_debit_adjustment")
            .unwrap_or("0".to_string()),
        total_credit_adjustment: row
            .try_get("total_credit_adjustment")
            .unwrap_or("0".to_string()),
        total_monetary_gain_loss: row
            .try_get("total_monetary_gain_loss")
            .unwrap_or("0".to_string()),
...
impl InflationAdjustmentRepository for PostgresInflationAdjustmentRepository {
    async fn create_index(
        ...
        .bind(currency_code)
        .bind(index_type)
        .bind(is_hyperinflationary)
        .bind(hyperinflationary_start_date)
        .bind(effective_from)
        .bind(effective_to)
        .bind(created_by)
        .fetch_one(&self.pool)
    ...
    async fn create_adjustment_line(
        ...
        .bind(restated_balance)
        .bind(adjustment_amount)
        .bind(inflation_factor)
    ...
    async fn get_dashboard_summary(&self, org_id: Uuid) -> AtlasResult<InflationDashboardSummary> {
        ...
        Ok(InflationDashboardSummary {
            ...
            total_adjustments: format!(
                "{:.2}",
                run_row.try_get::<f64, _>("total_adj").unwrap_or(0.0)
            ),
            total_gain_loss: format!(
                "{:.2}",
                run_row.try_get::<f64, _>("total_gl").unwrap_or(0.0)
            ),
        })
    }
}
