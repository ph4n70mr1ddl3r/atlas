...
use atlas_shared::{
    AtlasError, AtlasResult, IntercompanyBalance, IntercompanyBalanceSummary, IntercompanyBatch,
    IntercompanySettlement, IntercompanyTransaction,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
const VALID_TXN_STATUSES: &[&str] = &["draft", "approved", "posted", "settled", "cancelled"];
...
const VALID_SETTLEMENT_METHODS: &[&str] = &["cash", "netting", "offset"];
...
impl IntercompanyEngine {
    ...
    pub async fn submit_batch(
        ...
        info!(
            "Submitting intercompany batch '{}' for approval",
            batch.batch_number
        );
        self.repository
    ...
    pub async fn approve_batch(
        ...
        info!(
            "Approving intercompany batch '{}' by {}",
            batch.batch_number, approved_by
        );
        self.repository
            .update_batch_status(batch_id, "approved", Some(approved_by), None, None)
            .await
    }
    ...
    pub async fn post_batch(&self, batch_id: Uuid) -> AtlasResult<IntercompanyBatch> {
        ...
        for txn in &transactions {
            self.repository
                .update_transaction_status(txn.id, "posted", None)
                .await?;
        }
        ...
        info!(
            "Posting intercompany batch '{}' with {} transactions",
            batch.batch_number,
            transactions.len()
        );
        self.repository
            .update_batch_status(batch_id, "posted", None, Some(chrono::Utc::now()), None)
            .await
    }
    ...
    pub async fn reject_batch(
        ...
        info!(
            "Rejecting intercompany batch '{}': {}",
            batch.batch_number, reason
        );
        self.repository
            .update_batch_status(batch_id, "cancelled", None, None, Some(reason))
            .await
    }
    ...
    pub async fn create_transaction(
        ...
        let batch = self
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Batch '{batch_number}' not found"))
            })?;
        ...
        if let Some(er) = exchange_rate {
            let rate: f64 = er.parse().map_err(|_| {
                AtlasError::ValidationFailed("Exchange rate must be a valid number".to_string())
            })?;
            if rate <= 0.0 {
                return Err(AtlasError::ValidationFailed(
    ...
    pub async fn get_transaction(
        ...
        self.repository
            .get_transaction(org_id, transaction_number)
            .await
    }

    /// List transactions in a batch
    pub async fn list_transactions_by_batch(
        &self,
        batch_id: Uuid,
    ...
    pub async fn list_transactions_by_entity(
        ...
        self.repository
            .list_transactions_by_entity(org_id, entity_id, status)
            .await
    }
    ...
    pub async fn create_settlement(
        ...
        // Validate amount
        let amt: f64 = settled_amount.parse().map_err(|_| {
            AtlasError::ValidationFailed("Settled amount must be a valid number".to_string())
        })?;
        if amt <= 0.0 {
            return Err(AtlasError::ValidationFailed(
        ...
        for txn_id in transaction_ids {
            ...
            {
                self.repository
                    .update_transaction_status(*txn_id, "settled", Some(today))
                    .await?;
            }
        }
        ...
        self.recalculate_balances(org_id, from_entity_id, to_entity_id, currency_code)
            .await?;
    ...
    pub async fn get_balance(
        ...
        self.repository
            .get_balance(org_id, from_entity_id, to_entity_id, currency_code)
            .await
    }
    ...
    pub async fn get_balance_summary(
        &self,
        org_id: Uuid,
    ) -> AtlasResult<IntercompanyBalanceSummary> {
        let balances = self.repository.list_balances(org_id).await?;

        let total_outstanding: f64 = balances
            .iter()
            .filter_map(|b| b.total_outstanding.parse::<f64>().ok())
            .sum();

        let open_transactions: i32 = balances.iter().map(|b| b.open_transaction_count).sum();
    ...
    async fn recalculate_balances(
        ...
        let from_txns = self
            .repository
            .list_transactions_by_entity(org_id, from_entity_id, None)
            .await?;
        ...
        for txn in &from_txns {
            ...
            let matches_pair = (txn.from_entity_id == from_entity_id
                && txn.to_entity_id == to_entity_id)
                || (txn.from_entity_id == to_entity_id && txn.to_entity_id == from_entity_id);
            if !matches_pair {
                continue;
            }
            if txn.currency_code != currency_code {
                continue;
            }
