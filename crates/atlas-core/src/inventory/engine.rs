...
use atlas_shared::{
    AtlasError, AtlasResult, CycleCountHeader, CycleCountLine, InventoryDashboardSummary,
    InventoryOrganization, InventoryTransaction, InventoryTransactionType, Item, ItemCategory,
    Locator, OnHandBalance, Subinventory, TransactionReason,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
#[allow(dead_code)]
const VALID_TXN_STATUSES: &[&str] = &["pending", "approved", "processed", "cancelled"];

/// Valid cycle count statuses
#[allow(dead_code)]
const VALID_CYCLE_COUNT_STATUSES: &[&str] = &["draft", "in_progress", "completed", "cancelled"];

/// Valid cycle count methods
#[allow(dead_code)]
const VALID_COUNT_METHODS: &[&str] = &["full", "abc", "random", "by_category"];
...
impl InventoryEngine {
    ...
    pub async fn create_inventory_org(
        ...
        if !VALID_ORG_TYPES.contains(&org_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid org type '{}'. Must be one of: {}",
                org_type,
                VALID_ORG_TYPES.join(", ")
            )));
        }

        info!(
            "Creating inventory organization '{}' for org {}",
            code, org_id
        );

        self.repository
            .create_inventory_org(
                org_id,
                code,
                name,
                description,
                ...
                location_code,
                address,
                default_subinventory_code,
                default_currency_code,
                requires_approval_for_issues,
                requires_approval_for_transfers,
                enable_lot_control,
                enable_serial_control,
    ...
    pub async fn list_inventory_orgs(
        &self,
        org_id: Uuid,
    ) -> AtlasResult<Vec<InventoryOrganization>> {
    ...
    pub async fn delete_inventory_org(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        info!(
            "Deactivating inventory organization '{}' in org {}",
            code, org_id
        );
    ...
    pub async fn list_items(
        ...
        self.repository
            .list_items(org_id, category_code, item_type)
            .await
    }
    ...
    pub async fn create_locator(
        ...
        info!(
            "Creating locator '{}' for subinventory {}",
            code, subinventory_id
        );
        self.repository
            .create_locator(org_id, subinventory_id, code, description, picker_order)
            .await
    }
    ...
    pub async fn list_on_hand_balances(
        ...
        self.repository
            .list_on_hand_balances(org_id, item_id, inventory_org_id)
            .await
    }
    ...
    pub async fn create_transaction_type(
        ...
        if !VALID_TRANSACTION_ACTIONS.contains(&transaction_action) {
            return Err(AtlasError::ValidationFailed(format!(
                ...
                transaction_action,
                VALID_TRANSACTION_ACTIONS.join(", ")
            )));
        }
    ...
    pub async fn receive_item(
        ...
        info!(
            "Receiving {} {} of item {} into org {}",
            quantity, uom, item_id, to_inventory_org_id
        );
        ...
        let txn = self
            ...
            .create_transaction(
                ...
                None,
                None,
                Some(to_inventory_org_id),
                Some(to_subinventory_id),
                to_locator_id,
                quantity,
                uom,
                unit_cost,
    ...
    pub async fn issue_item(
        ...
        info!(
            "Issuing {} {} of item {} from org {}",
            quantity, uom, item_id, from_inventory_org_id
        );
        ...
        let txn = self
            ...
            .create_transaction(
                ...
                item_code,
                item_description,
                Some(from_inventory_org_id),
                Some(from_subinventory_id),
                from_locator_id,
                None,
                None,
                None,
    ...
    pub async fn transfer_item(
        ...
        info!(
            "Transferring {} {} of item {} from subinv {} to {}",
    ...
    pub async fn adjust_item(
        ...
        let delta: f64 = quantity_delta.parse().map_err(|_| {
            AtlasError::ValidationFailed("Quantity delta must be a valid number".to_string())
        })?;
        if delta == 0.0 {
            return Err(AtlasError::ValidationFailed(
        ...
        info!(
            "Adjusting item {} by {} in org {}",
            item_id, quantity_delta, inventory_org_id
        );
        ...
        let txn = self
            ...
            .create_transaction(
                ...
                None,
                None,
                Some(inventory_org_id),
                Some(subinventory_id),
                locator_id,
                quantity_delta,
                uom,
                unit_cost,
    ...
    pub async fn list_transactions(
        ...
        self.repository
            .list_transactions(org_id, item_id, transaction_action, status)
            .await
    }
    ...
    pub async fn list_transaction_reasons(
        &self,
        org_id: Uuid,
    ) -> AtlasResult<Vec<TransactionReason>> {
    ...
    pub async fn add_cycle_count_line(
        ...
        let cc = self
            .repository
            .get_cycle_count(cycle_count_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Cycle count {cycle_count_id} not found"))
            })?;

        if cc.status == "completed" || cc.status == "cancelled" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot add lines to cycle count in '{}' status",
                cc.status
            )));
        }
        ...
        let existing_lines = self
            .repository
            .list_cycle_count_lines(cycle_count_id)
            .await?;
        let line_number = (existing_lines.len() as i32) + 1;

        info!(
            "Adding line {} to cycle count {}",
            line_number, cc.count_number
        );
    ...
    pub async fn record_count(
        ...
        let qty: f64 = count_quantity.parse().map_err(|_| {
            AtlasError::ValidationFailed("Count quantity must be a valid number".to_string())
        })?;
        if qty < 0.0 {
            return Err(AtlasError::ValidationFailed(
        ...
        let line = self
            .repository
            .update_cycle_count_line_count(line_id, count_number, count_quantity, counted_by)
            .await?;

        // Update cycle count header summary
        let all_lines = self
            .repository
            .list_cycle_count_lines(line.cycle_count_id)
            .await?;
        ...
        self.repository
            .update_cycle_count_summary(
                ...
                counted,
                0,
                0, // matched/mismatched determined on approval
            )
            .await?;
    ...
    pub async fn approve_cycle_count_line(
        ...
        let approved: f64 = approved_quantity.parse().map_err(|_| {
            AtlasError::ValidationFailed("Approved quantity must be a valid number".to_string())
        })?;
        if approved < 0.0 {
            return Err(AtlasError::ValidationFailed(
        ...
        let line = self
            .repository
            .approve_cycle_count_line(line_id, approved_quantity)
            .await?;

        // If there's a variance, generate an adjustment transaction
        if !line.is_matched {
            if let (Some(_variance_qty), Some(variance_str)) = (
                line.variance_quantity.as_ref(),
                line.variance_quantity.as_ref(),
            ) {
                ...
                if variance.abs() > 0.001 {
                    let cc = self
                        .repository
                        .get_cycle_count(line.cycle_count_id)
                        .await?
                        .ok_or_else(|| {
                    ...
                    let _adj_tx = self
                        .adjust_item(
                            ...
                            Some(&format!(
                                "Variance of {} for item {}",
                                &variance_str,
                                line.item_code.as_deref().unwrap_or("unknown")
                            )),
                            None,
                        )
                        .await?;
        ...
        let mismatched = all_lines
            .iter()
            .filter(|l| !l.is_matched && l.status == "approved")
            .count() as i32;
        self.repository
            .update_cycle_count_summary(
                line.cycle_count_id,
    ...
    pub async fn list_cycle_count_lines(
        &self,
        cycle_count_id: Uuid,
    ...
    pub async fn get_dashboard_summary(
        &self,
        org_id: Uuid,
    ) -> AtlasResult<InventoryDashboardSummary> {
        ...
        let all_txns = self
            .repository
            .list_transactions(org_id, None, None, None)
            .await?;
        let all_on_hand = self
            .repository
            .list_on_hand_balances(org_id, None, None)
            .await?;
        let cycle_counts = self
            .repository
            .list_cycle_counts(org_id, Some("in_progress"))
            .await?;

        let total_items = all_items.len() as i32;
        let active_items = all_items.iter().filter(|i| i.is_active).count() as i32;

        let total_on_hand_value: f64 = all_on_hand
            .iter()
        ...
        let items_by_type: serde_json::Value = by_type
            .into_iter()
        ...
        let items_by_category: serde_json::Value = by_cat
            .into_iter()
            .map(|(k, v)| serde_json::json!({"category": k, "count": v}))
            .collect();

        // Transactions by action
        let mut by_action: std::collections::HashMap<String, i32> =
            std::collections::HashMap::new();
        for t in &all_txns {
            *by_action.entry(t.transaction_action.clone()).or_insert(0) += 1;
        }
        let transactions_by_action: serde_json::Value = by_action
            .into_iter()
            .map(|(k, v)| serde_json::json!({"action": k, "count": v}))
            .collect();

        // Top items by on-hand value
        let mut by_item_value: std::collections::HashMap<Uuid, (String, f64)> =
            std::collections::HashMap::new();
        for b in &all_on_hand {
            let entry = by_item_value
                .entry(b.item_id)
