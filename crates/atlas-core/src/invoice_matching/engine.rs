...
use super::{
    AtlasError, AtlasResult, InvoiceMatch, InvoiceMatchLine, InvoiceMatchingDashboard,
    InvoiceMatchingRepository,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
impl InvoiceMatchingEngine {
    ...
    pub async fn create_match(
        ...
        if po_amt <= 0.0 {
            return Err(AtlasError::ValidationFailed(
                "PO amount must be positive".into(),
            ));
        }
        ...
        if tol_pct < 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Price tolerance percentage must be non-negative".into(),
            ));
        }
        ...
        if qty_pct < 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Quantity tolerance percentage must be non-negative".into(),
            ));
        }
        ...
        if amt_tol < 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Amount tolerance must be non-negative".into(),
            ));
        }
        ...
        if self
            .repository
            .get_match_by_number(org_id, match_number)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Match number '{match_number}' already exists"
            )));
        }

        info!(
            "Creating invoice match {} (type: {}) for supplier {}",
            match_number, match_type, supplier_name
        );
        ...
        if status == "exception" {
            self.repository
                .update_match_hold(m.id, Some("Auto-hold: matching exception"), created_by)
                .await
                .ok();
        }
        ...
        self.repository
            .get_match(m.id)
            .await?
            .ok_or_else(|| AtlasError::EntityNotFound("Match not found after creation".into()))
    }

    /// Compute match status based on variances and tolerances
    fn compute_match_status(
        m: &InvoiceMatch,
        ...
        let price_var_pct = if po_amt > 0.0 {
            (price_var / po_amt * 100.0).abs()
        } else {
            0.0
        };
        ...
        if m.match_type == "three_way" || m.match_type == "four_way" {
            if let Some(ref rcpt_amt_str) = m.receipt_amount {
                ...
                receipt_var_pct = if rcpt_amt > 0.0 {
                    (receipt_var / rcpt_amt * 100.0).abs()
                } else {
                    0.0
                };
            }
        ...
        if m.match_type == "four_way" {
            if let Some(ref insp_amt_str) = m.inspection_amount {
                ...
                inspection_var_pct = if insp_amt > 0.0 {
                    (inspection_var / insp_amt * 100.0).abs()
                } else {
                    0.0
                };
            }
    ...
    /// Get match by number
    pub async fn get_match_by_number(
        &self,
        org_id: Uuid,
        number: &str,
    ...
    pub async fn list_matches(
        ...
        self.repository
            .list_matches(org_id, status, match_type, supplier_id)
            .await
    }
    ...
    pub async fn hold_match(
        ...
        if m.status == "cancelled" {
            return Err(AtlasError::WorkflowError(
                "Cannot hold a cancelled match".into(),
            ));
        }
        if m.status == "matched" {
            return Err(AtlasError::WorkflowError(
                "Cannot hold a matched record".into(),
            ));
        }

        info!("Placing match {} on hold", m.match_number);
        self.repository
            .update_match_hold(match_id, reason, held_by)
            .await
    }
    ...
    pub async fn override_match(
        ...
        if reason.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Override reason is required".into(),
            ));
        }
        ...
        if m.status != "exception" && m.status != "partial_match" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot override match in '{}' status. Must be 'exception' or 'partial_match'.",
                m.status
            )));
        }

        info!(
            "Overriding match {} exception (reason: {})",
            m.match_number, reason
        );
        self.repository
            .update_match_override(match_id, Some(reason), overridden_by)
            .await
    }
    ...
    pub async fn confirm_match(
        ...
        if m.status != "pending" && m.status != "partial_match" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot confirm match in '{}' status. Must be 'pending' or 'partial_match'.",
                m.status
            )));
        }
        ...
        self.repository
            .update_match_matched(match_id, confirmed_by)
            .await
    }
    ...
    pub async fn add_match_line(
        ...
        if m.status == "cancelled" {
            return Err(AtlasError::WorkflowError(
                "Cannot add lines to a cancelled match".into(),
            ));
        }
        ...
        {
            return Err(AtlasError::ValidationFailed(
                "Quantities, prices, and amounts must be non-negative".into(),
            ));
        }
        ...
        let tol_pct: f64 = m.price_tolerance_pct.parse().unwrap_or(0.0);
        let price_var_pct = if p_price > 0.0 {
            (price_var / p_price * 100.0).abs()
        } else {
            0.0
        };
        let qty_var_pct = if p_qty > 0.0 {
            (qty_var / p_qty * 100.0).abs()
        } else {
            0.0
        };
        ...
        info!(
            "Adding match line {} to match {} (status: {})",
            line_number, m.match_number, line_status
        );
        ...
        self.repository
            .create_match_line(
                ...
                po_amt,
                line_status,
                Some(&format!("{price_var:.4}")),
                Some(&format!("{qty_var:.4}")),
                notes,
            )
            .await
    }
    ...
    pub async fn override_match_line(&self, line_id: Uuid) -> AtlasResult<InvoiceMatchLine> {
        let line = self
            .repository
            .get_match_line(line_id)
            .await?
            .ok_or_else(|| AtlasError::EntityNotFound(format!("Match line {line_id} not found")))?;

        if line.status != "exception" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot override line in '{}' status. Must be 'exception'.",
                line.status
            )));
        }

        self.repository
            .update_match_line_status(line_id, "overridden")
            .await
    }
...
mod tests {
    ...
    impl MockRepo {
        fn new() -> Self {
            MockRepo {
                matches: std::sync::Mutex::new(vec![]),
    ...
    impl InvoiceMatchingRepository for MockRepo {
        async fn create_match(
            ...
            rcpt_num: Option<&str>,
            insp_id: Option<Uuid>,
            insp_status: Option<&str>,
            cb: Option<Uuid>,
        ) -> AtlasResult<InvoiceMatch> {
            let m = InvoiceMatch {
                id: Uuid::new_v4(),
                organization_id: org_id,
                match_number: mn.into(),
                invoice_id: inv_id,
                invoice_number: inv_num.map(Into::into),
                purchase_order_id: po_id,
                po_number: po_num.map(Into::into),
                supplier_id: sup_id,
                supplier_name: sup_name.into(),
                match_type: mt.into(),
                status: "pending".into(),
                invoice_amount: inv_amt.into(),
                po_amount: po_amt.into(),
                receipt_amount: rcpt_amt.map(Into::into),
                inspection_amount: insp_amt.map(Into::into),
                price_tolerance_pct: pt_pct.into(),
                quantity_tolerance_pct: qt_pct.into(),
                ...
                receipt_id: rcpt_id,
                receipt_number: rcpt_num.map(Into::into),
                inspection_id: insp_id,
                inspection_status: insp_status.map(Into::into),
                hold_reason: None,
                hold_at: None,
                hold_by: None,
        ...
        async fn get_match_by_number(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|m| m.organization_id == org_id && m.match_number == num)
                .cloned())
        }
        ...
        async fn create_match_line(
            ...
            let line = InvoiceMatchLine {
                ...
                po_line_id: pli,
                receipt_line_id: rli,
                inspection_line_id: insp_li,
                line_number: ln,
                item_description: desc.map(Into::into),
                invoice_quantity: iq.into(),
                po_quantity: pq.into(),
                receipt_quantity: rq.map(Into::into),
                inspected_quantity: insp_q.map(Into::into),
                invoice_unit_price: ip.into(),
                po_unit_price: pp.into(),
                invoice_line_amount: ia.into(),
                po_line_amount: pa.into(),
                status: status.into(),
                price_variance: pv.map(Into::into),
                quantity_variance: qv.map(Into::into),
    ...
    fn eng() -> InvoiceMatchingEngine {
        InvoiceMatchingEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_override_exception() {
        ...
        assert_eq!(m.status, "exception");
        let ovr = e
            .override_match(m.id, "Supplier provided justification", None)
            .await
            .unwrap();
    ...
    async fn test_hold_exception() {
        ...
        let m = e
            ...
            .unwrap();
        let held = e
            .hold_match(m.id, Some("Price variance exceeds threshold"), None)
            .await
            .unwrap();
        assert_eq!(held.status, "exception");
        assert_eq!(
            held.hold_reason,
            Some("Price variance exceeds threshold".into())
        );
    }
    ...
    async fn test_list_matches_invalid_status() {
        let r = eng()
            .list_matches(Uuid::new_v4(), Some("bad"), None, None)
            .await;
        assert!(r.is_err());
    }
    ...
    async fn test_list_matches_invalid_type() {
        let r = eng()
            .list_matches(Uuid::new_v4(), None, Some("bad"), None)
            .await;
        assert!(r.is_err());
    }
    ...
    async fn test_list_matches_valid() {
        let r = eng()
            .list_matches(Uuid::new_v4(), Some("matched"), Some("two_way"), None)
            .await;
