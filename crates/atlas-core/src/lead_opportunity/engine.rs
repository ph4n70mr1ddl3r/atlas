...
/// Valid lead ratings
const VALID_LEAD_RATINGS: &[&str] = &["hot", "warm", "cold"];

/// Valid opportunity statuses
const VALID_OPP_STATUSES: &[&str] = &["open", "won", "lost"];
...
#[allow(dead_code)]
const VALID_ACTIVITY_STATUSES: &[&str] = &["planned", "in_progress", "completed", "cancelled"];

/// Valid activity priorities
const VALID_ACTIVITY_PRIORITIES: &[&str] = &["low", "medium", "high", "critical"];
...
impl LeadOpportunityEngine {
    ...
    pub async fn create_lead_source(
        ...
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Lead source name is required".to_string(),
            ));
        }
        if self
            .repository
            .get_lead_source_by_code(org_id, &code)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Lead source '{code}' already exists"
            )));
        }
        info!("Creating lead source '{}' for org {}", code, org_id);
        self.repository
            .create_lead_source(org_id, &code, name, description, created_by)
            .await
    }
    ...
    pub async fn list_lead_sources(
        &self,
        org_id: Uuid,
    ) -> AtlasResult<Vec<atlas_shared::LeadSource>> {
    ...
    pub async fn create_lead_rating_model(
        ...
        if code.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Rating model code is required".to_string(),
            ));
        }
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
        ...
        if self
            .repository
            .get_lead_rating_model_by_code(org_id, &code)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Lead rating model '{code}' already exists"
            )));
        }
        info!("Creating lead rating model '{}' for org {}", code, org_id);
        self.repository
            .create_lead_rating_model(
                org_id,
                &code,
                name,
                description,
    ...
    pub async fn list_lead_rating_models(
        &self,
        org_id: Uuid,
    ) -> AtlasResult<Vec<atlas_shared::LeadRatingModel>> {
    ...
    pub async fn create_lead(
        ...
        if lead_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Lead number is required".to_string(),
            ));
        }
        ...
        let source_name = if let Some(src_id) = lead_source_id {
            let _src = self
                .repository
                .get_lead_source_by_code(org_id, &src_id.to_string())
                .await?;
        ...
        if self
            .repository
            .get_lead_by_number(org_id, lead_number)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Lead '{lead_number}' already exists"
            )));
        }
    ...
    pub async fn get_lead_by_number(
        &self,
        org_id: Uuid,
        lead_number: &str,
    ) -> AtlasResult<Option<atlas_shared::SalesLead>> {
        self.repository
            .get_lead_by_number(org_id, lead_number)
            .await
    }
    ...
    pub async fn update_lead_score(
        ...
        if !VALID_LEAD_RATINGS.contains(&rating) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid lead rating '{}'. Must be one of: {}",
                rating,
                VALID_LEAD_RATINGS.join(", ")
            )));
        }
        let score_val: f64 = score
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Lead score must be a number".to_string()))?;
        if !(0.0..=100.0).contains(&score_val) {
            return Err(AtlasError::ValidationFailed(
                "Lead score must be between 0 and 100".to_string(),
            ));
        }
    ...
    pub async fn convert_lead(
        ...
        let converted_lead = self
            .repository
            .convert_lead(id, Some(opportunity.id), customer_id)
            .await?;
    ...
    pub async fn create_opportunity_stage(
        ...
        if code.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Stage code is required".to_string(),
            ));
        }
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Stage name is required".to_string(),
            ));
        }
        let prob: f64 = probability.parse().unwrap_or(0.0);
        if !(0.0..=100.0).contains(&prob) {
            return Err(AtlasError::ValidationFailed(
                "Probability must be 0-100".to_string(),
            ));
        }
        if self
            .repository
            .get_opportunity_stage_by_code(org_id, &code)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Opportunity stage '{code}' already exists"
            )));
        }
    ...
    pub async fn list_opportunity_stages(
        &self,
        org_id: Uuid,
    ) -> AtlasResult<Vec<atlas_shared::OpportunityStage>> {
    ...
    pub async fn create_opportunity(
        ...
        if opportunity_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Opportunity number is required".to_string(),
            ));
        }
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
        ...
        if self
            .repository
            .get_opportunity_by_number(org_id, opportunity_number)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Opportunity '{opportunity_number}' already exists"
            )));
        }

        info!(
            "Creating opportunity '{}' for org {}",
            opportunity_number, org_id
        );
    ...
    pub async fn get_opportunity(
        &self,
        id: Uuid,
    ) -> AtlasResult<Option<atlas_shared::SalesOpportunity>> {
    ...
    pub async fn list_opportunities(
        ...
        if let Some(s) = status {
            if !VALID_OPP_STATUSES.contains(&s) {
                return Err(AtlasError::ValidationFailed(format!(
                    "Invalid opportunity status '{}'. Must be one of: {}",
                    s,
                    VALID_OPP_STATUSES.join(", ")
                )));
            }
        }
        self.repository
            .list_opportunities(org_id, status, owner_id, stage_id)
            .await
    }
    ...
    pub async fn update_opportunity_stage(
        ...
        if opp.status != "open" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot change stage of '{}' opportunity",
                opp.status
            )));
        }
        ...
        let updated = self
            .repository
            .update_opportunity_stage(id, stage_id, stage_name.as_deref(), &probability, &weighted)
            .await?;
        ...
        let _ = self
            ...
            .add_stage_history(
                ...
                id,
                old_stage.as_deref(),
                stage_name.as_deref().unwrap_or("Unknown"),
                changed_by,
                changed_by_name,
                notes,
            )
    ...
    pub async fn close_opportunity_won(
        ...
        if opp.status != "open" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot close '{}' opportunity as won. Must be 'open'.",
                opp.status
            )));
        }
    ...
    pub async fn close_opportunity_lost(
        ...
        if opp.status != "open" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot close '{}' opportunity as lost. Must be 'open'.",
                opp.status
            )));
        }
        info!(
            "Closing opportunity {} as LOST: {}",
            opp.opportunity_number,
            lost_reason.unwrap_or("N/A")
        );
        self.repository
            .close_opportunity_lost(id, lost_reason)
            .await
    }
    ...
    pub async fn list_stage_history(
        &self,
        opportunity_id: Uuid,
    ) -> AtlasResult<Vec<atlas_shared::OpportunityStageHistory>> {
    ...
    pub async fn add_opportunity_line(
        ...
        self.repository
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Opportunity {opportunity_id} not found"))
            })?;
        ...
        let existing = self
            .repository
            .list_opportunity_lines(opportunity_id)
            .await?;
        let line_number = (existing.len() as i32) + 1;

        info!(
            "Adding line {} to opportunity {}",
            product_name, opportunity_id
        );
    ...
    pub async fn list_opportunity_lines(
        &self,
        opportunity_id: Uuid,
    ) -> AtlasResult<Vec<atlas_shared::OpportunityLine>> {
    ...
    pub async fn create_activity(
        ...
        if subject.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Activity subject is required".to_string(),
            ));
        }
        ...
        if !VALID_ACTIVITY_PRIORITIES.contains(&priority) {
            return Err(AtlasError::ValidationFailed(format!(
                ...
                priority,
                VALID_ACTIVITY_PRIORITIES.join(", ")
            )));
        }
    ...
    pub async fn list_activities(
        ...
        self.repository
            .list_activities(org_id, lead_id, opportunity_id)
            .await
    }
    ...
    pub async fn cancel_activity(&self, id: Uuid) -> AtlasResult<atlas_shared::SalesActivity> {
        ...
        if activity.status == "completed" || activity.status == "cancelled" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot cancel activity in '{}' status",
                activity.status
            )));
        }
    ...
    pub async fn get_dashboard(
        &self,
        org_id: Uuid,
    ) -> AtlasResult<atlas_shared::SalesPipelineDashboard> {
