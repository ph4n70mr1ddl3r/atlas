...
pub trait LearningManagementRepository: Send + Sync {
    ...
    async fn get_learning_item_by_code(
        &self,
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<LearningItem>>;
    ...
    async fn update_learning_item_status(
        &self,
        id: Uuid,
        status: &str,
    ) -> AtlasResult<LearningItem>;
    ...
    async fn get_learning_category_by_code(
        &self,
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<LearningCategory>>;
    async fn list_learning_categories(
        &self,
        org_id: Uuid,
    ...
    async fn get_learning_path_by_code(
        &self,
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<LearningPath>>;
    ...
    async fn update_learning_path_status(
        &self,
        id: Uuid,
        status: &str,
    ) -> AtlasResult<LearningPath>;
    async fn update_learning_path_total_items(
        &self,
        id: Uuid,
        total: i32,
    ) -> AtlasResult<LearningPath>;
    ...
    async fn list_learning_path_items(
        &self,
        learning_path_id: Uuid,
    ...
    async fn update_learning_assignment_status(
        &self,
        id: Uuid,
        status: &str,
    ) -> AtlasResult<LearningAssignment>;
...
fn row_to_learning_item(row: &sqlx::postgres::PgRow) -> LearningItem {
    LearningItem {
        ...
        format: row
            .try_get("format")
            .unwrap_or_else(|_| "self_paced".to_string()),
...
impl LearningManagementRepository for PostgresLearningManagementRepository {
    ...
    async fn list_learning_items(
        ...
        let mut query =
            String::from("SELECT * FROM _atlas.learning_items WHERE organization_id = $1");
        let mut bind_idx = 2u32;
        let has_type = item_type.is_some();
        if has_type {
            query.push_str(&format!(" AND item_type = ${bind_idx}"));
            bind_idx += 1;
        }
    ...
    async fn delete_learning_category(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        ...
        if result.rows_affected() == 0 {
            return Err(AtlasError::EntityNotFound(format!(
                "Learning category '{code}' not found"
            )));
        }
        Ok(())
    }
    ...
    async fn list_learning_enrollments(
        ...
        let mut query =
            String::from("SELECT * FROM _atlas.learning_enrollments WHERE organization_id = $1");
        let mut bind_idx = 2u32;
        let has_item = learning_item_id.is_some();
        if has_item {
            query.push_str(&format!(" AND learning_item_id = ${bind_idx}"));
            bind_idx += 1;
        }
        let has_person = person_id.is_some();
        if has_person {
            query.push_str(&format!(" AND person_id = ${bind_idx}"));
            bind_idx += 1;
        }
        let has_status = status.is_some();
        if has_status {
        ...
        let mut q = sqlx::query(&query).bind(org_id);
        if let Some(li) = learning_item_id {
            q = q.bind(li);
        }
        if let Some(pi) = person_id {
            q = q.bind(pi);
        }
        if let Some(s) = status {
            q = q.bind(s);
    ...
    async fn list_learning_paths(
        ...
        let mut query =
            String::from("SELECT * FROM _atlas.learning_paths WHERE organization_id = $1");
        let mut bind_idx = 2u32;
        let has_status = status.is_some();
        if has_status {
            query.push_str(&format!(" AND status = ${bind_idx}"));
    ...
    async fn list_learning_assignments(
        ...
        let mut query =
            String::from("SELECT * FROM _atlas.learning_assignments WHERE organization_id = $1");
        let mut bind_idx = 2u32;
        let has_status = status.is_some();
        if has_status {
            query.push_str(&format!(" AND status = ${bind_idx}"));
        ...
        if has_type {
            query.push_str(&format!(" AND assignment_type = ${bind_idx}"));
        }
        query.push_str(" ORDER BY created_at DESC");
    ...
    async fn get_learning_dashboard(&self, org_id: Uuid) -> AtlasResult<LearningDashboard> {
        ...
        let completion_rate = if total_enrollments > 0 {
            Some(format!(
                "{:.1}",
                (f64::from(completed_count) / f64::from(total_enrollments)) * 100.0
            ))
        } else {
            None
        };
