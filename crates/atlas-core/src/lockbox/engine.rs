...
use super::{
    AtlasError, AtlasResult, LockboxApplication, LockboxBatch, LockboxDashboard, LockboxReceipt,
    LockboxRepository, LockboxTransmissionFormat,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
pub struct LockboxEngine {
    repository: Arc<dyn LockboxRepository>,
}

impl LockboxEngine {
    pub fn new(r: Arc<dyn LockboxRepository>) -> Self {
        Self { repository: r }
    }
    ...
    ) -> AtlasResult<LockboxBatch> {
        if batch_number.is_empty() || lockbox_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Batch number and lockbox number are required".into(),
            ));
        }
    ...
    pub async fn get_batch_by_id(&self, id: Uuid) -> AtlasResult<Option<LockboxBatch>> {
    ...
    pub async fn validate_batch(&self, id: Uuid) -> AtlasResult<LockboxBatch> {
        ...
        if batch.status != "imported" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot validate batch in '{}' status",
                batch.status
            )));
        }
        ...
        let total: f64 = receipts
            .iter()
            .map(|r| r.receipt_amount.parse::<f64>().unwrap_or(0.0))
            .sum();
        self.repository
            .update_batch_amounts(
                id,
                &total.to_string(),
                receipts.len() as i32,
                "0",
            ...
            .await?;
        self.repository
            .update_batch_status(id, "validated", None)
            .await
    }
    ...
    pub async fn create_receipt(
        ...
        let amt: f64 = receipt_amount
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid receipt amount".into()))?;
        if amt <= 0.0 {
            return Err(AtlasError::ValidationFailed(
    ...
    pub async fn manual_apply_receipt(
    ...
    ) -> AtlasResult<LockboxApplication> {
        let receipt = self
            .repository
        ...
        if receipt.status != "unapplied" && receipt.status != "partial" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot apply receipt in '{}' status",
                receipt.status
            )));
        }
        let amt: f64 = applied_amount
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid applied amount".into()))?;
        if amt <= 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Applied amount must be positive".into(),
            ));
        }
        ...
        if current_applied + amt > receipt_amt {
            return Err(AtlasError::ValidationFailed(
                "Applied amount exceeds receipt amount".into(),
            ));
        }
        let app = self
            .repository
        ...
        let new_status = if new_unapplied.abs() < 0.01 {
            "applied"
        } else {
            "partial"
        };
    ...
    pub async fn list_applications(
        &self,
        receipt_id: Uuid,
    ) -> AtlasResult<Vec<LockboxApplication>> {
        self.repository
            .list_applications_by_receipt(receipt_id)
            .await
    }
...
mod tests {
    ...
    impl LockboxRepository for MockRepo {
        async fn create_batch(
            ...
            let b = LockboxBatch {
                ...
                lockbox_number: ln.into(),
                bank_name: bank.map(Into::into),
                deposit_date: dd,
                status: "imported".into(),
                total_amount: "0".into(),
                total_receipts: 0,
                applied_amount: "0".into(),
                unapplied_amount: "0".into(),
                on_account_amount: "0".into(),
                currency_code: cc.into(),
                source_file_name: sfn.map(Into::into),
                error_message: None,
        ...
        async fn get_batch(&self, org_id: Uuid, bn: &str) -> AtlasResult<Option<LockboxBatch>> {
            Ok(self
                .batches
                .lock()
                .unwrap()
        ...
        async fn update_batch_amounts(
            ...
            if let Some(b) = bs.iter_mut().find(|b| b.id == id) {
                ...
                b.total_receipts = count;
                b.applied_amount = applied.into();
                b.unapplied_amount = unapplied.into();
                b.on_account_amount = on_account.into();
            }
            Ok(())
        }
        async fn create_receipt(
            ...
            let r = LockboxReceipt {
                id: Uuid::new_v4(),
                organization_id: org_id,
                batch_id: bid,
                receipt_number: rn.into(),
                customer_number: cn.map(Into::into),
                customer_id: cid,
                receipt_date: rd,
                receipt_amount: ra.into(),
                applied_amount: "0".into(),
                unapplied_amount: ra.into(),
                on_account_amount: "0".into(),
                status: "unapplied".into(),
                match_type: None,
                remittance_reference: rr.map(Into::into),
                error_message: None,
                metadata: serde_json::json!({}),
                created_at: chrono::Utc::now(),
                updated_at: chrono::Utc::now(),
        ...
        async fn list_receipts_by_batch(&self, bid: Uuid) -> AtlasResult<Vec<LockboxReceipt>> {
            Ok(self
                .receipts
                .lock()
                .unwrap()
        ...
        async fn update_receipt_status(
            ...
            r.on_account_amount = on_account.into();
            if let Some(m) = mt {
                r.match_type = Some(m.into());
            }
            Ok(r.clone())
        }
        async fn create_application(
            ...
            let a = LockboxApplication {
                id: Uuid::new_v4(),
                organization_id: org_id,
                receipt_id: rid,
                invoice_id: iid,
                invoice_number: inum.map(Into::into),
                applied_amount: amt.into(),
                application_date: ad,
                status: "applied".into(),
                applied_by: ab,
                metadata: serde_json::json!({}),
        ...
        async fn list_applications_by_receipt(
            &self,
            rid: Uuid,
        ) -> AtlasResult<Vec<LockboxApplication>> {
            Ok(self
                ...
                .unwrap()
                .iter()
                .filter(|a| a.receipt_id == rid)
                .cloned()
                .collect())
        }
        async fn reverse_application(&self, _id: Uuid) -> AtlasResult<LockboxApplication> {
        ...
        async fn list_formats(&self, _: Uuid) -> AtlasResult<Vec<LockboxTransmissionFormat>> {
            Ok(vec![])
        }
        async fn delete_format(&self, _: Uuid) -> AtlasResult<()> {
            Ok(())
        }
        async fn get_dashboard(&self, _: Uuid) -> AtlasResult<LockboxDashboard> {
            Ok(LockboxDashboard {
                total_batches: 0,
                pending_batches: 0,
                completed_batches: 0,
    ...
    fn eng() -> LockboxEngine {
        LockboxEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_manual_apply_receipt() {
        ...
        let _ = e
            ...
            .unwrap();
        let r = e
            .manual_apply_receipt(
                e.list_receipts(b.id).await.unwrap()[0].id,
                "INV-100",
                "600.00",
                None,
            )
