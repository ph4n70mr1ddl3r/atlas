...
pub trait LockboxRepository: Send + Sync {
    ...
    async fn get_batch(
        ...
        org_id: Uuid,
        batch_number: &str,
    ) -> AtlasResult<Option<LockboxBatch>>;
...
#[allow(dead_code)]
pub struct PostgresLockboxRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
impl PostgresLockboxRepository {
    #[must_use]
...
impl LockboxRepository for PostgresLockboxRepository {
    ...
    async fn get_batch(&self, _: Uuid, _: &str) -> AtlasResult<Option<LockboxBatch>> {
        Ok(None)
    }
    async fn get_batch_by_id(&self, _: Uuid) -> AtlasResult<Option<LockboxBatch>> {
        Ok(None)
    }
    async fn list_batches(&self, _: Uuid, _: Option<&str>) -> AtlasResult<Vec<LockboxBatch>> {
        Ok(vec![])
    }
    async fn update_batch_status(
        &self,
    ...
    async fn get_receipt(&self, _: Uuid) -> AtlasResult<Option<LockboxReceipt>> {
        Ok(None)
    }
    async fn list_receipts_by_batch(&self, _: Uuid) -> AtlasResult<Vec<LockboxReceipt>> {
        Ok(vec![])
    }
    async fn update_receipt_status(
        &self,
    ...
    async fn list_applications_by_receipt(&self, _: Uuid) -> AtlasResult<Vec<LockboxApplication>> {
        Ok(vec![])
    }
    async fn reverse_application(&self, _: Uuid) -> AtlasResult<LockboxApplication> {
        Err(AtlasError::EntityNotFound("Mock".into()))
    }
    async fn create_format(
        &self,
