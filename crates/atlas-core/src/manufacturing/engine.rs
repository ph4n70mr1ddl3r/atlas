...
use atlas_shared::{
    AtlasError, AtlasResult, CreateWorkDefinitionRequest, CreateWorkOrderRequest,
    IssueMaterialRequest, ManufacturingDashboard, ReportCompletionRequest, WorkDefinition,
    WorkDefinitionComponent, WorkDefinitionOperation, WorkOrder, WorkOrderMaterial,
    WorkOrderOperation,
};
use std::sync::Arc;
use tracing::info;
...
impl ManufacturingEngine {
    ...
    pub async fn create_work_definition(
        ...
        if !VALID_PRODUCTION_TYPES.contains(&production_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid production type '{}'. Must be one of: {}",
                production_type,
                VALID_PRODUCTION_TYPES.join(", ")
            )));
        }

        // Generate definition number if not provided
        let definition_number = req
            .definition_number
            .unwrap_or_else(|| format!("WD-{}", Uuid::new_v4().to_string()[..8].to_uppercase()));

        info!(
            "Creating work definition {} for org {}",
            definition_number, org_id
        );
        ...
        if let Some(s) = status {
            if !VALID_DEFINITION_STATUSES.contains(&s) {
                return Err(AtlasError::ValidationFailed(format!(
                    ...
                    s,
                    VALID_DEFINITION_STATUSES.join(", ")
                )));
            }
        }
    ...
    pub async fn activate_work_definition(
        ...
        if def.organization_id != org_id {
            return Err(AtlasError::Forbidden(
                "Work definition does not belong to your organization".to_string(),
            ));
        }
        ...
        self.repository
            .update_work_definition_status(id, "active")
            .await
    }
    ...
    pub async fn deactivate_work_definition(
        ...
        self.repository
            .update_work_definition_status(id, "inactive")
            .await
    }
    ...
    pub async fn add_work_definition_component(
        ...
        if component_item_code.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Component item code is required".to_string(),
            ));
        }

        let qty: f64 = quantity_required.parse().map_err(|_| {
            AtlasError::ValidationFailed("Quantity required must be a valid number".to_string())
        })?;
        if qty <= 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Quantity required must be positive".to_string(),
            ));
        }
        ...
        let def = self
            .repository
            .get_work_definition_by_id(work_definition_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!(
                    "Work definition {work_definition_id} not found"
                ))
            })?;
        ...
        let existing = self
            .repository
            .list_work_definition_components(work_definition_id)
            .await?;
        let line_number = (existing.len() as i32) + 1;

        info!(
            "Adding component {} to work definition {}",
            component_item_code, def.definition_number
        );
    ...
    pub async fn delete_work_definition_component(
        ...
        for def in &all_defs {
            let comps = self
                .repository
                .list_work_definition_components(def.id)
                .await?;
        ...
        Err(AtlasError::EntityNotFound(format!(
            "Component {id} not found in your organization"
        )))
    }
    ...
    pub async fn add_work_definition_operation(
        ...
        if operation_name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Operation name is required".to_string(),
            ));
        }
        if operation_sequence <= 0 {
            return Err(AtlasError::ValidationFailed(
                "Operation sequence must be positive".to_string(),
            ));
        }
        ...
        info!(
            "Adding operation {} (seq {}) to work definition {}",
            operation_name, operation_sequence, def.definition_number
        );
    ...
    pub async fn list_work_definition_operations(
        ...
        self.repository
            .list_work_definition_operations(work_definition_id)
            .await
    }
    ...
    pub async fn delete_work_definition_operation(
        ...
        for def in &all_defs {
            let ops = self
                .repository
                .list_work_definition_operations(def.id)
                .await?;
        ...
        Err(AtlasError::EntityNotFound(format!(
            "Operation {id} not found in your organization"
        )))
    }
    ...
    pub async fn create_work_order(
        ...
        // Validate quantity
        let qty: f64 = req.quantity_ordered.parse().map_err(|_| {
            AtlasError::ValidationFailed("Quantity ordered must be a valid number".to_string())
        })?;
        if qty <= 0.0 {
            return Err(AtlasError::ValidationFailed(
        ...
        let estimated_costs = if let Some(wd_id) = req.work_definition_id {
            let wd = self
                .repository
                .get_work_definition_by_id(wd_id)
                .await?
                .ok_or_else(|| {
                    AtlasError::EntityNotFound(format!("Work definition {wd_id} not found"))
                })?;
        ...
        let (mat_cost, lab_cost, oh_cost, total_cost) =
            estimated_costs.unwrap_or((0.0, 0.0, 0.0, 0.0));

        info!(
            "Creating work order {} for org {}",
            work_order_number, org_id
        );
        ...
        let wo = self
            ...
            .create_work_order(
                ...
                req.unit_of_measure.as_deref().unwrap_or("EA"),
                req.scheduled_start_date,
                req.scheduled_completion_date,
                req.due_date,
                priority,
                req.production_line.as_deref(),
                req.work_center_code.as_deref(),
                ...
                req.source_document_number.as_deref(),
                req.firm_planned.unwrap_or(false),
                req.company_id,
                req.plant_code.as_deref(),
                req.created_by,
            )
            .await?;
        ...
        if let Some(wd_id) = req.work_definition_id {
            ...
            let components = self
                .repository
                .list_work_definition_components(wd_id)
                .await?;
            ...
            let operations = self
                .repository
                .list_work_definition_operations(wd_id)
                .await?;
            for op in &operations {
                self.repository
                    .create_work_order_operation(
                        org_id,
                        wo.id,
    ...
    pub async fn release_work_order(&self, org_id: Uuid, id: Uuid) -> AtlasResult<WorkOrder> {
        ...
        if wo.organization_id != org_id {
            return Err(AtlasError::Forbidden(
                "Work order does not belong to your organization".to_string(),
            ));
        }
        ...
        self.repository
            .update_work_order_status(id, "released")
            .await
    }
    ...
    pub async fn start_work_order(&self, org_id: Uuid, id: Uuid) -> AtlasResult<WorkOrder> {
        ...
        if let Some(first_op) = operations.first() {
            self.repository
                .update_work_order_operation_status(first_op.id, "running")
                .await?;
        }

        self.repository
            .update_work_order_status(id, "started")
            .await?;
        self.repository
            .update_work_order_dates(id, Some(chrono::Utc::now().date_naive()), None)
            .await
    }
    ...
    pub async fn complete_work_order(&self, org_id: Uuid, id: Uuid) -> AtlasResult<WorkOrder> {
        ...
        for op in &operations {
            if op.status == "pending" || op.status == "in_queue" || op.status == "running" {
                self.repository
                    .update_work_order_operation_status(op.id, "completed")
                    .await?;
            }
        }

        self.repository
            .update_work_order_status(id, "completed")
            .await?;
        self.repository
            .update_work_order_dates(id, None, Some(chrono::Utc::now().date_naive()))
            .await
    }
    ...
    pub async fn cancel_work_order(
        ...
        info!(
            "Cancelling work order {} (reason: {:?})",
            wo.work_order_number, reason
        );
        self.repository
            .update_work_order_cancellation(id, reason)
            .await?;
        self.repository
            .update_work_order_status(id, "cancelled")
            .await
    }
    ...
    pub async fn report_completion(
        ...
        let wo = self
            .repository
            .get_work_order_by_id(work_order_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Work order {work_order_id} not found"))
            })?;
        ...
        let completed: f64 = req.quantity_completed.parse().map_err(|_| {
            AtlasError::ValidationFailed("Quantity completed must be a valid number".to_string())
        })?;
        let scrapped: f64 = req.quantity_scrapped.parse().map_err(|_| {
            AtlasError::ValidationFailed("Quantity scrapped must be a valid number".to_string())
        })?;
        ...
        if let Some(op_seq) = req.operation_sequence {
            let operations = self
                .repository
                .list_work_order_operations(work_order_id)
                .await?;
            if let Some(op) = operations.iter().find(|o| o.operation_sequence == op_seq) {
                ...
                self.repository
                    .update_work_order_operation_quantities(
                        op.id,
                ...
                if op_remaining <= 0.0 {
                    ...
                    if let Some(next_op) = operations
                        .iter()
                        .find(|o| o.operation_sequence > op_seq && o.status == "pending")
                    {
                        self.repository
                            .update_work_order_operation_status(next_op.id, "running")
                            .await?;
                    }
                } else {
                    self.repository
                        .update_work_order_operation_status(op.id, "running")
                        .await?;
                }
                ...
                if req.actual_run_hours.is_some() {
                    self.repository
                        .update_work_order_operation_time(
                            op.id,
                            None,
                            req.actual_run_hours.as_deref(),
                        )
                        .await?;
                }
                if req.actual_labor_cost.is_some() || req.actual_overhead_cost.is_some() {
                    self.repository
                        .update_work_order_operation_costs(
                            op.id,
        ...
        let new_labor: f64 = req
            .actual_labor_cost
            .as_deref()
            .unwrap_or("0")
            .parse()
            .unwrap_or(0.0);
        let new_overhead: f64 = req
        ...
        self.repository
            .update_work_order_actual_costs(
                work_order_id,
                None,
        ...
        if total_completed + total_scrapped >= ordered {
            ...
            self.repository
                .update_work_order_status(work_order_id, "completed")
                .await?;
            self.repository
                .update_work_order_dates(work_order_id, None, Some(chrono::Utc::now().date_naive()))
                .await
        } else {
            self.repository
                .get_work_order_by_id(work_order_id)
                ...
                .ok_or_else(|| {
                    AtlasError::EntityNotFound(format!(
                        "Work order {work_order_id} not found after progress update"
                    ))
                })
        }
    }
    ...
    pub async fn issue_materials(
        ...
        for issue in &issues {
            let mat = self
                .repository
                .get_work_order_material(issue.material_id)
                .await?
                .ok_or_else(|| {
                    AtlasError::EntityNotFound(format!(
            ...
            let qty: f64 = issue.quantity_issued.parse().map_err(|_| {
                AtlasError::ValidationFailed("Quantity issued must be a valid number".to_string())
            })?;
            if qty <= 0.0 {
                return Err(AtlasError::ValidationFailed(
                    "Quantity issued must be positive".to_string(),
                ));
            }
            ...
            if already_issued + qty > required {
                return Err(AtlasError::ValidationFailed(format!(
                    ...
                    qty,
                    required - already_issued,
                    required
                )));
            }

            info!(
                "Issuing {} units of {} to work order {}",
                qty, mat.component_item_code, wo.work_order_number
            );
            let updated = self
                .repository
                .update_work_order_material_issue(issue.material_id, &issue.quantity_issued)
                .await?;
    ...
    pub async fn return_material(
        ...
        let mat = self
            .repository
            .get_work_order_material(material_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Material {material_id} not found"))
            })?;

        if mat.organization_id != org_id {
            return Err(AtlasError::Forbidden(
                "Material does not belong to your organization".to_string(),
            ));
        }

        let qty: f64 = quantity_returned.parse().map_err(|_| {
            AtlasError::ValidationFailed("Quantity returned must be a valid number".to_string())
        })?;
        if qty <= 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Quantity returned must be positive".to_string(),
            ));
        }
        ...
        if already_returned + qty > issued {
            return Err(AtlasError::ValidationFailed(format!(
                ...
                qty,
                issued - already_returned,
                already_returned
            )));
        }

        info!(
            "Returning {} units of {} from work order",
            qty, mat.component_item_code
        );
        let updated = self
            .repository
            .update_work_order_material_return(material_id, quantity_returned)
            .await?;
        ...
        self.repository
            .update_work_order_material_status(material_id, new_status)
            .await
    }
    ...
    pub async fn update_operation_status(
        ...
        if !VALID_OPERATION_STATUSES.contains(&status) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid operation status '{}'. Must be one of: {}",
                status,
                VALID_OPERATION_STATUSES.join(", ")
            )));
        }

        let op = self
            .repository
            .get_work_order_operation(id)
            .await?
            .ok_or_else(|| AtlasError::EntityNotFound(format!("Operation {id} not found")))?;

        if op.organization_id != org_id {
            return Err(AtlasError::Forbidden(
                "Operation does not belong to your organization".to_string(),
            ));
        }

        info!(
            "Updating operation {} (seq {}) to status {}",
            op.operation_name, op.operation_sequence, status
        );
        self.repository
            .update_work_order_operation_status(id, status)
            .await
    }
    ...
    pub async fn list_work_order_materials(
        ...
        self.repository
            .list_work_order_materials(work_order_id)
            .await
    }
...
mod tests {
    ...
    impl ManufacturingRepository for MockMfgRepo {
        async fn create_work_definition(
            ...
            self.state
                ...
                .unwrap()
                .definitions
                .insert(id, ("draft".to_string(), def.clone()));
            Ok(def)
        }
        ...
        async fn get_work_definition(
            ...
            Ok(state
                ...
                .values()
                .find(|(_, d)| {
                    d.organization_id == org_id && d.definition_number == definition_number
                })
                .map(|(_, d)| d.clone()))
        }
        ...
        async fn create_work_order(
            ...
            let wo = WorkOrder {
                ...
                firm_planned,
                company_id,
                plant_code: plant_code.map(|s| s.to_string()),
                metadata: serde_json::json!({}),
                created_by,
                submitted_at: None,
                released_at: None,
        ...
        async fn get_work_order(
            ...
            Ok(state
                .orders
                .values()
                .find(|o| o.organization_id == org_id && o.work_order_number == work_order_number)
                .cloned())
        }
        ...
        async fn update_work_order_quantities(
            ...
            let mut state = self.state.lock().unwrap();
            if let Some(wo) = state.orders.get_mut(&id) {
                if let Some(qc) = quantity_completed {
                    wo.quantity_completed = qc.to_string();
                }
                if let Some(qs) = quantity_scrapped {
                    wo.quantity_scrapped = qs.to_string();
                }
                wo.updated_at = chrono::Utc::now();
                Ok(wo.clone())
            } else {
        ...
        async fn update_work_order_actual_costs(
            ...
            let mut state = self.state.lock().unwrap();
            if let Some(wo) = state.orders.get_mut(&id) {
                if let Some(c) = actual_material_cost {
                    wo.actual_material_cost = c.to_string();
                }
                if let Some(c) = actual_labor_cost {
                    wo.actual_labor_cost = c.to_string();
                }
                if let Some(c) = actual_overhead_cost {
                    wo.actual_overhead_cost = c.to_string();
                }
                if let Some(c) = actual_total_cost {
                    wo.actual_total_cost = c.to_string();
                }
                wo.updated_at = chrono::Utc::now();
                Ok(wo.clone())
            } else {
        ...
        async fn update_work_order_dates(
            ...
            let mut state = self.state.lock().unwrap();
            if let Some(wo) = state.orders.get_mut(&id) {
                if let Some(d) = actual_start_date {
                    wo.actual_start_date = Some(d);
                }
                if let Some(d) = actual_completion_date {
                    wo.actual_completion_date = Some(d);
                }
                wo.updated_at = chrono::Utc::now();
                Ok(wo.clone())
            } else {
        ...
        async fn get_work_order_operation(
            &self,
            id: Uuid,
        ) -> AtlasResult<Option<WorkOrderOperation>> {
            Ok(self
                .state
                .lock()
                .unwrap()
        ...
        async fn update_work_order_operation_quantities(
            ...
            if let Some(op) = state.order_operations.get_mut(&id) {
                if let Some(qc) = quantity_completed {
                    op.quantity_completed = qc.to_string();
                }
                if let Some(qs) = quantity_scrapped {
                    op.quantity_scrapped = qs.to_string();
                }
                if let Some(qr) = quantity_rejected {
                    op.quantity_rejected = qr.to_string();
                }
                op.updated_at = chrono::Utc::now();
                Ok(op.clone())
            } else {
        ...
        async fn update_work_order_operation_time(
            ...
            let mut state = self.state.lock().unwrap();
            if let Some(op) = state.order_operations.get_mut(&id) {
                if let Some(h) = actual_setup_hours {
                    op.actual_setup_hours = h.to_string();
                }
                if let Some(h) = actual_run_hours {
                    op.actual_run_hours = h.to_string();
                }
                op.updated_at = chrono::Utc::now();
                Ok(op.clone())
            } else {
        ...
        async fn update_work_order_operation_costs(
            ...
            if let Some(op) = state.order_operations.get_mut(&id) {
                if let Some(c) = actual_labor_cost {
                    op.actual_labor_cost = c.to_string();
                }
                if let Some(c) = actual_overhead_cost {
                    op.actual_overhead_cost = c.to_string();
                }
                if let Some(c) = actual_machine_cost {
                    op.actual_machine_cost = c.to_string();
                }
                op.updated_at = chrono::Utc::now();
                Ok(op.clone())
            } else {
        ...
        async fn update_work_order_operation_dates(
            ...
            if let Some(op) = state.order_operations.get_mut(&id) {
                if let Some(d) = actual_start_date {
                    op.actual_start_date = Some(d);
                }
                if let Some(d) = actual_completion_date {
                    op.actual_completion_date = Some(d);
                }
                op.updated_at = chrono::Utc::now();
                Ok(op.clone())
            } else {
        ...
        async fn get_work_order_material(
            &self,
            id: Uuid,
        ) -> AtlasResult<Option<WorkOrderMaterial>> {
        ...
        async fn update_work_order_material_issue(
            ...
            if let Some(mat) = state.order_materials.get_mut(&id) {
                let new_issued: f64 = mat.quantity_issued.parse().unwrap_or(0.0)
                    + quantity_issued.parse().unwrap_or(0.0);
                let required: f64 = mat.quantity_required.parse().unwrap_or(0.0);
                mat.quantity_issued = format!("{:.4}", new_issued);
                mat.status = if new_issued >= required {
                    "fully_issued"
                } else {
                    "partially_issued"
                }
        ...
        async fn update_work_order_material_return(
            ...
            let mut state = self.state.lock().unwrap();
            if let Some(mat) = state.order_materials.get_mut(&id) {
                let new_returned: f64 = mat.quantity_returned.parse().unwrap_or(0.0)
                    + quantity_returned.parse().unwrap_or(0.0);
    ...
    async fn test_activate_work_definition_success() {
        ...
        // Add a component
        engine
            .add_work_definition_component(org_id, def.id, "RAW-MAT-01", "5", "EA")
            .await
            .unwrap();

        // Add an operation
        engine
            .add_work_definition_operation(org_id, def.id, 10, "Assembly", Some("WC-ASSEMBLY"))
            .await
            .unwrap();
    ...
    async fn test_cannot_modify_active_definition() {
        ...
        engine
            .add_work_definition_component(org_id, def.id, "COMP-01", "1", "EA")
            .await
            .unwrap();
        engine
            .add_work_definition_operation(org_id, def.id, 10, "Cut", None)
            .await
            .unwrap();
        engine
            .activate_work_definition(org_id, def.id)
            .await
            .unwrap();

        // Should fail to add component to active definition
        let result = engine
            .add_work_definition_component(org_id, def.id, "COMP-02", "2", "EA")
            .await;
        assert!(result.is_err());
    ...
    async fn test_cancel_work_order() {
        ...
        let result = engine
            .cancel_work_order(org_id, wo.id, Some("Customer cancelled"))
            .await;
        assert!(result.is_ok());
    ...
    async fn test_create_work_order_copies_bom_and_routing() {
        ...
        engine
            .add_work_definition_component(org_id, def.id, "COMP-A", "2", "EA")
            .await
            .unwrap();
        engine
            .add_work_definition_component(org_id, def.id, "COMP-B", "3", "EA")
            .await
            .unwrap();
        engine
            .add_work_definition_operation(org_id, def.id, 10, "Cut", Some("WC-01"))
            .await
            .unwrap();
        engine
            .add_work_definition_operation(org_id, def.id, 20, "Assemble", Some("WC-02"))
            .await
            .unwrap();
        engine
            .activate_work_definition(org_id, def.id)
