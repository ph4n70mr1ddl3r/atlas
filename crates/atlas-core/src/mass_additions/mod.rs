...
const VALID_ASSET_TYPES: &[&str] = &["tangible", "intangible", "leased", "cip", "group"];
...
pub trait MassAdditionRepository: Send + Sync {
    async fn create(
        ...
        cost: &str,
        salvage_value: &str,
        salvage_value_percent: Option<&str>,
    ...
    async fn update_category_and_book(
        &self,
        id: Uuid,
        category_id: Option<Uuid>,
        category_code: Option<&str>,
...
#[allow(dead_code)]
pub struct PostgresMassAdditionRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
impl PostgresMassAdditionRepository {
    #[must_use]
...
impl MassAdditionRepository for PostgresMassAdditionRepository {
    ...
    async fn get(&self, _: Uuid) -> AtlasResult<Option<MassAddition>> {
        Ok(None)
    }
    async fn get_by_number(&self, _: Uuid, _: &str) -> AtlasResult<Option<MassAddition>> {
        Ok(None)
    }
    async fn list(
        &self,
    ...
    async fn update_status(&self, _: Uuid, _: &str, _: Option<&str>) -> AtlasResult<MassAddition> {
        Err(AtlasError::EntityNotFound("Not found".into()))
    }
    async fn update_merge(&self, _: Uuid, _: Uuid, _: &str) -> AtlasResult<MassAddition> {
        Err(AtlasError::EntityNotFound("Not found".into()))
    }
    async fn update_category_and_book(
        &self,
...
impl MassAdditionEngine {
    ...
    pub async fn create_from_invoice(
        ...
        // Validate cost
        let cost_val: f64 = cost
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Cost must be a valid number".into()))?;
        if cost_val <= 0.0 {
        ...
        self.repository
            .create(
                ...
                cost,
                "0",
                None, // salvage_value, salvage_value_percent
                None,
                None, // asset_account_code, depr_expense_account_code
                None, // location
                None,
                None, // department
                supplier_id,
    ...
    pub async fn get_by_number(
        ...
        org_id: Uuid,
        number: &str,
    ) -> AtlasResult<Option<MassAddition>> {
    ...
    pub async fn hold(&self, id: Uuid) -> AtlasResult<MassAddition> {
        let ma =
            self.repository.get(id).await?.ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Mass addition {id} not found"))
            })?;

        if ma.status != "posted" && ma.status != "pending_review" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot hold mass addition in '{}' status",
                ma.status
            )));
        }
    ...
    pub async fn release(&self, id: Uuid) -> AtlasResult<MassAddition> {
        ...
        if ma.status != "on_hold" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot release mass addition in '{}' status. Must be 'on_hold'.",
                ma.status
            )));
        }
    ...
    pub async fn reject(&self, id: Uuid, reason: &str) -> AtlasResult<MassAddition> {
        ...
        if ma.status == "converted" || ma.status == "rejected" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot reject mass addition in '{}' status",
                ma.status
            )));
        }
        info!(
            "Rejecting mass addition {}: {}",
            ma.mass_addition_number, reason
        );
        self.repository
    ...
    pub async fn merge(&self, id: Uuid, merge_to_id: Uuid) -> AtlasResult<MassAddition> {
        ...
        let target = self.repository.get(merge_to_id).await?.ok_or_else(|| {
            AtlasError::EntityNotFound(format!("Target mass addition {merge_to_id} not found"))
        })?;

        if ma.organization_id != target.organization_id {
            return Err(AtlasError::ValidationFailed(
                "Cannot merge across organizations".into(),
            ));
        }
        if ma.status == "converted" || ma.status == "rejected" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot merge mass addition in '{}' status",
                ma.status
            )));
        }
        if target.status == "converted" || target.status == "rejected" {
            return Err(AtlasError::ValidationFailed(
                "Cannot merge into a converted/rejected target".into(),
            ));
        }

        info!(
            "Merging mass addition {} into {}",
            ma.mass_addition_number, target.mass_addition_number
        );
        self.repository
            .update_merge(id, target.id, &target.mass_addition_number)
            .await
    }
    ...
    pub async fn update_category_and_book(
        ...
        if ma.status == "converted" {
            return Err(AtlasError::WorkflowError(
                "Cannot modify a converted mass addition".into(),
            ));
        }

        info!(
            "Updating category/book for mass addition {}",
            ma.mass_addition_number
        );
        self.repository
            .update_category_and_book(id, category_id, category_code, book_id, book_code)
            .await
    }
    ...
    pub async fn convert(&self, id: Uuid) -> AtlasResult<MassAddition> {
        ...
        if ma.status != "posted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot convert mass addition in '{}' status. Must be 'posted'.",
                ma.status
            )));
        }

        // Validate required fields for conversion
        if ma.category_code.is_none()
            || ma
                .category_code
                .as_ref()
        ...
        {
            return Err(AtlasError::ValidationFailed(
                "Category code is required for conversion".into(),
            ));
        }
        if ma.book_code.is_none()
            || ma
                .book_code
                .as_ref()
                .is_none_or(std::string::String::is_empty)
        {
            return Err(AtlasError::ValidationFailed(
                "Book code is required for conversion".into(),
            ));
        }

        info!(
            "Converting mass addition {} to fixed asset",
            ma.mass_addition_number
        );
...
mod tests {
    ...
    impl MockRepo {
        fn new() -> Self {
            Self {
                additions: std::sync::Mutex::new(vec![]),
            }
        }
    }
    ...
    impl MassAdditionRepository for MockRepo {
        async fn create(
            ...
            let ma = MassAddition {
                id: Uuid::new_v4(),
                organization_id: org_id,
                mass_addition_number: ma_number.into(),
                invoice_id,
                invoice_number: invoice_number.map(Into::into),
                invoice_line_id,
                invoice_line_number,
                description: description.map(Into::into),
                asset_key: asset_key.map(Into::into),
                category_id,
                ...
                supplier_id,
                supplier_number: supplier_number.map(Into::into),
                supplier_name: supplier_name.map(Into::into),
                po_number: po_number.map(Into::into),
                invoice_date,
                date_placed_in_service,
                merge_to_id: None,
                merge_to_number: None,
                reject_reason: None,
                status: status.into(),
        ...
        async fn get_by_number(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|m| m.organization_id == org_id && m.mass_addition_number == number)
                .cloned())
        }
        async fn list(
            &self,
            ...
            Ok(self
                ...
                .iter()
                .filter(|m| {
                    m.organization_id == org_id && (status.is_none() || m.status == status.unwrap())
                })
                .cloned()
                .collect())
        }
    ...
    fn eng() -> MassAdditionEngine {
        MassAdditionEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_hold_and_release() {
        ...
        let ma = e
            .create_from_invoice(
                org, None, None, None, None, None, "500", None, None, None, None, None, None, None,
            )
            .await
            .unwrap();
    ...
    async fn test_merge() {
        ...
        let ma2 = e
            .create_from_invoice(
                org, None, None, None, None, None, "300", None, None, None, None, None, None, None,
            )
            .await
            .unwrap();
    ...
    async fn test_update_category_and_book() {
        ...
        let ma = e
            .create_from_invoice(
                org, None, None, None, None, None, "5000", None, None, None, None, None, None, None,
            )
            .await
            .unwrap();
        let updated = e
            .update_category_and_book(
                ma.id,
                Some(Uuid::new_v4()),
                Some("VEHICLES"),
                Some(Uuid::new_v4()),
    ...
    async fn test_list_invalid_status() {
        assert!(eng()
            .list(Uuid::new_v4(), Some("invalid"), None)
            .await
            .is_err());
    }
