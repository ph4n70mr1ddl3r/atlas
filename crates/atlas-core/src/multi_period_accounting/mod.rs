...
#[allow(dead_code)]
pub struct PostgresMpaRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
impl PostgresMpaRepository {
    #[must_use]
...
impl MpaRepository for PostgresMpaRepository {
    ...
    async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<MpaDashboard> {
        ...
        Ok(MpaDashboard {
            ...
            total_scheduled_amount: row
                .try_get::<f64, _>("total_scheduled_amount")
                .map_or_else(|_| "0.00".into(), |v| format!("{v:.2}")),
            total_recognized_amount: row
                .try_get::<f64, _>("total_recognized_amount")
                .map_or_else(|_| "0.00".into(), |v| format!("{v:.2}")),
            total_remaining_amount: row
                .try_get::<f64, _>("total_remaining_amount")
                .map_or_else(|_| "0.00".into(), |v| format!("{v:.2}")),
