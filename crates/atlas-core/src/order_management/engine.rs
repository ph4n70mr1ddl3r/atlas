...
use atlas_shared::{
    AddOrderLineRequest, AtlasError, AtlasResult, CreateSalesOrderRequest, FulfillmentShipment,
    OrderHold, OrderManagementDashboard, SalesOrder, SalesOrderLine,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
impl OrderManagementEngine {
    ...
    pub async fn create_order(
        ...
        if let Some(channel) = &req.sales_channel {
            if channel.is_empty() {
                return Err(AtlasError::ValidationFailed(
                    "Sales channel cannot be empty".to_string(),
                ));
            }
        }
    ...
    pub async fn get_order_by_id_scoped(
        &self,
        org_id: Uuid,
        id: Uuid,
    ) -> AtlasResult<Option<SalesOrder>> {
        ...
        if let Some(fs) = fulfillment_status {
            if !VALID_FULFILLMENT_STATUSES.contains(&fs) {
                return Err(AtlasError::ValidationFailed(format!(
                    ...
                    fs,
                    VALID_FULFILLMENT_STATUSES.join(", ")
                )));
            }
        }
        self.repository
            .list_orders(org_id, status, fulfillment_status)
            .await
    }
    ...
    pub async fn submit_order(&self, org_id: Uuid, id: Uuid) -> AtlasResult<SalesOrder> {
        ...
        if order.organization_id != org_id {
            return Err(AtlasError::Forbidden(
                "Order does not belong to your organization".to_string(),
            ));
        }
    ...
    pub async fn confirm_order(&self, org_id: Uuid, id: Uuid) -> AtlasResult<SalesOrder> {
        ...
        self.repository
            .update_order_fulfillment(id, "reserved")
            .await
    }
    ...
    pub async fn add_order_line(&self, req: AddOrderLineRequest) -> AtlasResult<SalesOrderLine> {
        let order = self
            .repository
            .get_order_by_id(req.order_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Order {} not found", req.order_id))
            })?;
        ...
        let price: f64 = req.unit_selling_price.parse().map_err(|_| {
            AtlasError::ValidationFailed("Unit selling price must be a valid number".to_string())
        })?;
        ...
        if price < 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Unit selling price cannot be negative".to_string(),
            ));
        }

        // Validate discount
        if let Some(dp) = &req.discount_percent {
            let pct: f64 = dp.parse().map_err(|_| {
        ...
        info!(
            "Adding line {} to order {}",
            line_number, order.order_number
        );
        ...
        let line = self
            ...
            .create_order_line(
                ...
                &req.unit_selling_price,
                req.unit_list_price.as_deref(),
                req.discount_percent.as_deref(),
                req.discount_amount.as_deref(),
    ...
    pub async fn get_order_line_scoped(
        &self,
        org_id: Uuid,
        id: Uuid,
    ) -> AtlasResult<Option<SalesOrderLine>> {
    ...
    pub async fn list_order_lines_scoped(
        &self,
        org_id: Uuid,
        order_id: Uuid,
    ) -> AtlasResult<Vec<SalesOrderLine>> {
        let order = self.repository.get_order_by_id(order_id).await?;
        match order {
            Some(o) if o.organization_id == org_id => {
                self.repository.list_order_lines(order_id).await
            }
            _ => Ok(vec![]),
        }
    ...
    pub async fn ship_order_line(
        ...
        if line.organization_id != org_id {
            return Err(AtlasError::Forbidden(
                "Order line does not belong to your organization".to_string(),
            ));
        }
        ...
        let shipped: f64 = quantity_shipped.parse().map_err(|_| {
            AtlasError::ValidationFailed("Quantity shipped must be a valid number".to_string())
        })?;
        ...
        if total_shipped > ordered {
            return Err(AtlasError::ValidationFailed(format!(
                ...
                shipped,
                ordered - already_shipped,
                ordered
            )));
        }

        // Determine remaining and backorder
        let backordered = if total_shipped < ordered {
            ordered - total_shipped
        } else {
            0.0
        };
        let line_status = if total_shipped >= ordered {
            "shipped"
        } else {
            "partially_shipped"
        };
        let fulfillment = if total_shipped >= ordered {
            "shipped"
        } else {
            "released"
        };
        ...
        info!(
            "Shipping {} units for order line {} (status: {})",
            shipped, line.line_number, line_status
        );

        self.repository
            .update_line_quantities(
                id,
        ...
        let result = self
            .repository
            .update_line_status(id, line_status, fulfillment)
            .await?;
        Ok(result)
    }
    ...
    pub async fn cancel_order_line(
        ...
        if line.status == "cancelled" {
            return Err(AtlasError::WorkflowError(
                "Line is already cancelled".to_string(),
            ));
        }
        ...
        info!(
            "Cancelling order line {} ({} units)",
            line.line_number, to_cancel
        );

        self.repository
            .update_line_quantities(id, None, Some(&format!("{to_cancel:.4}")), Some("0"))
            .await?;

        self.repository
            .update_line_cancellation_reason(id, reason)
            .await?;
        let result = self
            .repository
            .update_line_status(id, "cancelled", "not_started")
            .await?;
    ...
    pub async fn apply_hold(
        ...
        if let Some(line_id) = order_line_id {
            let line = self
                .repository
                .get_order_line(line_id)
                .await?
                .ok_or_else(|| {
                    AtlasError::EntityNotFound(format!("Order line {line_id} not found"))
                })?;
    ...
    pub async fn release_hold(
        ...
        if hold.organization_id != org_id {
            return Err(AtlasError::Forbidden(
                "Hold does not belong to your organization".to_string(),
            ));
        }

        if !hold.is_active {
            return Err(AtlasError::WorkflowError(
                "Hold is already released".to_string(),
            ));
        }

        info!(
            "Releasing hold {} on order {}",
            hold.hold_type, hold.order_id
        );

        self.repository
            .release_hold(id, released_by, released_by_name)
            .await
    }
    ...
    pub async fn list_holds_scoped(
        &self,
        org_id: Uuid,
        order_id: Uuid,
        active_only: bool,
    ...
    pub async fn get_shipment(
        ...
        org_id: Uuid,
        id: Uuid,
    ) -> AtlasResult<Option<FulfillmentShipment>> {
        ...
        if let Some(s) = status {
            if !VALID_SHIPMENT_STATUSES.contains(&s) {
                return Err(AtlasError::ValidationFailed(format!(
                    ...
                    s,
                    VALID_SHIPMENT_STATUSES.join(", ")
                )));
            }
        }
        self.repository
            .list_shipments(org_id, status, order_id)
            .await
    }
    ...
    pub async fn confirm_shipment(
        ...
        if shipment.organization_id != org_id {
            return Err(AtlasError::Forbidden(
                "Shipment does not belong to your organization".to_string(),
            ));
        }
        ...
        info!(
            "Confirming shipment {} with ship date {}",
            shipment.shipment_number, ship_date
        );
        ...
        self.repository
            .update_order_dates(shipment.order_id, Some(ship_date), None)
            .await?;
        ...
        let order = self
            .repository
            .get_order_by_id(shipment.order_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Order {} not found", shipment.order_id))
            })?;
        if order.status == "confirmed" || order.status == "submitted" {
            self.repository
                .update_order_status(shipment.order_id, "processing")
                .await?;
        }
        self.repository
            .update_order_fulfillment(shipment.order_id, "released")
            .await?;
    ...
    pub async fn update_tracking(
        ...
        info!(
            "Updating tracking for shipment {}",
            shipment.shipment_number
        );

        self.repository
            .update_shipment_tracking(id, tracking_number, estimated_delivery, None)
            .await
    }
    ...
    pub async fn confirm_delivery(
        ...
        info!(
            "Confirming delivery for shipment {} on {}",
            shipment.shipment_number, delivery_date
        );
        ...
        let updated = self
            .repository
            .update_shipment_tracking(id, None, Some(delivery_date), delivery_confirmation)
            .await?;

        // Update order delivery date
        self.repository
            .update_order_dates(shipment.order_id, None, Some(delivery_date))
            .await?;

        // Only transition order to "delivered" when ALL shipments are delivered
        let all_shipments = self
            .repository
            .list_shipments(updated.organization_id, None, Some(shipment.order_id))
            .await?;
        let all_delivered = all_shipments.iter().all(|s| s.status == "delivered");

        if all_delivered {
            self.repository
                .update_order_status(shipment.order_id, "delivered")
                .await?;
            self.repository
                .update_order_fulfillment(shipment.order_id, "delivered")
                .await?;
        }
...
mod tests {
    use super::*;
    use atlas_shared::{AddOrderLineRequest, CreateSalesOrderRequest};
    ...
    fn test_org_id() -> Uuid {
        Uuid::parse_str(TEST_ORG).unwrap()
    }
    ...
    impl MockOrderRepo {
        ...
        fn make_order(&self, id: Uuid) -> SalesOrder {
            ...
            SalesOrder {
                ...
                subtotal_amount: "100".to_string(),
                tax_amount: "10".to_string(),
                shipping_charges: "5".to_string(),
                total_amount: "115".to_string(),
                payment_terms: None,
                shipping_method: None,
    ...
    impl OrderManagementRepository for MockOrderRepo {
        async fn create_order(
            ...
            Ok(SalesOrder {
                ...
                subtotal_amount: "0".to_string(),
                tax_amount: "0".to_string(),
                shipping_charges: "0".to_string(),
                ...
                salesperson_id,
                salesperson_name: salesperson_name.map(|s| s.to_string()),
        ...
        async fn list_orders(
            ...
            _org_id: Uuid,
            _status: Option<&str>,
            _fulfillment_status: Option<&str>,
        ) -> AtlasResult<Vec<SalesOrder>> {
            Ok(vec![])
        }
        ...
        async fn create_order_line(
            ...
            Ok(SalesOrderLine {
                ...
                ship_from_warehouse: ship_from_warehouse.map(|s| s.to_string()),
                fulfillment_status: "not_started".to_string(),
                status: "open".to_string(),
                cancellation_reason: None,
                metadata: serde_json::json!({}),
        ...
        async fn update_line_quantities(
            ...
            if let Some(qs) = quantity_shipped {
                line.quantity_shipped = qs.to_string();
            }
            if let Some(qc) = quantity_cancelled {
                line.quantity_cancelled = qc.to_string();
            }
            if let Some(qb) = quantity_backordered {
                line.quantity_backordered = qb.to_string();
            }
            self.state
                .lock()
                .unwrap()
        ...
        async fn create_hold(
            ...
            Ok(OrderHold {
                ...
                applied_by,
                applied_by_name: applied_by_name.map(|s| s.to_string()),
                released_by: None,
        ...
        async fn create_shipment(
            ...
            let shipment = FulfillmentShipment {
                ...
                shipped_by,
                shipped_by_name: shipped_by_name.map(|s| s.to_string()),
                metadata: serde_json::json!({}),
                created_at: chrono::Utc::now(),
                updated_at: chrono::Utc::now(),
            };
            ...
            self.state
                ...
                .unwrap()
                .shipment_data
                .insert(id, shipment.clone());
            Ok(shipment)
        }

        async fn get_shipment(&self, id: Uuid) -> AtlasResult<Option<FulfillmentShipment>> {
            Ok(self.state.lock().unwrap().shipment_data.get(&id).cloned())
        }
        ...
        async fn list_shipments(
            ...
            let shipments: Vec<FulfillmentShipment> = self
                .state
                .lock()
                .unwrap()
                .shipment_data
    ...
    async fn test_confirm_order_success_via_stateful_mock() {
        ...
        repo.state
            ...
            .unwrap()
            .order_status
            .insert(order_id, "submitted".to_string());
    ...
    async fn test_close_order_requires_shipped_or_delivered() {
        ...
        let msg = format!("{:?}", result.unwrap_err());
        assert!(
            msg.contains("Cannot close order") || msg.contains("Must be 'shipped' or 'delivered'")
        );
    }
    ...
    async fn test_cancel_order_success() {
        ...
        let order_id = Uuid::new_v4();
        let result = engine
            .cancel_order(test_org_id(), order_id, Some("Customer request"))
            .await;
        assert!(result.is_ok());
    ...
    async fn test_add_order_line_validates_quantity() {
        let engine = OrderManagementEngine::new(Arc::new(MockOrderRepo::new()));
        let result = engine
            .add_order_line(AddOrderLineRequest {
                org_id: test_org_id(),
                order_id: Uuid::new_v4(),
                item_code: Some("ITEM-01".to_string()),
                item_description: Some("Widget".to_string()),
    ...
    async fn test_ship_line_validates_zero() {
        let engine = OrderManagementEngine::new(Arc::new(MockOrderRepo::new()));
        let result = engine
            .ship_order_line(test_org_id(), Uuid::new_v4(), "0")
            .await;
        assert!(result.is_err());
        let msg = format!("{:?}", result.unwrap_err());
    ...
    async fn test_ship_line_partial_success() {
        ...
        repo.state.lock().unwrap().line_data.insert(
            line_id,
            SalesOrderLine {
                id: line_id,
                organization_id: test_org_id(),
    ...
    async fn test_cancel_line_success() {
        ...
        let engine = OrderManagementEngine::new(Arc::new(repo));
        let result = engine
            .cancel_order_line(test_org_id(), line_id, Some("No longer needed"))
            .await;
        assert!(result.is_ok());
        let line = result.unwrap();
    ...
    async fn test_create_shipment_success() {
        ...
        repo.state
            ...
            .unwrap()
            .order_status
            .insert(order_id, "confirmed".to_string());
        let engine = OrderManagementEngine::new(Arc::new(repo));
        let org_id = Uuid::new_v4();
        let line_id = Uuid::new_v4();
        let result = engine
    ...
    async fn test_list_shipments_validates_status() {
        let engine = OrderManagementEngine::new(Arc::new(MockOrderRepo::new()));
        let result = engine
            .list_shipments(Uuid::new_v4(), Some("invalid"), None)
            .await;
        assert!(result.is_err());
        let msg = format!("{:?}", result.unwrap_err());
    ...
    async fn test_list_orders_validates_status() {
        let engine = OrderManagementEngine::new(Arc::new(MockOrderRepo::new()));
        let result = engine
            .list_orders(Uuid::new_v4(), Some("invalid"), None)
            .await;
        assert!(result.is_err());
        let msg = format!("{:?}", result.unwrap_err());
    ...
    async fn test_list_orders_validates_fulfillment_status() {
        let engine = OrderManagementEngine::new(Arc::new(MockOrderRepo::new()));
        let result = engine
            .list_orders(Uuid::new_v4(), None, Some("bad_status"))
            .await;
        assert!(result.is_err());
        let msg = format!("{:?}", result.unwrap_err());
    ...
    async fn test_confirm_shipment_moves_order_to_processing() {
        ...
        // Confirm shipment
        let result = engine
            .confirm_shipment(test_org_id(), shipment.id, chrono::Utc::now().date_naive())
            .await;
        assert!(result.is_ok());
    ...
    async fn test_confirm_delivery_transitions_order_when_all_shipments_delivered() {
        ...
        repo.state
            ...
            .unwrap()
            .order_status
            .insert(order_id, "processing".to_string());
