...
#[allow(dead_code)]
pub struct PostgresPaymentFormatRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
impl PostgresPaymentFormatRepository {
    #[must_use]
...
impl PaymentFormatRepository for PostgresPaymentFormatRepository {
    ...
    async fn get(&self, _: Uuid) -> AtlasResult<Option<PaymentFormat>> {
        Ok(None)
    }
    async fn get_by_code(&self, _: Uuid, _: &str) -> AtlasResult<Option<PaymentFormat>> {
        Ok(None)
    }
    async fn list(
        &self,
    ...
    async fn deactivate(&self, _: Uuid) -> AtlasResult<PaymentFormat> {
        Err(AtlasError::EntityNotFound("Not found".into()))
    }
    async fn activate(&self, _: Uuid) -> AtlasResult<PaymentFormat> {
        Err(AtlasError::EntityNotFound("Not found".into()))
    }
    async fn delete(&self, _: Uuid, _: &str) -> AtlasResult<()> {
        Ok(())
    }
    async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<PaymentFormatDashboard> {
...
impl PaymentFormatEngine {
    ...
    pub async fn create(
        ...
        if let Some(max) = max_payments_per_file {
            if max <= 0 {
                return Err(AtlasError::ValidationFailed(
                    "Max payments per file must be positive".into(),
                ));
            }
        }
        ...
        if self.repository.get_by_code(org_id, code).await?.is_some() {
            return Err(AtlasError::Conflict(format!(
                "Payment format '{code}' already exists"
            )));
        }
    ...
    pub async fn get_by_code(
        ...
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<PaymentFormat>> {
    ...
    pub async fn list(
        ...
        self.repository
            .list(org_id, format_type, payment_method, is_active)
            .await
    }
    ...
    pub async fn update(
        ...
        let pf =
            self.repository.get(id).await?.ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Payment format {id} not found"))
            })?;

        if !pf.is_active {
            return Err(AtlasError::ValidationFailed(
                "Cannot update inactive payment format".into(),
            ));
        }
        ...
        self.repository
            .update(id, name, description, file_template, max_payments)
            .await
    }
    ...
    pub async fn deactivate(&self, id: Uuid) -> AtlasResult<PaymentFormat> {
        ...
        if !pf.is_active {
            return Err(AtlasError::ValidationFailed(
                "Payment format is already inactive".into(),
            ));
        }
    ...
    pub async fn activate(&self, id: Uuid) -> AtlasResult<PaymentFormat> {
        ...
        if pf.is_active {
            return Err(AtlasError::ValidationFailed(
                "Payment format is already active".into(),
            ));
        }
    ...
    pub async fn delete(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        let _pf = self
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Payment format '{code}' not found"))
            })?;
...
mod tests {
    ...
    impl MockRepo {
        fn new() -> Self {
            Self {
                formats: std::sync::Mutex::new(vec![]),
            }
        }
    }
    ...
    impl PaymentFormatRepository for MockRepo {
        async fn create(
            ...
            let pf = PaymentFormat {
                ...
                file_template: file_template.map(Into::into),
                requires_bank_details,
                supports_remittance,
                supports_void,
                max_payments_per_file,
        ...
        async fn get_by_code(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|f| f.organization_id == org_id && f.code == code)
                .cloned())
        }
        async fn list(
            &self,
        ...
        async fn update(
            ...
            if let Some(d) = description {
                pf.description = Some(d.into());
            }
            if let Some(t) = file_template {
                pf.file_template = Some(t.into());
            }
            if let Some(m) = max_payments {
                pf.max_payments_per_file = Some(m);
            }
        ...
        async fn delete(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
            self.formats
                .lock()
                .unwrap()
                .retain(|f| !(f.organization_id == org_id && f.code == code));
            Ok(())
        }
        async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<PaymentFormatDashboard> {
    ...
    fn eng() -> PaymentFormatEngine {
        PaymentFormatEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_create_valid() {
        let pf = eng()
            .create(
                ...
                "EFT_US",
                "US EFT Format",
                Some("Standard US EFT"),
                "file",
                "eft",
                Some("eft_us_template.dat"),
                true,
                true,
                true,
                Some(10000),
    ...
    async fn test_create_duplicate_code() {
        ...
        let _ = e
            .create(
                org, "DUP", "First", None, "file", "eft", None, false, true, true, None, "USD",
                None,
            )
            .await
            .unwrap();
        assert!(e
            .create(
                org, "DUP", "Second", None, "file", "eft", None, false, true, true, None, "USD",
                None
            )
            .await
            .is_err());
    ...
    async fn test_deactivate_activate() {
        ...
        let pf = e
            .create(
                org, "TEST", "Test", None, "xml", "ach", None, false, true, true, None, "USD", None,
            )
            .await
            .unwrap();
    ...
    async fn test_dashboard() {
        ...
        let _ = e
            .create(
                org, "F1", "Format 1", None, "file", "eft", None, false, true, true, None, "USD",
                None,
            )
            .await
            .unwrap();
        let _ = e
            .create(
                org, "F2", "Format 2", None, "xml", "ach", None, false, true, true, None, "USD",
                None,
            )
            .await
            .unwrap();
