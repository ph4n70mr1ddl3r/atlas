...
use super::repository::{
    FraudAlert, FraudAlertCreateParams, PaymentRiskRepository, RiskProfile,
    RiskProfileCreateParams, SanctionsScreeningCreateParams, SanctionsScreeningResult,
    SupplierRiskAssessment, SupplierRiskAssessmentCreateParams,
};
...
const VALID_PROFILE_TYPES: &[&str] = &["supplier_risk", "payment_risk", "invoice_risk", "global"];
...
impl PaymentRiskEngine {
    ...
    fn validate_alert_status(status: &str) -> AtlasResult<()> {
        if !VALID_ALERT_STATUSES.contains(&status) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid alert status '{}'. Must be one of: {}",
                status,
                VALID_ALERT_STATUSES.join(", ")
            )));
        }
        Ok(())
    }
    ...
    fn validate_screening_type(screening_type: &str) -> AtlasResult<()> {
        if !VALID_SCREENING_TYPES.contains(&screening_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid screening_type '{}'. Must be one of: {}",
                screening_type,
                VALID_SCREENING_TYPES.join(", ")
            )));
        }
        Ok(())
    }
    ...
    fn validate_screened_list(screened_list: &str) -> AtlasResult<()> {
        if !VALID_SCREENED_LISTS.contains(&screened_list) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid screened_list '{}'. Must be one of: {}",
                screened_list,
                VALID_SCREENED_LISTS.join(", ")
            )));
        }
        Ok(())
    }
    ...
    fn validate_match_status(match_status: &str) -> AtlasResult<()> {
        if !VALID_MATCH_STATUSES.contains(&match_status) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid match_status '{}'. Must be one of: {}",
                match_status,
                VALID_MATCH_STATUSES.join(", ")
            )));
        }
        Ok(())
    }
    ...
    fn validate_assessment_status(status: &str) -> AtlasResult<()> {
        if !VALID_ASSESSMENT_STATUSES.contains(&status) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid assessment status '{}'. Must be one of: {}",
                status,
                VALID_ASSESSMENT_STATUSES.join(", ")
            )));
        }
        Ok(())
    }
    ...
    pub async fn create_risk_profile(
        ...
        let params = RiskProfileCreateParams {
            ...
            duplicate_amount_tolerance_pct: duplicate_amount_tolerance_pct
                .map(std::string::ToString::to_string),
            duplicate_date_tolerance_days: duplicate_date_tolerance_days
                .map(std::string::ToString::to_string),
    ...
    pub async fn get_risk_profile(
        ...
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<RiskProfile>> {
    ...
    pub async fn list_risk_profiles(
        ...
        self.repo
            .list_risk_profiles(org_id, profile_type, is_active)
            .await
    }
    ...
    pub async fn create_fraud_alert(
        ...
        info!(
            "Creating fraud alert type '{}' severity '{}' for org {}",
            alert_type, severity, org_id
        );
    ...
    pub async fn list_fraud_alerts(
        ...
        self.repo
            .list_fraud_alerts(org_id, status, alert_type, severity)
            .await
    }
    ...
    pub async fn transition_fraud_alert(
        ...
        info!(
            "Transitioning fraud alert {} to status '{}'",
            id, new_status
        );
        ...
        self.repo
            .update_fraud_alert_status(id, new_status, resolution_notes, resolved_by)
            .await
    }
    ...
    pub async fn assign_fraud_alert(
        ...
        self.repo
            .assign_fraud_alert(id, assigned_to, assigned_team)
            .await
    }
    ...
    pub async fn create_screening_result(
        ...
        let seq = self
            .repo
            .get_next_screening_sequence(org_id)
            .await
            .unwrap_or(1);
    ...
    pub async fn list_screening_results(
        ...
        self.repo
            .list_screening_results(org_id, supplier_id, match_status)
            .await
    }
    ...
    pub async fn review_screening_result(
        ...
        self.repo
            .review_screening_result(id, reviewed_by, review_notes, action_taken)
            .await
    }
    ...
    pub async fn create_assessment(
        ...
        info!(
            "Creating risk assessment for supplier '{}' org {}",
            supplier_name, org_id
        );

        let seq = self
            .repo
            .get_next_assessment_sequence(org_id)
            .await
            .unwrap_or(1);
    ...
    pub async fn list_assessments(
        ...
        self.repo
            .list_assessments(org_id, supplier_id, status)
            .await
    }
