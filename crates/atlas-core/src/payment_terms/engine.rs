...
use super::{
    AtlasError, AtlasResult, NaiveDate, PaymentTerm, PaymentTermDashboard,
    PaymentTermDiscountSchedule, PaymentTermInstallment, PaymentTermsRepository,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
pub struct PaymentTermsEngine {
    repository: Arc<dyn PaymentTermsRepository>,
}

impl PaymentTermsEngine {
    pub fn new(r: Arc<dyn PaymentTermsRepository>) -> Self {
        Self { repository: r }
    }
    ...
    pub async fn create_term(
        ...
        base_due_days: i32,
        due_date_cutoff_day: Option<i32>,
        term_type: &str,
        ...
        if term_code.is_empty() || name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Term code and name are required".into(),
            ));
        }
        if !(0..=365).contains(&base_due_days) {
            return Err(AtlasError::ValidationFailed(
                "Base due days must be 0-365".into(),
            ));
        }
        if let Some(cutoff) = due_date_cutoff_day {
            if !(1..=31).contains(&cutoff) {
                return Err(AtlasError::ValidationFailed(
                    "Cutoff day must be 1-31".into(),
                ));
            }
        }
        if !VALID_TERM_TYPES.contains(&term_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid term_type '{term_type}'"
            )));
        }
        let discount: f64 = default_discount_percent
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid discount percent".into()))?;
        if !(0.0..=100.0).contains(&discount) {
            return Err(AtlasError::ValidationFailed(
                "Discount percent must be 0-100".into(),
            ));
        }
        if self.repository.get_term(org_id, term_code).await?.is_some() {
            return Err(AtlasError::Conflict(format!(
                "Payment term '{term_code}' already exists"
            )));
        }
    ...
    pub async fn activate_term(&self, id: Uuid) -> AtlasResult<PaymentTerm> {
        let term =
            self.repository.get_term_by_id(id).await?.ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Payment term {id} not found"))
            })?;
        if term.status == "active" {
            return Err(AtlasError::WorkflowError("Already active".into()));
        }
    ...
    pub async fn deactivate_term(&self, id: Uuid) -> AtlasResult<PaymentTerm> {
        ...
        if term.status == "inactive" {
            return Err(AtlasError::WorkflowError("Already inactive".into()));
        }
    ...
    pub async fn delete_term(&self, id: Uuid) -> AtlasResult<()> {
        self.repository
            .get_term_by_id(id)
            .await?
            .ok_or_else(|| AtlasError::EntityNotFound(format!("Payment term {id} not found")))?;
    ...
    pub async fn create_discount_schedule(
        ...
        discount_days: i32,
        discount_day_of_month: Option<i32>,
        discount_basis: &str,
        display_order: i32,
    ) -> AtlasResult<PaymentTermDiscountSchedule> {
        self.repository
            .get_term_by_id(term_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Payment term {term_id} not found"))
            })?;
        let discount: f64 = discount_percent
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid discount percent".into()))?;
        if !(0.0..=100.0).contains(&discount) {
            return Err(AtlasError::ValidationFailed(
    ...
    pub async fn list_discount_schedules(
        &self,
        term_id: Uuid,
    ) -> AtlasResult<Vec<PaymentTermDiscountSchedule>> {
    ...
    pub async fn create_installment(
        ...
        discount_percent: &str,
        discount_days: i32,
    ) -> AtlasResult<PaymentTermInstallment> {
        self.repository
            .get_term_by_id(term_id)
            .await?
            .ok_or_else(|| {
        ...
        if installment_number < 1 {
            return Err(AtlasError::ValidationFailed(
                "Installment number must be >= 1".into(),
            ));
        }
        if due_days_offset < 0 {
            return Err(AtlasError::ValidationFailed(
                "Due days offset must be >= 0".into(),
            ));
        }
        let pct: f64 = percentage
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid percentage".into()))?;
        if pct <= 0.0 || pct > 100.0 {
            return Err(AtlasError::ValidationFailed(
                "Percentage must be > 0 and <= 100".into(),
            ));
        }
        let disc: f64 = discount_percent
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid discount percent".into()))?;
    ...
    pub async fn list_installments(
        &self,
        term_id: Uuid,
    ) -> AtlasResult<Vec<PaymentTermInstallment>> {
    ...
    pub fn calculate_discount(
        &self,
        invoice_amount: f64,
        schedule: &PaymentTermDiscountSchedule,
...
mod tests {
    ...
    impl MockRepo {
        fn new() -> Self {
            Self {
                terms: std::sync::Mutex::new(vec![]),
                discounts: std::sync::Mutex::new(vec![]),
                installments: std::sync::Mutex::new(vec![]),
            }
        }
    }
    ...
    impl PaymentTermsRepository for MockRepo {
        async fn create_term(
            ...
            let t = PaymentTerm {
                ...
                base_due_days: due_days,
                due_date_cutoff_day: cutoff,
                status: "active".into(),
                term_type: tt.into(),
                default_discount_percent: dp.into(),
        ...
        async fn get_term(&self, org_id: Uuid, code: &str) -> AtlasResult<Option<PaymentTerm>> {
            Ok(self
                .terms
                .lock()
                .unwrap()
                .iter()
                .find(|t| t.organization_id == org_id && t.term_code == code)
                .cloned())
        }
        ...
        async fn list_terms(
            ...
            Ok(self
                ...
                .iter()
                .filter(|t| {
                    t.organization_id == org_id && (status.is_none() || t.status == status.unwrap())
                })
                .cloned()
                .collect())
        }
        ...
        async fn delete_term(&self, _id: Uuid) -> AtlasResult<()> {
            Ok(())
        }
        async fn create_discount_schedule(
            &self,
            org_id: Uuid,
            term_id: Uuid,
            dp: &str,
            ...
            let s = PaymentTermDiscountSchedule {
                id: Uuid::new_v4(),
                organization_id: org_id,
                payment_term_id: term_id,
                discount_percent: dp.into(),
                discount_days: dd,
                discount_day_of_month: dom,
                discount_basis: basis.into(),
                display_order: order,
                metadata: serde_json::json!({}),
                created_at: chrono::Utc::now(),
                updated_at: chrono::Utc::now(),
        ...
        async fn list_discount_schedules(
            &self,
            tid: Uuid,
        ) -> AtlasResult<Vec<PaymentTermDiscountSchedule>> {
            Ok(self
                ...
                .unwrap()
                .iter()
                .filter(|d| d.payment_term_id == tid)
                .cloned()
                .collect())
        }
        async fn delete_discount_schedule(&self, _: Uuid) -> AtlasResult<()> {
            Ok(())
        }
        ...
        async fn list_installments(&self, tid: Uuid) -> AtlasResult<Vec<PaymentTermInstallment>> {
            Ok(self
                .installments
                .lock()
                .unwrap()
                .iter()
                .filter(|i| i.payment_term_id == tid)
                .cloned()
                .collect())
        }
        async fn delete_installment(&self, _: Uuid) -> AtlasResult<()> {
            Ok(())
        }
        async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<PaymentTermDashboard> {
            ...
            let terms = ts
                ...
                .collect::<Vec<_>>();
            Ok(PaymentTermDashboard {
                total_terms: terms.len() as i32,
                active_terms: terms.iter().filter(|t| t.status == "active").count() as i32,
                terms_with_discounts: 0,
                terms_with_installments: 0,
            })
        }
    }

    fn eng() -> PaymentTermsEngine {
        PaymentTermsEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_create_term_duplicate() {
        ...
        let e = eng();
        let _ = e
            .create_term(org, "DUP", "T1", None, 30, None, "standard", "0", None)
            .await;
        assert!(e
            .create_term(org, "DUP", "T2", None, 30, None, "standard", "0", None)
            .await
            .is_err());
    }
    ...
    async fn test_create_discount_schedule_valid() {
        ...
        let t = e
            ...
            .unwrap();
        let ds = e
            .create_discount_schedule(Uuid::new_v4(), t.id, "2", 10, None, "invoice_amount", 0)
            .await
            .unwrap();
    ...
    async fn test_create_discount_schedule_invalid_percent() {
        ...
        let t = e
            ...
            .unwrap();
        assert!(e
            .create_discount_schedule(Uuid::new_v4(), t.id, "150", 10, None, "invoice_amount", 0)
            .await
            .is_err());
    }
    ...
    async fn test_create_discount_schedule_invalid_days() {
        ...
        let t = e
            ...
            .unwrap();
        assert!(e
            .create_discount_schedule(Uuid::new_v4(), t.id, "2", -1, None, "invoice_amount", 0)
            .await
            .is_err());
    }
    ...
    async fn test_create_discount_schedule_invalid_basis() {
        ...
        let t = e
            ...
            .unwrap();
        assert!(e
            .create_discount_schedule(Uuid::new_v4(), t.id, "2", 10, None, "bad_basis", 0)
            .await
            .is_err());
    }
    ...
    async fn test_create_installment_valid() {
        ...
        let t = e
            ...
            .unwrap();
        let inst = e
            .create_installment(Uuid::new_v4(), t.id, 1, 30, "50", "2", 10)
            .await
            .unwrap();
    ...
    async fn test_create_installment_zero_number() {
        ...
        let t = e
            ...
            .unwrap();
        assert!(e
            .create_installment(Uuid::new_v4(), t.id, 0, 30, "50", "2", 10)
            .await
            .is_err());
    }
    ...
    async fn test_create_installment_negative_offset() {
        ...
        let t = e
            ...
            .unwrap();
        assert!(e
            .create_installment(Uuid::new_v4(), t.id, 1, -1, "50", "2", 10)
            .await
            .is_err());
    }
    ...
    async fn test_create_installment_zero_pct() {
        ...
        let t = e
            ...
            .unwrap();
        assert!(e
            .create_installment(Uuid::new_v4(), t.id, 1, 30, "0", "2", 10)
            .await
            .is_err());
    }
    ...
    async fn test_create_installment_pct_over_100() {
        ...
        let t = e
            ...
            .unwrap();
        assert!(e
            .create_installment(Uuid::new_v4(), t.id, 1, 30, "150", "2", 10)
            .await
            .is_err());
    }
    ...
    async fn test_calculate_due_date_standard() {
        ...
        let t = PaymentTerm {
            ...
            term_code: "NET30".into(),
            name: "Net 30".into(),
            description: None,
            base_due_days: 30,
            due_date_cutoff_day: None,
            status: "active".into(),
            term_type: "standard".into(),
            default_discount_percent: "0".into(),
            metadata: serde_json::json!({}),
            created_by: None,
            created_at: chrono::Utc::now(),
            updated_at: chrono::Utc::now(),
    ...
    async fn test_calculate_discount_within_window() {
        let e = eng();
        let sched = PaymentTermDiscountSchedule {
            id: Uuid::new_v4(),
            organization_id: Uuid::new_v4(),
            payment_term_id: Uuid::new_v4(),
            discount_percent: "2".into(),
            discount_days: 10,
            discount_day_of_month: None,
            discount_basis: "invoice_amount".into(),
            display_order: 0,
            metadata: serde_json::json!({}),
            created_at: chrono::Utc::now(),
            updated_at: chrono::Utc::now(),
    ...
    async fn test_get_dashboard() {
        ...
        let org = Uuid::new_v4();
        let _ = e
            .create_term(org, "NET30", "T1", None, 30, None, "standard", "0", None)
            .await
            .unwrap();
