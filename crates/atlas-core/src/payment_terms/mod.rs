...
pub trait PaymentTermsRepository: Send + Sync {
    ...
    async fn list_terms(&self, org_id: Uuid, status: Option<&str>)
...
#[allow(dead_code)]
pub struct PostgresPaymentTermsRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
impl PostgresPaymentTermsRepository {
    #[must_use]
...
impl PaymentTermsRepository for PostgresPaymentTermsRepository {
    ...
    async fn get_term(&self, _: Uuid, _: &str) -> AtlasResult<Option<PaymentTerm>> {
        Ok(None)
    }
    async fn get_term_by_id(&self, _: Uuid) -> AtlasResult<Option<PaymentTerm>> {
        Ok(None)
    }
    async fn list_terms(&self, _: Uuid, _: Option<&str>) -> AtlasResult<Vec<PaymentTerm>> {
        Ok(vec![])
    }
    async fn update_term_status(&self, _: Uuid, _: &str) -> AtlasResult<PaymentTerm> {
        Err(AtlasError::EntityNotFound("Mock".into()))
    }
    async fn delete_term(&self, _: Uuid) -> AtlasResult<()> {
        Ok(())
    }
    async fn create_discount_schedule(
        &self,
    ...
    async fn list_installments(&self, _: Uuid) -> AtlasResult<Vec<PaymentTermInstallment>> {
        Ok(vec![])
    }
    async fn delete_installment(&self, _: Uuid) -> AtlasResult<()> {
        Ok(())
    }
    async fn get_dashboard(&self, _: Uuid) -> AtlasResult<PaymentTermDashboard> {
