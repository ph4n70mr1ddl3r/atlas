...
pub trait PrepaymentApplicationRepository: Send + Sync {
    ...
    async fn get_by_number(
        ...
        org_id: Uuid,
        number: &str,
    ) -> AtlasResult<Option<PrepaymentApplication>>;
    async fn list(
        &self,
        org_id: Uuid,
        status: Option<&str>,
        supplier_id: Option<Uuid>,
    ) -> AtlasResult<Vec<PrepaymentApplication>>;
...
#[allow(dead_code)]
pub struct PostgresPrepaymentApplicationRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
impl PostgresPrepaymentApplicationRepository {
    #[must_use]
...
impl PrepaymentApplicationRepository for PostgresPrepaymentApplicationRepository {
    ...
    async fn get(&self, _: Uuid) -> AtlasResult<Option<PrepaymentApplication>> {
        Ok(None)
    }
    async fn get_by_number(&self, _: Uuid, _: &str) -> AtlasResult<Option<PrepaymentApplication>> {
        Ok(None)
    }
    async fn list(
        &self,
    ...
    async fn update_status(&self, _: Uuid, _: &str) -> AtlasResult<PrepaymentApplication> {
        Err(AtlasError::EntityNotFound("Not found".into()))
    }
    async fn update_remaining(&self, _: Uuid, _: &str) -> AtlasResult<PrepaymentApplication> {
        Err(AtlasError::EntityNotFound("Not found".into()))
    }
    async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<PrepaymentDashboard> {
...
impl PrepaymentApplicationEngine {
    ...
    pub async fn apply(
        ...
        // Validate amounts
        let applied: f64 = applied_amount
            .parse()
        ...
        let remaining: f64 = remaining_prepayment_amount.parse().map_err(|_| {
            AtlasError::ValidationFailed("Invalid remaining prepayment amount".into())
        })?;
        if remaining < 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Remaining prepayment amount cannot be negative".into(),
            ));
        }
        ...
        if gl_date < application_date {
            return Err(AtlasError::ValidationFailed(
                "GL date cannot be before application date".into(),
            ));
        }
        ...
        info!(
            "Creating prepayment application {} for supplier {}",
            number, supplier_id
        );
        ...
        self.repository
            .create(
                ...
                prepayment_invoice_number,
                standard_invoice_id,
                standard_invoice_number,
                supplier_id,
                supplier_number,
                applied_amount,
    ...
    pub async fn confirm(&self, id: Uuid) -> AtlasResult<PrepaymentApplication> {
        let pa = self.repository.get(id).await?.ok_or_else(|| {
            AtlasError::EntityNotFound(format!("Prepayment application {id} not found"))
        })?;

        if pa.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot confirm application in '{}' status. Must be 'draft'.",
                pa.status
            )));
        }
        info!(
            "Confirming prepayment application {}",
    ...
    pub async fn cancel(&self, id: Uuid) -> AtlasResult<PrepaymentApplication> {
        ...
        if pa.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot cancel application in '{}' status. Must be 'draft'.",
                pa.status
            )));
        }
        info!(
            "Cancelling prepayment application {}",
            pa.application_number
        );
    ...
    pub async fn reverse(&self, id: Uuid) -> AtlasResult<PrepaymentApplication> {
        ...
        if pa.status != "applied" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot reverse application in '{}' status. Must be 'applied'.",
                pa.status
            )));
        }
...
mod tests {
    ...
    impl PrepaymentApplicationRepository for MockRepo {
        async fn create(
            ...
            number: &str,
            prepay_id: Uuid,
            prepay_num: Option<&str>,
            std_id: Uuid,
            std_num: Option<&str>,
            supplier_id: Uuid,
            supplier_num: Option<&str>,
            applied: &str,
            remaining: &str,
            currency: &str,
            ...
            let pa = PrepaymentApplication {
                id: Uuid::new_v4(),
                organization_id: org_id,
                application_number: number.into(),
                prepayment_invoice_id: prepay_id,
                prepayment_invoice_number: prepay_num.map(Into::into),
                standard_invoice_id: std_id,
                standard_invoice_number: std_num.map(Into::into),
                supplier_id,
                supplier_number: supplier_num.map(Into::into),
                applied_amount: applied.into(),
                remaining_prepayment_amount: remaining.into(),
        ...
        async fn get_by_number(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|p| p.organization_id == org_id && p.application_number == number)
                .cloned())
        }
        async fn list(
            &self,
        ...
        async fn update_remaining(
            ...
            let pa = all
                ...
                .find(|p| p.id == id)
                .ok_or_else(|| AtlasError::EntityNotFound("Not found".into()))?;
            pa.remaining_prepayment_amount = remaining.into();
            pa.updated_at = Utc::now();
            Ok(pa.clone())
        }
        async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<PrepaymentDashboard> {
            ...
            Ok(PrepaymentDashboard {
                ...
                applied_applications: org_items.iter().filter(|p| p.status == "applied").count()
                    as i32,
                cancelled_applications: org_items.iter().filter(|p| p.status == "cancelled").count()
                    as i32,
                total_applied_amount: org_items
                    .iter()
                    .filter(|p| p.status == "applied")
                    .map(|p| p.applied_amount.parse::<f64>().unwrap_or(0.0))
                    .sum::<f64>()
                    .to_string(),
                by_supplier: serde_json::json!([]),
            })
    ...
    fn eng() -> PrepaymentApplicationEngine {
        PrepaymentApplicationEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_list_valid() {
        assert!(eng()
            .list(Uuid::new_v4(), Some("draft"), None)
            .await
            .unwrap()
            .is_empty());
    }
