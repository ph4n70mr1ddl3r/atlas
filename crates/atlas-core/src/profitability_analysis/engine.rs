...
use super::repository::{
    ProfitabilityAnalysisRepository, ProfitabilityDashboard, ProfitabilityRun,
    ProfitabilityRunLine, ProfitabilitySegment, ProfitabilityTemplate, RunCreateParams,
    RunLineCreateParams, SegmentCreateParams, TemplateCreateParams,
};
...
// Valid run statuses
const VALID_RUN_STATUSES: &[&str] = &["draft", "calculated", "reviewed", "completed", "cancelled"];
...
impl ProfitabilityAnalysisEngine {
    ...
    fn validate_segment_type(segment_type: &str) -> AtlasResult<()> {
        if !VALID_SEGMENT_TYPES.contains(&segment_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid segment_type '{}'. Must be one of: {}",
                segment_type,
                VALID_SEGMENT_TYPES.join(", ")
            )));
        }
        Ok(())
    }
    ...
    pub async fn create_segment(
        ...
        info!(
            "Creating profitability segment '{}' ({}) for org {}",
            segment_code, segment_name, org_id
        );
    ...
    pub async fn list_segments(
        ...
        self.repo
            .list_segments(org_id, segment_type, is_active)
            .await
    }
    ...
    pub async fn delete_segment(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        info!(
            "Deleting profitability segment '{}' for org {}",
            code, org_id
        );
    ...
    pub async fn create_run(
        ...
        info!(
            "Creating profitability run '{}' for org {}",
            run_number, org_id
        );
    ...
    pub async fn delete_run(&self, org_id: Uuid, run_number: &str) -> AtlasResult<()> {
        info!(
            "Deleting profitability run '{}' for org {}",
            run_number, org_id
        );
        let run = self
            .repo
    ...
    pub async fn add_run_line(
        ...
        info!(
            "Adding line {} to profitability run {}",
            line_number, run_id
        );
    ...
    pub async fn remove_run_line(&self, run_id: Uuid, line_id: Uuid) -> AtlasResult<()> {
        info!(
            "Removing line {} from profitability run {}",
            line_id, run_id
        );
    ...
    async fn recalculate_run_totals(&self, run_id: Uuid) -> AtlasResult<()> {
        ...
        let total_operating_margin =
            Self::calculate_operating_margin(total_gross_margin, total_opex);
        ...
        let total_net_margin = Self::calculate_net_margin(
        ...
        let operating_margin_pct =
            Self::calculate_operating_margin_pct(total_revenue, total_operating_margin);
        ...
        for line in &lines {
            ...
            self.repo
                .update_run_line_margins(
                    ...
                    line.operating_margin,
                    line.operating_margin_pct,
                    line.net_margin,
                    line.net_margin_pct,
                    rev_contrib,
                    margin_contrib,
    ...
    pub async fn create_template(
        ...
        info!(
            "Creating profitability template '{}' for org {}",
            template_code, org_id
        );
        Self::validate_segment_type(segment_type)?;
    ...
    pub async fn delete_template(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        info!(
            "Deleting profitability template '{}' for org {}",
            code, org_id
        );
    ...
    #[must_use]
    pub const fn valid_segment_types() -> &'static [&'static str] {
        VALID_SEGMENT_TYPES
    }
    #[must_use]
    pub const fn valid_analysis_types() -> &'static [&'static str] {
...
mod tests {
    ...
    fn test_gross_margin() {
        assert!(
            (ProfitabilityAnalysisEngine::calculate_gross_margin(1000.0, 600.0) - 400.0).abs()
                < 0.01
        );
        assert!((ProfitabilityAnalysisEngine::calculate_gross_margin(500.0, 500.0)).abs() < 0.01);
        assert!(
            (ProfitabilityAnalysisEngine::calculate_gross_margin(200.0, 300.0) - (-100.0)).abs()
                < 0.01
        );
    }
    ...
    fn test_gross_margin_pct() {
        assert!(
            (ProfitabilityAnalysisEngine::calculate_gross_margin_pct(1000.0, 400.0) - 40.0).abs()
                < 0.01
        );
        assert!(
            (ProfitabilityAnalysisEngine::calculate_gross_margin_pct(1000.0, -100.0) - (-10.0))
                .abs()
                < 0.01
        );
    ...
    fn test_operating_margin() {
        assert!(
            (ProfitabilityAnalysisEngine::calculate_operating_margin(400.0, 150.0) - 250.0).abs()
                < 0.01
        );
        assert!(
            (ProfitabilityAnalysisEngine::calculate_operating_margin(100.0, 200.0) - (-100.0))
                .abs()
                < 0.01
        );
    }
    ...
    fn test_operating_margin_pct() {
        assert!(
            (ProfitabilityAnalysisEngine::calculate_operating_margin_pct(1000.0, 250.0) - 25.0)
                .abs()
                < 0.01
        );
    }
    ...
    fn test_net_margin() {
        assert!(
            (ProfitabilityAnalysisEngine::calculate_net_margin(250.0, 50.0, 25.0) - 275.0).abs()
                < 0.01
        );
        assert!(
            (ProfitabilityAnalysisEngine::calculate_net_margin(100.0, 0.0, 50.0) - 50.0).abs()
                < 0.01
        );
        assert!(
            (ProfitabilityAnalysisEngine::calculate_net_margin(200.0, 30.0, 250.0) - (-20.0)).abs()
                < 0.01
        );
    }
    ...
    fn test_net_margin_pct() {
        assert!(
            (ProfitabilityAnalysisEngine::calculate_net_margin_pct(1000.0, 275.0) - 27.5).abs()
                < 0.01
        );
    ...
    fn test_pct_change() {
        assert!(
            (ProfitabilityAnalysisEngine::calculate_pct_change(100.0, 120.0) - 20.0).abs() < 0.01
        );
        assert!(
            (ProfitabilityAnalysisEngine::calculate_pct_change(100.0, 80.0) - (-20.0)).abs() < 0.01
        );
        assert!(
            (ProfitabilityAnalysisEngine::calculate_pct_change(0.0, 50.0) - 100.0).abs() < 0.01
        );
    ...
    fn test_valid_status_transitions() {
        assert!(
            ProfitabilityAnalysisEngine::validate_status_transition("draft", "calculated").is_ok()
        );
        assert!(
            ProfitabilityAnalysisEngine::validate_status_transition("draft", "cancelled").is_ok()
        );
        assert!(
            ProfitabilityAnalysisEngine::validate_status_transition("calculated", "reviewed")
                .is_ok()
        );
        assert!(
            ProfitabilityAnalysisEngine::validate_status_transition("calculated", "cancelled")
                .is_ok()
        );
        assert!(
            ProfitabilityAnalysisEngine::validate_status_transition("calculated", "draft").is_ok()
        );
        assert!(
            ProfitabilityAnalysisEngine::validate_status_transition("reviewed", "completed")
                .is_ok()
        );
        assert!(
            ProfitabilityAnalysisEngine::validate_status_transition("reviewed", "cancelled")
                .is_ok()
        );
        assert!(
            ProfitabilityAnalysisEngine::validate_status_transition("reviewed", "calculated")
                .is_ok()
        );
    }
    ...
    fn test_invalid_status_transitions() {
        assert!(
            ProfitabilityAnalysisEngine::validate_status_transition("draft", "completed").is_err()
        );
        assert!(
            ProfitabilityAnalysisEngine::validate_status_transition("draft", "reviewed").is_err()
        );
        assert!(
            ProfitabilityAnalysisEngine::validate_status_transition("completed", "draft").is_err()
        );
        assert!(
            ProfitabilityAnalysisEngine::validate_status_transition("completed", "calculated")
                .is_err()
        );
        assert!(
            ProfitabilityAnalysisEngine::validate_status_transition("cancelled", "draft").is_err()
        );
    }
    ...
    fn test_full_margin_chain() {
        ...
        let operating_pct =
            ProfitabilityAnalysisEngine::calculate_operating_margin_pct(revenue, operating);
        assert!((operating_pct - 25.0).abs() < 0.01);

        let net = ProfitabilityAnalysisEngine::calculate_net_margin(
            operating,
            other_income,
            other_expense,
        );
