...
pub trait ProjectResourceManagementRepository: Send + Sync {
    ...
    async fn create_profile(
        ...
        available_hours_per_week: f64,
        cost_rate: f64,
        cost_rate_currency: &str,
        bill_rate: f64,
        bill_rate_currency: &str,
        location: &str,
    ...
    async fn get_profile_by_number(
        &self,
        org_id: Uuid,
        resource_number: &str,
    ...
    async fn update_availability_status(
        &self,
        id: Uuid,
        status: &str,
    ) -> AtlasResult<ResourceProfile>;
    ...
    async fn get_request_by_number(
        ...
        org_id: Uuid,
        request_number: &str,
    ) -> AtlasResult<Option<ResourceRequest>>;
    ...
    async fn get_assignment_by_number(
        &self,
        org_id: Uuid,
        assignment_number: &str,
    ) -> AtlasResult<Option<ResourceAssignment>>;
    ...
    async fn update_assignment_hours(
        &self,
        id: Uuid,
        actual_hours: f64,
        remaining_hours: f64,
    ...
    async fn list_utilization_entries(
        &self,
        org_id: Uuid,
    ...
    async fn update_utilization_status(
        &self,
        id: Uuid,
        status: &str,
    ) -> AtlasResult<UtilizationEntry>;
    async fn approve_utilization_entry(
        &self,
        id: Uuid,
        approved_by: Uuid,
    ) -> AtlasResult<UtilizationEntry>;
...
impl ProjectResourceManagementRepository for PostgresProjectResourceManagementRepository {
    ...
    async fn create_profile(
        ...
        .bind(cost_rate)
        .bind(cost_rate_currency)
        .bind(bill_rate)
        .bind(bill_rate_currency)
        .bind(location)
        .bind(manager_id)
        .bind(manager_name)
        .bind(hire_date)
    ...
    async fn update_availability_status(
        ...
        .map_err(|_| AtlasError::EntityNotFound(format!("Resource profile {id} not found")))?;
        Ok(row_to_profile(&row))
    }
    ...
    async fn delete_profile(&self, org_id: Uuid, resource_number: &str) -> AtlasResult<()> {
        ...
        if result.rows_affected() == 0 {
            return Err(AtlasError::EntityNotFound(format!(
                "Resource profile '{resource_number}' not found"
            )));
        }
        Ok(())
    }
    ...
    async fn create_request(
        ...
        .bind(end_date)
        .bind(hours_per_week)
        .bind(total_planned_hours)
        .bind(max_cost_rate)
        .bind(currency_code)
        .bind(resource_type_preference)
        .bind(location_requirement)
        .bind(notes)
        .bind(created_by)
        .fetch_one(&self.pool)
        .await?;
    ...
    async fn update_request_status(&self, id: Uuid, status: &str) -> AtlasResult<ResourceRequest> {
        ...
        .fetch_one(&self.pool).await
        .map_err(|_| AtlasError::EntityNotFound(format!("Resource request {id} not found")))?;
        Ok(row_to_request(&row))
    }
    ...
    async fn delete_request(&self, org_id: Uuid, request_number: &str) -> AtlasResult<()> {
        ...
        if result.rows_affected() == 0 {
            return Err(AtlasError::EntityNotFound(format!(
                "Draft request '{request_number}' not found"
            )));
        }
        Ok(())
    }
    ...
    async fn update_assignment_status(
        ...
        .fetch_one(&self.pool).await
        .map_err(|_| AtlasError::EntityNotFound(format!("Resource assignment {id} not found")))?;
        Ok(row_to_assignment(&row))
    }
    ...
    async fn delete_assignment(&self, org_id: Uuid, assignment_number: &str) -> AtlasResult<()> {
        ...
        if result.rows_affected() == 0 {
            return Err(AtlasError::EntityNotFound(format!(
                "Planned/cancelled assignment '{assignment_number}' not found"
            )));
        }
        Ok(())
    }
    ...
    async fn update_utilization_status(
        ...
        .fetch_one(&self.pool).await
        .map_err(|_| AtlasError::EntityNotFound(format!("Utilization entry {id} not found")))?;
        Ok(row_to_utilization(&row))
    }
    ...
    async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<ResourceDashboard> {
        ...
        let available_resources = profiles
            .iter()
            .filter(|r| {
                r.try_get::<String, _>("availability_status")
                    .unwrap_or_default()
                    == "available"
            })
            .count() as i64;
        let fully_allocated_resources = profiles
            .iter()
            .filter(|r| {
                r.try_get::<String, _>("availability_status")
                    .unwrap_or_default()
