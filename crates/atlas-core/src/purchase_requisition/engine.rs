...
use atlas_shared::{
    AtlasError, AtlasResult, AutocreateLink, AutocreateRequest, PurchaseRequisition,
    PurchaseRequisitionRequest, RequisitionApproval, RequisitionDashboardSummary,
    RequisitionDistribution, RequisitionDistributionRequest, RequisitionLine,
...
#[allow(dead_code)]
const VALID_AUTOCREATE_STATUSES: &[&str] =
...
impl PurchaseRequisitionEngine {
    ...
    pub async fn create_requisition(
        ...
        if !VALID_PRIORITIES.contains(&urgency_code) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid urgency code '{}'. Must be one of: {}",
                urgency_code,
        ...
        for (idx, line_req) in request.lines.iter().enumerate() {
            ...
            let qty: f64 = line_req
                .quantity
                .as_deref()
                .unwrap_or("1")
                .parse()
                .unwrap_or(1.0);
            let price: f64 = line_req
                .unit_price
                .as_deref()
                .unwrap_or("0")
                .parse()
                    ...
                    self.repository
                        .create_distribution(
                            ...
                            dist_req
                                .amount
                                .as_deref()
                                .unwrap_or(&format!("{line_amount:.4}")),
        ...
        self.repository
            .get_requisition_by_id(requisition.id)
            .await?
    ...
    pub async fn list_requisitions(
        ...
        self.repository
            .list_requisitions(org_id, status, requester_id)
            .await
    }
    ...
    pub async fn update_requisition(
        ...
        if requisition.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot update requisition in '{}' status. Must be 'draft'.",
                requisition.status
            )));
        }

        let urgency_code = request
            .urgency_code
            .as_deref()
            .unwrap_or(&requisition.urgency_code);
        if !VALID_PRIORITIES.contains(&urgency_code) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid urgency code '{}'. Must be one of: {}",
                urgency_code,
    ...
    pub async fn delete_requisition(&self, id: Uuid) -> AtlasResult<()> {
        ...
        if requisition.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot delete requisition in '{}' status. Must be 'draft'.",
                requisition.status
            )));
        }

        info!(
            "Deleting purchase requisition {}",
            requisition.requisition_number
        );
    ...
    pub async fn add_line(
        ...
        if requisition.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot add lines to requisition in '{}' status",
                requisition.status
            )));
        }
                ...
                self.repository
                    .create_distribution(
                        ...
                        dist_req
                            .amount
                            .as_deref()
                            .unwrap_or(&format!("{dist_amount:.4}")),
                        dist_req.project_code.as_deref(),
                        dist_req.cost_center.as_deref(),
                    )
                    .await?;
        ...
        let total = requisition
            ...
            + line_amount;
        self.repository
            .update_requisition_total(requisition_id, &format!("{total:.2}"))
            .await?;
    ...
    pub async fn add_distribution(
        ...
        if requisition.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot modify distributions on requisition in '{}' status",
                requisition.status
            )));
        }

        let line = requisition
            .lines
            .iter()
            .find(|l| l.id == line_id)
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Line {line_id} not found in requisition"))
            })?;
    ...
    pub async fn submit_requisition(
        ...
        if requisition.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot submit requisition in '{}' status. Must be 'draft'.",
                requisition.status
            )));
        }
        ...
        self.repository
            .update_requisition_status(id, "submitted", submitted_by, None)
            .await
    }
    ...
    pub async fn approve_requisition(
        ...
        if requisition.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot approve requisition in '{}' status. Must be 'submitted'.",
                requisition.status
            )));
        }
        ...
        self.repository
            .update_requisition_status(id, "approved", Some(approver_id), approver_name)
            .await?;
    ...
    pub async fn reject_requisition(
        ...
        if requisition.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot reject requisition in '{}' status. Must be 'submitted'.",
                requisition.status
            )));
        }

        info!(
            "Rejecting purchase requisition {}",
            requisition.requisition_number
        );
        ...
        self.repository
            .update_requisition_status(id, "rejected", None, None)
            .await?;
        ...
        self.repository
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::Internal("Requisition not found after rejection".to_string())
            })
    }
    ...
    pub async fn cancel_requisition(&self, id: Uuid) -> AtlasResult<PurchaseRequisition> {
        ...
        info!(
            "Cancelling purchase requisition {}",
            requisition.requisition_number
        );
        self.repository
            .update_requisition_status(id, "cancelled", None, None)
            .await?;
        self.repository
            .update_line_statuses(id, "cancelled")
            .await?;
        ...
        self.repository
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::Internal("Requisition not found after cancellation".to_string())
            })
    }
    ...
    pub async fn close_requisition(&self, id: Uuid) -> AtlasResult<PurchaseRequisition> {
        ...
        if requisition.status != "approved" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot close requisition in '{}' status. Must be 'approved'.",
                requisition.status
            )));
        }
        ...
        self.repository
            .update_requisition_status(id, "closed", None, None)
            .await?;
    ...
    pub async fn return_requisition(&self, id: Uuid) -> AtlasResult<PurchaseRequisition> {
        ...
        if requisition.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot return requisition in '{}' status. Must be 'submitted'.",
                requisition.status
            )));
        }

        info!(
            "Returning purchase requisition {} to draft",
            requisition.requisition_number
        );
        self.repository
            .update_requisition_status(id, "draft", None, None)
            .await?;
    ...
    pub async fn list_approvals(
        &self,
        requisition_id: Uuid,
    ) -> AtlasResult<Vec<RequisitionApproval>> {
    ...
    pub async fn autocreate(
        ...
        for line_id in &request.requisition_line_ids {
            let line = self
                ...
                .await?
                .ok_or_else(|| {
                    AtlasError::EntityNotFound(format!("Requisition line {line_id} not found"))
                })?;

            if line.status != "approved" {
                return Err(AtlasError::WorkflowError(format!(
                    "Line {} is in '{}' status. Only 'approved' lines can be AutoCreated.",
                    line.line_number, line.status
                )));
            }

            let default_po = format!("PO-{}", chrono::Utc::now().format("%Y%m%d%H%M%S"));
            let po_number = request
            ...
            self.repository
                .update_line_status(line.id, "ordered")
                .await?;
        ...
        for line_id in &request.requisition_line_ids {
            if let Some(line) = self.repository.get_line_by_id(*line_id).await? {
                let requisition = self
                    .repository
                    .get_requisition_by_id(line.requisition_id)
                    .await?;
                if let Some(req) = requisition {
                    let all_ordered = req.lines.iter().all(|l| {
                        l.status == "ordered"
                            || l.status == "partially_ordered"
                            || l.status == "closed"
                    });
                    if all_ordered {
                        self.repository
                            .update_requisition_status(req.id, "closed", None, None)
                            .await?;
                    }
                }
            }
    ...
    pub async fn cancel_autocreate_link(&self, link_id: Uuid) -> AtlasResult<()> {
        ...
        self.repository
            .update_autocreate_link_status(link_id, "cancelled")
            .await
    }
