...
use atlas_shared::{
    AtlasResult, CorrectiveAction, NonConformanceReport, QualityDashboardSummary, QualityHold,
    QualityInspection, QualityInspectionPlan, QualityInspectionPlanCriterion,
...
impl QualityManagementRepository for PostgresQualityManagementRepository {
    ...
    async fn create_plan(
        ...
        .bind(supplier_id)
        .bind(supplier_name)
        .bind(inspection_trigger)
        .bind(sampling_method)
        .bind(sample_size_percent)
        .bind(accept_number)
        .bind(reject_number)
        .bind(frequency)
        .bind(effective_from)
    ...
    async fn get_plan_by_id(&self, id: Uuid) -> AtlasResult<Option<QualityInspectionPlan>> {
        let row = sqlx::query("SELECT * FROM _atlas.quality_inspection_plans WHERE id=$1")
            .bind(id)
    ...
    async fn get_inspection(&self, id: Uuid) -> AtlasResult<Option<QualityInspection>> {
        let row = sqlx::query("SELECT * FROM _atlas.quality_inspections WHERE id=$1")
            .bind(id)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| atlas_shared::AtlasError::DatabaseError(e.to_string()))?;
    ...
    async fn get_ncr(&self, id: Uuid) -> AtlasResult<Option<NonConformanceReport>> {
        let row = sqlx::query("SELECT * FROM _atlas.quality_non_conformance_reports WHERE id=$1")
            .bind(id)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| atlas_shared::AtlasError::DatabaseError(e.to_string()))?;
    ...
    async fn create_corrective_action(
        ...
        .bind(description)
        .bind(root_cause)
        .bind(corrective_action_desc)
        .bind(preventive_action_desc)
        .bind(assigned_to)
        .bind(due_date)
        .bind(priority)
        .bind(created_by)
    ...
    async fn get_corrective_action(&self, id: Uuid) -> AtlasResult<Option<CorrectiveAction>> {
        let row = sqlx::query("SELECT * FROM _atlas.quality_corrective_actions WHERE id=$1")
            .bind(id)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| atlas_shared::AtlasError::DatabaseError(e.to_string()))?;
    ...
    async fn get_hold(&self, id: Uuid) -> AtlasResult<Option<QualityHold>> {
        let row = sqlx::query("SELECT * FROM _atlas.quality_holds WHERE id=$1")
            .bind(id)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| atlas_shared::AtlasError::DatabaseError(e.to_string()))?;
    ...
    async fn get_dashboard_summary(&self, org_id: Uuid) -> AtlasResult<QualityDashboardSummary> {
        ...
        let completed_corrective_actions: i64 =
            row.try_get("completed_corrective_actions").unwrap_or(0);
        ...
        let cap_rate = if total_ncrs > 0 {
            completed_corrective_actions as f64
                / (open_corrective_actions + completed_corrective_actions).max(1) as f64
                * 100.0
        } else {
            0.0
        };
