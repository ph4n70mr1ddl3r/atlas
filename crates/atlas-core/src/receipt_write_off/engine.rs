...
use super::{
    AtlasError, AtlasResult, ReceiptWriteOffRepository, WriteOffBatch, WriteOffDashboard,
    WriteOffPolicy, WriteOffReason, WriteOffRequest,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
impl ReceiptWriteOffEngine {
    pub fn new(r: Arc<dyn ReceiptWriteOffRepository>) -> Self {
        Self { repository: r }
    }
    ...
    pub async fn create_reason(
        ...
        default_gl_account: Option<&str>,
        requires_approval: bool,
        max_auto_approve_amount: Option<&str>,
        ...
        if let Some(gl) = default_gl_account {
            if gl.is_empty() {
                return Err(AtlasError::ValidationFailed(
                    "GL account cannot be empty".into(),
                ));
            }
        }
        if let Some(max) = max_auto_approve_amount {
            let amt: f64 = max.parse().map_err(|_| {
                AtlasError::ValidationFailed("Invalid max auto-approve amount".into())
            })?;
            if amt < 0.0 {
                return Err(AtlasError::ValidationFailed(
                    "Max auto-approve amount must be non-negative".into(),
                ));
            }
        }
    ...
    pub async fn get_reason(
        ...
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<WriteOffReason>> {
    ...
    pub async fn delete_reason(&self, id: Uuid) -> AtlasResult<()> {
        self.repository
            .get_reason_by_id(id)
            .await?
            .ok_or_else(|| AtlasError::EntityNotFound(format!("Reason {id} not found")))?;
    ...
    pub async fn create_request(
        ...
        if receipt_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Receipt number is required".into(),
            ));
        }
        ...
        let amt: f64 = write_off_amount
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid write-off amount".into()))?;
        if amt <= 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Write-off amount must be positive".into(),
            ));
        }

        let reason = self
            .repository
            .get_reason_by_id(reason_id)
            .await?
            .ok_or_else(|| AtlasError::EntityNotFound(format!("Reason {reason_id} not found")))?;
        if !reason.is_active {
            return Err(AtlasError::ValidationFailed(
                "Write-off reason is inactive".into(),
            ));
        }
        ...
        info!(
            "Creating write-off request {} for receipt {} amount {}",
            request_number, receipt_number, write_off_amount
        );
        ...
        self.repository
            .create_request(
                ...
                &reason.reason_code,
                comments,
                reason.default_gl_account.as_deref(),
                status,
                created_by,
            )
            .await
    ...
    pub async fn get_request_by_number(
        ...
        org_id: Uuid,
        number: &str,
    ) -> AtlasResult<Option<WriteOffRequest>> {
    ...
    pub async fn list_requests(
        ...
        self.repository
            .list_requests(org_id, status, reason_id)
            .await
    }
    ...
    pub async fn approve_request(
        ...
        if req.status != "pending_approval" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot approve request in '{}' status. Must be 'pending_approval'.",
                req.status
            )));
        }
        info!("Approving write-off request {}", req.request_number);
        self.repository
            .update_request_status(id, "approved", Some(approved_by), None, None)
            .await
    }
    ...
    pub async fn reject_request(&self, id: Uuid) -> AtlasResult<WriteOffRequest> {
        ...
        if req.status != "pending_approval" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot reject request in '{}' status. Must be 'pending_approval'.",
                req.status
            )));
        }
        self.repository
            .update_request_status(id, "rejected", None, None, None)
            .await
    }
    ...
    pub async fn post_request(
        ...
        if req.status != "approved" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot post request in '{}' status. Must be 'approved'.",
                req.status
            )));
        }
        info!("Posting write-off request {} to GL", req.request_number);
        self.repository
            .update_request_status(id, "posted", None, Some(posted_by), journal_entry_id)
            .await
    }
    ...
    pub async fn reverse_request(&self, id: Uuid) -> AtlasResult<WriteOffRequest> {
        ...
        if req.status != "posted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot reverse request in '{}' status. Must be 'posted'.",
                req.status
            )));
        }
        info!("Reversing write-off request {}", req.request_number);
        self.repository
            .update_request_status(id, "reversed", None, None, None)
            .await
    }
    ...
    pub async fn create_batch(
        ...
        currency_code: &str,
        created_by: Option<Uuid>,
    ) -> AtlasResult<WriteOffBatch> {
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Batch name is required".into(),
            ));
        }
        if currency_code.len() != 3 {
            return Err(AtlasError::ValidationFailed(
        ...
        info!(
            "Creating write-off batch {} for org {}",
            batch_number, org_id
        );
        self.repository
            .create_batch(
    ...
    pub async fn approve_batch(&self, id: Uuid, approved_by: Uuid) -> AtlasResult<WriteOffBatch> {
        ...
        self.repository
            .update_batch_status(id, "approved", Some(approved_by))
            .await
    }
    ...
    pub async fn post_batch(&self, id: Uuid) -> AtlasResult<WriteOffBatch> {
        ...
        self.repository
            .update_batch_status(id, "posted", None)
            .await
    }
    ...
    pub async fn create_policy(
        ...
        max_amount: &str,
        requires_approval: bool,
        auto_approve_below: Option<&str>,
        default_gl_account: Option<&str>,
        aging_threshold_days: Option<i32>,
        created_by: Option<Uuid>,
        ...
        let min: f64 = min_amount
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid min amount".into()))?;
        let max: f64 = max_amount
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid max amount".into()))?;
        if min < 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Min amount must be non-negative".into(),
            ));
        }
        if max <= min {
            return Err(AtlasError::ValidationFailed(
                "Max amount must be greater than min".into(),
            ));
        }
        if let Some(auto) = auto_approve_below {
            let a: f64 = auto
                .parse()
                .map_err(|_| AtlasError::ValidationFailed("Invalid auto-approve amount".into()))?;
            if a < 0.0 {
                return Err(AtlasError::ValidationFailed(
                    "Auto-approve amount must be non-negative".into(),
                ));
            }
        }
        if let Some(days) = aging_threshold_days {
            if days < 0 {
                return Err(AtlasError::ValidationFailed(
                    "Aging threshold days must be non-negative".into(),
                ));
            }
        }
    ...
    pub async fn get_applicable_policy(
        &self,
        org_id: Uuid,
        amount: &str,
    ) -> AtlasResult<Option<WriteOffPolicy>> {
        let amt: f64 = amount
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid amount".into()))?;
        if amt <= 0.0 {
            return Err(AtlasError::ValidationFailed(
...
mod tests {
    ...
    impl ReceiptWriteOffRepository for MockRepo {
        async fn create_reason(
            ...
            let r = WriteOffReason {
                ...
                default_gl_account: gl.map(Into::into),
                requires_approval: req_app,
                max_auto_approve_amount: max_auto.map(Into::into),
                is_active: true,
        ...
        async fn get_reason(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|r| r.organization_id == org_id && r.reason_code == code)
                .cloned())
        }
        ...
        async fn delete_reason(&self, _: Uuid) -> AtlasResult<()> {
            Ok(())
        }
        async fn create_request(
            ...
            let req = WriteOffRequest {
                id: Uuid::new_v4(),
                organization_id: org_id,
                request_number: rn.into(),
                receipt_id: rid,
                receipt_number: rcpt.into(),
                customer_id: cid,
                customer_number: cn.map(Into::into),
                write_off_amount: amt.into(),
                currency_code: cc.into(),
                reason_id: reas_id,
                reason_code: reas_code.into(),
                comments: comments.map(Into::into),
                status: status.into(),
                gl_account_code: gl.map(Into::into),
                approved_by: None,
                approved_at: None,
                posted_by: None,
                posted_at: None,
        ...
        async fn get_request_by_number(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|r| r.organization_id == org_id && r.request_number == rn)
                .cloned())
        }
        async fn list_requests(
            ...
            Ok(self
                ...
                .iter()
                .filter(|r| {
                    r.organization_id == org_id && (status.is_none() || r.status == status.unwrap())
                })
                .cloned()
                .collect())
        }
        async fn update_request_status(
            ...
            if let Some(a) = ab {
                req.approved_by = Some(a);
                req.approved_at = Some(chrono::Utc::now());
            }
            if let Some(p) = pb {
                req.posted_by = Some(p);
                req.posted_at = Some(chrono::Utc::now());
            }
            if let Some(j) = je {
                req.journal_entry_id = Some(j);
            }
            Ok(req.clone())
        }
        ...
        async fn update_batch_status(
            ...
            if let Some(a) = ab {
                b.approved_by = Some(a);
                b.approved_at = Some(chrono::Utc::now());
            }
            Ok(b.clone())
        }
        async fn update_batch_totals(&self, _: Uuid, _: &str, _: i32) -> AtlasResult<()> {
            Ok(())
        }
        async fn create_policy(
            ...
            let p = WriteOffPolicy {
                ...
                max_amount: max.into(),
                requires_approval: req_app,
                auto_approve_below: auto.map(Into::into),
                default_gl_account: gl.map(Into::into),
                aging_threshold_days: aging,
                is_active: true,
                metadata: serde_json::json!({}),
                created_by: cb,
                created_at: chrono::Utc::now(),
        ...
        async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<WriteOffDashboard> {
            ...
            Ok(WriteOffDashboard {
                ...
                pending_approval: reqs
                    .iter()
                    .filter(|r| r.organization_id == org_id && r.status == "pending_approval")
                    .count() as i32,
                approved: reqs
                    .iter()
                    .filter(|r| r.organization_id == org_id && r.status == "approved")
                    .count() as i32,
                rejected: reqs
                    .iter()
                    .filter(|r| r.organization_id == org_id && r.status == "rejected")
                    .count() as i32,
                posted: reqs
                    .iter()
                    .filter(|r| r.organization_id == org_id && r.status == "posted")
                    .count() as i32,
                total_write_off_amount: "0".into(),
                total_by_reason: serde_json::json!([]),
            })
        }
    }

    fn eng() -> ReceiptWriteOffEngine {
        ReceiptWriteOffEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_create_reason_empty_code() {
        assert!(eng()
            .create_reason(Uuid::new_v4(), "", "Name", None, None, false, None, None)
            .await
            .is_err());
    }
    ...
    async fn test_create_reason_empty_name() {
        assert!(eng()
            .create_reason(Uuid::new_v4(), "CODE", "", None, None, false, None, None)
            .await
            .is_err());
    }
    ...
    async fn test_create_reason_duplicate() {
        ...
        let e = eng();
        let _ = e
            .create_reason(org, "DUP", "T1", None, None, false, None, None)
            .await;
        assert!(e
            .create_reason(org, "DUP", "T2", None, None, false, None, None)
            .await
            .is_err());
    }
    ...
    async fn test_create_request_empty_receipt() {
        ...
        let org = Uuid::new_v4();
        let reason = e
            .create_reason(org, "R", "Reason", None, None, false, None, None)
            .await
            .unwrap();
        assert!(e
            .create_request(
    ...
    async fn test_approve_request() {
        ...
        let org = Uuid::new_v4();
        let reason = e
            .create_reason(org, "MAN", "Manual", None, None, true, None, None)
            .await
            .unwrap();
        let req = e
            .create_request(
    ...
    async fn test_approve_request_wrong_status() {
        ...
        let org = Uuid::new_v4();
        let reason = e
            .create_reason(org, "AUTO", "Auto", None, None, false, None, None)
            .await
            .unwrap();
        let req = e
            .create_request(
    ...
    async fn test_list_requests_invalid_status() {
        assert!(eng()
            .list_requests(Uuid::new_v4(), Some("bad"), None)
            .await
            .is_err());
    }
    ...
    async fn test_list_requests_valid() {
        let r = eng()
            .list_requests(Uuid::new_v4(), Some("pending_approval"), None)
            .await;
    ...
    async fn test_create_batch_valid() {
        let b = eng()
            .create_batch(Uuid::new_v4(), "Monthly Write-Off", None, "USD", None)
            .await
            .unwrap();
    ...
    async fn test_create_batch_empty_name() {
        assert!(eng()
            .create_batch(Uuid::new_v4(), "", None, "USD", None)
            .await
            .is_err());
    }
    ...
    async fn test_create_batch_bad_currency() {
        assert!(eng()
            .create_batch(Uuid::new_v4(), "Batch", None, "US", None)
            .await
            .is_err());
    }
    ...
    async fn test_approve_batch() {
        let e = eng();
        let b = e
            .create_batch(Uuid::new_v4(), "Batch", None, "USD", None)
            .await
            .unwrap();
    ...
    async fn test_list_batches_invalid_status() {
        assert!(eng()
            .list_batches(Uuid::new_v4(), Some("bad"))
            .await
            .is_err());
    }
    ...
    async fn test_get_applicable_policy_invalid_amount() {
        assert!(eng()
            .get_applicable_policy(Uuid::new_v4(), "0")
            .await
            .is_err());
        assert!(eng()
            .get_applicable_policy(Uuid::new_v4(), "abc")
            .await
            .is_err());
    }
}
