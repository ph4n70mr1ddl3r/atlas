...
use super::{
    AtlasError, AtlasResult, FactorCompany, FactoringAgreement, FactoringDashboard,
    FactoringRequest, FactoringRequestLine, FactoringSettlement, ReceivablesFactoringRepository,
    VALID_AGREEMENT_STATUSES, VALID_AGREEMENT_TYPES, VALID_RECOURSE_TYPES, VALID_REQUEST_STATUSES,
...
impl ReceivablesFactoringEngine {
    ...
    pub async fn create_factor_company(
        ...
        if !VALID_RECOURSE_TYPES.contains(&recourse_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid recourse type '{}'. Must be one of: {}",
                recourse_type,
                VALID_RECOURSE_TYPES.join(", ")
            )));
        }
        ...
        advance_rate
            .parse::<f64>()
            .map_err(|_| AtlasError::ValidationFailed("Invalid advance rate".into()))?;
        fee_rate
            .parse::<f64>()
            .map_err(|_| AtlasError::ValidationFailed("Invalid fee rate".into()))?;

        if let Some(min) = minimum_invoice_amount {
            if min.parse::<f64>().unwrap_or(-1.0) < 0.0 {
                return Err(AtlasError::ValidationFailed(
                    "Minimum invoice amount must be non-negative".into(),
                ));
            }
        }
        if let Some(max) = maximum_invoice_amount {
            if max.parse::<f64>().unwrap_or(-1.0) < 0.0 {
                return Err(AtlasError::ValidationFailed(
                    "Maximum invoice amount must be non-negative".into(),
                ));
            }
        }
        ...
        if self
            .repository
            .get_factor_company_by_code(org_id, code)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Factor company code '{code}' already exists"
            )));
        }

        info!("Creating factor company '{}' for org {}", code, org_id);
        self.repository
            .create_factor_company(
                org_id,
                code,
                name,
                description,
    ...
    pub async fn get_factor_company_by_code(
        ...
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<FactorCompany>> {
        self.repository
            .get_factor_company_by_code(org_id, code)
            .await
    }
    ...
    ) -> AtlasResult<Vec<FactorCompany>> {
        self.repository
            .list_factor_companies(org_id, is_active)
            .await
    }
    ...
    pub async fn activate_factor_company(&self, id: Uuid) -> AtlasResult<FactorCompany> {
        ...
        if fc.is_active {
            return Err(AtlasError::ValidationFailed(
                "Factor company is already active".into(),
            ));
        }
    ...
    pub async fn create_agreement(
        ...
        if agreement_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Agreement number is required".into(),
            ));
        }
        if agreement_name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Agreement name is required".into(),
            ));
        }
        if !VALID_AGREEMENT_TYPES.contains(&agreement_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid agreement type '{}'. Must be one of: {}",
                agreement_type,
                VALID_AGREEMENT_TYPES.join(", ")
            )));
        }
        if !VALID_RECOURSE_TYPES.contains(&recourse_type) {
            return Err(AtlasError::ValidationFailed(format!(
        ...
        factoring_fee_rate
            .parse::<f64>()
            .map_err(|_| AtlasError::ValidationFailed("Invalid factoring fee rate".into()))?;
        late_fee_rate
            .parse::<f64>()
            .map_err(|_| AtlasError::ValidationFailed("Invalid late fee rate".into()))?;
        reserve_rate
            .parse::<f64>()
            .map_err(|_| AtlasError::ValidationFailed("Invalid reserve rate".into()))?;
        minimum_fee
            .parse::<f64>()
            .map_err(|_| AtlasError::ValidationFailed("Invalid minimum fee".into()))?;
        ...
        let fc = self
            .repository
            .get_factor_company(factor_company_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Factor company {factor_company_id} not found"))
            })?;
        if !fc.is_active {
            return Err(AtlasError::ValidationFailed(
                "Factor company is not active".into(),
            ));
        }

        info!(
            "Creating factoring agreement '{}' with factor '{}'",
            agreement_number, fc.code
        );
        self.repository
            .create_agreement(
                org_id,
    ...
    pub async fn list_agreements(
        ...
        self.repository
            .list_agreements(org_id, status, factor_company_id)
            .await
    }
    ...
    pub async fn activate_agreement(
        ...
        self.repository
            .update_agreement_status(id, "active", approved_by)
            .await
    }
    ...
    pub async fn suspend_agreement(&self, id: Uuid) -> AtlasResult<FactoringAgreement> {
        ...
        if a.status != "active" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot suspend agreement in '{}' status. Must be 'active'.",
                a.status
            )));
        }
        info!("Suspending factoring agreement '{}'", a.agreement_number);
        self.repository
            .update_agreement_status(id, "suspended", None)
            .await
    }
    ...
    pub async fn terminate_agreement(&self, id: Uuid) -> AtlasResult<FactoringAgreement> {
        ...
        if a.status != "active" && a.status != "suspended" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot terminate agreement in '{}' status. Must be 'active' or 'suspended'.",
                a.status
            )));
        }
        info!("Terminating factoring agreement '{}'", a.agreement_number);
        self.repository
            .update_agreement_status(id, "terminated", None)
            .await
    }
    ...
    pub async fn create_request(
        ...
        if request_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Request number is required".into(),
            ));
        }
        if !VALID_RECOURSE_TYPES.contains(&recourse_type) {
            return Err(AtlasError::ValidationFailed(format!(
        ...
        let agreement = self
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Agreement {agreement_id} not found"))
            })?;
        ...
        info!(
            "Creating factoring request '{}' against agreement '{}'",
            request_number, agreement.agreement_number
        );
        self.repository
            .create_request(
                org_id,
    ...
    pub async fn list_requests(
        ...
        self.repository
            .list_requests(org_id, agreement_id, status)
            .await
    }
    ...
    pub async fn submit_request(&self, id: Uuid) -> AtlasResult<FactoringRequest> {
        ...
        if r.status != "draft" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot submit request in '{}' status. Must be 'draft'.",
                r.status
            )));
        }
        info!(
            "Submitting factoring request '{}' for approval",
            r.request_number
        );
    ...
    pub async fn approve_request(&self, id: Uuid) -> AtlasResult<FactoringRequest> {
        ...
        if r.status != "submitted" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot approve request in '{}' status. Must be 'submitted'.",
                r.status
            )));
        }
    ...
    pub async fn fund_request(&self, id: Uuid) -> AtlasResult<FactoringRequest> {
        ...
        if r.status != "approved" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot fund request in '{}' status. Must be 'approved'.",
                r.status
            )));
        }
    ...
    pub async fn settle_request(&self, id: Uuid) -> AtlasResult<FactoringRequest> {
        ...
        if r.status != "funded" && r.status != "partially_settled" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot settle request in '{}' status. Must be 'funded' or 'partially_settled'.",
                r.status
            )));
        }
    ...
    pub async fn cancel_request(&self, id: Uuid) -> AtlasResult<FactoringRequest> {
        ...
        if r.status != "draft" && r.status != "submitted" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot cancel request in '{}' status. Must be 'draft' or 'submitted'.",
                r.status
            )));
        }
    ...
    pub async fn add_request_line(
        ...
        let inv_amt: f64 = invoice_amount
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid invoice amount".into()))?;
        let elig_amt: f64 = eligible_amount
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid eligible amount".into()))?;
        if inv_amt <= 0.0 {
            return Err(AtlasError::ValidationFailed(
        ...
        if elig_amt < 0.0 || elig_amt > inv_amt {
            return Err(AtlasError::ValidationFailed(
                "Eligible amount must be between 0 and invoice amount".into(),
            ));
        }
        if line_number < 1 {
            return Err(AtlasError::ValidationFailed(
        ...
        info!(
            "Adding line {} to factoring request '{}'",
            line_number, request.request_number
        );
        self.repository
            .add_request_line(
                org_id,
    ...
    pub async fn list_request_lines(
        &self,
        request_id: Uuid,
    ) -> AtlasResult<Vec<FactoringRequestLine>> {
    ...
    pub async fn create_settlement(
        ...
        if settlement_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Settlement number is required".into(),
            ));
        }
        ...
        if let Some(s) = status {
            if !VALID_SETTLEMENT_STATUSES.contains(&s) {
                return Err(AtlasError::ValidationFailed(format!(
                    "Invalid status '{}'. Must be one of: {}",
                    s,
                    VALID_SETTLEMENT_STATUSES.join(", ")
                )));
            }
        }
        self.repository
            .list_settlements(org_id, agreement_id, status)
            .await
    }
    ...
    pub async fn process_settlement(
        ...
        self.repository
            .update_settlement_status(id, "processed", processed_by)
            .await
    }
...
mod tests {
    ...
    impl ReceivablesFactoringRepository for MockRepo {
        async fn create_factor_company(
            ...
            let fc = FactorCompany {
                ...
                contact_name: cn.map(Into::into),
                contact_email: ce.map(Into::into),
                contact_phone: cp.map(Into::into),
                bank_name: bn.map(Into::into),
                bank_account_number: ba.map(Into::into),
                default_advance_rate: ar.into(),
                default_fee_rate: fr.into(),
        ...
        async fn get_factor_company_by_code(
            &self,
            org_id: Uuid,
            code: &str,
        ) -> AtlasResult<Option<FactorCompany>> {
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|c| c.organization_id == org_id && c.code == code)
                .cloned())
        }

        async fn list_factor_companies(
            &self,
            org_id: Uuid,
            is_active: Option<bool>,
        ) -> AtlasResult<Vec<FactorCompany>> {
        ...
        async fn create_agreement(
            ...
            let a = FactoringAgreement {
                id: Uuid::new_v4(),
                organization_id: org_id,
                agreement_number: num.into(),
                factor_company_id: fc_id,
                factor_company_code: None,
                agreement_name: name.into(),
                description: desc.map(Into::into),
                agreement_type: at.into(),
                recourse_type: rt.into(),
                advance_rate: ar.into(),
                factoring_fee_rate: ffr.into(),
                late_fee_rate: lfr.into(),
                reserve_rate: rr.into(),
                minimum_fee: mf.into(),
                currency_code: cc.into(),
                start_date: sd,
                end_date: ed,
                credit_limit: cl.map(Into::into),
                total_factored_amount: "0.00".into(),
                total_advance_amount: "0.00".into(),
                total_fee_amount: "0.00".into(),
                total_reserve_amount: "0.00".into(),
                total_settled_amount: "0.00".into(),
                status: "draft".into(),
                approved_by: None,
                approved_at: None,
                metadata: serde_json::json!({}),
        ...
        async fn update_agreement_status(
            ...
            if status == "active" {
                a.approved_by = ab;
                a.approved_at = Some(chrono::Utc::now());
            }
        ...
        async fn create_request(
            ...
            let r = FactoringRequest {
                id: Uuid::new_v4(),
                organization_id: org_id,
                request_number: num.into(),
                agreement_id: agr_id,
                agreement_number: None,
                factor_company_id: None,
                factor_company_name: None,
                request_date: rd,
                funding_date: None,
                settlement_date: None,
                total_invoice_amount: "0.00".into(),
                eligible_amount: "0.00".into(),
                advance_rate: "0.8000".into(),
                advance_amount: "0.00".into(),
                factoring_fee_rate: "0.0150".into(),
                factoring_fee_amount: "0.00".into(),
                reserve_rate: "0.0500".into(),
                reserve_amount: "0.00".into(),
                recourse_type: rt.into(),
                currency_code: cc.into(),
                status: "draft".into(),
                approved_by: None,
        ...
        async fn update_request_funding(
            &self,
            id: Uuid,
            fb: Option<Uuid>,
        ) -> AtlasResult<FactoringRequest> {
            ...
            let r = rs
                ...
                .find(|r| r.id == id)
                .ok_or_else(|| AtlasError::EntityNotFound("Not found".into()))?;
            r.status = "funded".into();
            r.funded_by = fb;
            r.funded_at = Some(chrono::Utc::now());
            r.updated_at = chrono::Utc::now();
            Ok(r.clone())
        }

        async fn update_request_settlement(
            &self,
            id: Uuid,
            sb: Option<Uuid>,
        ) -> AtlasResult<FactoringRequest> {
            ...
            let r = rs
                ...
                .find(|r| r.id == id)
                .ok_or_else(|| AtlasError::EntityNotFound("Not found".into()))?;
            r.status = "settled".into();
            r.settled_by = sb;
            r.settled_at = Some(chrono::Utc::now());
            r.updated_at = chrono::Utc::now();
            Ok(r.clone())
        }
        ...
        async fn add_request_line(
            ...
            cnum: Option<&str>,
            cname: Option<&str>,
            idate: Option<chrono::NaiveDate>,
            idd: Option<chrono::NaiveDate>,
            iam: &str,
            eam: &str,
            do_: Option<i32>,
            dov: Option<i32>,
            ...
            let line = FactoringRequestLine {
                ...
                line_number: ln,
                transaction_id: tid,
                transaction_number: tnum.map(Into::into),
                customer_id: cid,
                customer_number: cnum.map(Into::into),
                customer_name: cname.map(Into::into),
                invoice_date: idate,
                invoice_due_date: idd,
                invoice_amount: iam.into(),
                eligible_amount: eam.into(),
                days_outstanding: do_,
                days_overdue: dov,
                advance_amount: "0.00".into(),
                factoring_fee_amount: "0.00".into(),
                reserve_amount: "0.00".into(),
                settlement_amount: "0.00".into(),
                is_eligible: true,
        ...
        async fn create_settlement(
            ...
            let s = FactoringSettlement {
                id: Uuid::new_v4(),
                organization_id: org_id,
                settlement_number: num.into(),
                agreement_id: agr_id,
                request_id: req_id,
                settlement_date: sd,
                total_settled: "0.00".into(),
                total_reserve_released: "0.00".into(),
                total_chargebacks: "0.00".into(),
                total_late_fees: "0.00".into(),
                net_to_customer: "0.00".into(),
                currency_code: cc.into(),
                status: "draft".into(),
                processed_by: None,
                processed_at: None,
        ...
        async fn update_settlement_status(
            ...
            if status == "processed" {
                s.processed_by = pb;
                s.processed_at = Some(chrono::Utc::now());
            }
        ...
        async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<FactoringDashboard> {
            ...
            let org_companies: Vec<_> = companies
                .iter()
                .filter(|c| c.organization_id == org_id)
                .collect();
            let org_agreements: Vec<_> = agreements
                .iter()
                .filter(|a| a.organization_id == org_id)
                .collect();
            let org_requests: Vec<_> = requests
                .iter()
                .filter(|r| r.organization_id == org_id)
                .collect();
            let org_settlements: Vec<_> = settlements
                .iter()
                .filter(|s| s.organization_id == org_id)
                .collect();
            ...
            Ok(FactoringDashboard {
                ...
                active_factor_companies: org_companies.iter().filter(|c| c.is_active).count()
                    as i64,
                total_agreements: org_agreements.len() as i64,
                active_agreements: org_agreements
                    .iter()
                    .filter(|a| a.status == "active")
                    .count() as i64,
                total_requests: org_requests.len() as i64,
                draft_requests: org_requests.iter().filter(|r| r.status == "draft").count() as i64,
                approved_requests: org_requests
                    .iter()
                    .filter(|r| r.status == "approved")
                    .count() as i64,
                funded_requests: org_requests.iter().filter(|r| r.status == "funded").count()
                    as i64,
                settled_requests: org_requests
                    .iter()
    ...
    async fn make_active_agreement(
        eng: &ReceivablesFactoringEngine,
        org_id: Uuid,
        fc_id: Uuid,
    ) -> FactoringAgreement {
        let a = eng
    ...
    async fn test_create_factor_company_duplicate_code_fails() {
        ...
        let _ = e
            .create_factor_company(
                org, "FC-DUP", "Name", None, None, None, None, None, None, "0.8", "0.01",
                "recourse", None, None, None,
            )
            .await
            .unwrap();
        let r = e
            .create_factor_company(
                org, "FC-DUP", "Name 2", None, None, None, None, None, None, "0.8", "0.01",
                "recourse", None, None, None,
            )
            .await;
    ...
    async fn test_deactivate_activate_factor_company() {
        ...
        let fc = e
            .create_factor_company(
                org, "FC-DA", "Name", None, None, None, None, None, None, "0.8", "0.01",
                "recourse", None, None, None,
            )
            .await
            .unwrap();
    ...
    async fn test_deactivate_already_inactive_fails() {
        ...
        let fc = e
            .create_factor_company(
                org, "FC-DI", "Name", None, None, None, None, None, None, "0.8", "0.01",
                "recourse", None, None, None,
            )
            .await
            .unwrap();
    ...
    async fn test_add_line_to_submitted_request_fails() {
        ...
        let res = e
            .add_request_line(
                org, r.id, 1, None, None, None, None, None, None, None, "1000.00", "1000.00", None,
                None,
            )
            .await;
        assert!(res.is_err());
    ...
    async fn test_settlement_lifecycle() {
        ...
        let s = e
            .process_settlement(s.id, Some(Uuid::new_v4()))
            .await
            .unwrap();
    ...
    async fn test_create_agreement_inactive_company_fails() {
        ...
        let fc = e
            .create_factor_company(
                org, "FC-IN", "Name", None, None, None, None, None, None, "0.8", "0.01",
                "recourse", None, None, None,
            )
            .await
            .unwrap();
