...
use atlas_shared::{
    ...
    ReceiptLine, ReceiptReturn, ReceivingDashboard, ReceivingLocation,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
impl ReceivingEngine {
    ...
    pub async fn list_receipts(
        ...
        self.repository
            .list_receipts(org_id, status, supplier_id)
            .await
    }
    ...
    pub async fn confirm_receipt(
        ...
        self.repository
            .update_receipt_status(receipt_id, "received", Some(received_by), None)
            .await
    }
    ...
    pub async fn close_receipt(&self, receipt_id: Uuid) -> AtlasResult<ReceiptHeader> {
        ...
        self.repository
            .update_receipt_status(receipt_id, "closed", None, None)
            .await
    }
    ...
    pub async fn cancel_receipt(&self, receipt_id: Uuid) -> AtlasResult<ReceiptHeader> {
        ...
        if receipt.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot cancel receipt in '{}' status. Must be 'draft'.",
                receipt.status
            )));
        }
        ...
        self.repository
            .update_receipt_status(receipt_id, "cancelled", None, None)
            .await
    }
    ...
    pub async fn add_receipt_line(
        ...
        if receipt.status != "draft" && receipt.status != "received" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot add lines to receipt in '{}' status",
                receipt.status
            )));
        }

        let received: f64 = received_qty.parse().map_err(|_| {
            AtlasError::ValidationFailed("Received quantity must be a valid number".to_string())
        })?;
        if received < 0.0 {
            return Err(AtlasError::ValidationFailed(
        ...
        info!(
            "Adding line {} to receipt {}",
            line_number, receipt.receipt_number
        );
    ...
    pub async fn create_inspection(
        ...
        let _line = self
            .repository
            .get_receipt_line(receipt_line_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Receipt line {receipt_line_id} not found"))
            })?;

        let inspection_number = format!("INS-{}", Uuid::new_v4().to_string()[..8].to_uppercase());

        info!(
            "Creating inspection {} for line {}",
            inspection_number, receipt_line_id
        );
    ...
    pub async fn complete_inspection(
        ...
        if inspection.status != "pending" && inspection.status != "in_progress" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot complete inspection in '{}' status",
                inspection.status
            )));
        }
        ...
        let inspected: f64 = quantity_inspected.parse().map_err(|_| {
            AtlasError::ValidationFailed("Quantity inspected must be a valid number".to_string())
        })?;
        let accepted: f64 = quantity_accepted.parse().map_err(|_| {
            AtlasError::ValidationFailed("Quantity accepted must be a valid number".to_string())
        })?;
        let rejected: f64 = quantity_rejected.parse().map_err(|_| {
            AtlasError::ValidationFailed("Quantity rejected must be a valid number".to_string())
        })?;
        ...
        info!(
            "Completing inspection {} with disposition {}",
            inspection.inspection_number, disposition
        );
    ...
    pub async fn create_delivery(
        ...
        let qty: f64 = quantity_delivered.parse().map_err(|_| {
            AtlasError::ValidationFailed("Delivered quantity must be a valid number".to_string())
        })?;
        if qty <= 0.0 {
            return Err(AtlasError::ValidationFailed(
        ...
        info!(
            "Creating delivery {} for receipt line {}",
            delivery_number, receipt_line_id
        );
    ...
    pub async fn create_return(
        ...
        let qty: f64 = quantity_returned.parse().map_err(|_| {
            AtlasError::ValidationFailed("Return quantity must be a valid number".to_string())
        })?;
        if qty <= 0.0 {
            return Err(AtlasError::ValidationFailed(
        ...
        info!(
            "Creating return {} for supplier {:?}",
            return_number, supplier_id
        );
    ...
    pub async fn submit_return(&self, return_id: Uuid) -> AtlasResult<ReceiptReturn> {
        ...
        if rtv.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot submit return in '{}' status. Must be 'draft'.",
                rtv.status
            )));
        }
        ...
        self.repository
            .update_return_status(return_id, "submitted", None, None)
            .await
    }
    ...
    pub async fn ship_return(
        ...
        if rtv.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot ship return in '{}' status. Must be 'submitted'.",
                rtv.status
            )));
        }
        ...
        self.repository
            .update_return_status(return_id, "shipped", carrier, tracking_number)
            .await
    }
    ...
    pub async fn credit_return(&self, return_id: Uuid) -> AtlasResult<ReceiptReturn> {
        ...
        if rtv.status != "shipped" && rtv.status != "received_by_supplier" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot credit return in '{}' status.",
                rtv.status
            )));
        }
        ...
        self.repository
            .update_return_status(return_id, "credited", None, None)
            .await
    }
    ...
    pub async fn cancel_return(&self, return_id: Uuid) -> AtlasResult<ReceiptReturn> {
        ...
        if rtv.status != "draft" && rtv.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot cancel return in '{}' status.",
                rtv.status
            )));
        }
        ...
        self.repository
            .update_return_status(return_id, "cancelled", None, None)
            .await
    }
    ...
    pub async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<ReceivingDashboard> {
        ...
        let pending_inspections = self
            .repository
            .list_inspections(org_id, None)
            .await?
            .iter()
        ...
        let receipts_by_status: serde_json::Value = status_counts
            .into_iter()
            .map(|(k, v)| serde_json::json!({"status": k, "count": v}))
            .collect();

        // Top suppliers by receipt count
        let mut supplier_counts: std::collections::HashMap<String, i32> =
            std::collections::HashMap::new();
        for r in &receipts {
        ...
        let top_suppliers: serde_json::Value = supplier_vec
            .into_iter()
        ...
        recent.truncate(5);
        let recent_receipts: serde_json::Value = recent
            .iter()
            .map(|r| {
                serde_json::json!({
                    "id": r.id,
