...
use super::{
    AtlasError, AtlasResult, RevMgmtContract, RevMgmtDashboard, RevMgmtObligation,
    RevMgmtRecognitionEvent, RevMgmtSSP, RevenueManagementRepository,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
const VALID_RECOGNITION_PATTERNS: &[&str] = &[
    "straight_line",
...
impl RevenueManagementEngine {
    pub fn new(repository: Arc<dyn RevenueManagementRepository>) -> Self {
        Self { repository }
    }
    ...
    pub async fn create_contract(
        ...
        currency_code: &str,
        contract_start_date: chrono::NaiveDate,
        contract_end_date: Option<chrono::NaiveDate>,
        created_by: Option<Uuid>,
    ) -> AtlasResult<RevMgmtContract> {
        if contract_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Contract number is required".into(),
            ));
        }
        ...
        if price < 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Transaction price must be non-negative".into(),
            ));
        }
        ...
        if let Some(end) = contract_end_date {
            if end < contract_start_date {
                return Err(AtlasError::ValidationFailed(
                    "Contract end date must be after start date".into(),
                ));
            }
        }
        if self
            .repository
            .get_contract(org_id, contract_number)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Contract number '{contract_number}' already exists"
            )));
        }
        info!(
            "Creating revenue contract {} for org {}",
            contract_number, org_id
        );
        self.repository
            .create_contract(
                org_id,
    ...
    pub async fn get_contract(
        ...
        org_id: Uuid,
        contract_number: &str,
    ) -> AtlasResult<Option<RevMgmtContract>> {
        self.repository.get_contract(org_id, contract_number).await
    }
    pub async fn get_contract_by_id(&self, id: Uuid) -> AtlasResult<Option<RevMgmtContract>> {
        self.repository.get_contract_by_id(id).await
    }
    ...
    pub async fn list_contracts(
        ...
        org_id: Uuid,
        status: Option<&str>,
    ) -> AtlasResult<Vec<RevMgmtContract>> {
    ...
    pub async fn activate_contract(&self, contract_id: Uuid) -> AtlasResult<RevMgmtContract> {
        let c = self
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Contract {contract_id} not found"))
            })?;
        if c.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot activate contract in '{}' status",
                c.status
            )));
        }
        info!("Activating revenue contract {}", c.contract_number);
        self.repository
            .update_contract_status(contract_id, "active")
            .await
    }
    ...
    pub async fn complete_contract(&self, contract_id: Uuid) -> AtlasResult<RevMgmtContract> {
        ...
        if c.status != "active" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot complete contract in '{}' status",
                c.status
            )));
        }
        let obs = self
            .repository
            .list_obligations_by_contract(contract_id)
            .await?;
        if !obs.iter().all(|o| o.satisfaction_status == "satisfied") {
            return Err(AtlasError::WorkflowError(
                "Not all obligations satisfied".into(),
            ));
        }
        info!("Completing revenue contract {}", c.contract_number);
        self.repository
            .update_contract_status(contract_id, "complete")
            .await
    }
    ...
    pub async fn create_obligation(
        ...
        if contract.status != "draft" && contract.status != "active" {
            return Err(AtlasError::WorkflowError(
                "Can only add obligations to draft/active contracts".into(),
            ));
        }
        ...
        if !VALID_OBLIGATION_TYPES.contains(&obligation_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid obligation_type '{obligation_type}'"
            )));
        }
        if !VALID_SATISFACTION_METHODS.contains(&satisfaction_method) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid satisfaction_method '{satisfaction_method}'"
            )));
        }
        if !VALID_RECOGNITION_PATTERNS.contains(&recognition_pattern) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid recognition_pattern '{recognition_pattern}'"
            )));
        }
        let ssp: f64 = standalone_selling_price.parse().unwrap_or(-1.0);
        if ssp < 0.0 {
            return Err(AtlasError::ValidationFailed(
                "SSP must be non-negative".into(),
            ));
        }
        self.repository
            .create_obligation(
    ...
    pub async fn list_obligations(&self, contract_id: Uuid) -> AtlasResult<Vec<RevMgmtObligation>> {
        self.repository
            .list_obligations_by_contract(contract_id)
            .await
    }
    ...
    pub async fn allocate_transaction_price(
        &self,
        contract_id: Uuid,
    ) -> AtlasResult<Vec<RevMgmtObligation>> {
        let contract = self
            .repository
            .get_contract_by_id(contract_id)
            .await?
        ...
        if contract.status != "active" {
            return Err(AtlasError::WorkflowError(
                "Can only allocate for active contracts".into(),
            ));
        }
        let obs = self
            .repository
        ...
        let tot_rec: f64 = updated
            .iter()
            .map(|o| o.recognized_amount.parse::<f64>().unwrap_or(0.0))
            .sum();
        let sat = updated
            .iter()
            .filter(|o| o.satisfaction_status == "satisfied")
            .count() as i32;
        self.repository
            .update_contract_totals(
                contract_id,
                &allocated_total.to_string(),
                &tot_rec.to_string(),
                &(allocated_total - tot_rec).to_string(),
                updated.len() as i32,
                sat,
            )
            .await?;
    ...
    pub async fn create_ssp(
        ...
        created_by: Option<Uuid>,
    ) -> AtlasResult<RevMgmtSSP> {
        if item_code.is_empty() || item_name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Item code and name required".into(),
            ));
        }
        if !VALID_SSP_METHODS.contains(&estimation_method) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid estimation_method '{estimation_method}'"
            )));
        }
    ...
    ) -> AtlasResult<Option<RevMgmtSSP>> {
        self.repository.get_ssp(org_id, item_code, on_date).await
    }
    pub async fn list_ssps(&self, org_id: Uuid) -> AtlasResult<Vec<RevMgmtSSP>> {
        self.repository.list_ssps(org_id).await
    }
    ...
    pub async fn satisfy_obligation(
        ...
        created_by: Option<Uuid>,
    ) -> AtlasResult<RevMgmtRecognitionEvent> {
        let o = self
            .repository
            .get_obligation(obligation_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Obligation {obligation_id} not found"))
            })?;
        ...
        if new_total > alloc + 0.01 {
            return Err(AtlasError::ValidationFailed(format!(
                "Amount {new_total} exceeds allocated {alloc}"
            )));
        }
        ...
        let new_pct = if alloc > 0.0 {
            new_total / alloc * 100.0
        } else {
            0.0
        };
        ...
        let event = self
            ...
            .create_recognition_event(
                ...
                obligation_id,
                &evnum,
                &format!("{} of obligation {}", etype, o.obligation_number),
                etype,
                amount,
                recognition_date,
                gl_account_code,
        ...
        let all_obs = self
            .repository
            .list_obligations_by_contract(o.contract_id)
            .await?;
        let tot_rec: f64 = all_obs
            .iter()
            .map(|ob| ob.recognized_amount.parse::<f64>().unwrap_or(0.0))
            .sum();
        let tot_alloc: f64 = all_obs
            .iter()
            .map(|ob| ob.allocated_amount.parse::<f64>().unwrap_or(0.0))
            .sum();
        let sat = all_obs
            .iter()
            .filter(|ob| ob.satisfaction_status == "satisfied")
            .count() as i32;
        self.repository
            .update_contract_totals(
                o.contract_id,
    ...
    pub async fn list_recognition_events(
        &self,
        contract_id: Uuid,
    ) -> AtlasResult<Vec<RevMgmtRecognitionEvent>> {
        self.repository.list_events_by_contract(contract_id).await
    }
    pub async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<RevMgmtDashboard> {
...
mod tests {
    ...
    impl MockRepo {
        fn new() -> Self {
            MockRepo {
                contracts: std::sync::Mutex::new(vec![]),
            }
        }
    }
    ...
    impl RevenueManagementRepository for MockRepo {
        async fn create_contract(
            ...
            tp: &str,
            cc: &str,
            csd: chrono::NaiveDate,
            ced: Option<chrono::NaiveDate>,
            cb: Option<Uuid>,
        ) -> AtlasResult<RevMgmtContract> {
            let c = RevMgmtContract {
                id: Uuid::new_v4(),
                organization_id: org_id,
                contract_number: cn.into(),
                customer_id: cid,
                customer_name: cname.into(),
                description: desc.map(Into::into),
                status: "draft".into(),
                transaction_price: tp.into(),
                total_allocated: "0".into(),
                total_recognized: "0".into(),
                total_unrecognized: tp.into(),
                currency_code: cc.into(),
                contract_start_date: csd,
                contract_end_date: ced,
                performance_obligation_count: 0,
        ...
        async fn get_contract(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|c| c.organization_id == org_id && c.contract_number == cn)
                .cloned())
        }
        ...
        async fn create_obligation(
            ...
            Ok(RevMgmtObligation {
                id: Uuid::new_v4(),
                organization_id: org_id,
                contract_id,
                contract_number: cn.map(Into::into),
                obligation_number: on.into(),
                description: desc.into(),
                obligation_type: ot.into(),
                satisfaction_status: "not_started".into(),
                satisfaction_method: sm.into(),
                recognition_pattern: rp.into(),
                standalone_selling_price: ssp.into(),
                allocated_amount: aa.into(),
                recognized_amount: "0".into(),
                unrecognized_amount: aa.into(),
                recognition_start_date: rsd,
                recognition_end_date: red,
                percent_complete: "0".into(),
                metadata: serde_json::json!({}),
                created_at: chrono::Utc::now(),
                updated_at: chrono::Utc::now(),
            })
        }
        async fn get_obligation(&self, _: Uuid) -> AtlasResult<Option<RevMgmtObligation>> {
            Ok(None)
        }
        async fn list_obligations_by_contract(
            &self,
            _: Uuid,
        ) -> AtlasResult<Vec<RevMgmtObligation>> {
            Ok(vec![])
        ...
        async fn list_ssps(&self, _: Uuid) -> AtlasResult<Vec<RevMgmtSSP>> {
            Ok(vec![])
        }
        async fn create_recognition_event(
            &self,
            org_id: Uuid,
            contract_id: Uuid,
            obl_id: Uuid,
            ...
            Ok(RevMgmtRecognitionEvent {
                id: Uuid::new_v4(),
                organization_id: org_id,
                contract_id,
                obligation_id: obl_id,
                event_number: en.into(),
                description: desc.into(),
                event_type: et.into(),
                amount: amt.into(),
                recognition_date: rd,
                gl_account_code: glac.map(Into::into),
                is_posted: false,
                posted_at: None,
                metadata: serde_json::json!({}),
                created_by: cb,
        ...
        async fn list_events_by_contract(
            &self,
            _: Uuid,
        ) -> AtlasResult<Vec<RevMgmtRecognitionEvent>> {
            Ok(vec![])
        }
        async fn list_events_by_obligation(
            &self,
            _: Uuid,
        ) -> AtlasResult<Vec<RevMgmtRecognitionEvent>> {
            Ok(vec![])
        }
        async fn get_dashboard(&self, _: Uuid) -> AtlasResult<RevMgmtDashboard> {
            Ok(RevMgmtDashboard {
                total_contracts: 0,
                active_contracts: 0,
                total_performance_obligations: 0,
                satisfied_obligations: 0,
                total_transaction_price: "0".into(),
                total_allocated: "0".into(),
                total_recognized: "0".into(),
                total_unrecognized: "0".into(),
                unrecognized_by_period: serde_json::json!([]),
            })
        }
    }

    fn eng() -> RevenueManagementEngine {
        RevenueManagementEngine::new(Arc::new(MockRepo::new()))
    }
