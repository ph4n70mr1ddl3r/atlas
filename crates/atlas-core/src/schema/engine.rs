...
use atlas_shared::{AtlasError, AtlasResult, EntityDefinition, FieldDefinition};
use dashmap::DashMap;
use std::sync::Arc;
...
impl SchemaEngine {
    ...
    pub async fn upsert_entity(&self, entity: EntityDefinition) -> AtlasResult<()> {
        ...
        self.repository
            .set_entity_version(&name, new_version)
            .await?;
    ...
    fn validate_entity(&self, entity: &EntityDefinition) -> AtlasResult<()> {
        ...
        for field in &entity.fields {
            if !field_names.insert(&field.name) {
                return Err(AtlasError::SchemaError(format!(
        ...
        if let Some(workflow) = &entity.workflow {
            ...
            for trans in &workflow.transitions {
                if !workflow.states.iter().any(|s| s.name == trans.from_state) {
                    return Err(AtlasError::SchemaError(format!(
                        "Invalid from_state in transition: {}",
                        trans.from_state
                    )));
                }
                if !workflow.states.iter().any(|s| s.name == trans.to_state) {
                    return Err(AtlasError::SchemaError(format!(
                        "Invalid to_state in transition: {}",
                        trans.to_state
                    )));
                }
            }
