...
use super::{AccessDecision, FieldCheck, FieldSecurity, SecurityContext};
...
impl SecurityEngine {
    ...
    pub fn check_access(
        ...
        if ctx.roles.contains(&"admin".to_string()) || ctx.roles.contains(&"system".to_string()) {
            return AccessDecision {
                allowed: true,
                reason: Some("Admin bypass".to_string()),
            };
        }
        ...
        match action {
            "read" | "execute_action" | "list" | "history" | "transitions" => AccessDecision {
                allowed: true,
                reason: Some("Default read/action allowed".to_string()),
            },
            "create" | "update" | "delete" => AccessDecision {
                allowed: false,
                reason: Some("No write access".to_string()),
            },
            _ => AccessDecision {
                allowed: false,
                ...
                match user_field {
                    "organization_id" => {
                        if let (Some(rec_org), Some(ctx_org)) =
                            (record_value.and_then(|v| v.as_str()), ctx.organization_id)
                        {
                    ...
                    _ => {
                        tracing::warn!(
                            "Unknown user field reference in security condition: '{}'",
                            user_field
                        );
                        return false;
                    }
    ...
    pub fn check_field_access(
        ...
        // Get field security
        let field_sec = self.field_security.get(entity).and_then(|m| m.get(field));

        if let Some(sec) = field_sec {
            FieldCheck {
                field: field.to_string(),
                can_read: sec.read_roles.is_empty()
                    || sec.read_roles.iter().any(|r| ctx.roles.contains(r)),
