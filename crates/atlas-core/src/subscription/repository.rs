...
pub trait SubscriptionRepository: Send + Sync {
    ...
    async fn list_price_tiers(
        ...
        org_id: Uuid,
        product_id: Uuid,
    ) -> AtlasResult<Vec<SubscriptionPriceTier>>;
    ...
    async fn create_subscription(
        ...
        billing_frequency: &str,
        billing_day_of_month: i32,
        billing_alignment: &str,
        currency_code: &str,
        quantity: &str,
        unit_price: &str,
        list_price: &str,
    ...
    async fn create_amendment(
        ...
        amendment_type: &str,
        description: Option<&str>,
        old_quantity: Option<&str>,
        new_quantity: Option<&str>,
        old_unit_price: Option<&str>,
        new_unit_price: Option<&str>,
        old_recurring_amount: Option<&str>,
        new_recurring_amount: Option<&str>,
        old_end_date: Option<&chrono::NaiveDate>,
        new_end_date: Option<&chrono::NaiveDate>,
        effective_date: chrono::NaiveDate,
        proration_credit: &str,
        proration_charge: &str,
        status: &str,
        created_by: Option<Uuid>,
    ...
    async fn create_revenue_line(
        ...
        org_id: Uuid,
        subscription_id: Uuid,
        billing_schedule_id: Option<Uuid>,
        period_name: &str,
        period_start: chrono::NaiveDate,
        period_end: chrono::NaiveDate,
        revenue_amount: &str,
        deferred_amount: &str,
        recognized_to_date: &str,
        status: &str,
...
impl SubscriptionRepository for PostgresSubscriptionRepository {
    async fn create_product(
        ...
        .bind(default_duration_months)
        .bind(is_auto_renew)
        .bind(cancellation_notice_days)
        .bind(setup_fee)
        .bind(tier_type)
        .bind(created_by)
        .fetch_one(&self.pool)
    ...
    async fn delete_product(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        sqlx::query(
            "DELETE FROM _atlas.subscription_products WHERE organization_id=$1 AND product_code=$2",
        )
        .bind(org_id)
        .bind(code)
        .execute(&self.pool)
    ...
    async fn create_subscription(
        ...
        .bind(renewal_date)
        .bind(billing_frequency)
        .bind(billing_day_of_month)
        .bind(billing_alignment)
        .bind(currency_code)
        .bind(quantity)
        .bind(unit_price)
        .bind(list_price)
        ...
        .bind(setup_fee)
        .bind(recurring_amount)
        .bind(total_contract_value)
        .bind(total_billed)
        .bind(total_revenue_recognized)
    ...
    async fn create_amendment(
        ...
        .bind(old_unit_price)
        .bind(new_unit_price)
        .bind(old_recurring_amount)
        .bind(new_recurring_amount)
        .bind(old_end_date)
        .bind(new_end_date)
        .bind(effective_date)
        .bind(proration_credit)
        .bind(proration_charge)
        .bind(status)
        .bind(created_by)
        .fetch_one(&self.pool)
        .await
