...
pub trait SuccessionPlanningRepository: Send + Sync {
    ...
    async fn get_succession_plan_by_code(
        &self,
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<SuccessionPlan>>;
    ...
    async fn update_succession_plan_status(
        &self,
        id: Uuid,
        status: &str,
    ) -> AtlasResult<SuccessionPlan>;
    ...
    async fn list_succession_candidates(
        &self,
        plan_id: Uuid,
    ...
    async fn update_candidate_readiness(
        &self,
        id: Uuid,
        readiness: &str,
    ) -> AtlasResult<SuccessionCandidate>;
    ...
    async fn get_talent_pool_by_code(
        &self,
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<TalentPool>>;
    ...
    async fn update_pool_member_status(
        &self,
        id: Uuid,
        status: &str,
    ) -> AtlasResult<TalentPoolMember>;
    ...
    async fn get_talent_review_by_code(
        &self,
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<TalentReview>>;
    ...
    async fn update_talent_review_status(
        &self,
        id: Uuid,
        status: &str,
    ) -> AtlasResult<TalentReview>;
    ...
    async fn get_talent_review_assessment(
        &self,
        id: Uuid,
    ) -> AtlasResult<Option<TalentReviewAssessment>>;
    async fn list_talent_review_assessments(
        &self,
        review_id: Uuid,
    ...
    async fn get_career_path_by_code(
        &self,
        org_id: Uuid,
        code: &str,
    ) -> AtlasResult<Option<CareerPath>>;
...
fn row_to_candidate(row: &sqlx::postgres::PgRow) -> SuccessionCandidate {
    SuccessionCandidate {
        ...
        readiness: row
            .try_get("readiness")
            .unwrap_or_else(|_| "not_ready".to_string()),
...
fn row_to_pool(row: &sqlx::postgres::PgRow) -> TalentPool {
    TalentPool {
        ...
        pool_type: row
            .try_get("pool_type")
            .unwrap_or_else(|_| "high_potential".to_string()),
...
fn row_to_review(row: &sqlx::postgres::PgRow) -> TalentReview {
    TalentReview {
        ...
        review_type: row
            .try_get("review_type")
            .unwrap_or_else(|_| "nine_box".to_string()),
...
impl SuccessionPlanningRepository for PostgresSuccessionPlanningRepository {
    ...
    async fn create_succession_plan(
        ...
        .bind(job_id)
        .bind(department_id)
        .bind(current_incumbent_id)
        .bind(current_incumbent_name)
        .bind(risk_level)
        .bind(urgency)
        .bind(effective_date)
        .bind(created_by)
    ...
    async fn list_succession_plans(
        ...
        let mut query =
            String::from("SELECT * FROM _atlas.succession_plans WHERE organization_id = $1");
        ...
        if has_risk {
            query.push_str(&format!(" AND risk_level = ${bind_idx}"));
        }
    ...
    async fn delete_succession_plan(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        ...
        if result.rows_affected() == 0 {
            return Err(AtlasError::EntityNotFound(format!(
                "Succession plan '{code}' not found"
            )));
        }
        Ok(())
    }
    ...
    async fn create_succession_candidate(
        ...
        .bind(potential_rating)
        .bind(flight_risk)
        .bind(development_notes)
    ...
    async fn list_talent_pools(
        ...
        let mut query =
            String::from("SELECT * FROM _atlas.talent_pools WHERE organization_id = $1");
        let mut bind_idx = 2u32;
        let has_status = status.is_some();
        if has_status {
            query.push_str(&format!(" AND status = ${bind_idx}"));
        ...
        if has_type {
            query.push_str(&format!(" AND pool_type = ${bind_idx}"));
        }
    ...
    async fn delete_talent_pool(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        let result =
            sqlx::query("DELETE FROM _atlas.talent_pools WHERE organization_id = $1 AND code = $2")
                .bind(org_id)
                .bind(code)
                .execute(&self.pool)
                .await?;
    ...
    async fn delete_talent_pool_member(&self, id: Uuid) -> AtlasResult<()> {
        ...
        if result.rows_affected() == 0 {
            return Err(AtlasError::EntityNotFound(
                "Pool member not found".to_string(),
            ));
        }
        Ok(())
    }
    ...
    async fn list_talent_reviews(
        ...
        let mut query =
            String::from("SELECT * FROM _atlas.talent_reviews WHERE organization_id = $1");
        let mut bind_idx = 2u32;
        let has_status = status.is_some();
        if has_status {
            query.push_str(&format!(" AND status = ${bind_idx}"));
        ...
        if has_type {
            query.push_str(&format!(" AND review_type = ${bind_idx}"));
        }
    ...
    async fn create_talent_review_assessment(
        ...
        .bind(performance_rating)
        .bind(potential_rating)
        .bind(nine_box_position)
        .bind(strengths)
        .bind(weaknesses)
        .bind(career_aspiration)
        .bind(development_needs)
        .bind(succession_readiness)
        .bind(assessor_id)
        .bind(notes)
        .fetch_one(&self.pool)
        .await?;
    ...
    async fn create_career_path(
        ...
        .bind(to_job_id)
        .bind(to_job_title)
        .bind(typical_duration_months)
        .bind(required_competencies)
        .bind(required_certifications)
        .bind(development_activities)
        .bind(created_by)
        .fetch_one(&self.pool)
        .await?;
    ...
    async fn list_career_paths(
        ...
        let mut query =
            String::from("SELECT * FROM _atlas.career_paths WHERE organization_id = $1");
        let mut bind_idx = 2u32;
        let has_status = status.is_some();
        if has_status {
            query.push_str(&format!(" AND status = ${bind_idx}"));
        ...
        if has_type {
            query.push_str(&format!(" AND path_type = ${bind_idx}"));
        }
        query.push_str(" ORDER BY name");
    ...
    async fn delete_career_path(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        let result =
            sqlx::query("DELETE FROM _atlas.career_paths WHERE organization_id = $1 AND code = $2")
                .bind(org_id)
                .bind(code)
                .execute(&self.pool)
                .await?;
    ...
    async fn get_succession_dashboard(&self, org_id: Uuid) -> AtlasResult<SuccessionDashboard> {
        ...
        let coverage_pct = if active_plans > 0 {
            Some(format!(
                "{:.1}",
                (covered as f64 / f64::from(active_plans)) * 100.0
            ))
        } else {
            None
        };
