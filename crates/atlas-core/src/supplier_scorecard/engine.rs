...
const VALID_SCORECARD_STATUSES: &[&str] =
...
impl SupplierScorecardEngine {
    ...
    pub async fn get_template(
        &self,
        id: Uuid,
    ) -> atlas_shared::AtlasResult<Option<atlas_shared::ScorecardTemplate>> {
    ...
    pub async fn list_templates(
        &self,
        org_id: Uuid,
    ) -> atlas_shared::AtlasResult<Vec<atlas_shared::ScorecardTemplate>> {
    ...
    pub async fn create_category(
        ...
        let weight_val: f64 = weight
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Weight must be a number".to_string()))?;
        ...
        info!(
            "Creating scorecard category '{}' for template {}",
            code, template_id
        );
        self.repository
            .create_category(
    ...
    pub async fn list_categories(
        &self,
        template_id: Uuid,
    ) -> atlas_shared::AtlasResult<Vec<atlas_shared::ScorecardCategory>> {
    ...
    pub async fn create_scorecard(
        ...
        if scorecard_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Scorecard number is required".to_string(),
            ));
        }
        if evaluation_period_start >= evaluation_period_end {
            return Err(AtlasError::ValidationFailed(
                "Evaluation period start must be before end".to_string(),
            ));
        }
        if self
            .repository
            .get_scorecard_by_number(org_id, scorecard_number)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Scorecard '{scorecard_number}' already exists"
            )));
        }
        // Verify template exists
        self.repository
        ...
        info!(
            "Creating scorecard '{}' for supplier {} in org {}",
            scorecard_number, supplier_id, org_id
        );
        self.repository
            .create_scorecard(
                org_id,
    ...
    pub async fn get_scorecard(
        &self,
        id: Uuid,
    ) -> atlas_shared::AtlasResult<Option<atlas_shared::SupplierScorecard>> {
        ...
        if let Some(s) = status {
            if !VALID_SCORECARD_STATUSES.contains(&s) {
                return Err(AtlasError::ValidationFailed(format!(
                    ...
                    s,
                    VALID_SCORECARD_STATUSES.join(", ")
                )));
            }
        }
        self.repository
            .list_scorecards(org_id, supplier_id, status)
            .await
    }
    ...
    pub async fn submit_scorecard(
        ...
        if sc.status != "draft" && sc.status != "rejected" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot submit scorecard in '{}' status. Must be 'draft' or 'rejected'.",
                sc.status
            )));
        }
        ...
        info!(
            "Submitting scorecard {} with score {} ({})",
            sc.scorecard_number, overall, grade
        );
        self.repository
            .submit_scorecard(
                id,
                &format!("{overall:.2}"),
                if lines.is_empty() { "N/A" } else { &grade },
                reviewer_id,
                reviewer_name,
            )
            .await
    ...
    pub async fn approve_scorecard(
        ...
        if sc.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot approve scorecard in '{}' status. Must be 'submitted'.",
                sc.status
            )));
        }
    ...
    pub async fn reject_scorecard(
        ...
        if sc.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot reject scorecard in '{}' status. Must be 'submitted'.",
                sc.status
            )));
        }
        info!("Rejecting scorecard {}", sc.scorecard_number);
        self.repository
            .update_scorecard_status(id, "rejected")
            .await
    }
    ...
    pub async fn add_scorecard_line(
        ...
        let score_val: f64 = score
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Score must be a number".to_string()))?;
        ...
        let sc = self
            .repository
            .get_scorecard(scorecard_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Scorecard {scorecard_id} not found"))
            })?;
        if sc.status != "draft" {
        ...
        info!(
            "Adding KPI line '{}' to scorecard {}",
            kpi_name, sc.scorecard_number
        );
        self.repository
            .add_scorecard_line(
                org_id,
    ...
    pub async fn list_scorecard_lines(
        &self,
        scorecard_id: Uuid,
    ) -> atlas_shared::AtlasResult<Vec<atlas_shared::ScorecardLine>> {
    ...
    pub async fn create_review(
        ...
        if review_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Review number is required".to_string(),
            ));
        }
        ...
        if self
            .repository
            .get_review_by_number(org_id, review_number)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Review '{review_number}' already exists"
            )));
        }
        info!(
            "Creating performance review '{}' for supplier {}",
            review_number, supplier_id
        );
        self.repository
            .create_review(
                org_id,
    ...
    pub async fn get_review(
        &self,
        id: Uuid,
    ) -> atlas_shared::AtlasResult<Option<atlas_shared::SupplierPerformanceReview>> {
    ...
    pub async fn list_reviews(
        ...
        self.repository
            .list_reviews(org_id, supplier_id, status)
            .await
    }
    ...
    pub async fn update_review_status(
        ...
        info!(
            "Updating review {} status to {}",
            review.review_number, status
        );
    ...
    pub async fn complete_review(
        ...
        if review.status != "in_progress" && review.status != "scheduled" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot complete review in '{}' status.",
                review.status
            )));
        }
    ...
    pub async fn create_action_item(
        ...
        if description.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Action item description is required".to_string(),
            ));
        }
    ...
    pub async fn list_action_items(
        &self,
        review_id: Uuid,
    ) -> atlas_shared::AtlasResult<Vec<atlas_shared::ReviewActionItem>> {
    ...
    pub async fn complete_action_item(
        ...
        if item.status != "open" && item.status != "in_progress" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot complete action item in '{}' status.",
                item.status
            )));
        }
    ...
    pub async fn get_dashboard(
        &self,
        org_id: Uuid,
    ) -> atlas_shared::AtlasResult<atlas_shared::SupplierScorecardDashboard> {
