...
use atlas_shared::{
    AtlasResult, ReviewActionItem, ScorecardCategory, ScorecardLine, ScorecardTemplate,
    SupplierPerformanceReview, SupplierScorecard, SupplierScorecardDashboard,
};
use sqlx::{PgPool, Row};
use uuid::Uuid;
...
pub trait ScorecardRepository: Send + Sync {
    ...
    async fn create_scorecard(
        ...
        notes: Option<&str>,
        created_by: Option<Uuid>,
    ) -> AtlasResult<SupplierScorecard>;
    async fn get_scorecard(&self, id: Uuid) -> AtlasResult<Option<SupplierScorecard>>;
    async fn get_scorecard_by_number(
        &self,
        org_id: Uuid,
        scorecard_number: &str,
    ) -> AtlasResult<Option<SupplierScorecard>>;
    async fn list_scorecards(
        &self,
        org_id: Uuid,
        supplier_id: Option<Uuid>,
        status: Option<&str>,
    ) -> AtlasResult<Vec<SupplierScorecard>>;
    async fn update_scorecard_status(
        &self,
        id: Uuid,
        status: &str,
    ) -> AtlasResult<SupplierScorecard>;
    ...
    async fn create_review(
        ...
        period_end: chrono::NaiveDate,
        created_by: Option<Uuid>,
    ) -> AtlasResult<SupplierPerformanceReview>;
    async fn get_review(&self, id: Uuid) -> AtlasResult<Option<SupplierPerformanceReview>>;
    async fn get_review_by_number(
        &self,
        org_id: Uuid,
        review_number: &str,
    ) -> AtlasResult<Option<SupplierPerformanceReview>>;
    async fn list_reviews(
        &self,
        org_id: Uuid,
        supplier_id: Option<Uuid>,
        status: Option<&str>,
    ) -> AtlasResult<Vec<SupplierPerformanceReview>>;
...
#[allow(dead_code)]
pub struct PostgresScorecardRepository {
    #[allow(dead_code)]
    pool: PgPool,
}
...
fn row_to_scorecard(row: &sqlx::postgres::PgRow) -> SupplierScorecard {
    SupplierScorecard {
        ...
        organization_id: row.get("organization_id"),
        template_id: row.get("template_id"),
        scorecard_number: row.get("scorecard_number"),
        ...
        overall_score: get_num(row, "overall_score"),
...
fn row_to_line(row: &sqlx::postgres::PgRow) -> ScorecardLine {
    ScorecardLine {
        ...
        kpi_description: row.get("kpi_description"),
        weight: get_num(row, "weight"),
        target_value: row
            .get::<Option<f64>, _>("target_value")
            .map(|v| format!("{v:.2}")),
        actual_value: row
            .get::<Option<f64>, _>("actual_value")
            .map(|v| format!("{v:.2}")),
...
fn row_to_review(row: &sqlx::postgres::PgRow) -> SupplierPerformanceReview {
    SupplierPerformanceReview {
        ...
        previous_score: row
            .get::<Option<f64>, _>("previous_score")
            .map(|v| format!("{v:.2}")),
        current_score: row
            .get::<Option<f64>, _>("current_score")
            .map(|v| format!("{v:.2}")),
        score_change: row
            .get::<Option<f64>, _>("score_change")
            .map(|v| format!("{v:.2}")),
...
macro_rules! db_err {
    ($e:expr) => {
        atlas_shared::AtlasError::DatabaseError($e.to_string())
    };
}
...
impl ScorecardRepository for PostgresScorecardRepository {
    ...
    async fn get_template(&self, id: Uuid) -> AtlasResult<Option<ScorecardTemplate>> {
        let row = sqlx::query(
            "SELECT * FROM _atlas.scorecard_templates WHERE id = $1 AND is_active = true",
        )
        .bind(id)
    ...
    async fn delete_template(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        sqlx::query(
            "DELETE FROM _atlas.scorecard_templates WHERE organization_id = $1 AND code = $2",
        )
        .bind(org_id)
        .bind(code)
        .execute(&self.pool)
    ...
    async fn list_categories(&self, template_id: Uuid) -> AtlasResult<Vec<ScorecardCategory>> {
        let rows = sqlx::query(
            "SELECT * FROM _atlas.scorecard_categories WHERE template_id = $1 ORDER BY sort_order",
        )
        .bind(template_id)
        .fetch_all(&self.pool)
        .await
    ...
    async fn list_scorecard_lines(&self, scorecard_id: Uuid) -> AtlasResult<Vec<ScorecardLine>> {
        let rows = sqlx::query(
            "SELECT * FROM _atlas.scorecard_lines WHERE scorecard_id=$1 ORDER BY line_number",
        )
        .bind(scorecard_id)
        .fetch_all(&self.pool)
        .await
    ...
    async fn list_action_items(&self, review_id: Uuid) -> AtlasResult<Vec<ReviewActionItem>> {
        let rows = sqlx::query(
            "SELECT * FROM _atlas.review_action_items WHERE review_id=$1 ORDER BY action_number",
        )
        .bind(review_id)
        .fetch_all(&self.pool)
        .await
