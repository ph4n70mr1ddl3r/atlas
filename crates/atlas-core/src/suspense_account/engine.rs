...
use super::{
    AtlasError, AtlasResult, SuspenseAccountDefinition, SuspenseAccountRepository,
    SuspenseAgingSnapshot, SuspenseClearingBatch, SuspenseClearingLine, SuspenseDashboard,
    SuspenseEntry,
};
use std::sync::Arc;
use tracing::info;
...
pub struct SuspenseAccountEngine {
    repository: Arc<dyn SuspenseAccountRepository>,
}

impl SuspenseAccountEngine {
    pub fn new(r: Arc<dyn SuspenseAccountRepository>) -> Self {
        Self { repository: r }
    }
    ...
    pub async fn create_definition(
        ...
        if balancing_segment.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Balancing segment is required".into(),
            ));
        }
        if suspense_account.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Suspense account is required".into(),
            ));
        }
        if self
            .repository
            .get_definition_by_code(org_id, code)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Suspense definition '{code}' already exists"
            )));
        }
        info!(
            "Creating suspense account definition {} for org {}",
            code, org_id
        );
        self.repository
            .create_definition(
    ...
    pub async fn list_definitions(
        &self,
        org_id: Uuid,
    ) -> AtlasResult<Vec<SuspenseAccountDefinition>> {
    ...
    pub async fn activate_definition(&self, id: Uuid) -> AtlasResult<SuspenseAccountDefinition> {
        ...
        if def.enabled && def.status == "active" {
            return Err(AtlasError::WorkflowError(
                "Definition is already active".into(),
            ));
        }
        self.repository
            .update_definition_status(id, true, "active")
            .await
    }
    ...
    pub async fn deactivate_definition(&self, id: Uuid) -> AtlasResult<SuspenseAccountDefinition> {
        ...
        if !def.enabled || def.status == "inactive" {
            return Err(AtlasError::WorkflowError(
                "Definition is already inactive".into(),
            ));
        }
        // Check for open entries
        let open_entries = self
            .repository
            .list_entries_by_definition(id, Some("open"))
            .await?;
        if !open_entries.is_empty() {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot deactivate: {} open entries exist",
                open_entries.len()
            )));
        }
        self.repository
            .update_definition_status(id, false, "inactive")
            .await
    }
    ...
    pub async fn delete_definition(&self, id: Uuid) -> AtlasResult<()> {
        ...
        if def.status == "active" {
            return Err(AtlasError::WorkflowError(
                "Cannot delete an active definition. Deactivate first.".into(),
            ));
        }
    ...
    pub async fn create_entry(
        ...
        let def = self
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Definition {definition_id} not found"))
            })?;
        if !def.enabled || def.status != "active" {
        ...
        let amt: f64 = suspense_amount
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid suspense amount".into()))?;
        if amt == 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Suspense amount cannot be zero".into(),
            ));
        }
        ...
        info!(
            "Creating suspense entry for definition {} amount {}",
            def.code, suspense_amount
        );
        self.repository
            .create_entry(
                org_id,
    ...
    pub async fn list_entries_by_definition(
        ...
        self.repository
            .list_entries_by_definition(definition_id, status)
            .await
    }
    ...
    pub async fn write_off_entry(
        ...
        if entry.status != "open" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot write off entry in '{}' status",
                entry.status
            )));
        }
        if resolution_notes.is_none() || resolution_notes.unwrap().is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Resolution notes are required for write-off".into(),
            ));
        }
        self.repository
            .update_entry_status(
    ...
    pub async fn create_clearing_batch(
        ...
        if batch_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Batch number is required".into(),
            ));
        }
        if self
            .repository
            .get_clearing_batch_by_number(org_id, batch_number)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
                "Clearing batch '{batch_number}' already exists"
            )));
        }
        info!(
            "Creating suspense clearing batch {} for org {}",
            batch_number, org_id
        );
        self.repository
            .create_clearing_batch(org_id, batch_number, description, clearing_date, created_by)
            .await
    }
    ...
    pub async fn add_clearing_line(
        ...
        if entry.status != "open" {
            return Err(AtlasError::WorkflowError(format!(
                "Entry {entry_id} is not open"
            )));
        }
        if clearing_account.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Clearing account is required".into(),
            ));
        }
        let amt: f64 = cleared_amount
            .parse()
            .map_err(|_| AtlasError::ValidationFailed("Invalid cleared amount".into()))?;
        if amt <= 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Cleared amount must be positive".into(),
            ));
        }
        let entry_amt: f64 = entry.suspense_amount.parse().unwrap_or(0.0);
        if amt > entry_amt.abs() {
            return Err(AtlasError::ValidationFailed(
                "Cleared amount exceeds suspense amount".into(),
            ));
        }
        self.repository
            .create_clearing_line(
                org_id,
                batch_id,
                entry_id,
                clearing_account,
    ...
    pub async fn list_clearing_lines(
        &self,
        batch_id: Uuid,
    ) -> AtlasResult<Vec<SuspenseClearingLine>> {
    ...
    pub async fn submit_clearing_batch(&self, id: Uuid) -> AtlasResult<SuspenseClearingBatch> {
        let batch = self
            ...
            .await?
            .ok_or_else(|| AtlasError::EntityNotFound(format!("Batch {id} not found")))?;
        if batch.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot submit batch in '{}' status",
                batch.status
            )));
        }
        ...
        if lines.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Cannot submit an empty clearing batch".into(),
            ));
        }
        let total: f64 = lines
            .iter()
            .map(|l| l.cleared_amount.parse::<f64>().unwrap_or(0.0))
            .sum();
        self.repository
            .update_clearing_batch(id, "submitted", lines.len() as i32, &total.to_string())
            .await
    }
    ...
    pub async fn approve_clearing_batch(&self, id: Uuid) -> AtlasResult<SuspenseClearingBatch> {
        ...
        if batch.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot approve batch in '{}' status",
                batch.status
            )));
        }
        ...
        for line in &lines {
            let _ = self
                .repository
                .update_entry_status(
                    line.suspense_entry_id,
                    "cleared",
                    None,
                    Some(clearing_date),
                    line.resolution_notes.as_deref(),
                )
                .await;
        }
        self.repository
            .update_clearing_batch(
                id,
                "approved",
                batch.total_entries,
                &batch.total_cleared_amount,
            )
            .await
    }
    ...
    pub async fn create_aging_snapshot(
        ...
        info!(
            "Creating suspense aging snapshot for org {} on {}",
            org_id, snapshot_date
        );
        self.repository
            .create_aging_snapshot(org_id, snapshot_date)
            .await
    }
...
mod tests {
    ...
    impl SuspenseAccountRepository for MockRepo {
        async fn create_definition(
            ...
            let d = SuspenseAccountDefinition {
                ...
                balancing_segment: seg.into(),
                suspense_account: acct.into(),
                enabled: true,
                status: "active".into(),
        ...
        async fn get_definition_by_code(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|d| d.organization_id == org_id && d.code == code)
                .cloned())
        }
        ...
        async fn create_entry(
            ...
            let e = SuspenseEntry {
                id: Uuid::new_v4(),
                organization_id: org_id,
                suspense_definition_id: def_id,
                journal_entry_id: je_id,
                journal_batch_id: jb_id,
                balancing_segment_value: bsv.into(),
                suspense_account: sa.into(),
                suspense_amount: amt.into(),
                original_amount: oa.map(Into::into),
                entry_type: et.into(),
                entry_date: ed,
                currency_code: cc.into(),
                status: "open".into(),
                cleared_by_journal_id: None,
                clearing_date: None,
        ...
        async fn list_entries(
            ...
            Ok(self
                ...
                .iter()
                .filter(|e| {
                    e.organization_id == org_id && (status.is_none() || e.status == status.unwrap())
                })
                .cloned()
                .collect())
        }
        async fn list_entries_by_definition(
            ...
            Ok(self
                ...
                .iter()
                .filter(|e| {
                    e.suspense_definition_id == def_id
        ...
        async fn update_entry_status(
            ...
            e.cleared_by_journal_id = cjid;
            e.clearing_date = cd;
            e.resolution_notes = rn.map(Into::into);
            Ok(e.clone())
        }
        ...
        async fn get_clearing_batch_by_number(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .find(|b| b.organization_id == org_id && b.batch_number == bn)
                .cloned())
        }
        async fn list_clearing_batches(
            ...
            Ok(self
                ...
                .iter()
                .filter(|b| {
                    b.organization_id == org_id && (status.is_none() || b.status == status.unwrap())
                })
                .cloned()
                .collect())
        }
        async fn update_clearing_batch(
            ...
            b.total_entries = te;
            b.total_cleared_amount = tca.into();
            Ok(b.clone())
        }
        async fn create_clearing_line(
            ...
            let l = SuspenseClearingLine {
                ...
                suspense_entry_id: entry_id,
                clearing_account: ca.into(),
                cleared_amount: amt.into(),
                status: "pending".into(),
                resolution_notes: rn.map(Into::into),
                metadata: serde_json::json!({}),
                created_at: chrono::Utc::now(),
                updated_at: chrono::Utc::now(),
            };
        ...
        async fn list_clearing_lines(
            ...
            Ok(self
                ...
                .unwrap()
                .iter()
                .filter(|l| l.clearing_batch_id == batch_id)
                .cloned()
                .collect())
        }
    ...
    fn eng() -> SuspenseAccountEngine {
        SuspenseAccountEngine::new(Arc::new(MockRepo::new()))
    }
    ...
    async fn test_create_definition_empty_code() {
        let r = eng()
            .create_definition(Uuid::new_v4(), "", "Name", None, "seg", "acct", None)
            .await;
        assert!(r.is_err());
    }
    ...
    async fn test_create_definition_empty_name() {
        let r = eng()
            .create_definition(Uuid::new_v4(), "CODE", "", None, "seg", "acct", None)
            .await;
        assert!(r.is_err());
    }
    ...
    async fn test_create_definition_empty_segment() {
        let r = eng()
            .create_definition(Uuid::new_v4(), "CODE", "Name", None, "", "acct", None)
            .await;
        assert!(r.is_err());
    }
    ...
    async fn test_create_definition_empty_account() {
        let r = eng()
            .create_definition(Uuid::new_v4(), "CODE", "Name", None, "seg", "", None)
            .await;
        assert!(r.is_err());
    }
    ...
    async fn test_create_definition_duplicate() {
        ...
        let e = eng();
        let _ = e
            .create_definition(org, "DUP", "Name", None, "seg", "acct", None)
            .await;
        let r = e
            .create_definition(org, "DUP", "Name", None, "seg", "acct", None)
            .await;
    ...
    async fn test_deactivate_definition() {
        let e = eng();
        let d = e
            .create_definition(Uuid::new_v4(), "DEACT", "Name", None, "seg", "acct", None)
            .await
            .unwrap();
    ...
    async fn test_deactivate_already_inactive() {
        let e = eng();
        let d = e
            .create_definition(Uuid::new_v4(), "DEACT2", "Name", None, "seg", "acct", None)
            .await
            .unwrap();
    ...
    async fn test_activate_definition() {
        let e = eng();
        let d = e
            .create_definition(Uuid::new_v4(), "ACT", "Name", None, "seg", "acct", None)
            .await
            .unwrap();
        let _ = e.deactivate_definition(d.id).await.unwrap();
    ...
    async fn test_activate_already_active() {
        let e = eng();
        let d = e
            .create_definition(Uuid::new_v4(), "ACT2", "Name", None, "seg", "acct", None)
            .await
            .unwrap();
    ...
    async fn test_delete_active_definition() {
        let e = eng();
        let d = e
            .create_definition(Uuid::new_v4(), "DEL1", "Name", None, "seg", "acct", None)
            .await
            .unwrap();
    ...
    async fn test_delete_inactive_definition() {
        let e = eng();
        let d = e
            .create_definition(Uuid::new_v4(), "DEL2", "Name", None, "seg", "acct", None)
            .await
            .unwrap();
        let _ = e.deactivate_definition(d.id).await.unwrap();
    ...
    async fn test_list_definitions() {
        ...
        let e = eng();
        let _ = e
            .create_definition(org, "L1", "Name 1", None, "seg", "acct", None)
            .await;
        let _ = e
            .create_definition(org, "L2", "Name 2", None, "seg", "acct", None)
            .await;
    ...
    async fn test_create_entry_auto() {
        ...
        let org = Uuid::new_v4();
        let d = e
            .create_definition(org, "E1", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let entry = e
            .create_entry(
    ...
    async fn test_create_entry_manual() {
        ...
        let org = Uuid::new_v4();
        let d = e
            .create_definition(org, "E2", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let entry = e
            .create_entry(
    ...
    async fn test_create_entry_invalid_type() {
        ...
        let org = Uuid::new_v4();
        let d = e
            .create_definition(org, "E3", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let r = e
            .create_entry(
    ...
    async fn test_create_entry_zero_amount() {
        ...
        let org = Uuid::new_v4();
        let d = e
            .create_definition(org, "E4", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let r = e
            .create_entry(
    ...
    async fn test_create_entry_empty_segment() {
        ...
        let org = Uuid::new_v4();
        let d = e
            .create_definition(org, "E5", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let r = e
            .create_entry(
    ...
    async fn test_create_entry_bad_currency() {
        ...
        let org = Uuid::new_v4();
        let d = e
            .create_definition(org, "E6", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let r = e
            .create_entry(
    ...
    async fn test_create_entry_inactive_definition() {
        ...
        let org = Uuid::new_v4();
        let d = e
            .create_definition(org, "E7", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let _ = e.deactivate_definition(d.id).await.unwrap();
        let r = e
    ...
    async fn test_reverse_entry() {
        ...
        let org = Uuid::new_v4();
        let d = e
            .create_definition(org, "R1", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let entry = e
            .create_entry(
            ...
            .unwrap();
        let entry = e
            .reverse_entry(entry.id, Some("Reversed in error"))
            .await
            .unwrap();
    ...
    async fn test_reverse_non_open_entry() {
        ...
        let org = Uuid::new_v4();
        let d = e
            .create_definition(org, "R2", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let entry = e
            .create_entry(
    ...
    async fn test_write_off_entry() {
        ...
        let org = Uuid::new_v4();
        let d = e
            .create_definition(org, "W1", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let entry = e
            .create_entry(
            ...
            .unwrap();
        let entry = e
            .write_off_entry(entry.id, Some("Immaterial amount"))
            .await
            .unwrap();
    ...
    async fn test_write_off_without_notes() {
        ...
        let org = Uuid::new_v4();
        let d = e
            .create_definition(org, "W2", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let entry = e
            .create_entry(
    ...
    async fn test_list_entries_invalid_status() {
        assert!(eng()
            .list_entries(Uuid::new_v4(), Some("bad"))
            .await
            .is_err());
    }
    ...
    async fn test_submit_clearing_batch() {
        ...
        let org = Uuid::new_v4();
        let def = e
            .create_definition(org, "S1", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let entry = e
            .create_entry(
        ...
        let batch = e
            ...
            .unwrap();
        let _ = e
            .add_clearing_line(org, batch.id, entry.id, "4100-000", "500", Some("Cleared"))
            .await
            .unwrap();
    ...
    async fn test_approve_clearing_batch() {
        ...
        let org = Uuid::new_v4();
        let def = e
            .create_definition(org, "A1", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let entry = e
            .create_entry(
        ...
        let batch = e
            ...
            .unwrap();
        let _ = e
            .add_clearing_line(org, batch.id, entry.id, "4100-000", "500", None)
            .await
            .unwrap();
    ...
    async fn test_post_clearing_batch() {
        ...
        let org = Uuid::new_v4();
        let def = e
            .create_definition(org, "P1", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let entry = e
            .create_entry(
    ...
    async fn test_add_clearing_line_exceeds_amount() {
        ...
        let org = Uuid::new_v4();
        let def = e
            .create_definition(org, "X1", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let entry = e
            .create_entry(
        ...
        let batch = e
            ...
            .unwrap();
        let r = e
            .add_clearing_line(org, batch.id, entry.id, "4100-000", "300", None)
            .await;
        assert!(r.is_err());
    }
    ...
    async fn test_add_clearing_line_zero_amount() {
        ...
        let org = Uuid::new_v4();
        let def = e
            .create_definition(org, "X2", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let entry = e
            .create_entry(
        ...
        let batch = e
            ...
            .unwrap();
        let r = e
            .add_clearing_line(org, batch.id, entry.id, "4100-000", "0", None)
            .await;
        assert!(r.is_err());
    }
    ...
    async fn test_add_clearing_line_to_posted_batch() {
        ...
        let org = Uuid::new_v4();
        let def = e
            .create_definition(org, "X3", "Name", None, "company", "9999-000", None)
            .await
            .unwrap();
        let entry = e
            .create_entry(
        ...
        let batch = e
            ...
            .unwrap();
        let _ = e
            .add_clearing_line(org, batch.id, entry.id, "4100-000", "200", None)
            .await
            .unwrap();
        let _ = e.submit_clearing_batch(batch.id).await.unwrap();
        let _ = e.approve_clearing_batch(batch.id).await.unwrap();
        ...
        let entry2 = e
            ...
            .unwrap();
        assert!(e
            .add_clearing_line(org, batch.id, entry2.id, "4100-000", "100", None)
            .await
            .is_err());
    }
    ...
    async fn test_list_clearing_batches_invalid_status() {
        assert!(eng()
            .list_clearing_batches(Uuid::new_v4(), Some("bad"))
            .await
            .is_err());
    }
}
