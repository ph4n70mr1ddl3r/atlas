...
use atlas_shared::{
    AtlasError, AtlasResult, CarbonOffset, EmissionFactor, EnvironmentalActivity, EsgMetric,
    EsgMetricReading, SustainabilityDashboard, SustainabilityFacility, SustainabilityGoal,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
const VALID_FACILITY_STATUSES: &[&str] = &["active", "inactive", "decommissioned"];

const VALID_EMISSION_SCOPES: &[&str] = &["scope_1", "scope_2", "scope_3"];
...
const VALID_GAS_TYPES: &[&str] = &["co2", "ch4", "n2o", "hfcs", "pfcs", "sf6", "co2e"];
...
const VALID_ACTIVITY_STATUSES: &[&str] = &["draft", "confirmed", "verified", "adjusted"];

const VALID_ESG_PILLARS: &[&str] = &["environmental", "social", "governance"];

const VALID_METRIC_DIRECTIONS: &[&str] = &["lower_is_better", "higher_is_better"];
...
const VALID_GOAL_STATUSES: &[&str] = &["on_track", "at_risk", "off_track", "achieved", "cancelled"];
...
#[allow(dead_code)]
const VALID_OFFSET_STATUSES: &[&str] = &["active", "retired", "expired", "cancelled"];

const VALID_OFFSET_REGISTRIES: &[&str] = &["Verra", "Gold_Standard", "ACR", "CAR", "Other"];
...
impl SustainabilityEngine {
    ...
    pub async fn create_facility(
        ...
        if facility_code.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Facility code is required".to_string(),
            ));
        }
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
        ...
        if self
            .repository
            .get_facility_by_code(org_id, facility_code)
            .await?
            .is_some()
        {
        ...
        info!(
            "Creating sustainability facility '{}' ({}) for org {}",
            facility_code, name, org_id
        );
        self.repository
            .create_facility(
                org_id,
    ...
    pub async fn get_facility_by_code(
        &self,
        org_id: Uuid,
        facility_code: &str,
    ) -> AtlasResult<Option<SustainabilityFacility>> {
        self.repository
            .get_facility_by_code(org_id, facility_code)
            .await
    }
    ...
    pub async fn list_facilities(
        ...
        self.repository
            .list_facilities(org_id, status, facility_type)
            .await
    }
    ...
    pub async fn create_emission_factor(
        ...
        if factor_code.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Factor code is required".to_string(),
            ));
        }
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Factor name is required".to_string(),
            ));
        }
        ...
        if self
            .repository
            .get_emission_factor_by_code(org_id, factor_code)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
        ...
        info!(
            "Creating emission factor '{}' ({}) for org {} [scope={}, category={}]",
    ...
    pub async fn get_emission_factor_by_code(
        &self,
        org_id: Uuid,
        factor_code: &str,
    ) -> AtlasResult<Option<EmissionFactor>> {
        self.repository
            .get_emission_factor_by_code(org_id, factor_code)
            .await
    }
    ...
    pub async fn list_emission_factors(
        ...
        self.repository
            .list_emission_factors(org_id, scope, category, activity_type)
            .await
    }
    ...
    pub async fn delete_emission_factor(&self, org_id: Uuid, factor_code: &str) -> AtlasResult<()> {
        info!(
            "Deleting emission factor '{}' for org {}",
            factor_code, org_id
        );
        self.repository
    ...
    pub async fn create_activity(
        ...
        if activity_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Activity number is required".to_string(),
            ));
        }
        validate_enum("scope", scope, VALID_EMISSION_SCOPES)?;
        ...
        if let Some(ef_id) = emission_factor_id {
            self.repository
                .get_emission_factor(ef_id)
                .await?
                .ok_or_else(|| {
                    AtlasError::EntityNotFound(format!("Emission factor {ef_id} not found"))
                })?;
        }
        ...
        if let Some(f_id) = facility_id {
            self.repository
                .get_facility(f_id)
                .await?
                .ok_or_else(|| AtlasError::EntityNotFound(format!("Facility {f_id} not found")))?;
        }

        if self
            .repository
            .get_activity_by_number(org_id, activity_number)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
        ...
        info!(
            "Creating environmental activity '{}' for org {} [scope={}, type={}, co2e={:.2}kg]",
    ...
    pub async fn get_activity_by_number(
        &self,
        org_id: Uuid,
        activity_number: &str,
    ) -> AtlasResult<Option<EnvironmentalActivity>> {
        self.repository
            .get_activity_by_number(org_id, activity_number)
            .await
    }
    ...
    pub async fn list_activities(
        ...
        self.repository
            .list_activities(org_id, scope, facility_id, activity_type, reporting_period)
            .await
    }
    ...
    pub async fn create_metric(
        ...
        if metric_code.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Metric code is required".to_string(),
            ));
        }
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Metric name is required".to_string(),
            ));
        }
        ...
        if self
            .repository
            .get_metric_by_code(org_id, metric_code)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
        ...
        info!(
            "Creating ESG metric '{}' ({}) for org {} [pillar={}, category={}]",
    ...
    pub async fn get_metric_by_code(
        &self,
        org_id: Uuid,
        metric_code: &str,
    ) -> AtlasResult<Option<EsgMetric>> {
        self.repository
            .get_metric_by_code(org_id, metric_code)
            .await
    }
    ...
    pub async fn create_metric_reading(
        ...
        self.repository
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("ESG metric {metric_id} not found"))
            })?;
        ...
        info!(
            "Recording ESG metric reading for metric {} [value={}, date={}]",
    ...
    pub async fn create_goal(
        ...
        if goal_code.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Goal code is required".to_string(),
            ));
        }
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
        ...
        if baseline_unit.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Baseline unit is required".to_string(),
            ));
        }
        if target_unit.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Target unit is required".to_string(),
            ));
        }
        ...
        if self
            .repository
            .get_goal_by_code(org_id, goal_code)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
        ...
        info!(
            "Creating sustainability goal '{}' ({}) for org {} [type={}, baseline={}, target={}]",
    ...
    /// Get a goal by code
    pub async fn get_goal_by_code(
        &self,
        org_id: Uuid,
        goal_code: &str,
    ) -> AtlasResult<Option<SustainabilityGoal>> {
    ...
    pub async fn create_carbon_offset(
        ...
        if offset_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Offset number is required".to_string(),
            ));
        }
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Offset name is required".to_string(),
            ));
        }
        ...
        if self
            .repository
            .get_offset_by_number(org_id, offset_number)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
        ...
        info!(
            "Creating carbon offset '{}' ({}) for org {} [type={}, qty={:.2}t]",
        ...
        self.repository
            .create_carbon_offset(
                ...
                quantity_tonnes,
                quantity_tonnes, // remaining = quantity initially
                unit_price,
                total_cost,
                currency_code,
                vintage_year,
    ...
    pub async fn get_offset_by_number(
        &self,
        org_id: Uuid,
        offset_number: &str,
    ) -> AtlasResult<Option<CarbonOffset>> {
        self.repository
            .get_offset_by_number(org_id, offset_number)
            .await
    }
    ...
    pub async fn list_carbon_offsets(
        ...
        self.repository
            .list_carbon_offsets(org_id, status, project_type)
            .await
    }
    ...
    pub async fn retire_carbon_offset(
        ...
        info!(
            "Retiring {:.2} tonnes from carbon offset {}",
            retire_quantity, id
        );
        self.repository
    ...
    pub async fn delete_carbon_offset(&self, org_id: Uuid, offset_number: &str) -> AtlasResult<()> {
        info!(
            "Deleting carbon offset '{}' for org {}",
            offset_number, org_id
        );
        self.repository
...
mod tests {
    ...
    async fn test_update_facility_status_bad_status() {
        let engine = create_engine();
        let result = engine
            .update_facility_status(Uuid::new_v4(), "unknown")
            .await;
        assert!(result.is_err());
    }
    ...
    async fn test_update_activity_status_bad_status() {
        let engine = create_engine();
        let result = engine
            .update_activity_status(Uuid::new_v4(), "deleted")
            .await;
        assert!(result.is_err());
    }
    ...
    async fn test_create_metric_success() {
        ...
        let result = engine
            .create_metric(
                ...
                Some("Total direct and energy-indirect emissions"),
                "environmental",
                "climate",
                "tonnes CO2e",
                Some("GRI 305-4"),
                Some("EM-EP-110a.1"),
                Some("Metrics and Targets"),
                None,
                Some(5000.0),
                Some(6000.0),
                "lower_is_better",
    ...
    async fn test_create_goal_success() {
        ...
        let result = engine
            .create_goal(
                ...
                "tonnes CO2e",
                Some(50.0),
                Some(serde_json::json!([{"year": 2025, "value": 7500}])),
                None,
                Some(test_user_id()),
                Some("Sustainability Director"),
                Some("SBTi"),
                Some("SBTi-2024-001"),
    ...
    async fn test_create_offset_success() {
        ...
        let result = engine
            .create_carbon_offset(
                ...
                Some("GreenCarbon Inc."),
                None,
                Some("Purchased for 2024 offset"),
                Some(test_user_id()),
            )
            .await;
        assert!(result.is_ok());
