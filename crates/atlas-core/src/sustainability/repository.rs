...
fn row_to_facility(row: &sqlx::postgres::PgRow) -> SustainabilityFacility {
    SustainabilityFacility {
        ...
        total_area_sqm: row.try_get("total_area_sqm").unwrap_or_default(),
...
fn row_to_emission_factor(row: &sqlx::postgres::PgRow) -> EmissionFactor {
    EmissionFactor {
        ...
        factor_value: row.try_get("factor_value").unwrap_or(0.0),
...
fn row_to_activity(row: &sqlx::postgres::PgRow) -> EnvironmentalActivity {
    EnvironmentalActivity {
        ...
        co2e_kg: row.try_get("co2e_kg").unwrap_or(0.0),
        co2_kg: row.try_get("co2_kg").unwrap_or_default(),
        ch4_kg: row.try_get("ch4_kg").unwrap_or_default(),
        n2o_kg: row.try_get("n2o_kg").unwrap_or_default(),
        cost_amount: row.try_get("cost_amount").unwrap_or_default(),
...
fn row_to_reading(row: &sqlx::postgres::PgRow) -> EsgMetricReading {
    EsgMetricReading {
        ...
        metric_value: row.try_get("metric_value").unwrap_or(0.0),
...
fn row_to_offset(row: &sqlx::postgres::PgRow) -> CarbonOffset {
    CarbonOffset {
        ...
        quantity_tonnes: row.try_get("quantity_tonnes").unwrap_or(0.0),
        remaining_tonnes: row.try_get("remaining_tonnes").unwrap_or(0.0),
        unit_price: row.try_get("unit_price").unwrap_or_default(),
        total_cost: row.try_get("total_cost").unwrap_or_default(),
        currency_code: row.try_get("currency_code").unwrap_or_default(),
        vintage_year: row.try_get("vintage_year").unwrap_or(2024),
        retired_quantity: row.try_get("retired_quantity").unwrap_or(0.0),
...
impl SustainabilityRepository for PostgresSustainabilityRepository {
    ...
    async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<SustainabilityDashboard> {
        ...
        for row in &act_rows {
            ...
            let co2e: f64 = row.try_get("co2e_kg").unwrap_or(0.0);
        ...
        let total_offsets: f64 = offset_rows.iter()
            .map(|r| r.try_get::<f64, _>("retired_quantity").unwrap_or(0.0))
            .sum();
