...
use atlas_shared::{
    AtlasError, AtlasResult, TaxFilingCalendarEntry, TaxReportingDashboardSummary, TaxReturn,
    TaxReturnLine, TaxReturnTemplate, TaxReturnTemplateLine,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
/// Valid tax types
#[allow(dead_code)]
const VALID_TAX_TYPES: &[&str] = &["vat", "gst", "sales_tax", "corporate_income", "withholding"];

/// Valid filing frequencies
#[allow(dead_code)]
const VALID_FILING_FREQUENCIES: &[&str] = &["monthly", "quarterly", "semi_annual", "annual"];

/// Valid line types
#[allow(dead_code)]
const VALID_LINE_TYPES: &[&str] = &["input", "calculated", "total", "informational"];
...
#[allow(dead_code)]
const VALID_FILING_STATUSES: &[&str] = &["upcoming", "due_soon", "overdue", "filed", "extended"];
...
impl TaxReportingEngine {
    ...
    pub async fn add_template_line(
        ...
        info!(
            "Adding template line {} ({}) to template",
            box_code, box_name
        );
    ...
    pub async fn create_return(
        ...
        info!(
            "Creating tax return {} for period {} to {}",
    ...
    pub async fn update_return_line(
        ...
        let final_amount = if let Some(override_amt) = override_amount {
            let oamt: f64 = override_amt.parse().map_err(|_| {
                AtlasError::ValidationFailed("Override amount must be a valid number".to_string())
            })?;
            oamt
        } else {
            amt
    ...
    pub async fn submit_return(
        ...
        info!(
            "Submitting tax return {} (net due: {:.2})",
            tax_return.return_number, net_due
        );
        self.repository
            .update_return_status(return_id, "submitted", submitted_by, None, None)
            .await
    }
    ...
    pub async fn file_return(
        ...
        if tax_return.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot file return in '{}' status. Must be 'submitted'.",
                tax_return.status
            )));
        }
        ...
        if !VALID_FILING_METHODS.contains(&filing_method) {
            return Err(AtlasError::ValidationFailed(format!(
                ...
                filing_method,
                VALID_FILING_METHODS.join(", ")
            )));
        }

        info!(
            "Filing tax return {} via {}",
            tax_return.return_number, filing_method
        );
        self.repository
            .update_return_filing(
                return_id,
                "filed",
                filing_method,
                filing_reference,
    ...
    pub async fn mark_paid(
        ...
        if tax_return.status != "filed" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot mark paid from '{}' status. Must be 'filed'.",
                tax_return.status
            )));
        }

        let amt: f64 = payment_amount.parse().map_err(|_| {
        ...
        info!(
            "Marking tax return {} as paid ({:.2})",
            tax_return.return_number, amt
        );
        self.repository
            .update_return_payment(return_id, "paid", payment_amount, payment_reference)
            .await
    }
    ...
    pub async fn list_filing_calendar(
        &self,
        org_id: Uuid,
    ) -> AtlasResult<Vec<TaxFilingCalendarEntry>> {
