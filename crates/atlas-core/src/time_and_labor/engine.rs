...
use atlas_shared::{
    AtlasError, AtlasResult, LaborDistribution, OvertimeRule, TimeAndLaborDashboard, TimeCard,
    TimeCardHistory, TimeEntry, WorkSchedule,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
const VALID_SCHEDULE_TYPES: &[&str] = &["fixed", "flexible", "rotating", "shift"];

/// Valid threshold types
const VALID_THRESHOLD_TYPES: &[&str] = &["daily", "weekly", "both"];

/// Valid time card statuses
const VALID_CARD_STATUSES: &[&str] = &["draft", "submitted", "approved", "rejected", "cancelled"];
...
impl TimeAndLaborEngine {
    ...
    pub async fn create_work_schedule(
        ...
        info!(
            "Creating work schedule '{}' ({}) for org {}",
    ...
    pub async fn get_work_schedule(
        ...
        self.repository
            .get_work_schedule(org_id, &code.to_uppercase())
            .await
    }
    ...
    pub async fn delete_work_schedule(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        ...
        self.repository
            .delete_work_schedule(org_id, &code.to_uppercase())
            .await
    }
    ...
    pub async fn create_overtime_rule(
        ...
        if !VALID_THRESHOLD_TYPES.contains(&threshold_type) {
            return Err(AtlasError::ValidationFailed(format!(
                "Invalid threshold_type '{}'. Must be one of: {}",
                threshold_type,
        ...
        info!(
            "Creating overtime rule '{}' ({}) for org {}",
            code_upper, name, org_id
        );

        self.repository
            .create_overtime_rule(
                org_id,
                &code_upper,
                name,
                description,
    ...
    pub async fn get_overtime_rule(
        ...
        self.repository
            .get_overtime_rule(org_id, &code.to_uppercase())
            .await
    }
    ...
    pub async fn delete_overtime_rule(&self, org_id: Uuid, code: &str) -> AtlasResult<()> {
        ...
        self.repository
            .delete_overtime_rule(org_id, &code.to_uppercase())
            .await
    }
    ...
    pub async fn create_time_card(
        ...
        let schedule_id = if let Some(sc) = schedule_code {
            let schedule = self.get_work_schedule(org_id, sc).await?.ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Work schedule '{sc}' not found"))
            })?;
            Some(schedule.id)
        } else {
            None
        ...
        let overtime_rule_id = if let Some(oc) = overtime_rule_code {
            let rule = self.get_overtime_rule(org_id, oc).await?.ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Overtime rule '{oc}' not found"))
            })?;
            Some(rule.id)
        } else {
            None
        ...
        info!(
            "Creating time card {} for employee {} period {} to {}",
        ...
        self.repository
            .add_history(card.id, "create", None, Some("draft"), created_by, None)
            .await?;
    ...
    pub async fn get_time_card_by_number(
        &self,
        org_id: Uuid,
        card_number: &str,
    ) -> AtlasResult<Option<TimeCard>> {
        self.repository
            .get_time_card_by_number(org_id, card_number)
            .await
    }
    ...
    pub async fn list_time_cards(
        ...
        self.repository
            .list_time_cards(org_id, employee_id, status)
            .await
    }
    ...
    pub async fn submit_time_card(
        ...
        let card = self
            .repository
            .get_time_card(card_id)
            .await?
            .ok_or_else(|| AtlasError::EntityNotFound(format!("Time card {card_id} not found")))?;

        if card.organization_id != org_id {
            return Err(AtlasError::EntityNotFound(format!(
                "Time card {card_id} not found"
            )));
        }

        if card.status != "draft" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot submit card in '{}' status. Must be 'draft'.",
                card.status
            )));
        }

        info!(
            "Submitting time card {} by {:?}",
            card.card_number, submitted_by
        );
        let updated = self
            .repository
            .update_time_card_status(card_id, "submitted", None, None)
            .await?;
    ...
    pub async fn approve_time_card(
        ...
        if card.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot approve card in '{}' status. Must be 'submitted'.",
                card.status
            )));
        }

        info!(
            "Approving time card {} by {}",
            card.card_number, approved_by
        );
        let updated = self
            .repository
            .update_time_card_status(card_id, "approved", Some(approved_by), None)
            .await?;
    ...
    pub async fn reject_time_card(
        ...
        if card.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot reject card in '{}' status. Must be 'submitted'.",
                card.status
            )));
        }

        info!(
            "Rejecting time card {} by {}",
            card.card_number, rejected_by
        );
        let updated = self
            .repository
            .update_time_card_status(card_id, "rejected", Some(rejected_by), reason)
            .await?;
    ...
    pub async fn cancel_time_card(
        ...
        if card.status != "draft" && card.status != "submitted" {
            return Err(AtlasError::WorkflowError(format!(
                "Cannot cancel card in '{}' status. Must be 'draft' or 'submitted'.",
                card.status
            )));
        }
        ...
        info!(
            "Cancelling time card {} reason: {:?}",
            card.card_number, reason
        );
        let updated = self
            .repository
            .update_time_card_status(card_id, "cancelled", None, reason)
            .await?;
    ...
    pub async fn create_time_entry(
        ...
        let card = self
            .repository
            .get_time_card(time_card_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Time card {time_card_id} not found"))
            })?;

        if card.organization_id != org_id {
            return Err(AtlasError::EntityNotFound(format!(
                "Time card {time_card_id} not found"
            )));
        }
        ...
        if entry_date < card.period_start || entry_date > card.period_end {
            return Err(AtlasError::ValidationFailed(format!(
                "Entry date {} is outside the time card period {} to {}",
        ...
        info!(
            "Creating time entry for card {} date {} type {} hours {}",
    ...
    pub async fn list_time_entries(
        ...
        self.repository
            .list_time_entries_by_card(time_card_id)
            .await
    }
    ...
    pub async fn delete_time_entry(&self, org_id: Uuid, id: Uuid) -> AtlasResult<()> {
        let entry = self
            ...
            .get_time_entry(id)
            .await?
            .ok_or_else(|| AtlasError::EntityNotFound(format!("Time entry {id} not found")))?;

        if entry.organization_id != org_id {
            return Err(AtlasError::EntityNotFound(format!(
                "Time entry {id} not found"
            )));
        }

        let card = self
            .repository
            .get_time_card(entry.time_card_id)
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Time card {} not found", entry.time_card_id))
            })?;
    ...
    pub async fn get_time_card_history(
        ...
        org_id: Uuid,
        time_card_id: Uuid,
    ) -> AtlasResult<Vec<TimeCardHistory>> {
        // Verify the time card belongs to org
        let card = self
            .repository
            .get_time_card(time_card_id)
    ...
    pub async fn create_labor_distribution(
        ...
        let entry = self
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Time entry {time_entry_id} not found"))
            })?;

        if entry.organization_id != org_id {
            return Err(AtlasError::EntityNotFound(format!(
                "Time entry {time_entry_id} not found"
            )));
        }
        ...
        info!(
            "Creating labor distribution for entry {} percent {}%",
            time_entry_id, distribution_percent
        );

        self.repository
            .create_labor_distribution(
                org_id,
                time_entry_id,
    ...
    pub async fn list_labor_distributions(
        ...
        self.repository
            .list_labor_distributions_by_entry(time_entry_id)
            .await
    }
    ...
    pub async fn delete_labor_distribution(&self, org_id: Uuid, id: Uuid) -> AtlasResult<()> {
        ...
        self.repository
            .delete_labor_distribution_org_scoped(org_id, id)
            .await
    }
    ...
    pub async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<TimeAndLaborDashboard> {
        ...
        let pending_approval_count =
            all_cards.iter().filter(|c| c.status == "submitted").count() as i64;

        let today = chrono::Utc::now().date_naive();
        let submitted_today_count = all_cards
            .iter()
            .filter(|c| c.submitted_at.is_some_and(|at| at.date_naive() == today))
            .count() as i64;
        ...
        for card in &all_cards {
            ...
            for entry in &entries {
                ...
                let current = hours_by_type
                    .get(&entry.entry_type)
                    .and_then(serde_json::Value::as_f64)
                    .unwrap_or(0.0);
                hours_by_type.insert(
                    entry.entry_type.clone(),
                    serde_json::json!(((current + hours) * 100.0).round() / 100.0),
                );
            }
        }
