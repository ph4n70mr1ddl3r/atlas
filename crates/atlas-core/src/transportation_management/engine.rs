...
use atlas_shared::{
    AtlasError, AtlasResult, Carrier, CarrierService, FreightRate, TransportLane,
    TransportShipment, TransportShipmentLine, TransportShipmentStop,
    TransportShipmentTrackingEvent, TransportationDashboard,
};
use std::sync::Arc;
use tracing::info;
use uuid::Uuid;
...
const VALID_CARRIER_STATUSES: &[&str] = &["active", "inactive", "suspended", "blacklisted"];

const VALID_SERVICE_LEVELS: &[&str] = &["standard", "express", "economy", "premium", "same_day"];

const VALID_LANE_TYPES: &[&str] = &["domestic", "international", "intercompany"];

const VALID_LANE_STATUSES: &[&str] = &["active", "inactive", "seasonal"];
...
const VALID_SHIPMENT_TYPES: &[&str] = &["outbound", "inbound", "transfer", "returns"];

const VALID_PRIORITIES: &[&str] = &["low", "normal", "high", "critical"];

const VALID_STOP_TYPES: &[&str] = &["pickup", "delivery", "transfer"];

const VALID_STOP_STATUSES: &[&str] = &["pending", "arrived", "departed", "skipped", "failed"];
...
const VALID_RATE_STATUSES: &[&str] = &["active", "expired", "pending", "superseded"];
...
impl TransportationManagementEngine {
    ...
    pub async fn create_carrier(
        ...
        if carrier_code.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Carrier code is required".to_string(),
            ));
        }
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Carrier name is required".to_string(),
            ));
        }
        ...
        if self
            .repository
            .get_carrier_by_code(org_id, carrier_code)
            .await?
            .is_some()
        {
    ...
    pub async fn get_carrier_by_code(
        &self,
        org_id: Uuid,
        carrier_code: &str,
    ) -> AtlasResult<Option<Carrier>> {
        self.repository
            .get_carrier_by_code(org_id, carrier_code)
            .await
    }
    ...
    pub async fn list_carriers(
        ...
        self.repository
            .list_carriers(org_id, status, carrier_type)
            .await
    }
    ...
    pub async fn suspend_carrier(&self, id: Uuid) -> AtlasResult<Carrier> {
        ...
        if carrier.status != "active" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot suspend carrier in '{}' status. Must be 'active'.",
                carrier.status
            )));
        }
        ...
        info!(
            "Suspending carrier {} ({})",
            carrier.carrier_code, carrier.name
        );
    ...
    pub async fn reactivate_carrier(&self, id: Uuid) -> AtlasResult<Carrier> {
        ...
        if carrier.status != "inactive" && carrier.status != "suspended" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot reactivate carrier in '{}' status. Must be 'inactive' or 'suspended'.",
                carrier.status
            )));
        }

        info!(
            "Reactivating carrier {} ({})",
            carrier.carrier_code, carrier.name
        );
    ...
    pub async fn blacklist_carrier(&self, id: Uuid) -> AtlasResult<Carrier> {
        ...
        info!(
            "Blacklisting carrier {} ({})",
            carrier.carrier_code, carrier.name
        );
        self.repository
            .update_carrier_status(id, "blacklisted")
            .await
    }
    ...
    pub async fn update_carrier_performance(
        ...
        info!(
            "Updating carrier {} performance: rating={:.2}, on_time={:.1}%, claims={:.4}",
            id, rating, on_time_pct, claims
        );
        self.repository
            .update_carrier_performance(id, rating, on_time_pct, claims)
            .await
    }
    ...
    pub async fn create_carrier_service(
        ...
        if service_code.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Service code is required".to_string(),
            ));
        }
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
        ...
        if rate_per_kg < 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Rate per kg cannot be negative".to_string(),
            ));
        }
        ...
        if carrier.status != "active" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot add services to carrier in '{}' status",
                carrier.status
            )));
        }

        info!(
            "Creating service '{}' ({}) for carrier {} [level={}]",
    ...
    pub async fn list_carrier_services(
        ...
        carrier_id: Uuid,
        active_only: bool,
    ) -> AtlasResult<Vec<CarrierService>> {
        self.repository
            .list_carrier_services(carrier_id, active_only)
            .await
    }
    ...
    pub async fn toggle_carrier_service(
        ...
        self.repository
            .update_carrier_service_active(id, is_active)
            .await
    }
    ...
    pub async fn create_lane(
        ...
        if lane_code.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Lane code is required".to_string(),
            ));
        }
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Lane name is required".to_string(),
            ));
        }
        ...
        if let Some(pc_id) = preferred_carrier_id {
            self.repository.get_carrier(pc_id).await?.ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Preferred carrier {pc_id} not found"))
            })?;
        }

        if self
            .repository
            .get_lane_by_code(org_id, lane_code)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
    ...
    /// Get a lane by code
    pub async fn get_lane_by_code(
        &self,
        org_id: Uuid,
        lane_code: &str,
    ) -> AtlasResult<Option<TransportLane>> {
    ...
    pub async fn deactivate_lane(&self, id: Uuid) -> AtlasResult<TransportLane> {
        ...
        if lane.status != "active" && lane.status != "seasonal" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot deactivate lane in '{}' status",
                lane.status
            )));
        }
    ...
    pub async fn create_shipment(
        ...
        if shipment_number.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "TransportShipment number is required".to_string(),
            ));
        }
        ...
        if let Some(cid) = carrier_id {
            let carrier =
                self.repository.get_carrier(cid).await?.ok_or_else(|| {
                    AtlasError::EntityNotFound(format!("Carrier {cid} not found"))
                })?;
            if carrier.status != "active" {
                return Err(AtlasError::ValidationFailed(format!(
                    "Carrier '{}' is not active (status: {})",
                    carrier.carrier_code, carrier.status
                )));
            }
            ...
            if let Some(sid) = carrier_service_id {
                let svc = self
                    .repository
                    .get_carrier_service(sid)
                    .await?
                    .ok_or_else(|| {
                        AtlasError::EntityNotFound(format!("Carrier service {sid} not found"))
                    })?;
                if !svc.is_active {
                    return Err(AtlasError::ValidationFailed(format!(
                        "Carrier service '{}' is not active",
                        svc.service_code
                    )));
                }
        ...
        if let Some(lid) = lane_id {
            let lane = self
                ...
                .get_lane(lid)
                .await?
                .ok_or_else(|| AtlasError::EntityNotFound(format!("Lane {lid} not found")))?;
            if lane.status != "active" {
                return Err(AtlasError::ValidationFailed(format!(
                    "Lane '{}' is not active",
                    lane.lane_code
        ...
        if self
            .repository
            .get_shipment_by_number(org_id, shipment_number)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
        ...
        info!(
            "Creating shipment '{}' [type={}, priority={}, carrier={:?}]",
    ...
    pub async fn get_shipment_by_number(
        ...
        org_id: Uuid,
        shipment_number: &str,
    ) -> AtlasResult<Option<TransportShipment>> {
        self.repository
            .get_shipment_by_number(org_id, shipment_number)
            .await
    }
    ...
    pub async fn list_shipments(
        ...
        self.repository
            .list_shipments(org_id, status, shipment_type)
            .await
    }
    ...
    pub async fn book_shipment(
        ...
        let shipment = self.repository.get_shipment(id).await?.ok_or_else(|| {
            AtlasError::EntityNotFound(format!("TransportShipment {id} not found"))
        })?;

        if shipment.status != "draft" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot book shipment in '{}' status. Must be 'draft'.",
                shipment.status
            )));
        }
        ...
        info!(
            "Booking shipment {} by {:?}",
            shipment.shipment_number, booked_by
        );
    ...
    pub async fn confirm_pickup(
        ...
        if shipment.status != "booked" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot confirm pickup for shipment in '{}' status. Must be 'booked'.",
                shipment.status
            )));
        }

        info!(
            "Confirming pickup for shipment {}",
            shipment.shipment_number
        );

        let mut s = self
            .repository
            .update_shipment_status(id, "picked_up")
            .await?;
        if tracking_number.is_some() || pro_number.is_some() {
            s = self
                .repository
                .update_shipment_tracking(id, tracking_number, None, pro_number, None)
                .await?;
        }
        self.repository
            .update_shipment_dates(id, Some(chrono::Utc::now().date_naive()), None)
            .await?;
        Ok(s)
    }
    ...
    pub async fn start_transit(
        ...
        if shipment.status != "picked_up" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot start transit for shipment in '{}' status. Must be 'picked_up'.",
                shipment.status
            )));
        }
        ...
        self.repository
            .update_shipment_status(id, "in_transit")
            .await
    }
    ...
    pub async fn arrive_at_destination(&self, id: Uuid) -> AtlasResult<TransportShipment> {
        ...
        if shipment.status != "in_transit" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot mark arrival for shipment in '{}' status. Must be 'in_transit'.",
                shipment.status
            )));
        }

        info!(
            "TransportShipment {} arrived at destination",
            shipment.shipment_number
        );
        self.repository
            .update_shipment_status(id, "at_destination")
            .await
    }
    ...
    pub async fn confirm_delivery(
        ...
        if shipment.status != "at_destination" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot confirm delivery for shipment in '{}' status. Must be 'at_destination'.",
                shipment.status
            )));
        }

        info!(
            "Confirming delivery for shipment {} by {:?}",
            shipment.shipment_number, received_by
        );
        let _s = self
            .repository
            .update_shipment_status(id, "delivered")
            .await?;
        let s = self
            .repository
            .update_shipment_delivery(id, received_by)
            .await?;
        Ok(s)
    }
    ...
    pub async fn cancel_shipment(&self, id: Uuid) -> AtlasResult<TransportShipment> {
        ...
        if shipment.status == "delivered" || shipment.status == "cancelled" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot cancel shipment in '{}' status",
                shipment.status
            )));
        }
        ...
        self.repository
            .update_shipment_status(id, "cancelled")
            .await
    }
    ...
    pub async fn mark_exception(&self, id: Uuid) -> AtlasResult<TransportShipment> {
        ...
        if shipment.status == "delivered" || shipment.status == "cancelled" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot mark exception for shipment in '{}' status",
                shipment.status
            )));
        }

        info!(
            "Marking exception for shipment {}",
            shipment.shipment_number
        );
        self.repository
            .update_shipment_status(id, "exception")
            .await
    }
    ...
    pub async fn assign_carrier(
        ...
        info!(
            "Assigning carrier {} to shipment {}",
            carrier.carrier_code, shipment.shipment_number
        );
        self.repository
    ...
    pub async fn delete_shipment(&self, org_id: Uuid, shipment_number: &str) -> AtlasResult<()> {
        ...
        self.repository
            .delete_shipment(org_id, shipment_number)
            .await
    }
    ...
    pub async fn add_stop(
        ...
        let shipment = self
            ...
            .await?
            .ok_or_else(|| {
                AtlasError::EntityNotFound(format!("TransportShipment {shipment_id} not found"))
            })?;

        if shipment.status != "draft" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot add stops to shipment in '{}' status",
                shipment.status
            )));
        }

        info!(
            "Adding stop #{} ({}) to shipment {}",
            stop_number, stop_type, shipment.shipment_number
        );
    ...
    pub async fn update_stop_status(
        ...
        self.repository
            .update_stop_status(id, status, actual_arrival, actual_departure)
            .await
    }
    ...
    pub async fn add_shipment_line(
        ...
        if shipment.status != "draft" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot add lines to shipment in '{}' status",
                shipment.status
            )));
        }

        info!(
            "Adding line #{} to shipment {}",
            line_number, shipment.shipment_number
        );

        self.repository
            .create_shipment_line(
                org_id,
                shipment_id,
                line_number,
                item_id,
    ...
    pub async fn list_shipment_lines(
        &self,
        shipment_id: Uuid,
    ) -> AtlasResult<Vec<TransportShipmentLine>> {
    ...
    pub async fn recalculate_shipment_totals(
        ...
        info!(
            "Recalculating shipment {} totals: weight={:.2}, pieces={}, cost={:.2}",
    ...
    pub async fn add_tracking_event(
        ...
        info!(
            "Adding tracking event '{}' for shipment {}",
            event_type, shipment.shipment_number
        );

        // Auto-transition shipment status based on tracking event
        let _ = self
            .auto_transition_from_event(shipment_id, event_type, &shipment.status)
            .await;
    ...
    pub async fn list_tracking_events(
        &self,
        shipment_id: Uuid,
    ) -> AtlasResult<Vec<TransportShipmentTrackingEvent>> {
    ...
    pub async fn create_freight_rate(
        ...
        if rate_code.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Rate code is required".to_string(),
            ));
        }
        if name.is_empty() {
            return Err(AtlasError::ValidationFailed(
                "Rate name is required".to_string(),
            ));
        }
        validate_enum("rate_type", rate_type, VALID_RATE_TYPES)?;
        if rate_amount < 0.0 {
            return Err(AtlasError::ValidationFailed(
                "Rate amount cannot be negative".to_string(),
            ));
        }
        if minimum_charge < 0.0 {
            return Err(AtlasError::ValidationFailed(
        ...
        if self
            .repository
            .get_freight_rate_by_code(org_id, rate_code)
            .await?
            .is_some()
        {
            return Err(AtlasError::Conflict(format!(
        ...
        info!(
            "Creating freight rate '{}' ({}) [type={}, amount={:.4}]",
    ...
    pub async fn list_freight_rates(
        ...
        self.repository
            .list_freight_rates(org_id, carrier_id, status)
            .await
    }
    ...
    pub async fn expire_freight_rate(&self, id: Uuid) -> AtlasResult<FreightRate> {
        let rate =
            self.repository.get_freight_rate(id).await?.ok_or_else(|| {
                AtlasError::EntityNotFound(format!("Freight rate {id} not found"))
            })?;

        if rate.status != "active" {
            return Err(AtlasError::ValidationFailed(format!(
                "Cannot expire rate in '{}' status",
                rate.status
            )));
        }
        ...
        self.repository
            .update_freight_rate_status(id, "expired")
            .await
    }
    ...
    async fn auto_transition_from_event(
        ...
        match event_type {
            "picked_up" if current_status == "booked" => {
                let _ = self
                    .repository
                    .update_shipment_status(shipment_id, "picked_up")
                    .await;
            }
            "in_transit" if current_status == "picked_up" => {
                let _ = self
                    .repository
                    .update_shipment_status(shipment_id, "in_transit")
                    .await;
            }
            "delivered"
                if current_status == "in_transit"
                    || current_status == "at_destination"
            ...
            "exception" if current_status != "delivered" && current_status != "cancelled" => {
                let _ = self
                    .repository
                    .update_shipment_status(shipment_id, "exception")
                    .await;
            }
            _ => {}
        }
...
mod tests {
    ...
    async fn test_update_carrier_performance_validation_bad_rating() {
        let engine = create_engine();
        let result = engine
            .update_carrier_performance(Uuid::new_v4(), 6.0, 90.0, 0.01)
            .await;
        assert!(result.is_err());
        match result.unwrap_err() {
    ...
    async fn test_update_carrier_performance_validation_bad_on_time() {
        let engine = create_engine();
        let result = engine
            .update_carrier_performance(Uuid::new_v4(), 4.0, 150.0, 0.01)
            .await;
        assert!(result.is_err());
        match result.unwrap_err() {
    ...
    async fn test_update_carrier_performance_validation_bad_claims() {
        let engine = create_engine();
        let result = engine
            .update_carrier_performance(Uuid::new_v4(), 4.0, 90.0, 2.0)
            .await;
        assert!(result.is_err());
        match result.unwrap_err() {
    ...
    async fn test_create_lane_success() {
        ...
        let result = engine
            .create_lane(
                ...
                None,
                Some("New York Warehouse"),
                Some("New York"),
                Some("NY"),
                "USA",
                Some("10001"),
                None,
                Some("LA Distribution Center"),
                Some("Los Angeles"),
                Some("CA"),
                "USA",
                Some("90001"),
    ...
    async fn test_create_shipment_success() {
        ...
        let result = engine
            .create_shipment(
                ...
                None,
                Some("NYC Warehouse"),
                serde_json::json!({"city": "New York", "state": "NY"}),
                None,
                Some("LA Customer"),
                serde_json::json!({"city": "Los Angeles", "state": "CA"}),
    ...
    async fn test_book_shipment_not_found() {
        let engine = create_engine();
        let result = engine
            .book_shipment(Uuid::new_v4(), Some(test_user_id()))
            .await;
        assert!(result.is_err());
    }
