...
pub trait TransportationManagementRepository: Send + Sync {
    ...
    async fn get_freight_rate_by_code(
        ...
        org_id: Uuid,
        rate_code: &str,
    ) -> AtlasResult<Option<FreightRate>>;
...
fn row_to_carrier(row: &sqlx::postgres::PgRow) -> Carrier {
    Carrier {
        ...
        carrier_type: row
            .try_get("carrier_type")
            .unwrap_or_else(|_| "parcel".to_string()),
        status: row
            .try_get("status")
        ...
        payment_terms: row
            .try_get("payment_terms")
            .unwrap_or_else(|_| "net_30".to_string()),
        ...
        default_service_level: row
            .try_get("default_service_level")
...
fn row_to_lane(row: &sqlx::postgres::PgRow) -> TransportLane {
    TransportLane {
        ...
        origin_country: row
            .try_get("origin_country")
            .unwrap_or_else(|_| "USA".to_string()),
        ...
        lane_type: row
            .try_get("lane_type")
            .unwrap_or_else(|_| "domestic".to_string()),
...
fn row_to_freight_rate(row: &sqlx::postgres::PgRow) -> FreightRate {
    FreightRate {
        ...
        rate_type: row
            .try_get("rate_type")
            .unwrap_or_else(|_| "per_kg".to_string()),
        ...
        currency_code: row
            ...
            .unwrap_or_else(|_| "USD".to_string()),
        fuel_surcharge_pct: row.try_get("fuel_surcharge_pct").unwrap_or(0.0),
        accessorial_rates: row
            .try_get("accessorial_rates")
            .unwrap_or(serde_json::json!({})),
        effective_from: row
            .try_get("effective_from")
            .unwrap_or(chrono::NaiveDate::from_ymd_opt(2025, 1, 1).unwrap()),
        effective_to: row.try_get("effective_to").unwrap_or_default(),
        status: row
            .try_get("status")
            .unwrap_or_else(|_| "active".to_string()),
...
impl TransportationManagementRepository for PostgresTransportationManagementRepository {
    ...
    async fn delete_carrier(&self, org_id: Uuid, carrier_code: &str) -> AtlasResult<()> {
        ...
        if result.rows_affected() == 0 {
            return Err(AtlasError::EntityNotFound(format!(
                "Carrier '{carrier_code}' not found"
            )));
        }
        Ok(())
    }
    ...
    async fn create_carrier_service(
        ...
        .bind(description)
        .bind(service_level)
        .bind(transit_days_min)
        .bind(transit_days_max)
        .bind(max_weight_kg)
        .bind(&max_dimensions)
        .bind(cutoff_time)
        .bind(operates_on_weekends)
        .bind(is_international)
        .bind(rate_per_kg)
        .bind(minimum_charge)
        .bind(fuel_surcharge_pct)
        .bind(is_active)
        .bind(&metadata)
        .fetch_one(&self.pool)
        .await?;
    ...
    async fn delete_carrier_service(&self, id: Uuid) -> AtlasResult<()> {
        ...
        if result.rows_affected() == 0 {
            return Err(AtlasError::EntityNotFound(
                "Carrier service not found".to_string(),
            ));
        }
        Ok(())
    }
    ...
    async fn update_shipment_carrier(
        ...
        .bind(carrier_code)
        .bind(carrier_name)
        .bind(carrier_service_id)
        .bind(carrier_service_code)
        .fetch_one(&self.pool)
        .await
        .map_err(|_| AtlasError::EntityNotFound(format!("TransportShipment {id} not found")))?;
        Ok(row_to_shipment(&row))
    ...
    async fn delete_shipment(&self, org_id: Uuid, shipment_number: &str) -> AtlasResult<()> {
        ...
        if result.rows_affected() == 0 {
            return Err(AtlasError::EntityNotFound(format!(
                "TransportShipment '{shipment_number}' not found"
            )));
        }
        Ok(())
    }
    ...
    async fn update_shipment_line_quantities(
        ...
        .await
        .map_err(|_| {
            AtlasError::EntityNotFound(format!("TransportShipment line {id} not found"))
        })?;
        Ok(row_to_shipment_line(&row))
    }
    ...
    async fn delete_shipment_line(&self, id: Uuid) -> AtlasResult<()> {
        ...
        if result.rows_affected() == 0 {
            return Err(AtlasError::EntityNotFound(
                "TransportShipment line not found".to_string(),
            ));
        }
        Ok(())
    }
    ...
    async fn create_tracking_event(
        ...
        .bind(longitude)
        .bind(description)
        .bind(carrier_event_code)
        .bind(carrier_event_description)
        .bind(updated_by)
        .bind(&metadata)
        .fetch_one(&self.pool)
        .await?;
    ...
    async fn create_freight_rate(
        ...
        .bind(currency_code)
        .bind(fuel_surcharge_pct)
        .bind(&accessorial_rates)
        .bind(effective_from)
        .bind(effective_to)
        .bind(status)
        .bind(is_contract_rate)
        .bind(contract_number)
        .bind(volume_threshold_min)
        .bind(volume_threshold_max)
        .bind(&metadata)
        .bind(created_by)
        .fetch_one(&self.pool)
        .await?;
    ...
    async fn delete_freight_rate(&self, org_id: Uuid, rate_code: &str) -> AtlasResult<()> {
        ...
        if result.rows_affected() == 0 {
            return Err(AtlasError::EntityNotFound(format!(
                "Freight rate '{rate_code}' not found"
            )));
        }
        Ok(())
    }
    ...
    async fn get_dashboard(&self, org_id: Uuid) -> AtlasResult<TransportationDashboard> {
        ...
        let active_shipments = shipment_rows
            .iter()
            .filter(|r| {
                matches!(
        ...
        let total_freight_cost: f64 = shipment_rows
            .iter()
            .map(|r| r.try_get("total_cost").unwrap_or(0.0))
            .sum();
        let total_weight: f64 = shipment_rows
            .iter()
            .map(|r| r.try_get("total_weight_kg").unwrap_or(0.0))
            .sum();
        let avg_cost_per_kg = if total_weight > 0.0 {
            total_freight_cost / total_weight
        } else {
            0.0
        };
        ...
        let carrier_rows =
            sqlx::query("SELECT status FROM _atlas.carriers WHERE organization_id = $1")
                .bind(org_id)
                .fetch_all(&self.pool)
                .await
                .unwrap_or_default();
        let total_carriers = carrier_rows.len() as i32;
        let active_carriers = carrier_rows
            .iter()
            .filter(|r| r.try_get::<String, _>("status").unwrap_or_default() == "active")
            .count() as i32;

        let lane_rows =
            sqlx::query("SELECT status FROM _atlas.transport_lanes WHERE organization_id = $1")
                .bind(org_id)
                .fetch_all(&self.pool)
                .await
                .unwrap_or_default();
        let total_lanes = lane_rows.len() as i32;
        let active_lanes = lane_rows
            .iter()
            .filter(|r| r.try_get::<String, _>("status").unwrap_or_default() == "active")
            .count() as i32;
        ...
        Ok(TransportationDashboard {
            ...
            total_freight_cost,
            avg_cost_per_kg,
            shipments_by_status: serde_json::to_value(shipments_by_status)
                .unwrap_or(serde_json::json!({})),
            shipments_by_carrier: serde_json::to_value(shipments_by_carrier)
                .unwrap_or(serde_json::json!({})),
