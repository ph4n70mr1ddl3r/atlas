            ...
            FieldType::Integer { min, max } => {
                if let Some(n) = value.as_i64() {
                    if let Some(min_val) = min {
                        if n < *min_val {
                            result.add_error(
                                &field.name,
                                "min",
                                &format!("{} must be at least {}", field.label, min_val),
                            );
                        }
                    }
                    if let Some(max_val) = max {
                        if n > *max_val {
                            result.add_error(
                                &field.name,
                                "max",
                                &format!("{} must be at most {}", field.label, max_val),
                            );
                        }
                    }
                    ...
                    result.add_error(
                        &field.name,
                        "type",
                        &format!("{} must be an integer", field.label),
                    );
                }
            }
            ...
            FieldType::Enum { values } => {
                if let Some(s) = value.as_str() {
                    ...
                    result.add_error(
                        &field.name,
                        "type",
                        &format!("{} must be a string", field.label),
                    );
                }
            }
                ...
                if let Some(s) = value.as_str() {
                    if uuid::Uuid::parse_str(s).is_err() {
                        result.add_error(
                            &field.name,
                            "uuid",
                            &format!("{} must be a valid UUID", field.label),
                        );
                    }
                } else if !value.is_null() {
                    result.add_error(
                        &field.name,
                        "type",
                        &format!("{} must be a string (UUID)", field.label),
                    );
                }
            }
            ...
            FieldType::OneToMany { .. } => {
                if let Some(arr) = value.as_array() {
                        ...
                        if let Some(s) = v.as_str() {
                            if uuid::Uuid::parse_str(s).is_err() {
                                result.add_error(
                                    ...
                                    "uuid",
                                    &format!(
                                        "{} contains an invalid UUID at index {}",
                                        field.label, i
                                    ),
                                );
                            }
                            ...
                            result.add_error(
                                &field.name,
                                "type",
                                &format!("{} must contain only UUID strings", field.label),
                            );
                        }
                    }
                } else if !value.is_null() {
                    result.add_error(
                        &field.name,
                        "type",
                        &format!("{} must be an array of UUIDs", field.label),
                    );
                }
            }
                ...
                if let Some(s) = value.as_str() {
                    if !s.contains('@') || !s.contains('.') {
                        result.add_error(
                            &field.name,
                            "email",
                            &format!("{} must be a valid email", field.label),
                        );
                    }
                }
            }
                    ...
                    if let Some(re) = self.get_cached_regex(pattern) {
                        if !re.is_match(s) {
                            result.add_error(
                                &field.name,
                                "pattern",
                                &format!("{} does not match required format", field.label),
                            );
                        }
                    } else {
                        result.add_error(
