...
pub trait ReferenceDataLookup {
    ...
    fn get_segment_from_salesperson(&self, salesperson_id: &str, segment_name: &str) -> Option<String>;
    fn get_segment_from_standard_line(&self, std_line_id: &str, segment_name: &str) -> Option<String>;
}
...
impl AutoAccountingService {
    ...
    pub fn derive_account<T: ReferenceDataLookup>(
        ...
        for &segment in segments_to_build {
            ...
            let rule = rules.iter().find(|r| r.account_class == account_class && r.segment_name == segment);
            ...
            let segment_value = match rule {
                ...
                None => return Err(format!("No AutoAccounting rule found for class '{}', segment '{}'", account_class, segment)),
            };

            match segment_value {
                Some(val) => combination.push(val),
                None => return Err(format!("Failed to derive segment '{}' for class '{}'", segment, account_class)),
            }
        }
...
mod tests {
    ...
    impl ReferenceDataLookup for MockLookup {
        fn get_segment_from_trx_type(&self, trx_type_id: &str, segment_name: &str) -> Option<String> {
            self.trx_type_data.get(&(trx_type_id.to_string(), segment_name.to_string())).cloned()
        }
        fn get_segment_from_salesperson(&self, salesperson_id: &str, segment_name: &str) -> Option<String> {
            self.salesperson_data.get(&(salesperson_id.to_string(), segment_name.to_string())).cloned()
        }
        fn get_segment_from_standard_line(&self, std_line_id: &str, segment_name: &str) -> Option<String> {
            self.std_line_data.get(&(std_line_id.to_string(), segment_name.to_string())).cloned()
        }
    }
    ...
    fn test_derive_revenue_account() {
        ...
        trx_data.insert(("TRX-INV".to_string(), "COMPANY".to_string()), "01".to_string());
        
        let mut rep_data = HashMap::new();
        rep_data.insert(("REP-500".to_string(), "COST_CENTER".to_string()), "200".to_string());
        
        let mut line_data = HashMap::new();
        line_data.insert(("ITEM-A".to_string(), "ACCOUNT".to_string()), "4000".to_string());
        ...
        let result = AutoAccountingService::derive_account("REVENUE", &segments, &rules, &context, &lookup);
    ...
    fn test_derive_receivable_account_with_constant() {
        let mut trx_data = HashMap::new();
        trx_data.insert(("TRX-INV".to_string(), "COMPANY".to_string()), "01".to_string());
        trx_data.insert(("TRX-INV".to_string(), "COST_CENTER".to_string()), "000".to_string());
        ...
        let segments = ["COMPANY", "COST_CENTER", "ACCOUNT"];
        let result = AutoAccountingService::derive_account("RECEIVABLE", &segments, &rules, &context, &lookup);
    ...
    fn test_missing_rule_error() {
        ...
        let result = AutoAccountingService::derive_account("FREIGHT", &segments, &rules, &context, &lookup);

        assert!(result.is_err());
        assert_eq!(result.err().unwrap(), "No AutoAccounting rule found for class 'FREIGHT', segment 'ACCOUNT'");
    }
    ...
    fn test_failed_to_derive_segment() {
        ...
        assert!(result.is_err());
        assert_eq!(result.err().unwrap(), "Failed to derive segment 'COMPANY' for class 'REVENUE'");
    }
}
