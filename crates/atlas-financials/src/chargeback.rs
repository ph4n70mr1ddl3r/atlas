...
impl ChargebackService {
    ...
    pub fn create_chargeback(
        ...
        if chargebacks
            .iter()
            .any(|c| c.organization_id == organization_id && c.chargeback_number == number)
        {
    ...
    pub fn add_line(
        ...
        let line_number = (lines
            .iter()
            .filter(|l| l.chargeback_id == chargeback_id)
            .count() as i32)
            + 1;
...
mod tests {
    ...
    fn test_create_and_resolve_chargeback() {
        ...
        let cb = service
            .create_chargeback(
                ...
                "CB-101".to_string(),
                500.0,
                "DAMAGED_GOODS".to_string(),
            )
            .unwrap();
        assert_eq!(cb.status, "open");

        service
            .add_line(cb.id, 250.0, Some("Box crushed".to_string()))
            .unwrap();
        service
            .add_line(cb.id, 250.0, Some("Item missing".to_string()))
            .unwrap();

        service
            .resolve_chargeback(cb.id, "Validated with warehouse".to_string())
            .unwrap();
    ...
    fn test_duplicate_chargeback() {
        ...
        let org_id = Uuid::new_v4();
        service
            .create_chargeback(org_id, "CB-1".to_string(), 100.0, "R".to_string())
            .unwrap();
