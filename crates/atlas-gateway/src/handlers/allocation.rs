...
pub async fn create_allocation_pool(
    ...
    let pool_type = body["pool_type"]
        .as_str()
        .unwrap_or("cost_center")
        .to_string();
    ...
    let source_department_id = body["source_department_id"]
        .as_str()
        .and_then(|s| Uuid::parse_str(s).ok());
    let source_project_id = body["source_project_id"]
        .as_str()
        .and_then(|s| Uuid::parse_str(s).ok());
...
pub async fn activate_allocation_pool(
    ...
    match state.financials.allocation_engine.activate_pool(id).await {
        Ok(pool) => Ok(Json(serde_json::to_value(pool).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err((
...
pub async fn create_allocation_basis(
    ...
    let name = body["name"].as_str().unwrap_or("").to_string();
    let description = body["description"].as_str();
    let basis_type = body["basis_type"]
        .as_str()
        ...
        .await
    {
        Ok(basis) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(basis).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn get_allocation_basis(
    ...
    match state
        .financials
        .allocation_engine
        .get_basis(org_id, &code)
        .await
    {
        Ok(Some(basis)) => Ok(Json(serde_json::to_value(basis).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err((
            StatusCode::NOT_FOUND,
            Json(json!({"error": "Basis not found"})),
        )),
        Err(e) => Err((
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error": e.to_string()})),
...
pub async fn activate_allocation_basis(
    ...
    match state.financials.allocation_engine.activate_basis(id).await {
        Ok(basis) => Ok(Json(serde_json::to_value(basis).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err((
...
pub async fn add_allocation_basis_detail(
    ...
    match state
        .financials
        .allocation_engine
        .add_basis_detail(org_id, &basis_code, &request, parse_uuid(&claims.sub).ok())
        .await
    {
        Ok(detail) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(detail).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn list_allocation_basis_details(
    ...
    match state
        .financials
        .allocation_engine
        .list_basis_details(org_id, &basis_code, query.period_name.as_deref())
        .await
    {
...
pub async fn recalculate_basis_percentages(
    ...
    match state
        .financials
        .allocation_engine
        .recalculate_basis_percentages(org_id, &basis_code)
        .await
    {
        Ok(details) => Ok(Json(json!({"data": details}))),
        Err(e) => Err((
...
pub async fn create_allocation_rule(
    ...
    let mut request = atlas_shared::GlAllocationRuleRequest {
        ...
        allocation_method: body["allocation_method"]
            ...
            .to_string(),
        offset_method: body["offset_method"]
            .as_str()
            .unwrap_or("same_account")
            .to_string(),
    ...
    if let Some(lines_array) = body["target_lines"].as_array() {
        ...
        for line_val in lines_array {
            lines.push(atlas_shared::GlAllocationTargetLineRequest {
                target_department_id: line_val["target_department_id"]
                    .as_str()
                    .and_then(|s| Uuid::parse_str(s).ok()),
                target_department_name: line_val["target_department_name"]
                    .as_str()
                    .map(String::from),
                target_cost_center: line_val["target_cost_center"].as_str().map(String::from),
                target_project_id: line_val["target_project_id"]
                    .as_str()
                    .and_then(|s| Uuid::parse_str(s).ok()),
                target_project_name: line_val["target_project_name"].as_str().map(String::from),
                target_account_code: line_val["target_account_code"]
                    .as_str()
                    .unwrap_or("")
                    .to_string(),
    ...
    match state
        .financials
        .allocation_engine
        .create_rule(org_id, &request, parse_uuid(&claims.sub).ok())
        .await
    {
        Ok(rule) => Ok((
            StatusCode::CREATED,
...
pub async fn execute_allocation(
    ...
    match state
        .financials
        .allocation_engine
        .execute_allocation(org_id, &request, parse_uuid(&claims.sub).ok())
        .await
    {
        Ok(run) => Ok((
            StatusCode::CREATED,
...
pub async fn reverse_allocation_run(
    ...
    match state
        .financials
        .allocation_engine
        .reverse_run(id, Some(reversed_by))
        .await
    {
        Ok(run) => Ok(Json(serde_json::to_value(run).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
