...
pub async fn create_pool(
    ...
    let concentration_account_id = body["concentration_account_id"]
        .as_str()
    ...
    let termination_date = body["termination_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
        ...
        .await
    {
        Ok(pool) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(pool).unwrap_or(Value::Null)),
        )),
        Err(e) => Err((
...
pub async fn get_pool(
        ...
        .await
    {
        ...
        Ok(None) => Err((
            ...
            Json(json!({"error": "Cash pool not found"})),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn add_participant(
    ...
    let participant_type = body["participant_type"]
        .as_str()
        .unwrap_or("source")
        .to_string();
    let sweep_direction = body["sweep_direction"]
        .as_str()
        .unwrap_or("to_concentration")
        .to_string();
        ...
        .await
    {
        Ok(participant) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(participant).unwrap_or(Value::Null)),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn list_participants(
    ...
    match state
        .financials
        .cash_concentration_engine
        .list_participants(pool_id, org_id, params.status.as_deref())
        .await
    {
...
pub async fn remove_participant(
    ...
    match state
        .financials
        .cash_concentration_engine
        .remove_participant(pool_id, &participant_code, org_id)
        .await
    {
...
pub async fn create_sweep_rule(
    ...
    let sweep_type = body["sweep_type"]
        .as_str()
...
pub async fn list_sweep_rules(
    ...
    match state
        .financials
        .cash_concentration_engine
        .list_sweep_rules(pool_id, org_id)
        .await
    {
...
pub async fn delete_sweep_rule(
    ...
    match state
        .financials
        .cash_concentration_engine
        .delete_sweep_rule(pool_id, &rule_code, org_id)
        .await
    {
...
pub async fn list_sweep_runs(
    ...
    match state
        .financials
        .cash_concentration_engine
        .list_sweep_runs(pool_id, org_id)
        .await
    {
...
pub async fn list_sweep_run_lines(
    ...
    match state
        .financials
        .cash_concentration_engine
        .list_sweep_run_lines(sweep_run_id, org_id)
        .await
    {
