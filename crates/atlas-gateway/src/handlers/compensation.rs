...
pub async fn create_component(
    ...
    let plan = state
        .hcm
        .compensation_engine
        .get_plan_by_code(org_id, &plan_code)
        .await
        ...
        .await
    {
        Ok(comp) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(comp)),
        )),
        Err(e) => {
...
pub async fn create_cycle(
        ...
        .await
    {
        Ok(cycle) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(cycle)),
        )),
        Err(e) => {
...
pub async fn list_cycles(
    ...
    match state
        .hcm
        .compensation_engine
        .list_cycles(org_id, query.status.as_deref())
        .await
    {
...
pub async fn create_budget_pool(
        ...
        .await
    {
        Ok(pool) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(pool)),
        )),
        Err(e) => {
...
pub async fn create_worksheet(
        ...
        .await
    {
        Ok(ws) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(ws)),
        )),
        Err(e) => {
...
pub async fn list_worksheets(
    ...
    match state
        .hcm
        .compensation_engine
        .list_worksheets(cycle_id, query.status.as_deref())
        .await
    {
...
pub async fn list_worksheet_lines(
    ...
    match state
        .hcm
        .compensation_engine
        .list_worksheet_lines(worksheet_id)
        .await
    {
...
pub async fn generate_statement(
        ...
        .await
    {
        Ok(stmt) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(stmt)),
        )),
        Err(e) => {
