...
pub async fn create_program(
    ...
    let single_limit = body["default_single_purchase_limit"]
        .as_str()
        .unwrap_or("5000.00");
    ...
    let auto_deactivate = body["auto_deactivate_on_termination"]
        .as_bool()
        .unwrap_or(true);
        ...
        .await
    {
        Ok(program) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(program).unwrap_or_else(|e| {
...
pub async fn list_programs(
    ...
    match state
        .financials
        .corporate_card_engine
        .list_programs(org_id, active_only)
        .await
    {
...
pub async fn issue_card(
    ...
    let issue_date = body["issue_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok())
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "issue_date is required (YYYY-MM-DD)"})),
            )
        })?;
    let expiry_date = body["expiry_date"]
        ...
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "expiry_date is required (YYYY-MM-DD)"})),
            )
        })?;
        ...
        .await
    {
        Ok(card) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(card).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn list_cards(
    ...
    match state
        .financials
        .corporate_card_engine
        .list_cards(org_id, program_id, cardholder_id, query.status.as_deref())
        .await
    {
...
pub async fn import_transaction(
    ...
    let posting_date = body["posting_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok())
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "posting_date is required"})),
            )
        })?;
    let transaction_date = body["transaction_date"]
        ...
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "transaction_date is required"})),
            )
        })?;
        ...
        .await
    {
        Ok(txn) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(txn).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn list_transactions(
    ...
    match state
        .financials
        .corporate_card_engine
        .list_transactions(org_id, card_id, query.status.as_deref(), date_from, date_to)
        .await
    {
...
pub async fn match_transaction(
    ...
    let expense_report_id = body["expense_report_id"]
        .as_str()
        .and_then(|s| Uuid::parse_str(s).ok())
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "expense_report_id is required"})),
            )
        })?;
    let expense_line_id = body["expense_line_id"]
        .as_str()
        .and_then(|s| Uuid::parse_str(s).ok());
...
pub async fn resolve_dispute(
    ...
    match state
        .financials
        .corporate_card_engine
        .resolve_dispute(id, resolution, resolved_status)
        .await
    {
        Ok(txn) => Ok((
            StatusCode::OK,
...
pub async fn import_statement(
    ...
    let statement_date = body["statement_date"]
        ...
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "statement_date is required"})),
            )
        })?;
    let period_start = body["billing_period_start"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok())
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "billing_period_start is required"})),
            )
        })?;
    let period_end = body["billing_period_end"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok())
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "billing_period_end is required"})),
            )
        })?;
    ...
    let payment_due_date = body["payment_due_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
        ...
        .await
    {
        Ok(stmt) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(stmt).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn list_statements(
    ...
    match state
        .financials
        .corporate_card_engine
        .list_statements(org_id, program_id, query.status.as_deref())
        .await
    {
...
pub async fn pay_statement(
    ...
    match state
        .financials
        .corporate_card_engine
        .pay_statement(id, payment_reference)
        .await
    {
        Ok(stmt) => Ok((
            StatusCode::OK,
...
pub async fn request_limit_override(
    ...
    let effective_from = body["effective_from"]
        ...
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "effective_from is required"})),
            )
        })?;
        ...
        .await
    {
        Ok(ovr) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(ovr).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn approve_limit_override(
    ...
    match state
        .financials
        .corporate_card_engine
        .approve_limit_override(id, approved_by)
        .await
    {
        Ok(ovr) => Ok((
            StatusCode::OK,
...
pub async fn reject_limit_override(
    ...
    match state
        .financials
        .corporate_card_engine
        .reject_limit_override(id, rejected_by)
        .await
    {
        Ok(ovr) => Ok((
            StatusCode::OK,
...
pub async fn list_limit_overrides(
    ...
    match state
        .financials
        .corporate_card_engine
        .list_limit_overrides(card_id, query.status.as_deref())
        .await
    {
