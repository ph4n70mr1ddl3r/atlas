...
pub async fn delete_scoring_model(
    ...
    match state
        .financials
        .credit_management_engine
        .delete_scoring_model(org_id, &code)
        .await
    {
...
pub async fn create_profile(
    ...
    match state
        ...
        .create_profile(
            ...
            payload.customer_group_name.as_deref(),
            payload.scoring_model_id,
            payload.review_frequency_days,
            Some(user_id),
        )
        .await
    {
        Ok(profile) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(profile).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn get_profile(
    ...
    match state
        ...
        .await
    {
        Ok(Some(p)) => Ok(Json(serde_json::to_value(p).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err(StatusCode::NOT_FOUND),
...
pub async fn list_profiles(
    ...
    match state
        .financials
        .credit_management_engine
        .list_profiles(org_id, params.status.as_deref())
        .await
    {
...
pub async fn update_profile_status(
    ...
    match state
        .financials
        .credit_management_engine
        .update_profile_status(id, &payload.status)
        .await
    {
        Ok(p) => Ok(Json(serde_json::to_value(p).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => {
...
pub async fn delete_profile(
        ...
        .await
    {
        Ok(Some(profile)) => {
            let org_id =
                Uuid::parse_str(&_claims.org_id).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
            match state
                .financials
                .credit_management_engine
                .delete_profile(org_id, &profile.profile_number)
                .await
            {
                Ok(()) => Ok(StatusCode::NO_CONTENT),
                Err(e) => {
...
pub async fn update_credit_limit(
    ...
    match state
        .financials
        .credit_management_engine
        .update_credit_limit_amount(id, &payload.credit_limit)
        .await
    {
        Ok(l) => Ok(Json(serde_json::to_value(l).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => {
...
pub async fn set_temp_limit(
    ...
    match state
        ...
        .set_temp_limit(
            id,
            &payload.temp_limit_increase,
            Some(payload.temp_limit_expiry),
        )
        .await
    {
        Ok(l) => Ok(Json(serde_json::to_value(l).unwrap_or_else(|e| {
...
pub async fn calculate_exposure(
    ...
    match state
        ...
        .calculate_exposure(
            ...
            &payload.open_receivables,
            &payload.open_orders,
            &payload.open_shipments,
            &payload.open_invoices,
            &payload.unapplied_cash,
            &payload.on_hold_amount,
        )
        .await
    {
        Ok(exposure) => Ok(Json(serde_json::to_value(exposure).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => {
...
pub async fn get_latest_exposure(
    ...
    match state
        .financials
        .credit_management_engine
        .get_latest_exposure(profile_id)
        .await
    {
        Ok(Some(e)) => Ok(Json(serde_json::to_value(e).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err(StatusCode::NOT_FOUND),
...
pub async fn create_hold(
    ...
    match state
        ...
        .create_hold(
            ...
            &payload.entity_type,
            payload.entity_id,
            payload.entity_number.as_deref(),
            payload.hold_amount.as_deref(),
...
pub async fn list_holds(
    ...
    match state
        .financials
        .credit_management_engine
        .list_holds(org_id, params.status.as_deref(), params.profile_id)
        .await
    {
...
pub async fn release_hold(
    ...
    match state
        .financials
        .credit_management_engine
        .release_hold(id, Some(user_id), payload.release_reason.as_deref())
        .await
    {
        Ok(h) => Ok(Json(serde_json::to_value(h).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => {
...
pub async fn override_hold(
    ...
    match state
        .financials
        .credit_management_engine
        .override_hold(id, Some(user_id), Some(&payload.override_reason))
        .await
    {
        Ok(h) => Ok(Json(serde_json::to_value(h).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
...
pub async fn create_review(
    ...
    match state
        ...
        .create_review(
            ...
            &payload.review_type,
            None,
            payload.recommended_credit_limit.as_deref(),
            None,
            None,
            payload.due_date,
            Some(user_id),
        ...
        .await
    {
        Ok(review) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(review).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn list_reviews(
    ...
    match state
        .financials
        .credit_management_engine
        .list_reviews(org_id, params.status.as_deref(), params.profile_id)
        .await
    {
...
pub async fn complete_review(
    ...
    match state
        ...
        .complete_review(
            id,
            payload.new_score.as_deref(),
            payload.new_rating.as_deref(),
            payload.approved_credit_limit.as_deref(),
            payload.findings.as_deref(),
            payload.recommendations.as_deref(),
            user_id,
            payload.reviewer_name.as_deref(),
        )
        .await
    {
        Ok(r) => Ok(Json(serde_json::to_value(r).unwrap_or_else(|e| {
...
pub async fn approve_review(
    ...
    match state
        .financials
        .credit_management_engine
        .approve_review(id, user_id, None)
        .await
    {
        Ok(r) => Ok(Json(serde_json::to_value(r).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
...
pub async fn reject_review(
    ...
    match state
        .financials
        .credit_management_engine
        .reject_review(id, Some(&payload.reason))
        .await
    {
        Ok(r) => Ok(Json(serde_json::to_value(r).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
