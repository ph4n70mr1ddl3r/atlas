...
pub async fn create_revaluation_definition(
    ...
    let request = atlas_shared::CurrencyRevaluationDefinitionRequest {
        ...
        revaluation_type: body["revaluation_type"]
            .as_str()
        ...
        unrealized_gain_account_code: body["unrealized_gain_account_code"]
            .as_str()
            .map(String::from),
        unrealized_loss_account_code: body["unrealized_loss_account_code"]
            .as_str()
            .map(String::from),
    ...
    match state
        ...
        .create_definition(org_id, &request, None)
        .await
    {
        Ok(def) => Ok((
            StatusCode::CREATED,
...
pub async fn list_revaluation_definitions(
    ...
    match state
        .financials
        .currency_revaluation_engine
        .list_definitions(org_id, active_only)
        .await
    {
...
pub async fn add_revaluation_account(
    ...
    match state
        .financials
        .currency_revaluation_engine
        .add_account(org_id, &code, &request)
        .await
    {
        Ok(acct) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(acct).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn execute_revaluation(
        ...
        Some(arr) => arr
            ...
            .filter_map(|b| {
                Some(atlas_shared::CurrencyRevaluationBalanceRequest {
                    ...
                    original_exchange_rate: b["original_exchange_rate"]
                        .as_str()
                        .unwrap_or("1")
                        .to_string(),
                    original_base_amount: b["original_base_amount"]
                        .as_str()
                        .unwrap_or("0")
                        .to_string(),
                })
    ...
    match state
        .financials
        .currency_revaluation_engine
        .execute_revaluation(org_id, &request, None)
        .await
    {
        Ok(run) => Ok((
            StatusCode::CREATED,
