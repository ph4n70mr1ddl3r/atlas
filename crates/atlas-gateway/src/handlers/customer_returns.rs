...
fn default_standard_return() -> String {
    "standard_return".to_string()
}
#[allow(dead_code)]
fn default_return_to_stock() -> String {
    "return_to_stock".to_string()
}
...
pub async fn create_return_reason(
    ...
    match state
        ...
        .create_return_reason(
            ...
            &req.return_type,
            req.default_disposition.as_deref(),
            req.requires_approval,
            req.credit_issued_automatically,
            None,
        )
        .await
    {
        Ok(reason) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(reason).unwrap_or_else(|e| {
...
pub async fn get_return_reason(
    ...
    match state
        .scm
        .customer_returns_engine
        .get_return_reason(parse_uuid(&claims.org_id)?, &code)
        .await
    {
        Ok(Some(reason)) => Ok(Json(serde_json::to_value(reason).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err((
...
pub async fn list_return_reasons(
    ...
    match state
        .scm
        .customer_returns_engine
        .list_return_reasons(parse_uuid(&claims.org_id)?, query.return_type.as_deref())
        .await
    {
...
pub async fn delete_return_reason(
    ...
    match state
        .scm
        .customer_returns_engine
        .delete_return_reason(parse_uuid(&claims.org_id)?, &code)
        .await
    {
...
pub async fn create_rma(
    ...
    match state
        ...
        .create_rma(
            ...
            req.original_order_number.as_deref(),
            req.original_order_id,
            req.customer_contact.as_deref(),
            req.customer_email.as_deref(),
            req.customer_phone.as_deref(),
            return_date,
            req.expected_receipt_date,
            &req.currency_code,
            req.notes.as_deref(),
            None,
        )
        .await
    {
        Ok(rma) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(rma).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn get_rma(
    ...
    match state.scm.customer_returns_engine.get_rma(id).await {
        Ok(Some(rma)) => Ok(Json(serde_json::to_value(rma).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err((
...
pub async fn submit_rma(
    ...
    match state.scm.customer_returns_engine.submit_rma(id).await {
        Ok(rma) => Ok(Json(serde_json::to_value(rma).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err((
...
pub async fn add_return_line(
    ...
    match state
        ...
        .add_return_line(
            ...
            req.original_line_id,
            &req.original_quantity,
            &req.return_quantity,
            &req.unit_price,
            req.reason_code.as_deref(),
...
pub async fn receive_return_line(
    ...
    match state
        .scm
        .customer_returns_engine
        .receive_return_line(line_id, &req.received_quantity)
        .await
    {
...
pub async fn generate_credit_memo(
    ...
    match state
        .scm
        .customer_returns_engine
        .generate_credit_memo(rma_id, req.gl_account_code.as_deref(), None)
        .await
    {
        Ok(memo) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(memo).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn get_credit_memo(
    ...
    match state.scm.customer_returns_engine.get_credit_memo(id).await {
        Ok(Some(memo)) => Ok(Json(serde_json::to_value(memo).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err((
...
pub async fn issue_credit_memo(
    ...
    match state
        .scm
        .customer_returns_engine
        .issue_credit_memo(id)
        .await
    {
        Ok(memo) => Ok(Json(serde_json::to_value(memo).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err((
...
pub async fn get_returns_dashboard(
    ...
    match state
        .scm
        .customer_returns_engine
        .get_dashboard_summary(parse_uuid(&claims.org_id)?)
        .await
    {
