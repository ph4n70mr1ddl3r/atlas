...
pub async fn create_template(
    ...
    match state
        ...
        .create_template(
            ...
            &payload.deferral_type,
            &payload.recognition_method,
            &payload.deferral_account_code,
            &payload.recognition_account_code,
            payload.contra_account_code.as_deref(),
            payload.default_periods,
            &payload.period_type,
            &payload.start_date_basis,
            &payload.end_date_basis,
            payload.prorate_partial_periods,
            payload.auto_generate_schedule,
            payload.auto_post,
...
pub async fn create_schedule(
    ...
    match state
        ...
        .create_schedule(
            ...
            payload.start_date,
            payload.end_date,
            payload.original_journal_entry_id,
            Some(user_id),
        )
        .await
    {
...
pub async fn recognize_pending(
    ...
    match state
        .financials
        .deferred_revenue_engine
        .recognize_pending(org_id, payload.as_of_date)
        .await
    {
        Ok(lines) => Ok(Json(serde_json::json!({ "data": lines }))),
        Err(e) => {
            error!("Failed to recognize pending: {}", e);
            Err(StatusCode::INTERNAL_SERVER_ERROR)
        }
    }
}
...
pub async fn hold_schedule(
    ...
    match state
        .financials
        .deferred_revenue_engine
        .hold_schedule(id, &payload.reason)
        .await
    {
        Ok(s) => Ok(to_json(s)),
        Err(e) => {
...
pub async fn get_deferred_revenue_dashboard(
        ...
        .await
    {
        ...
        Err(e) => {
            error!("Failed to get deferred revenue dashboard: {}", e);
            Err(StatusCode::INTERNAL_SERVER_ERROR)
        }
    }
}
