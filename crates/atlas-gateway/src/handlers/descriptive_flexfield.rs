...
pub async fn create_value_set(
        ...
        .await
    {
        Ok(vs) => {
            ...
            Ok((
                StatusCode::CREATED,
                Json(serde_json::to_value(vs).unwrap_or_else(|e| {
...
pub async fn get_value_set(
    ...
    match state.shared.dff_engine.get_value_set(org_id, &code).await {
        Ok(Some(vs)) => Ok(Json(serde_json::to_value(vs).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
...
pub async fn list_value_set_entries(
    ...
    match state
        .shared
        .dff_engine
        .list_value_set_entries(org_id, &code, query.parent_value.as_deref())
        .await
    {
        Ok(list) => Ok(Json(json!(list))),
        Err(e) => Err(map_error_status(&e)),
...
pub async fn create_flexfield(
        ...
        .await
    {
        Ok(ff) => {
            info!(
                "Created flexfield '{}' for entity '{}'",
                ff.code, ff.entity_name
            );
            Ok((
                StatusCode::CREATED,
                Json(serde_json::to_value(ff).unwrap_or_else(|e| {
                    tracing::error!("Serialization error: {}", e);
                    serde_json::Value::Null
                })),
            ))
...
pub async fn get_flexfield(
    ...
    match state.shared.dff_engine.get_flexfield(org_id, &code).await {
        Ok(Some(ff)) => Ok(Json(serde_json::to_value(ff).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err(StatusCode::NOT_FOUND),
...
pub async fn activate_flexfield(
    ...
    match state.shared.dff_engine.activate_flexfield(id).await {
        Ok(ff) => {
            ...
            Ok(Json(serde_json::to_value(ff).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })))
        }
...
pub async fn create_context(
        ...
        .await
    {
        Ok(ctx) => {
            info!(
                "Created context '{}' in flexfield '{}'",
                ctx.code, flexfield_code
            );
            Ok((
                StatusCode::CREATED,
                Json(serde_json::to_value(ctx).unwrap_or_else(|e| {
                    tracing::error!("Serialization error: {}", e);
                    serde_json::Value::Null
                })),
            ))
...
pub async fn list_contexts(
    ...
    match state
        .shared
        .dff_engine
        .list_contexts(org_id, &flexfield_code)
        .await
    {
        Ok(list) => Ok(Json(json!(list))),
        Err(e) => Err(map_error_status(&e)),
...
pub async fn disable_context(
    ...
    match state.shared.dff_engine.disable_context(id).await {
        Ok(ctx) => Ok(Json(serde_json::to_value(ctx).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err(map_error_status(&e)),
...
pub async fn create_segment(
        ...
        .await
    {
        Ok(seg) => {
            info!(
                "Created segment '{}' in '{}/{}'",
                seg.segment_code, flexfield_code, context_code
            );
            Ok((
                StatusCode::CREATED,
                Json(serde_json::to_value(seg).unwrap_or_else(|e| {
                    tracing::error!("Serialization error: {}", e);
                    serde_json::Value::Null
                })),
            ))
...
pub async fn list_segments_by_context(
    ...
    match state
        .shared
        .dff_engine
        .list_segments_by_context(org_id, &flexfield_code, &context_code)
        .await
    {
        Ok(list) => Ok(Json(json!(list))),
        Err(e) => Err(map_error_status(&e)),
...
pub async fn list_segments_by_flexfield(
    ...
    match state
        .shared
        .dff_engine
        .list_segments_by_flexfield(org_id, &flexfield_code)
        .await
    {
        Ok(list) => Ok(Json(json!(list))),
        Err(e) => Err(map_error_status(&e)),
...
pub async fn set_flexfield_data(
        ...
        .await
    {
        Ok(data) => {
            ...
            Ok((
                StatusCode::OK,
                Json(serde_json::to_value(data).unwrap_or_else(|e| {
                    tracing::error!("Serialization error: {}", e);
                    serde_json::Value::Null
                })),
            ))
...
pub async fn get_flexfield_data(
    ...
    match state
        .shared
        .dff_engine
        .get_data(&entity_name, entity_id, query.context_code.as_deref())
        .await
    {
        Ok(list) => Ok(Json(json!(list))),
        Err(e) => Err(map_error_status(&e)),
...
pub async fn delete_flexfield_data(
    ...
    match state
        .shared
        .dff_engine
        .delete_data(&entity_name, entity_id)
        .await
    {
        Ok(()) => Ok(StatusCode::NO_CONTENT),
        Err(e) => Err(map_error_status(&e)),
