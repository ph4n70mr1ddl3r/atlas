...
pub async fn create_mandate(
    ...
    match state
        ...
        .create_mandate(
            ...
            payload.creditor_scheme_id.as_deref(),
            payload.creditor_name.as_deref(),
            payload.mandate_reference.as_deref(),
            payload
                .mandate_date
                .unwrap_or_else(|| chrono::Utc::now().date_naive()),
            payload.expiry_date,
            payload.max_collection_amount.as_deref(),
...
pub async fn list_mandates(
    ...
    match state
        .financials
        .direct_debit_mandate_engine
        .list_mandates(org_id, query.customer_id, query.status.as_deref())
        .await
    {
...
pub async fn cancel_mandate(
    ...
    match state
        .financials
        .direct_debit_mandate_engine
        .cancel_mandate(id, &payload.reason)
        .await
    {
        Ok(m) => Ok(to_json(m)),
        Err(e) => {
...
pub async fn revoke_mandate(
    ...
    match state
        .financials
        .direct_debit_mandate_engine
        .revoke_mandate(id, &payload.reason)
        .await
    {
        Ok(m) => Ok(to_json(m)),
        Err(e) => {
...
pub async fn list_collections(
    ...
    match state
        .financials
        .direct_debit_mandate_engine
        .list_collections(mandate_id, query.status.as_deref())
        .await
    {
...
pub async fn complete_collection(
    ...
    match state
        .financials
        .direct_debit_mandate_engine
        .complete_collection(id, payload.bank_reference.as_deref())
        .await
    {
        Ok(c) => Ok(to_json(c)),
        Err(e) => {
...
pub async fn fail_collection(
    ...
    match state
        .financials
        .direct_debit_mandate_engine
        .fail_collection(id, &payload.reason_code, payload.reason_text.as_deref())
        .await
    {
        Ok(c) => Ok(to_json(c)),
        Err(e) => {
...
pub async fn return_collection(
    ...
    match state
        .financials
        .direct_debit_mandate_engine
        .return_collection(id, &payload.reason_code, payload.reason_text.as_deref())
        .await
    {
        Ok(c) => Ok(to_json(c)),
        Err(e) => {
...
pub async fn reverse_collection(
    ...
    match state
        .financials
        .direct_debit_mandate_engine
        .reverse_collection(id, &payload.reason_code, payload.reason_text.as_deref())
        .await
    {
        Ok(c) => Ok(to_json(c)),
        Err(e) => {
