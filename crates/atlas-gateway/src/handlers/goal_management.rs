...
pub async fn create_library_category(
    ...
    Ok((
        StatusCode::CREATED,
        Json(crate::handlers::records::to_json_or_null(cat)),
    ))
}
...
pub async fn list_library_categories(
    ...
    let cats = state
        .hcm
        .goal_management_engine
        .list_library_categories(org_id)
        .await
        .map_err(|e| {
            error!("List categories error: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })?;
    Ok(Json(
        serde_json::json!({ "data": cats, "meta": { "total": cats.len() } }),
    ))
}
...
pub async fn delete_library_category(
    ...
    state
        .hcm
        .goal_management_engine
        .delete_library_category(org_id, &code)
        .await
        .map_err(|e| {
...
pub async fn create_library_template(
    ...
    Ok((
        StatusCode::CREATED,
        Json(crate::handlers::records::to_json_or_null(tmpl)),
    ))
}
...
pub async fn list_library_templates(
    ...
    let tmpls = state
        .hcm
        .goal_management_engine
        .list_library_templates(org_id, params.category_id, params.goal_type.as_deref())
        .await
        .map_err(|e| {
            error!("List templates error: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })?;
    Ok(Json(
        serde_json::json!({ "data": tmpls, "meta": { "total": tmpls.len() } }),
    ))
}
...
pub async fn delete_library_template(
    ...
    state
        .hcm
        .goal_management_engine
        .delete_library_template(org_id, &code)
        .await
        .map_err(|e| {
...
pub async fn list_goal_plans(
    ...
    let plans = state
        .hcm
        .goal_management_engine
        .list_goal_plans(org_id, params.status.as_deref())
        .await
        .map_err(|e| {
            error!("List plans error: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })?;
    Ok(Json(
        serde_json::json!({ "data": plans, "meta": { "total": plans.len() } }),
    ))
}
...
pub async fn update_goal_plan_status(
    ...
    let plan = state
        .hcm
        .goal_management_engine
        .update_goal_plan_status(id, &payload.status)
        .await
        .map_err(|e| {
...
pub async fn list_goals(
    ...
    let goals = state
        ...
        .list_goals(
            ...
            params.plan_id,
            params.owner_id,
            params.goal_type.as_deref(),
            params.status.as_deref(),
            params.parent_goal_id,
        )
        .await
        .map_err(|e| {
            error!("List goals error: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })?;
    Ok(Json(
        serde_json::json!({ "data": goals, "meta": { "total": goals.len() } }),
    ))
}
...
pub async fn update_goal_progress(
    ...
    let goal = state
        ...
        .update_goal_progress(
            id,
            payload.actual_value.as_deref(),
            payload.progress_pct.as_deref(),
...
pub async fn create_goal_alignment(
    ...
    let alignment = state
        ...
        .create_goal_alignment(
            org_id,
            payload.source_goal_id,
            payload.aligned_to_goal_id,
    ...
    Ok((
        StatusCode::CREATED,
        Json(crate::handlers::records::to_json_or_null(alignment)),
    ))
}
...
pub async fn list_goal_alignments(
    ...
    let alignments = state
        .hcm
        .goal_management_engine
        .list_goal_alignments(goal_id)
        .await
        .map_err(|e| {
            error!("List alignments error: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })?;
    Ok(Json(
        serde_json::json!({ "data": alignments, "meta": { "total": alignments.len() } }),
    ))
}
...
pub async fn create_goal_note(
    ...
    Ok((
        StatusCode::CREATED,
        Json(crate::handlers::records::to_json_or_null(note)),
    ))
}
...
pub async fn list_goal_notes(
    ...
    let notes = state
        .hcm
        .goal_management_engine
        .list_goal_notes(goal_id)
        .await
        .map_err(|e| {
            error!("List notes error: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })?;
    Ok(Json(
        serde_json::json!({ "data": notes, "meta": { "total": notes.len() } }),
    ))
}
