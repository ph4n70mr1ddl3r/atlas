...
pub async fn create_derivative(
    ...
    let trade_date = body["trade_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
    let effective_date = body["effective_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
    let maturity_date = body["maturity_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
    let settlement_date = body["settlement_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
        ...
        .await
    {
        Ok(deriv) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(deriv).unwrap_or(Value::Null)),
        )),
        Err(e) => Err((
...
pub async fn get_derivative(
        ...
        .await
    {
        ...
        Ok(None) => Err((
            ...
            Json(json!({"error": "Derivative not found"})),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn update_derivative_valuation(
    ...
    match state
        .financials
        .hedge_management_engine
        .update_derivative_valuation(id, fair_value, unrealized_gain_loss, valuation_method)
        .await
    {
        Ok(deriv) => Ok(Json(serde_json::to_value(deriv).unwrap_or(Value::Null))),
        Err(e) => Err((
...
pub async fn create_hedge_relationship(
    ...
    let designated_start_date = body["designated_start_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
    let designated_end_date = body["designated_end_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
    let effectiveness_method = body["effectiveness_method"]
        .as_str()
        .unwrap_or("dollar_offset")
        .to_string();
        ...
        .await
    {
        Ok(hedge) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(hedge).unwrap_or(Value::Null)),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn get_hedge_relationship(
        ...
        .await
    {
        ...
        Ok(None) => Err((
            StatusCode::NOT_FOUND,
            Json(json!({"error": "Hedge relationship not found"})),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn run_effectiveness_test(
    ...
    let hedge_relationship_id = body["hedge_relationship_id"]
        .as_str()
        .and_then(|s| Uuid::parse_str(s).ok())
        .unwrap_or_else(Uuid::nil);
    let test_type = body["test_type"].as_str().unwrap_or("ongoing").to_string();
    let test_date = body["test_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok())
        .unwrap_or_else(|| chrono::Utc::now().date_naive());
    let derivative_fair_value_change = body["derivative_fair_value_change"]
        .as_str()
        .unwrap_or("0")
        .to_string();
    let hedged_item_fair_value_change = body["hedged_item_fair_value_change"]
        .as_str()
        .unwrap_or("0")
        .to_string();
    let test_period_start = body["test_period_start"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
    let test_period_end = body["test_period_end"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
    let notes = body["notes"].as_str();
        ...
        .await
    {
        Ok(test) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(test).unwrap_or(Value::Null)),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn get_effectiveness_test(
    ...
    match state
        .financials
        .hedge_management_engine
        .get_effectiveness_test(id)
        .await
    {
        Ok(Some(test)) => Ok(Json(serde_json::to_value(test).unwrap_or(Value::Null))),
        Ok(None) => Err((
            StatusCode::NOT_FOUND,
            Json(json!({"error": "Effectiveness test not found"})),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn create_documentation(
    ...
    let hedge_type = body["hedge_type"]
        .as_str()
        .unwrap_or("fair_value")
        .to_string();
    ...
    let designation_date = body["designation_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
    let documentation_date = body["documentation_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
    ...
    match state
        ...
        .create_documentation(
            ...
            hedging_strategy_description,
            hedged_item_description,
            hedged_risk_description,
            derivative_description,
            effectiveness_method_description,
...
pub async fn get_documentation(
    ...
    match state
        .financials
        .hedge_management_engine
        .get_documentation(org_id, &document_number)
        .await
    {
        Ok(Some(doc)) => Ok(Json(serde_json::to_value(doc).unwrap_or(Value::Null))),
        Ok(None) => Err((
            StatusCode::NOT_FOUND,
            Json(json!({"error": "Documentation not found"})),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn list_documentation(
    ...
    let hedge_relationship_id = params
        .hedge_relationship_id
        .as_deref()
        .and_then(|s| Uuid::parse_str(s).ok());
    ...
    match state
        .financials
        .hedge_management_engine
        .list_documentation(org_id, hedge_relationship_id)
        .await
    {
...
pub async fn approve_documentation(
    ...
    match state
        .financials
        .hedge_management_engine
        .approve_documentation(id, parse_uuid(&claims.sub).ok())
        .await
    {
