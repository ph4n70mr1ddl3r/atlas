...
pub async fn create_impairment_test(
    ...
    match state
        ...
        .create_test(
            ...
            payload.asset_id,
            None,
            payload.discount_rate.as_deref(),
            None,
            Some(user_id),
        )
        .await
...
pub async fn submit_impairment_test(
    ...
    match state
        .financials
        .impairment_management_engine
        .submit_test(id, Some(user_id))
        .await
    {
        Ok(test) => Ok(to_json(test)),
        Err(e) => {
            error!("Failed to submit test: {}", e);
            Err(StatusCode::BAD_REQUEST)
        }
    }
}
...
pub async fn approve_impairment_test(
    ...
    match state
        .financials
        .impairment_management_engine
        .approve_test(id, Some(user_id))
        .await
    {
        Ok(test) => Ok(to_json(test)),
        Err(e) => {
            error!("Failed to approve test: {}", e);
            Err(StatusCode::BAD_REQUEST)
        }
    }
}
