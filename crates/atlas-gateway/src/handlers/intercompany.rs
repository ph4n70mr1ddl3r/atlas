...
pub async fn submit_intercompany_batch(
    ...
    match state
        .financials
        .intercompany_engine
        .submit_batch(batch_id, Some(user_id))
        .await
    {
        Ok(batch) => Ok(Json(serde_json::to_value(batch).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => {
...
pub async fn approve_intercompany_batch(
    ...
    match state
        .financials
        .intercompany_engine
        .approve_batch(batch_id, user_id)
        .await
    {
        Ok(batch) => Ok(Json(serde_json::to_value(batch).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
...
pub async fn reject_intercompany_batch(
    ...
    match state
        .financials
        .intercompany_engine
        .reject_batch(batch_id, &payload.reason)
        .await
    {
        Ok(batch) => Ok(Json(serde_json::to_value(batch).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
...
pub async fn list_intercompany_transactions(
    ...
    match state
        .financials
        .intercompany_engine
        .list_transactions_by_batch(batch_id)
        .await
    {
...
pub async fn list_entity_transactions(
    ...
    match state
        .financials
        .intercompany_engine
        .list_transactions_by_entity(org_id, entity_id, params.status.as_deref())
        .await
    {
        Ok(transactions) => Ok(Json(serde_json::json!({ "data": transactions }))),
        Err(e) => {
...
fn default_settlement_method() -> String {
    "cash".to_string()
}
...
pub async fn list_intercompany_settlements(
    ...
    match state
        .financials
        .intercompany_engine
        .list_settlements(org_id, params.entity_id)
        .await
    {
...
pub async fn get_intercompany_balance(
    ...
    match state
        .financials
        .intercompany_engine
        .get_balance(org_id, from_entity_id, to_entity_id, &currency)
        .await
    {
        Ok(Some(balance)) => Ok(Json(serde_json::to_value(balance).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err(StatusCode::NOT_FOUND),
