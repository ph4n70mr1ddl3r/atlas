...
pub async fn create_schedule(
    ...
    let compounding_frequency = body["compounding_frequency"]
        .as_str()
        .unwrap_or("daily")
        .to_string();
    let charge_type = body["charge_type"]
        .as_str()
        .unwrap_or("interest")
        .to_string();
...
pub async fn list_schedules(
    ...
    match state
        .financials
        .interest_invoice_engine
        .list_schedules(org_id, params.status.as_deref())
        .await
    {
...
pub async fn delete_schedule(
    ...
    match state
        .financials
        .interest_invoice_engine
        .delete_schedule(org_id, &schedule_code)
        .await
    {
...
pub async fn register_overdue_invoice(
    ...
    let outstanding_amount = body["outstanding_amount"]
        .as_str()
        .unwrap_or("0")
        .to_string();
        ...
        .await
    {
        Ok(inv) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(inv).unwrap_or(Value::Null)),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn list_overdue_invoices(
    ...
    match state
        .financials
        .interest_invoice_engine
        .list_overdue_invoices(org_id, params.status.as_deref())
        .await
    {
...
pub async fn calculate_interest(
    ...
    let schedule_id = body["schedule_id"]
        .as_str()
        .and_then(|s| Uuid::parse_str(s).ok());
    let calculation_date = body["calculation_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok())
        .unwrap_or_else(|| chrono::Utc::now().date_naive());
...
pub async fn get_calculation_run(
        ...
        .await
    {
        ...
        Ok(None) => Err((
            StatusCode::NOT_FOUND,
            Json(json!({"error": "Calculation run not found"})),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn generate_interest_invoices(
    ...
    let invoice_date = body["invoice_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok())
        .unwrap_or_else(|| chrono::Utc::now().date_naive());
    let due_date = body["due_date"]
...
pub async fn get_interest_invoice(
        ...
        .await
    {
        ...
        Ok(None) => Err((
            StatusCode::NOT_FOUND,
            Json(json!({"error": "Interest invoice not found"})),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn list_interest_invoices(
    ...
    match state
        .financials
        .interest_invoice_engine
        .list_interest_invoices(org_id, params.status.as_deref())
        .await
    {
        Ok(invoices) => Ok(Json(json!({"data": invoices}))),
        Err(e) => Err((
...
pub async fn list_interest_invoice_lines(
    ...
    match state
        .financials
        .interest_invoice_engine
        .list_interest_invoice_lines(invoice_id)
        .await
    {
        Ok(lines) => Ok(Json(json!({"data": lines}))),
        Err(e) => Err((
