...
pub async fn create_match(
    ...
    match state
        ...
        .create_match(
            ...
            &payload.po_amount,
            payload.receipt_amount.as_deref(),
            payload.inspection_amount.as_deref(),
            ...
            payload.inspection_status.as_deref(),
            Some(user_id),
        )
        .await
    {
...
pub async fn hold_match(
    ...
    match state
        .financials
        .invoice_matching_engine
        .hold_match(id, payload.reason.as_deref(), Some(user_id))
        .await
    {
        Ok(m) => Ok(to_json(m)),
        Err(e) => {
...
#[derive(Debug, Deserialize)]
pub struct OverrideMatchRequest {
    pub reason: String,
}
...
pub async fn override_match(
    ...
    match state
        .financials
        .invoice_matching_engine
        .override_match(id, &payload.reason, Some(user_id))
        .await
    {
        Ok(m) => Ok(to_json(m)),
        Err(e) => {
...
pub async fn cancel_match(
    ...
    match state
        .financials
        .invoice_matching_engine
        .cancel_match(id, payload.reason.as_deref())
        .await
    {
        Ok(m) => Ok(to_json(m)),
        Err(e) => {
...
pub async fn add_match_line(
    ...
    match state
        ...
        .add_match_line(
            ...
            payload.inspection_line_id,
            payload.line_number,
            payload.item_description.as_deref(),
            &payload.invoice_quantity,
            &payload.po_quantity,
            payload.receipt_quantity.as_deref(),
            payload.inspected_quantity.as_deref(),
            &payload.invoice_unit_price,
            &payload.po_unit_price,
            &payload.invoice_line_amount,
            &payload.po_line_amount,
...
pub async fn list_match_lines(
        ...
        .await
    {
        ...
        Err(e) => {
            error!("Failed to list match lines: {}", e);
            Err(StatusCode::INTERNAL_SERVER_ERROR)
        }
    }
}
