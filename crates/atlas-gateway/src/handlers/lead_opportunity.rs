...
pub async fn create_lead_source(
        ...
        .await
    {
        Ok(src) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(src)),
        )),
        Err(e) => {
...
pub async fn delete_lead_source(
    ...
    match state
        .crm
        .lead_opportunity_engine
        .delete_lead_source(org_id, &code)
        .await
    {
...
pub async fn create_lead(
    ...
    match state
        ...
        .create_lead(
            ...
            payload.lead_source_name.as_deref(),
            payload.lead_rating_model_id,
        ...
        .await
    {
        Ok(lead) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(lead)),
        )),
        Err(e) => {
...
pub async fn list_leads(
    ...
    match state
        .crm
        .lead_opportunity_engine
        .list_leads(org_id, params.status.as_deref(), params.owner_id)
        .await
    {
...
pub async fn update_lead_status(
    ...
    match state
        .crm
        .lead_opportunity_engine
        .update_lead_status(id, &payload.status)
        .await
    {
...
pub async fn update_lead_score(
    ...
    match state
        .crm
        .lead_opportunity_engine
        .update_lead_score(id, &payload.score, &payload.rating)
        .await
    {
        Ok(l) => Ok(Json(crate::handlers::records::to_json_or_null(l))),
        Err(e) => {
    ...
    match state.crm.lead_opportunity_engine.get_lead(id).await {
        Ok(Some(lead)) => {
            match state
                .crm
                .lead_opportunity_engine
                .delete_lead(org_id, &lead.lead_number)
                .await
            {
                Ok(()) => Ok(StatusCode::NO_CONTENT),
                Err(e) => {
...
pub async fn create_opportunity_stage(
        ...
        .await
    {
        Ok(stage) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(stage)),
        )),
        Err(e) => {
...
pub async fn list_opportunity_stages(
    ...
    match state
        .crm
        .lead_opportunity_engine
        .list_opportunity_stages(org_id)
        .await
    {
...
pub async fn delete_opportunity_stage(
    ...
    match state
        .crm
        .lead_opportunity_engine
        .delete_opportunity_stage(org_id, &code)
        .await
    {
        Ok(()) => Ok(StatusCode::NO_CONTENT),
        Err(e) => {
...
pub async fn create_opportunity(
        ...
        .await
    {
        Ok(opp) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(opp)),
        )),
        Err(e) => {
...
pub async fn update_opportunity_stage(
    ...
    match state
        .crm
        .lead_opportunity_engine
        .update_opportunity_stage(id, payload.stage_id, user_id, user_name.as_deref(), None)
        .await
    {
...
pub async fn close_opportunity_lost(
    ...
    match state
        .crm
        .lead_opportunity_engine
        .close_opportunity_lost(id, payload.lost_reason.as_deref())
        .await
    {
        Ok(o) => Ok(Json(crate::handlers::records::to_json_or_null(o))),
        Err(e) => {
    ...
    match state.crm.lead_opportunity_engine.get_opportunity(id).await {
        Ok(Some(opp)) => {
            match state
                .crm
                .lead_opportunity_engine
                .delete_opportunity(org_id, &opp.opportunity_number)
                .await
            {
                Ok(()) => Ok(StatusCode::NO_CONTENT),
                Err(e) => {
...
pub async fn create_activity(
        ...
        .await
    {
        Ok(act) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(act)),
        )),
        Err(e) => {
...
pub async fn list_activities(
    ...
    match state
        .crm
        .lead_opportunity_engine
        .list_activities(org_id, params.lead_id, params.opportunity_id)
        .await
    {
...
pub async fn complete_activity(
    ...
    match state
        .crm
        .lead_opportunity_engine
        .complete_activity(id, payload.outcome.as_deref())
        .await
    {
