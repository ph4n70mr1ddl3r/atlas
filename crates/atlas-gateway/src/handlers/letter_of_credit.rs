...
pub async fn create_lc(
    ...
    let lc_number = body["lcNumber"]
        .as_str()
        .or_else(|| body["lc_number"].as_str())
        .unwrap_or("")
        .to_string();
    let lc_type = body["lcType"]
        .as_str()
        .or_else(|| body["lc_type"].as_str())
        .unwrap_or("import")
        .to_string();
    let lc_form = body["lcForm"]
        .as_str()
        .or_else(|| body["lc_form"].as_str())
        .unwrap_or("irrevocable")
        .to_string();
    ...
    let applicant_name = body["applicantName"]
        ...
        .unwrap_or("")
        .to_string();
    let applicant_address = body["applicantAddress"]
        .as_str()
        .or_else(|| body["applicant_address"].as_str());
    let applicant_bank_name = body["applicantBankName"]
        .as_str()
        .or_else(|| body["applicant_bank_name"].as_str())
        .unwrap_or("")
        .to_string();
    let applicant_bank_swift = body["applicantBankSwift"]
        .as_str()
    ...
    let beneficiary_name = body["beneficiaryName"]
        ...
        .unwrap_or("")
        .to_string();
    let beneficiary_address = body["beneficiaryAddress"]
        .as_str()
        .or_else(|| body["beneficiary_address"].as_str());
    let beneficiary_bank_name = body["beneficiaryBankName"]
        .as_str()
        .or_else(|| body["beneficiary_bank_name"].as_str());
    let beneficiary_bank_swift = body["beneficiaryBankSwift"]
        .as_str()
        .or_else(|| body["beneficiary_bank_swift"].as_str());
    let advising_bank_name = body["advisingBankName"]
        .as_str()
        .or_else(|| body["advising_bank_name"].as_str());
    let advising_bank_swift = body["advisingBankSwift"]
        .as_str()
        .or_else(|| body["advising_bank_swift"].as_str());
    let confirming_bank_name = body["confirmingBankName"]
        .as_str()
        .or_else(|| body["confirming_bank_name"].as_str());
    let confirming_bank_swift = body["confirmingBankSwift"]
        .as_str()
        .or_else(|| body["confirming_bank_swift"].as_str());
    let lc_amount = body["lcAmount"]
        .as_str()
    ...
    let currency_code = body["currencyCode"]
        ...
        .unwrap_or("USD")
        .to_string();
    let tolerance_plus = body["tolerancePlus"]
        .as_str()
        .or_else(|| body["tolerance_plus"].as_str())
        .unwrap_or("0")
        .to_string();
    let tolerance_minus = body["toleranceMinus"]
        .as_str()
        .or_else(|| body["tolerance_minus"].as_str())
        .unwrap_or("0")
        .to_string();
    let available_with = body["availableWith"]
        .as_str()
        .or_else(|| body["available_with"].as_str());
    let available_by = body["availableBy"]
        .as_str()
        .or_else(|| body["available_by"].as_str())
        .unwrap_or("payment")
        .to_string();
    let draft_at = body["draftAt"]
        .as_str()
    ...
    let place_of_expiry = body["placeOfExpiry"]
        .as_str()
        .or_else(|| body["place_of_expiry"].as_str());
    let partial_shipments = body["partialShipments"]
        .as_str()
        .or_else(|| body["partial_shipments"].as_str())
        .unwrap_or("allowed")
        .to_string();
    let transshipment = body["transshipment"]
        .as_str()
        .or_else(|| body["transshipment"].as_str())
        .unwrap_or("allowed")
        .to_string();
    let port_of_loading = body["portOfLoading"]
        .as_str()
        .or_else(|| body["port_of_loading"].as_str());
    let port_of_discharge = body["portOfDischarge"]
        .as_str()
        .or_else(|| body["port_of_discharge"].as_str());
    let shipment_period = body["shipmentPeriod"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
    let latest_shipment_date = body["latestShipmentDate"]
        .as_str()
        .or_else(|| body["latest_shipment_date"].as_str())
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
    let goods_description = body["goodsDescription"]
        .as_str()
        .or_else(|| body["goods_description"].as_str());
    let incoterms = body["incoterms"]
        .as_str()
        .or_else(|| body["incoterms"].as_str());
    let additional_conditions = body["additionalConditions"]
        .as_str()
        .or_else(|| body["additional_conditions"].as_str());
    let bank_charges = body["bankCharges"]
        .as_str()
        .or_else(|| body["bank_charges"].as_str())
        .unwrap_or("beneficiary")
        .to_string();
    let reference_po_number = body["referencePoNumber"]
        .as_str()
        ...
        .await
    {
        Ok(lc) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(&lc).unwrap_or(Value::Null)),
        )),
        Err(e) => Err((
...
pub async fn get_lc(
        ...
        .await
    {
        ...
        Ok(None) => Err((
            ...
            Json(json!({"error": "Letter of credit not found"})),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn list_lcs(
    ...
    match state
        .financials
        .letter_of_credit_engine
        .list_lcs(org_id, params.status.as_deref(), params.lc_type.as_deref())
        .await
    {
...
pub async fn delete_lc(
    ...
    match state
        .financials
        .letter_of_credit_engine
        .delete_lc(org_id, &lc_number)
        .await
    {
        Ok(()) => Ok((
            StatusCode::OK,
            Json(json!({"message": "Letter of credit deleted"})),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn process_expired(
    ...
    match state
        .financials
        .letter_of_credit_engine
        .process_expired(org_id, as_of_date)
        .await
    {
        Ok(expired) => Ok((
            StatusCode::OK,
...
pub async fn create_amendment(
    ...
    let bank_reference = body["bankReference"]
        .as_str()
...
pub async fn add_required_document(
    ...
    let document_type = body["documentType"]
        .as_str()
        .or_else(|| body["document_type"].as_str())
        .unwrap_or("")
        .to_string();
    let document_code = body["documentCode"]
        .as_str()
        .or_else(|| body["document_code"].as_str());
    let description = body["description"].as_str();
    let original_copies = body["originalCopies"]
        .as_i64()
        .or_else(|| body["original_copies"].as_i64())
        .unwrap_or(1) as i32;
    let copy_count = body["copyCount"]
        .as_i64()
        .or_else(|| body["copy_count"].as_i64())
        .unwrap_or(0) as i32;
    let is_mandatory = body["isMandatory"]
        .as_bool()
        .or_else(|| body["is_mandatory"].as_bool())
        .unwrap_or(true);
    let special_instructions = body["specialInstructions"]
        .as_str()
...
pub async fn delete_required_document(
    ...
    match state
        .financials
        .letter_of_credit_engine
        .delete_required_document(doc_id)
        .await
    {
        Ok(()) => Ok((
            StatusCode::OK,
            Json(json!({"message": "Required document deleted"})),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn create_shipment(
    ...
    let shipment_number = body["shipmentNumber"]
        .as_str()
        .or_else(|| body["shipment_number"].as_str())
        .unwrap_or("")
        .to_string();
    let vessel_name = body["vesselName"]
        .as_str()
        .or_else(|| body["vessel_name"].as_str());
    let voyage_number = body["voyageNumber"]
        .as_str()
        .or_else(|| body["voyage_number"].as_str());
    let bill_of_lading_number = body["billOfLadingNumber"]
        .as_str()
        .or_else(|| body["bill_of_lading_number"].as_str());
    let carrier_name = body["carrierName"]
        .as_str()
    ...
    let port_of_discharge = body["portOfDischarge"]
        .as_str()
        .or_else(|| body["port_of_discharge"].as_str());
    let shipment_date = body["shipmentDate"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
    let expected_arrival_date = body["expectedArrivalDate"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
    let goods_description = body["goodsDescription"]
        .as_str()
    ...
    let unit_price = body["unitPrice"]
        .as_str()
        .or_else(|| body["unit_price"].as_str());
    let shipment_amount = body["shipmentAmount"]
        .as_str()
        ...
        .await
    {
        Ok(shipment) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(&shipment).unwrap_or(Value::Null)),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn create_presentation(
    ...
    let presentation_number = body["presentationNumber"]
        .as_str()
        .or_else(|| body["presentation_number"].as_str())
        .unwrap_or("")
        .to_string();
    let shipment_id = body["shipmentId"]
        .as_str()
        .and_then(|s| Uuid::parse_str(s).ok());
    let presentation_date = body["presentationDate"]
        .as_str()
        .or_else(|| body["presentation_date"].as_str())
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok())
        .unwrap_or_else(|| chrono::Utc::now().date_naive());
    let presenting_bank_name = body["presentingBankName"]
        .as_str()
        .or_else(|| body["presenting_bank_name"].as_str());
    let total_amount = body["totalAmount"]
        .as_str()
        ...
        .await
    {
        Ok(presentation) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(&presentation).unwrap_or(Value::Null)),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn accept_presentation(
    ...
    match state
        .financials
        .letter_of_credit_engine
        .accept_presentation(presentation_id)
        .await
    {
        Ok(presentation) => Ok((
            StatusCode::OK,
...
pub async fn pay_presentation(
    ...
    let paid_amount = body["paidAmount"]
        .as_str()
        .or_else(|| body["paid_amount"].as_str())
        .unwrap_or("0")
        .to_string();
    let payment_date = body["paymentDate"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok())
        .unwrap_or_else(|| chrono::Utc::now().date_naive());
    ...
    match state
        .financials
        .letter_of_credit_engine
        .pay_presentation(presentation_id, &paid_amount, payment_date)
        .await
    {
        Ok(presentation) => Ok((
            StatusCode::OK,
...
pub async fn reject_presentation(
    ...
    match state
        .financials
        .letter_of_credit_engine
        .reject_presentation(presentation_id)
        .await
    {
        Ok(presentation) => Ok((
            StatusCode::OK,
...
pub async fn add_presentation_document(
    ...
    let required_document_id = body["requiredDocumentId"]
        .as_str()
        .and_then(|s| Uuid::parse_str(s).ok());
    let document_type = body["documentType"]
        .as_str()
        ...
        .unwrap_or("")
        .to_string();
    let document_reference = body["documentReference"]
        .as_str()
    ...
    let copy_count = body["copyCount"]
        ...
        .or_else(|| body["copy_count"].as_i64())
        .unwrap_or(0) as i32;
    let is_compliant = body["isCompliant"]
        .as_bool()
