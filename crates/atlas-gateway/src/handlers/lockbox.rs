...
pub async fn create_format(
    ...
    match state
        ...
        .create_format(
            ...
            &payload.format_type,
            payload.field_delimiter.as_deref(),
            payload.record_delimiter.as_deref(),
            payload.header_identifier.as_deref(),
            payload.detail_identifier.as_deref(),
            payload.trailer_identifier.as_deref(),
            Some(user_id),
        )
        .await
    {
...
pub async fn list_formats(
    ...
    match state.financials.lockbox_engine.list_formats(org_id).await {
        ...
        Err(e) => {
            error!("Failed to list formats: {}", e);
            Err(StatusCode::INTERNAL_SERVER_ERROR)
        }
    }
}
