...
pub async fn delete_campaign_type(
    ...
    match state
        .crm
        .marketing_engine
        .delete_campaign_type(org_id, &code)
        .await
    {
...
pub async fn create_campaign(
    ...
    match state
        ...
        .create_campaign(
            ...
            &payload.name,
            payload.description.as_deref(),
            payload.campaign_type_id,
            payload.campaign_type_name.as_deref(),
            &payload.channel,
            &payload.budget,
            ...
            payload.parent_campaign_name.as_deref(),
            payload.tags,
        ...
        .await
    {
        Ok(campaign) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(campaign)),
        )),
        Err(e) => {
    ...
    match state.crm.marketing_engine.get_campaign(id).await {
        Ok(Some(campaign)) => {
            match state
                .crm
                .marketing_engine
                .delete_campaign(org_id, &campaign.campaign_number)
                .await
            {
                Ok(()) => Ok(StatusCode::NO_CONTENT),
                Err(e) => {
...
pub async fn add_campaign_member(
        ...
        .await
    {
        Ok(member) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(member)),
        )),
        Err(e) => {
...
pub async fn list_campaign_members(
    ...
    match state
        .crm
        .marketing_engine
        .list_campaign_members(campaign_id, params.status.as_deref())
        .await
    {
...
pub async fn update_member_status(
    ...
    match state
        .crm
        .marketing_engine
        .update_member_status(id, &payload.status, payload.response.as_deref())
        .await
    {
...
pub async fn create_campaign_response(
        ...
        .await
    {
        Ok(resp) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(resp)),
        )),
        Err(e) => {
...
pub async fn list_campaign_responses(
    ...
    match state
        .crm
        .marketing_engine
        .list_responses(campaign_id, params.response_type.as_deref())
        .await
    {
