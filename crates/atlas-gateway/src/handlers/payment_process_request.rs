...
pub async fn create_request(
    ...
    let payment_date =
        chrono::NaiveDate::parse_from_str(&req.payment_date, "%Y-%m-%d").map_err(|e| {
            (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": format!("Invalid payment_date: {}", e)})),
            )
        })?;
    let gl_date = chrono::NaiveDate::parse_from_str(&req.gl_date, "%Y-%m-%d").map_err(|e| {
        (
            StatusCode::BAD_REQUEST,
    ...
    let due_from = req
        ...
        .map_err(|e| {
            (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": format!("Invalid due_date_from: {}", e)})),
            )
        })?;
    let due_to = req
        .due_date_to
        ...
        .map_err(|e| {
            (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": format!("Invalid due_date_to: {}", e)})),
            )
        })?;
        ...
        .await
    {
        Ok(ppr) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(ppr).unwrap_or(serde_json::Value::Null)),
        )),
        Err(e) => {
...
pub async fn list_requests(
    ...
    match state
        .financials
        .payment_process_request_engine
        .list_requests(org_id, query.status.as_deref())
        .await
    {
...
pub async fn get_request(
    ...
    match state
        .financials
        .payment_process_request_engine
        .get_request(org_id, id)
        .await
    {
        Ok(ppr) => Ok(Json(
            serde_json::to_value(ppr).unwrap_or(serde_json::Value::Null),
        )),
        Err(e) => {
...
pub async fn cancel_request(
    ...
    match state
        .financials
        .payment_process_request_engine
        .cancel_request(org_id, id, user_id, req.reason.as_deref())
        .await
    {
        Ok(ppr) => Ok(Json(
            serde_json::to_value(ppr).unwrap_or(serde_json::Value::Null),
...
pub async fn add_document(
    ...
    let invoice_id = match parse_uuid(&req.invoice_id) {
        ...
        Err(_) => {
            return Err((
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": "Invalid invoice_id"})),
            ))
        }
    };
    let supplier_id = req
    ...
    let invoice_date = req
        ...
        .map_err(|e| {
            (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": format!("Invalid invoice_date: {}", e)})),
            )
        })?;
    let discount_date = req
        .discount_date
        .as_deref()
        .map(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d"))
        .transpose()
        .map_err(|e| {
            (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": format!("Invalid discount_date: {}", e)})),
            )
        })?;
...
pub async fn remove_document(
    ...
    match state
        .financials
        .payment_process_request_engine
        .remove_document(org_id, ppr_id, doc_id)
        .await
    {
        Ok(()) => Ok(StatusCode::NO_CONTENT),
        Err(e) => {
