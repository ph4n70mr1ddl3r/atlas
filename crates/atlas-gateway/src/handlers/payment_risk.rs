...
pub async fn create_risk_profile(
    ...
    match state
        ...
        .create_risk_profile(
            ...
            body["enable_behavioral_analysis"]
                .as_bool()
                .unwrap_or(false),
        ...
        .await
    {
        Ok(profile) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(profile).unwrap_or(Value::Null)),
        )),
        Err(e) => Err((
...
pub async fn list_risk_profiles(
    ...
    match state
        .financials
        .payment_risk_engine
        .list_risk_profiles(org_id, params.profile_type.as_deref(), params.is_active)
        .await
    {
...
pub async fn set_risk_profile_active(
    ...
    match state
        .financials
        .payment_risk_engine
        .set_risk_profile_active(id, is_active)
        .await
    {
...
pub async fn delete_risk_profile(
    ...
    match state
        .financials
        .payment_risk_engine
        .delete_risk_profile(org_id, &code)
        .await
    {
...
pub async fn create_fraud_alert(
    ...
    if alert_type.is_empty() || severity.is_empty() {
        return Err((
            StatusCode::BAD_REQUEST,
            Json(json!({"error": "alert_type and severity are required"})),
        ));
    }
        ...
        .await
    {
        Ok(alert) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(alert).unwrap_or(Value::Null)),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn get_fraud_alert(
    ...
    match state
        .financials
        .payment_risk_engine
        .get_fraud_alert(org_id, &alert_number)
        .await
    {
        Ok(Some(alert)) => Ok(Json(serde_json::to_value(alert).unwrap_or(Value::Null))),
        Ok(None) => Err((
            StatusCode::NOT_FOUND,
            Json(json!({"error": "Fraud alert not found"})),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn create_screening_result(
    ...
    if screening_type.is_empty() || screened_list.is_empty() {
        return Err((
            StatusCode::BAD_REQUEST,
            Json(json!({"error": "screening_type and screened_list are required"})),
        ));
    }
...
pub async fn get_screening_result(
    ...
    match state
        .financials
        .payment_risk_engine
        .get_screening_result(org_id, &screening_id)
        .await
    {
        Ok(Some(result)) => Ok(Json(serde_json::to_value(result).unwrap_or(Value::Null))),
        Ok(None) => Err((
            StatusCode::NOT_FOUND,
            Json(json!({"error": "Screening result not found"})),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn list_screening_results(
    ...
    match state
        .financials
        .payment_risk_engine
        .list_screening_results(org_id, supplier_id, params.match_status.as_deref())
        .await
    {
...
pub async fn create_assessment(
    ...
    if supplier_name.is_empty() {
        return Err((
            StatusCode::BAD_REQUEST,
            Json(json!({"error": "supplier_name is required"})),
        ));
    }
        ...
        .await
    {
        Ok(assessment) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(assessment).unwrap_or(Value::Null)),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn get_assessment(
    ...
    match state
        .financials
        .payment_risk_engine
        .get_assessment(org_id, &assessment_number)
        .await
    {
        Ok(Some(assessment)) => Ok(Json(
...
pub async fn list_assessments(
    ...
    match state
        .financials
        .payment_risk_engine
        .list_assessments(org_id, supplier_id, params.status.as_deref())
        .await
    {
...
pub async fn transition_assessment(
    ...
    match state
        .financials
        .payment_risk_engine
        .transition_assessment(id, &body.status)
        .await
    {
        Ok(assessment) => Ok(Json(
            serde_json::to_value(assessment).unwrap_or(Value::Null),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
...
pub async fn delete_assessment(
    ...
    match state
        .financials
        .payment_risk_engine
        .delete_assessment(org_id, &assessment_number)
        .await
    {
        Ok(()) => Ok(StatusCode::NO_CONTENT),
        Err(e) => Err((
