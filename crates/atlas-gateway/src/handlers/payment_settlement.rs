    ...
    let settlement_date = chrono::NaiveDate::parse_from_str(&req.settlement_date, "%Y-%m-%d")
        .map_err(|e| {
            (
                ...
                Json(serde_json::json!({"error": format!("Invalid settlement_date: {}", e)})),
            )
        })?;
...
pub async fn get_batch(
    ...
    match state
        ...
        .await
    {
        Ok(Some(batch)) => Ok(Json(
            serde_json::to_value(batch).unwrap_or(serde_json::Value::Null),
        )),
        Ok(None) => Err((
            StatusCode::NOT_FOUND,
            Json(serde_json::json!({"error": "Settlement batch not found"})),
        )),
        Err(e) => {
...
pub async fn cancel_batch(
    ...
    match state
        .financials
        .payment_settlement_engine
        .cancel_batch(id, None, req.reason.as_deref())
        .await
    {
        Ok(batch) => Ok(Json(
            serde_json::to_value(batch).unwrap_or(serde_json::Value::Null),
...
pub async fn add_line(
    ...
    let org_id = parse_uuid(&claims.org_id)?;
    let invoice_id = req.invoice_id.parse::<Uuid>().map_err(|e| {
        (
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({"error": format!("Invalid invoice_id: {}", e)})),
        )
    })?;
