...
pub async fn get_cost_transaction(
        ...
        .await
    {
        ...
        Ok(None) => Err((
            ...
            Json(json!({"error": "Cost transaction not found"})),
        )),
        Err(e) => Err(error_response(e)),
    }
}
...
pub async fn approve_cost_transaction(
    ...
    match state
        .projects
        .project_costing_engine
        .approve_cost_transaction(id, approved_by)
        .await
    {
...
pub async fn reverse_cost_transaction(
    ...
    match state
        .projects
        .project_costing_engine
        .reverse_cost_transaction(org_id, id, reason, created_by)
        .await
    {
        Ok(txn) => Ok(Json(serde_json::to_value(txn).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
...
> {
        ...
        .await
    {
        Ok(schedule) => Ok((
            axum::http::StatusCode::CREATED,
            Json(serde_json::to_value(schedule).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn get_burden_schedule(
    ...
    match state
        .projects
        .project_costing_engine
        .get_burden_schedule(org_id, &code)
        .await
    {
        Ok(Some(schedule)) => Ok(Json(serde_json::to_value(schedule).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err((
            axum::http::StatusCode::NOT_FOUND,
            Json(json!({"error": "Burden schedule not found"})),
        )),
        Err(e) => Err(error_response(e)),
    }
}
...
pub async fn activate_burden_schedule(
    ...
    match state
        .projects
        .project_costing_engine
        .activate_burden_schedule(id)
        .await
    {
        Ok(schedule) => Ok(Json(serde_json::to_value(schedule).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err(error_response(e)),
...
> {
    ...
    match state
        .projects
        .project_costing_engine
        .add_burden_schedule_line(
            org_id,
            schedule_id,
            &req.cost_type,
            req.expenditure_category.as_deref(),
            &req.burden_rate_percent,
...
> {
    ...
    match state
        ...
        .create_cost_adjustment(
            ...
            req.description.as_deref(),
            req.effective_date,
            req.transfer_to_project_id,
            req.transfer_to_task_id,
            created_by,
        )
        .await
    {
        Ok(adj) => Ok((
            axum::http::StatusCode::CREATED,
            Json(serde_json::to_value(adj).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn list_cost_adjustments(
    ...
    match state
        .projects
        .project_costing_engine
        .list_cost_adjustments(org_id, status)
        .await
    {
...
pub async fn approve_cost_adjustment(
    ...
    match state
        .projects
        .project_costing_engine
        .approve_cost_adjustment(id, approved_by)
        .await
    {
        Ok(adj) => Ok(Json(serde_json::to_value(adj).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => {
...
pub async fn distribute_cost_transaction(
    ...
    match state
        .projects
        .project_costing_engine
        .distribute_cost_transaction(
            org_id,
            id,
            &req.raw_cost_account,
            &req.burden_account,
            &req.ap_ar_account,
        )
        .await
...
pub async fn post_distributions(
    ...
    match state
        .projects
        .project_costing_engine
        .post_distributions(org_id, gl_batch_id)
        .await
    {
