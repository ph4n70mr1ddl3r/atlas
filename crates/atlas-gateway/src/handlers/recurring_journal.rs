...
pub async fn create_schedule(
    ...
    let recurrence_type = body["recurrence_type"]
        ...
        .unwrap_or("monthly")
        .to_string();
    let journal_type = body["journal_type"]
        .as_str()
        .unwrap_or("standard")
        .to_string();
...
pub async fn activate_schedule(
    ...
    match state
        .financials
        .recurring_journal_engine
        .activate_schedule(id, Some(approved_by))
        .await
    {
...
pub async fn generate_journal(
    ...
    let generation_date = body["generation_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok())
        .unwrap_or_else(|| chrono::Utc::now().date_naive());

    let override_amounts: Option<Vec<(i32, String)>> = body
        .get("override_amounts")
        ...
        .await
    {
        Ok(gen) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(gen).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn get_generation(
    ...
    match state
        ...
        .await
    {
        Ok(Some(gen)) => Ok(Json(serde_json::to_value(gen).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err((
...
pub async fn post_generation(
    ...
    match state
        ...
        .await
    {
        Ok(gen) => Ok(Json(serde_json::to_value(gen).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err((
...
pub async fn list_generation_lines(
    ...
    match state
        .financials
        .recurring_journal_engine
        .list_generation_lines(generation_id)
        .await
    {
        Ok(lines) => Ok(Json(json!({"data": lines}))),
        Err(e) => Err((
