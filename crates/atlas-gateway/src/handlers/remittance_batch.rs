...
pub async fn create_batch(
    ...
    let batch_date = body["batch_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok())
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "batch_date is required (YYYY-MM-DD)"})),
            )
        })?;
    let gl_date = body["gl_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
        ...
        .await
    {
        Ok(batch) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(batch).unwrap_or(Value::Null)),
        )),
        Err(e) => Err((
...
pub async fn get_batch(
    ...
    match state.financials.remittance_batch_engine.get_batch(id).await {
        ...
        Ok(None) => Err((
            ...
            Json(json!({"error": "Remittance batch not found"})),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn list_batches(
    ...
    match state
        ...
        .list_batches(
            ...
            params.remittance_method.as_deref(),
        )
        .await
    {
...
pub async fn transmit_batch(
    ...
    match state
        .financials
        .remittance_batch_engine
        .transmit_batch(id, reference_number)
        .await
    {
        Ok(batch) => Ok(Json(serde_json::to_value(batch).unwrap_or(Value::Null))),
        Err(e) => Err((
...
pub async fn add_receipt(
    ...
    let receipt_id = body["receipt_id"]
        ...
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "receipt_id is required"})),
            )
        })?;
    ...
    let receipt_date = body["receipt_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
...
pub async fn remove_receipt(
    ...
    match state
        .financials
        .remittance_batch_engine
        .remove_receipt(batch_id, receipt_id)
        .await
    {
