...
pub async fn create_policy(
    ...
    match state
        ...
        .create_policy(
            ...
            payload
                .allocation_basis
                .as_deref()
                .unwrap_or("standalone_selling_price"),
        ...
        .await
    {
        ...
        Err(e) => {
            error!("Failed to create revenue policy: {}", e);
            Err(rev_map_err(e))
        }
    }
}
...
pub async fn list_policies(
    ...
    match state.financials.revenue_engine.list_policies(org_id).await {
        ...
        Err(e) => {
            error!("Failed to list revenue policies: {}", e);
...
pub async fn get_policy(
        ...
        .await
    {
        ...
        Err(e) => {
            error!("Failed to get revenue policy: {}", e);
            Err(StatusCode::INTERNAL_SERVER_ERROR)
        }
    }
}
...
pub async fn delete_policy(
        ...
        .await
    {
        ...
        Err(e) => {
            error!("Failed to delete revenue policy: {}", e);
            Err(rev_map_err(e))
        }
    }
}
...
pub async fn list_contracts(
    ...
    match state
        .financials
        .revenue_engine
        .list_contracts(org_id, params.status.as_deref(), params.customer_id)
        .await
    {
        Ok(contracts) => Ok(Json(serde_json::json!({ "data": contracts }))),
        Err(e) => {
            error!("Failed to list revenue contracts: {}", e);
            Err(rev_map_err(e))
        }
    }
}
...
pub async fn get_contract(
    ...
    match state.financials.revenue_engine.get_contract(id).await {
        ...
        Ok(None) => Err(StatusCode::NOT_FOUND),
        Err(e) => {
            error!("Failed to get revenue contract: {}", e);
            Err(StatusCode::INTERNAL_SERVER_ERROR)
        }
    }
}
...
pub async fn activate_contract(
    ...
    match state.financials.revenue_engine.activate_contract(id).await {
        ...
        Err(e) => {
            error!("Failed to activate revenue contract: {}", e);
            Err(rev_map_err(e))
        }
    }
}
...
pub async fn cancel_contract(
    ...
    match state
        .financials
        .revenue_engine
        .cancel_contract(id, payload.reason.as_deref())
        .await
    {
        Ok(c) => Ok(Json(crate::handlers::records::to_json_or_null(c))),
        Err(e) => {
            error!("Failed to cancel revenue contract: {}", e);
            Err(rev_map_err(e))
        }
    }
}
...
pub async fn create_obligation(
        ...
        .await
    {
        Ok(o) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(o)),
        )),
        Err(e) => {
...
pub async fn list_obligations(
    ...
    match state
        .financials
        .revenue_engine
        .list_obligations(contract_id)
        .await
    {
        Ok(obligations) => Ok(Json(serde_json::json!({ "data": obligations }))),
        Err(e) => {
            error!("Failed to list obligations: {}", e);
            Err(StatusCode::INTERNAL_SERVER_ERROR)
        }
    }
}
...
pub async fn get_obligation(
    ...
    match state.financials.revenue_engine.get_obligation(id).await {
        ...
        Ok(None) => Err(StatusCode::NOT_FOUND),
        Err(e) => {
            error!("Failed to get obligation: {}", e);
            Err(StatusCode::INTERNAL_SERVER_ERROR)
        }
    }
}
...
pub async fn allocate_transaction_price(
    ...
    match state
        .financials
        .revenue_engine
        .allocate_transaction_price(contract_id)
        .await
    {
        Ok(obligations) => Ok(Json(serde_json::json!({ "data": obligations }))),
        Err(e) => {
            error!("Failed to allocate transaction price: {}", e);
            Err(rev_map_err(e))
        }
    }
}
...
pub async fn generate_straight_line_schedule(
    ...
    match state
        .financials
        .revenue_engine
        .generate_straight_line_schedule(obligation_id, payload.start_date, payload.end_date)
        .await
    {
...
pub async fn schedule_point_in_time(
    ...
    match state
        .financials
        .revenue_engine
        .schedule_point_in_time(obligation_id, payload.recognition_date)
        .await
    {
        Ok(line) => Ok((
            StatusCode::CREATED,
        ...
        Err(e) => {
            error!("Failed to schedule point-in-time: {}", e);
            Err(rev_map_err(e))
        }
    }
}
...
pub async fn reverse_recognition(
    ...
    match state
        .financials
        .revenue_engine
        .reverse_recognition(line_id, &payload.reason)
        .await
    {
        Ok(line) => Ok(Json(crate::handlers::records::to_json_or_null(line))),
        Err(e) => {
            error!("Failed to reverse recognition: {}", e);
            Err(rev_map_err(e))
        }
    }
}
...
pub async fn list_schedule_lines(
    ...
    match state
        .financials
        .revenue_engine
        .list_schedule_lines(obligation_id)
        .await
    {
        Ok(lines) => Ok(Json(serde_json::json!({ "data": lines }))),
        Err(e) => {
...
pub async fn list_contract_schedule_lines(
    ...
    match state
        .financials
        .revenue_engine
        .list_contract_schedule_lines(contract_id)
        .await
    {
        Ok(lines) => Ok(Json(serde_json::json!({ "data": lines }))),
        Err(e) => {
            error!("Failed to list contract schedule lines: {}", e);
            Err(StatusCode::INTERNAL_SERVER_ERROR)
        }
    }
}
...
pub async fn create_modification(
        ...
        .await
    {
        ...
        Err(e) => {
            error!("Failed to create modification: {}", e);
            Err(rev_map_err(e))
        }
    }
}
...
pub async fn list_modifications(
    ...
    match state
        .financials
        .revenue_engine
        .list_modifications(contract_id)
        .await
    {
        Ok(modifications) => Ok(Json(serde_json::json!({ "data": modifications }))),
        Err(e) => {
            error!("Failed to list modifications: {}", e);
            Err(StatusCode::INTERNAL_SERVER_ERROR)
        }
    }
}
