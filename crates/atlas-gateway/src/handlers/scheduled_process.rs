...
pub async fn create_template(
    ...
    let process_type = body["process_type"]
        .as_str()
        .unwrap_or("report")
        .to_string();
    let executor_type = body["executor_type"]
        .as_str()
        .unwrap_or("built_in")
        .to_string();
    ...
    let approval_chain_id = body["approval_chain_id"]
        .as_str()
...
pub async fn list_templates(
    ...
    match state
        .shared
        .scheduled_process_engine
        .list_templates(org_id, params.process_type.as_deref(), params.is_active)
        .await
    {
...
pub async fn activate_template(
    ...
    match state
        ...
        .await
    {
        Ok(template) => Ok(Json(serde_json::to_value(template).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err((
...
pub async fn submit_process(
    ...
    let scheduled_start_at = body["scheduled_start_at"]
        .as_str()
        ...
        .await
    {
        Ok(process) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(process).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn get_process(
    ...
    match state.shared.scheduled_process_engine.get_process(id).await {
        Ok(Some(process)) => Ok(Json(serde_json::to_value(process).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err((
...
pub async fn start_process(
    ...
    match state
        ...
        .await
    {
        Ok(process) => Ok(Json(serde_json::to_value(process).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err((
...
pub async fn complete_process(
    ...
    match state
        .shared
        .scheduled_process_engine
        .complete_process(id, result_summary, output_file_url, log_output)
        .await
    {
        Ok(process) => Ok(Json(serde_json::to_value(process).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
...
pub async fn cancel_process(
    ...
    match state
        .shared
        .scheduled_process_engine
        .cancel_process(id, cancelled_by, reason)
        .await
    {
        Ok(process) => Ok(Json(serde_json::to_value(process).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
...
pub async fn update_progress(
    ...
    match state
        .shared
        .scheduled_process_engine
        .update_progress(id, progress_percent)
        .await
    {
        Ok(process) => Ok(Json(serde_json::to_value(process).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
...
pub async fn create_recurrence(
    ...
    let parameters = body.get("parameters").cloned().unwrap_or(json!({}));
    let recurrence_type = body["recurrence_type"]
        .as_str()
        .unwrap_or("daily")
        .to_string();
        ...
        .await
    {
        Ok(recurrence) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(recurrence).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn get_recurrence(
    ...
    match state
        ...
        .await
    {
        Ok(Some(recurrence)) => Ok(Json(serde_json::to_value(recurrence).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err((
            StatusCode::NOT_FOUND,
            Json(json!({"error": "Recurrence not found"})),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn list_recurrences(
    ...
    match state
        .shared
        .scheduled_process_engine
        .list_recurrences(org_id, params.is_active)
        .await
    {
        Ok(recurrences) => Ok(Json(
            json!({"data": recurrences, "total": recurrences.len()}),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn deactivate_recurrence(
    ...
    match state
        .shared
        .scheduled_process_engine
        .deactivate_recurrence(id)
        .await
    {
        Ok(recurrence) => Ok(Json(serde_json::to_value(recurrence).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err((
...
pub async fn list_process_logs(
    ...
    match state
        .shared
        .scheduled_process_engine
        .list_logs(id, params.log_level.as_deref(), params.limit)
        .await
    {
...
pub async fn add_process_log(
        ...
        .await
    {
        Ok(log) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(log).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
