...
pub async fn create_sod_rule(
    ...
    let first_duties: Vec<String> = body["first_duties"]
        .as_array()
        .map(|arr| {
            arr.iter()
        ...
        .unwrap_or_default();
    let second_duties: Vec<String> = body["second_duties"]
        .as_array()
        .map(|arr| {
            arr.iter()
                .filter_map(|v| v.as_str().map(String::from))
        ...
        .unwrap_or_default();
    let enforcement_mode = body["enforcement_mode"]
        .as_str()
        .unwrap_or("detective")
        .to_string();
...
pub async fn list_sod_assignments(
    ...
    match state
        .shared
        .sod_engine
        .list_role_assignments(org_id, user_id)
        .await
    {
...
pub async fn remove_sod_assignment(
    ...
    match state.shared.sod_engine.remove_role_assignment(id).await {
        Ok(assignment) => Ok(Json(serde_json::to_value(assignment).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err((
...
pub async fn get_sod_violation(
    ...
    match state.shared.sod_engine.get_violation(id).await {
        Ok(Some(v)) => Ok(Json(serde_json::to_value(v).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err((
...
pub async fn resolve_sod_violation(
    ...
    match state
        .shared
        .sod_engine
        .resolve_violation(id, resolved_by)
        .await
    {
        Ok(v) => Ok(Json(serde_json::to_value(v).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err((
...
pub async fn accept_sod_exception(
    ...
    match state
        .shared
        .sod_engine
        .accept_exception(id, accepted_by)
        .await
    {
        Ok(v) => Ok(Json(serde_json::to_value(v).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
...
pub async fn create_sod_mitigation(
    ...
    let violation_id = body["violation_id"]
        ...
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "violation_id is required"})),
            )
        })?;
    let control_name = body["control_name"].as_str().unwrap_or("").to_string();
    let control_description = body["control_description"]
        .as_str()
        .unwrap_or("")
        .to_string();
    let control_owner_id = body["control_owner_id"]
        .as_str()
        .and_then(|s| Uuid::parse_str(s).ok());
    let review_frequency = body["review_frequency"]
        .as_str()
        .unwrap_or("monthly")
        .to_string();
    let effective_from = body["effective_from"]
        ...
        .await
    {
        Ok(control) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(control).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn list_sod_mitigations(
    ...
    match state
        .shared
        .sod_engine
        .get_mitigating_controls(violation_id)
        .await
    {
...
pub async fn approve_sod_mitigation(
    ...
    match state
        .shared
        .sod_engine
        .approve_mitigating_control(id, approved_by)
        .await
    {
        Ok(control) => Ok(Json(serde_json::to_value(control).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err((
