...
pub async fn create_carrier(
    ...
    match state
        ...
        .create_carrier(
            ...
            payload.tracking_url_template.as_deref(),
        ...
        .await
    {
        ...
        Err(e) => {
            error!("Failed to create carrier: {}", e);
...
pub async fn create_shipping_method(
    ...
    match state
        ...
        .create_method(
            ...
            payload.description.as_deref(),
            payload.carrier_id,
            payload.transit_time_days.unwrap_or(1),
            payload.is_express.unwrap_or(false),
            user_id,
        )
        .await
    {
...
pub async fn create_shipment(
    ...
    match state
        ...
        .create_shipment(
            ...
            payload.shipping_method_name.as_deref(),
            payload.order_id,
            payload.order_number.as_deref(),
            payload.customer_id,
            payload.customer_name.as_deref(),
            payload.ship_from_warehouse.as_deref(),
            payload.ship_to_name.as_deref(),
            payload.ship_to_address.as_deref(),
            payload.ship_to_city.as_deref(),
            payload.ship_to_state.as_deref(),
            payload.ship_to_postal_code.as_deref(),
            payload.ship_to_country.as_deref(),
...
pub async fn list_shipments(
    ...
    match state
        .scm
        .shipping_engine
        .list_shipments(org_id, query.status.as_deref())
        .await
    {
...
pub async fn ship_confirm(
    ...
    match state
        .scm
        .shipping_engine
        .ship_confirm(id, tracking.as_deref(), user_id)
        .await
    {
        Ok(s) => Ok(Json(crate::handlers::records::to_json_or_null(s))),
        Err(e) => {
...
pub async fn add_shipment_line(
    ...
    Json(payload): Json<AddShipmentLineRequest>,
) -> Result<(StatusCode, Json<serde_json::Value>), StatusCode> {
    let org_id = Uuid::parse_str(&claims.org_id).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    match state
        .scm
        ...
        .add_shipment_line(
            ...
            payload.is_fragile.unwrap_or(false),
            payload.is_hazardous.unwrap_or(false),
            payload.notes.as_deref(),
...
pub async fn list_shipment_lines(
    ...
    match state
        .scm
        .shipping_engine
        .list_shipment_lines(shipment_id)
        .await
    {
...
pub async fn delete_shipment_line(
    State(state): State<Arc<AppState>>,
    _claims: Extension<Claims>,
    Path(id): Path<Uuid>,
) -> Result<StatusCode, StatusCode> {
...
#[derive(Debug, Deserialize)]
pub struct UpdateShippedQtyRequest {
...
pub async fn update_shipped_quantity(
    ...
    match state
        .scm
        .shipping_engine
        .update_line_shipped_quantity(id, &payload.shipped_quantity)
        .await
    {
...
pub struct CreatePackingSlipRequest {
    ...
    pub weight: Option<String>,
    pub weight_unit: Option<String>,
    pub dimensions_length: Option<String>,
    pub dimensions_width: Option<String>,
    pub dimensions_height: Option<String>,
...
pub async fn create_packing_slip(
    ...
    match state
        ...
        .create_packing_slip(
            org_id,
            shipment_id,
            &payload.packing_slip_number,
            payload.package_number.unwrap_or(1),
            payload.package_type.as_deref(),
            payload.weight.as_deref(),
            payload.weight_unit.as_deref(),
            payload.dimensions_length.as_deref(),
            payload.dimensions_width.as_deref(),
            payload.dimensions_height.as_deref(),
            payload.dimensions_unit.as_deref(),
            payload.notes.as_deref(),
            user_id,
        )
        .await
    {
        Ok(ps) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(ps)),
        )),
        Err(e) => {
            error!("Failed: {}", e);
            Err(ship_map_err(e))
...
pub async fn list_packing_slips(
    ...
    match state
        .scm
        .shipping_engine
        .list_packing_slips(shipment_id)
        .await
    {
...
pub async fn delete_packing_slip(
    State(state): State<Arc<AppState>>,
    _claims: Extension<Claims>,
    Path(id): Path<Uuid>,
) -> Result<StatusCode, StatusCode> {
...
pub async fn add_packing_slip_line(
    State(state): State<Arc<AppState>>,
    claims: Extension<Claims>,
    Path(packing_slip_id): Path<Uuid>,
    Json(payload): Json<AddPackingSlipLineRequest>,
) -> Result<(StatusCode, Json<serde_json::Value>), StatusCode> {
    let org_id = Uuid::parse_str(&claims.org_id).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    match state
        .scm
...
pub async fn delete_packing_slip_line(
    State(state): State<Arc<AppState>>,
    _claims: Extension<Claims>,
    Path(id): Path<Uuid>,
) -> Result<StatusCode, StatusCode> {
...
pub async fn get_shipping_dashboard(
    State(state): State<Arc<AppState>>,
    claims: Extension<Claims>,
) -> Result<Json<serde_json::Value>, StatusCode> {
    let org_id = Uuid::parse_str(&claims.org_id).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
