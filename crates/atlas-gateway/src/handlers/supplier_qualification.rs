...
pub async fn create_qualification_area(
    ...
    let area_type = body["area_type"]
        .as_str()
        .unwrap_or("questionnaire")
        .to_string();
    let scoring_model = body["scoring_model"]
        .as_str()
        .unwrap_or("manual")
        .to_string();
        ...
        .await
    {
        Ok(area) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(area).unwrap_or_else(|e| {
...
pub async fn get_qualification_area(
    ...
    match state
        .scm
        .supplier_qualification_engine
        .get_area(org_id, &code)
        .await
    {
        Ok(Some(area)) => Ok(Json(serde_json::to_value(area).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err((
...
pub async fn create_qualification_question(
        ...
        .await
    {
        Ok(question) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(question).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn create_initiative(
    ...
    let name = body["name"].as_str().unwrap_or("").to_string();
    let description = body["description"].as_str();
    let area_id: Uuid = body["area_id"]
        .as_str()
        .and_then(|s| s.parse().ok())
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "area_id is required"})),
            )
        })?;
    let qualification_purpose = body["qualification_purpose"]
        .as_str()
        .unwrap_or("new_supplier")
        .to_string();
        ...
        .await
    {
        Ok(initiative) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(initiative).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn get_initiative(
    ...
    match state
        ...
        .await
    {
        Ok(Some(initiative)) => Ok(Json(serde_json::to_value(initiative).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err((
            StatusCode::NOT_FOUND,
            Json(json!({"error": "Initiative not found"})),
        )),
        Err(e) => Err((
            StatusCode::from_u16(e.status_code()).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR),
            Json(json!({"error": e.to_string()})),
...
pub async fn list_initiatives(
    ...
    match state
        .scm
        .supplier_qualification_engine
        .list_initiatives(org_id, params.status.as_deref())
        .await
    {
...
pub async fn activate_initiative(
    ...
    match state
        .scm
        .supplier_qualification_engine
        .activate_initiative(id)
        .await
    {
        Ok(initiative) => Ok(Json(serde_json::to_value(initiative).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err((
...
pub async fn invite_supplier(
    ...
    let supplier_id: Uuid = body["supplier_id"]
        .as_str()
        .and_then(|s| s.parse().ok())
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "supplier_id is required"})),
            )
        })?;
        ...
        .await
    {
        Ok(invitation) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(invitation).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn list_invitations(
    ...
    match state
        .scm
        .supplier_qualification_engine
        .list_invitations(initiative_id)
        .await
    {
...
pub async fn submit_invitation_response(
    ...
    match state
        .scm
        .supplier_qualification_engine
        .submit_response(invitation_id)
        .await
    {
        Ok(invitation) => Ok(Json(serde_json::to_value(invitation).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err((
...
pub async fn start_evaluation(
    ...
    match state
        .scm
        .supplier_qualification_engine
        .start_evaluation(invitation_id)
        .await
    {
        Ok(invitation) => Ok(Json(serde_json::to_value(invitation).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
...
pub async fn qualify_supplier(
    ...
    match state
        .scm
        .supplier_qualification_engine
        .qualify_supplier(invitation_id, Some(user_id), evaluation_notes)
        .await
    {
        Ok(invitation) => Ok(Json(serde_json::to_value(invitation).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
...
pub async fn disqualify_supplier(
    ...
    match state
        .scm
        .supplier_qualification_engine
        .disqualify_supplier(invitation_id, reason, Some(user_id))
        .await
    {
        Ok(invitation) => Ok(Json(serde_json::to_value(invitation).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
...
pub async fn create_response(
    ...
    let question_id: Uuid = body["question_id"]
        .as_str()
        .and_then(|s| s.parse().ok())
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "question_id is required"})),
            )
        })?;
...
pub async fn score_response(
    ...
    match state
        .scm
        .supplier_qualification_engine
        .score_response(response_id, score, evaluator_notes, Some(user_id))
        .await
    {
        Ok(response) => Ok(Json(serde_json::to_value(response).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err((
...
pub async fn create_certification(
    ...
    let supplier_name = body["supplier_name"].as_str().unwrap_or("").to_string();
    let certification_type = body["certification_type"]
        .as_str()
        .unwrap_or("")
        .to_string();
    let certification_name = body["certification_name"]
        .as_str()
        .unwrap_or("")
        .to_string();
    let certifying_body = body["certifying_body"].as_str();
    let certificate_number = body["certificate_number"].as_str();
    let issued_date = body["issued_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
    let expiry_date = body["expiry_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
    let renewal_date = body["renewal_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok());
    let qualification_invitation_id = body["qualification_invitation_id"]
        .as_str()
        .and_then(|s| s.parse().ok());
        ...
        .await
    {
        Ok(cert) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(cert).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn list_certifications(
    ...
    match state
        .scm
        .supplier_qualification_engine
        .list_certifications(org_id, params.supplier_id, params.status.as_deref())
        .await
    {
...
pub async fn revoke_certification(
    ...
    match state
        .scm
        .supplier_qualification_engine
        .revoke_certification(id)
        .await
    {
        Ok(cert) => Ok(Json(serde_json::to_value(cert).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => Err((
...
pub async fn renew_certification(
    ...
    let new_expiry = match body["expiry_date"]
        .as_str()
        .and_then(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d").ok())
    {
        Some(d) => d,
        None => {
            return Err((
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "expiry_date is required"})),
            ))
        }
    };
    ...
    match state
        .scm
        .supplier_qualification_engine
        .renew_certification(id, new_expiry, new_cert_number)
        .await
    {
        Ok(cert) => Ok(Json(serde_json::to_value(cert).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
