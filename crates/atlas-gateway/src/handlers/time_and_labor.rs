...
const fn default_eight() -> f64 {
    8.0
}
const fn default_forty() -> f64 {
    40.0
}
const fn default_five() -> i32 {
    5
}
const fn default_sixty() -> i32 {
    60
}
...
pub async fn create_work_schedule(
    ...
    match state
        ...
        .create_work_schedule(
            ...
            payload.standard_hours_per_day,
            payload.standard_hours_per_week,
            payload.work_days_per_week,
            payload.start_time,
            payload.end_time,
            payload.break_duration_minutes,
            Some(user_id),
        )
        .await
    {
        Ok(s) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(s)),
        )),
        Err(e) => {
            error!("Failed to create work schedule: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn get_work_schedule(
    ...
    match state
        .hcm
        .time_and_labor_engine
        .get_work_schedule(org_id, &code)
        .await
    {
        Ok(Some(s)) => Ok(Json(crate::handlers::records::to_json_or_null(s))),
        Ok(None) => Err(StatusCode::NOT_FOUND),
        Err(e) => {
            error!("Failed to get work schedule: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn list_work_schedules(
    ...
    match state
        .hcm
        .time_and_labor_engine
        .list_work_schedules(org_id)
        .await
    {
        Ok(schedules) => Ok(Json(serde_json::json!({ "data": schedules }))),
        Err(e) => {
            error!("Failed to list work schedules: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn delete_work_schedule(
    ...
    match state
        .hcm
        .time_and_labor_engine
        .delete_work_schedule(org_id, &code)
        .await
    {
        Ok(()) => Ok(StatusCode::NO_CONTENT),
        Err(e) => {
            error!("Failed to delete work schedule: {}", e);
            Err(map_error(e))
        }
    }
}
...
fn default_weekly_threshold() -> String {
    "weekly".to_string()
}
const fn default_one_point_five() -> f64 {
    1.5
}
const fn default_two() -> f64 {
    2.0
}
...
pub async fn create_overtime_rule(
    ...
    match state
        ...
        .create_overtime_rule(
            ...
            payload.daily_threshold_hours,
            payload.weekly_threshold_hours,
            payload.overtime_multiplier,
            payload.double_time_threshold_hours,
            payload.double_time_multiplier,
            payload.include_holidays,
            payload.include_weekends,
        ...
        .await
    {
        ...
        Err(e) => {
            error!("Failed to create overtime rule: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn get_overtime_rule(
    ...
    match state
        .hcm
        .time_and_labor_engine
        .get_overtime_rule(org_id, &code)
        .await
    {
        Ok(Some(r)) => Ok(Json(crate::handlers::records::to_json_or_null(r))),
        Ok(None) => Err(StatusCode::NOT_FOUND),
        Err(e) => {
            error!("Failed to get overtime rule: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn list_overtime_rules(
    ...
    match state
        .hcm
        .time_and_labor_engine
        .list_overtime_rules(org_id)
        .await
    {
        Ok(rules) => Ok(Json(serde_json::json!({ "data": rules }))),
        Err(e) => {
            error!("Failed to list overtime rules: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn delete_overtime_rule(
    ...
    match state
        .hcm
        .time_and_labor_engine
        .delete_overtime_rule(org_id, &code)
        .await
    {
        Ok(()) => Ok(StatusCode::NO_CONTENT),
        Err(e) => {
            error!("Failed to delete overtime rule: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn create_time_card(
    ...
    match state
        ...
        .create_time_card(
            ...
            payload.period_end,
            payload.schedule_code.as_deref(),
            payload.overtime_rule_code.as_deref(),
            Some(user_id),
        )
        .await
    {
        ...
        Err(e) => {
            error!("Failed to create time card: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn list_time_cards(
    ...
    match state
        .hcm
        .time_and_labor_engine
        .list_time_cards(org_id, params.employee_id, params.status.as_deref())
        .await
    {
        Ok(cards) => Ok(Json(serde_json::json!({ "data": cards }))),
        Err(e) => {
            error!("Failed to list time cards: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn submit_time_card(
    ...
    match state
        .hcm
        .time_and_labor_engine
        .submit_time_card(org_id, id, Some(user_id))
        .await
    {
        Ok(c) => Ok(Json(crate::handlers::records::to_json_or_null(c))),
        Err(e) => {
            error!("Failed to submit time card: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn approve_time_card(
    ...
    match state
        .hcm
        .time_and_labor_engine
        .approve_time_card(org_id, id, user_id)
        .await
    {
        Ok(c) => Ok(Json(crate::handlers::records::to_json_or_null(c))),
        Err(e) => {
            error!("Failed to approve time card: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn reject_time_card(
    ...
    match state
        .hcm
        .time_and_labor_engine
        .reject_time_card(org_id, id, user_id, payload.reason.as_deref())
        .await
    {
        Ok(c) => Ok(Json(crate::handlers::records::to_json_or_null(c))),
        Err(e) => {
            error!("Failed to reject time card: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn cancel_time_card(
    ...
    match state
        .hcm
        .time_and_labor_engine
        .cancel_time_card(org_id, id, payload.reason.as_deref())
        .await
    {
        Ok(c) => Ok(Json(crate::handlers::records::to_json_or_null(c))),
        Err(e) => {
            error!("Failed to cancel time card: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn create_time_entry(
    ...
    match state
        ...
        .create_time_entry(
            ...
            payload.task_name.as_deref(),
            payload.location.as_deref(),
            payload.cost_center.as_deref(),
            payload.labor_category.as_deref(),
        ...
        .await
    {
        Ok(e) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(e)),
        )),
        Err(e2) => {
            error!("Failed to create time entry: {}", e2);
            Err(map_error(e2))
        }
    }
}
...
pub async fn list_time_entries(
    ...
    match state
        .hcm
        .time_and_labor_engine
        .list_time_entries(org_id, time_card_id)
        .await
    {
        Ok(entries) => Ok(Json(serde_json::json!({ "data": entries }))),
        Err(e) => {
            error!("Failed to list time entries: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn delete_time_entry(
    ...
    match state
        .hcm
        .time_and_labor_engine
        .delete_time_entry(org_id, id)
        .await
    {
        Ok(()) => Ok(StatusCode::NO_CONTENT),
        Err(e) => {
            error!("Failed to delete time entry: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn get_time_card_history(
    ...
    match state
        .hcm
        .time_and_labor_engine
        .get_time_card_history(org_id, time_card_id)
        .await
    {
        Ok(history) => Ok(Json(serde_json::json!({ "data": history }))),
        Err(e) => {
            error!("Failed to get time card history: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn create_labor_distribution(
        ...
        .await
    {
        ...
        Err(e) => {
            error!("Failed to create labor distribution: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn list_labor_distributions(
    ...
    match state
        .hcm
        .time_and_labor_engine
        .list_labor_distributions(org_id, time_entry_id)
        .await
    {
        Ok(dists) => Ok(Json(serde_json::json!({ "data": dists }))),
        Err(e) => {
            error!("Failed to list labor distributions: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn delete_labor_distribution(
    ...
    match state
        .hcm
        .time_and_labor_engine
        .delete_labor_distribution(org_id, id)
        .await
    {
        Ok(()) => Ok(StatusCode::NO_CONTENT),
        Err(e) => {
            error!("Failed to delete labor distribution: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn get_time_and_labor_dashboard(
    ...
    let org_id = claims.org_uuid()?;
    match state.hcm.time_and_labor_engine.get_dashboard(org_id).await {
        Ok(dashboard) => Ok(Json(crate::handlers::records::to_json_or_null(dashboard))),
        Err(e) => {
            error!("Failed to get time and labor dashboard: {}", e);
            Err(map_error(e))
        }
    }
}
