...
pub async fn create_policy(
    ...
    let from_entity_id = body["from_entity_id"]
        .as_str()
        .and_then(|s| Uuid::parse_str(s).ok());
    let from_entity_name = body["from_entity_name"].as_str();
    let to_entity_id = body["to_entity_id"]
        .as_str()
        .and_then(|s| Uuid::parse_str(s).ok());
...
pub async fn list_policies(
    ...
    match state
        .shared
        .transfer_pricing_engine
        .list_policies(org_id, params.status.as_deref())
        .await
    {
...
pub async fn create_transaction(
    ...
    let transaction_date: chrono::NaiveDate = body["transaction_date"]
        .as_str()
        .and_then(|s| s.parse().ok())
        .ok_or_else(|| {
            (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "transaction_date is required (YYYY-MM-DD)"})),
            )
        })?;
...
pub async fn get_transaction(
    ...
    match state
        ...
        .await
    {
        Ok(Some(txn)) => Ok(Json(serde_json::to_value(txn).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err((
...
pub async fn list_transactions(
    ...
    match state
        .shared
        .transfer_pricing_engine
        .list_transactions(org_id, params.status.as_deref(), policy_id)
        .await
    {
...
pub async fn submit_transaction(
    ...
    match state
        .shared
        .transfer_pricing_engine
        .submit_transaction(id)
        .await
    {
        Ok(txn) => Ok(Json(serde_json::to_value(txn).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => {
...
pub async fn approve_transaction(
    ...
    match state
        .shared
        .transfer_pricing_engine
        .approve_transaction(id, approved_by)
        .await
    {
        Ok(txn) => Ok(Json(serde_json::to_value(txn).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
...
pub async fn create_benchmark(
    ...
    let prepared_by = body["prepared_by"]
        .as_str()
        .and_then(|s| Uuid::parse_str(s).ok());
        ...
        .await
    {
        Ok(bm) => Ok((
            StatusCode::CREATED,
            Json(serde_json::to_value(bm).unwrap_or_else(|e| {
                tracing::error!("Serialization error: {}", e);
                serde_json::Value::Null
            })),
        )),
...
pub async fn get_benchmark(
    ...
    match state.shared.transfer_pricing_engine.get_benchmark(id).await {
        Ok(Some(bm)) => Ok(Json(serde_json::to_value(bm).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err((
            StatusCode::NOT_FOUND,
            Json(json!({"error": "Benchmark not found"})),
        )),
        Err(e) => Err((
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error": format!("{}", e)})),
...
pub async fn list_benchmarks(
    ...
    match state
        .shared
        .transfer_pricing_engine
        .list_benchmarks(org_id, params.status.as_deref())
        .await
    {
...
pub async fn submit_benchmark(
    ...
    match state
        .shared
        .transfer_pricing_engine
        .submit_benchmark_for_review(id)
        .await
    {
        Ok(bm) => Ok(Json(serde_json::to_value(bm).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Err(e) => {
...
pub async fn approve_benchmark(
    ...
    match state
        .shared
        .transfer_pricing_engine
        .approve_benchmark(id, reviewed_by, None)
        .await
    {
        Ok(bm) => Ok(Json(serde_json::to_value(bm).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
...
pub async fn get_documentation(
    ...
    match state
        ...
        .await
    {
        Ok(Some(doc)) => Ok(Json(serde_json::to_value(doc).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
            serde_json::Value::Null
        }))),
        Ok(None) => Err((
...
pub async fn list_documentation(
    ...
    match state
        .shared
        .transfer_pricing_engine
        .list_documentation(org_id, params.doc_type.as_deref(), params.status.as_deref())
        .await
    {
...
pub async fn submit_documentation(
    ...
    match state
        .shared
        .transfer_pricing_engine
        .submit_documentation_for_review(id)
        .await
    {
...
pub async fn approve_documentation(
    ...
    match state
        .shared
        .transfer_pricing_engine
        .approve_documentation(id, approved_by)
        .await
    {
        Ok(doc) => Ok(Json(serde_json::to_value(doc).unwrap_or_else(|e| {
            tracing::error!("Serialization error: {}", e);
