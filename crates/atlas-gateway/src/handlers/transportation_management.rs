...
pub async fn toggle_carrier_service(
    ...
    let service = state
        .scm
        .transportation_engine
        .toggle_carrier_service(id, is_active)
        .await
        .map_err(|e| match e {
            atlas_shared::AtlasError::EntityNotFound(_) => StatusCode::NOT_FOUND,
            _ => StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn start_transit(
    ...
    let shipment = state
        .scm
        .transportation_engine
        .start_transit(id, body["driverName"].as_str(), body["vehicleId"].as_str())
        .await
        .map_err(|e| match e {
            atlas_shared::AtlasError::EntityNotFound(_) => StatusCode::NOT_FOUND,
            atlas_shared::AtlasError::ValidationFailed(_) => StatusCode::BAD_REQUEST,
...
pub async fn confirm_delivery(
    ...
    let shipment = state
        .scm
        .transportation_engine
        .confirm_delivery(id, Some(user_id))
        .await
        .map_err(|e| match e {
            atlas_shared::AtlasError::EntityNotFound(_) => StatusCode::NOT_FOUND,
            atlas_shared::AtlasError::ValidationFailed(_) => StatusCode::BAD_REQUEST,
...
pub async fn assign_carrier(
    ...
    let carrier_id = body["carrierId"]
        .as_str()
        .and_then(|s| Uuid::parse_str(s).ok())
        .ok_or(StatusCode::BAD_REQUEST)?;
    let carrier_service_id = body["carrierServiceId"]
        .as_str()
        .and_then(|s| Uuid::parse_str(s).ok());
    ...
    let shipment = state
        .scm
        .transportation_engine
        .assign_carrier(id, carrier_id, carrier_service_id)
        .await
        .map_err(|e| match e {
            atlas_shared::AtlasError::EntityNotFound(_) => StatusCode::NOT_FOUND,
            _ => StatusCode::INTERNAL_SERVER_ERROR,
    ...
    let stop = state
        ...
        .add_stop(
            ...
            body.get("address")
                .cloned()
                .unwrap_or(serde_json::json!({})),
            body["plannedArrival"].as_str().and_then(|d| {
                chrono::DateTime::parse_from_rfc3339(d)
                    .ok()
                    .map(|dt| dt.to_utc())
            }),
            body["plannedDeparture"].as_str().and_then(|d| {
                chrono::DateTime::parse_from_rfc3339(d)
                    .ok()
                    .map(|dt| dt.to_utc())
            }),
...
pub async fn update_stop_status(
    ...
    let stop = state
        ...
        .update_stop_status(
            ...
            body["actualArrival"].as_str().and_then(|d| {
                chrono::DateTime::parse_from_rfc3339(d)
                    .ok()
                    .map(|dt| dt.to_utc())
            }),
            body["actualDeparture"].as_str().and_then(|d| {
                chrono::DateTime::parse_from_rfc3339(d)
                    .ok()
                    .map(|dt| dt.to_utc())
            }),
...
pub async fn recalculate_shipment_totals(
    ...
    let shipment = state
        .scm
        .transportation_engine
        .recalculate_shipment_totals(id)
        .await
        .map_err(|e| match e {
            atlas_shared::AtlasError::EntityNotFound(_) => StatusCode::NOT_FOUND,
            _ => StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn list_tracking_events(
    ...
    let events = state
        .scm
        .transportation_engine
        .list_tracking_events(shipment_id)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
pub async fn list_freight_rates(
    ...
    let rates = state
        .scm
        .transportation_engine
        .list_freight_rates(org_id, params.carrier_id.as_ref(), params.status.as_deref())
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
pub async fn delete_freight_rate(
    ...
    state
        .scm
        .transportation_engine
        .delete_freight_rate(org_id, &code)
        .await
        .map_err(|e| match e {
            atlas_shared::AtlasError::EntityNotFound(_) => StatusCode::NOT_FOUND,
            _ => StatusCode::INTERNAL_SERVER_ERROR,
