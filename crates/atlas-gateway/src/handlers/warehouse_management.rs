...
pub async fn create_warehouse(
    ...
    match state
        ...
        .create_warehouse(
            ...
            payload.location_code.as_deref(),
            Some(user_id),
        )
        .await
    {
        Ok(wh) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(wh)),
        )),
        Err(e) => {
...
pub async fn list_warehouses(
    ...
    match state
        .scm
        .warehouse_management_engine
        .list_warehouses(org_id, params.active_only.unwrap_or(false))
        .await
    {
        Ok(warehouses) => Ok(Json(serde_json::json!({ "data": warehouses }))),
        Err(e) => {
            error!("Failed to list warehouses: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn create_zone(
        ...
        .await
    {
        Ok(zone) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(zone)),
        )),
        Err(e) => {
...
pub async fn list_zones(
    ...
    match state
        .scm
        .warehouse_management_engine
        .list_zones(org_id, warehouse_id)
        .await
    {
...
pub async fn create_put_away_rule(
    ...
    match state
        ...
        .create_put_away_rule(
            ...
            payload.item_category.as_deref(),
            &payload.target_zone_type,
            &payload.strategy,
        )
        .await
    {
...
pub async fn list_put_away_rules(
    ...
    match state
        .scm
        .warehouse_management_engine
        .list_put_away_rules(org_id, warehouse_id)
        .await
    {
        Ok(rules) => Ok(Json(serde_json::json!({ "data": rules }))),
        Err(e) => {
            error!("Failed to list put-away rules: {}", e);
            Err(map_error(e))
        }
    }
}
...
pub async fn delete_put_away_rule(
    ...
    match state
        .scm
        .warehouse_management_engine
        .delete_put_away_rule(org_id, id)
        .await
    {
        Ok(()) => Ok(StatusCode::NO_CONTENT),
        Err(e) => {
...
pub async fn create_task(
    ...
    let warehouse_id = if let Some(wave_id) = payload.wave_id {
        if let Ok(Some(wave)) = state
            .scm
            .warehouse_management_engine
            .get_wave(org_id, wave_id)
            .await
        {
            wave.warehouse_id
        } else {
        ...
        if let Ok(Some(zone)) = state
            .scm
            .warehouse_management_engine
            .get_zone(org_id, zone_id)
            .await
        {
            zone.warehouse_id
        } else {
    ...
    match state
        ...
        .create_task(
            ...
            payload.from_zone_id,
            payload.to_zone_id,
            payload.from_location.as_deref(),
            payload.to_location.as_deref(),
            payload.quantity,
            payload.uom.as_deref(),
            payload.source_document.as_deref(),
        ...
        .await
    {
        Ok(task) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(task)),
        )),
        Err(e) => {
...
pub async fn create_wave(
        ...
        .await
    {
        Ok(wave) => Ok((
            StatusCode::CREATED,
            Json(crate::handlers::records::to_json_or_null(wave)),
        )),
        Err(e) => {
...
pub async fn list_waves(
    ...
    match state
        .scm
        .warehouse_management_engine
        .list_waves(org_id, params.warehouse_id, params.status.as_deref())
        .await
    {
        Ok(waves) => Ok(Json(serde_json::json!({ "data": waves }))),
        Err(e) => {
            error!("Failed to list waves: {}", e);
            Err(map_error(e))
        }
    }
}
