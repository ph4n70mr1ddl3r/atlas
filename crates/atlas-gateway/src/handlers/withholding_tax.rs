...
pub async fn create_tax_code(
    ...
    let effective_from = req.effective_from.as_deref()
        ...
        .transpose()
        .map_err(|e| (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": format!("Invalid effective_from: {}", e)}))))?;
    ...
    match state.financials.withholding_tax_engine.create_tax_code(
        ...
        Ok(tc) => Ok((StatusCode::CREATED, Json(serde_json::to_value(tc).unwrap_or(serde_json::Value::Null)))),
        Err(e) => {
...
pub async fn create_tax_group(
    ...
    let org_id = parse_uuid(&claims.org_id)?;
    let user_id = parse_uuid(&claims.sub).ok();
    let tax_code_ids: Result<Vec<Uuid>, _> = req.tax_code_ids.iter()
        .map(|s| s.parse::<Uuid>())
        .collect();
    ...
    match state.financials.withholding_tax_engine.create_tax_group(
        ...
        Ok(group) => Ok((StatusCode::CREATED, Json(serde_json::to_value(group).unwrap_or(serde_json::Value::Null)))),
        Err(e) => {
...
pub async fn assign_supplier(
    ...
    let exemption_valid_until = req.exemption_valid_until.as_deref()
        .map(|d| chrono::NaiveDate::parse_from_str(d, "%Y-%m-%d"))
        .transpose()
    ...
    match state.financials.withholding_tax_engine.assign_supplier(
        ...
        exemption_valid_until, user_id,
    ).await {
        Ok(assignment) => Ok((StatusCode::CREATED, Json(serde_json::to_value(assignment).unwrap_or(serde_json::Value::Null)))),
        Err(e) => {
...
pub async fn record_withholding(
    ...
    match state.financials.withholding_tax_engine.record_withholding(
        ...
        Ok(lines) => Ok((StatusCode::CREATED, Json(serde_json::json!({"data": lines})))),
        Err(e) => {
...
pub async fn remit_withholding(
    ...
    let line_ids: Result<Vec<Uuid>, _> = req.line_ids.iter()
        .map(|s| s.parse::<Uuid>())
        .collect();
    let line_ids = line_ids
        .map_err(|_| (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": "Invalid line_ids"}))))?;
...
pub async fn generate_certificate(
    ...
    let period_start = chrono::NaiveDate::parse_from_str(&req.period_start, "%Y-%m-%d")
        .map_err(|e| (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": format!("Invalid period_start: {}", e)}))))?;
    ...
    match state.financials.withholding_tax_engine.generate_certificate(
        ...
        Ok(cert) => Ok((StatusCode::CREATED, Json(serde_json::to_value(cert).unwrap_or(serde_json::Value::Null)))),
        Err(e) => {
