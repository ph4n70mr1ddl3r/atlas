...
use atlas_core::{
    ...
    DistributionSetEngine,
    invoice_matching::PostgresInvoiceMatchingRepository,
    distribution_set::PostgresDistributionSetRepository,
...
impl AppState {
    pub async fn new() -> anyhow::Result<Self> {
        ...
        // Initialize Hedge Management engine (Oracle Fusion: Treasury > Hedge Management)
        let hedge_management_engine = Arc::new(HedgeManagementEngine::new(Arc::new(
        ...
        // Initialize Cash Concentration / Pooling engine (Oracle Fusion: Treasury > Cash Pooling)
        let cash_concentration_engine = Arc::new(CashConcentrationEngine::new(Arc::new(
        ...
        // Initialize Payment Process Request engine (Oracle Fusion: Financials > Payables > Payment Process Requests)
        let payment_process_request_engine = Arc::new(PaymentProcessRequestEngine::new(Arc::new(
            PostgresPaymentProcessRequestRepository::new(db_pool.clone())
        )));

        // Initialize Invoice Batch engine (Oracle Fusion: Financials > Payables > Invoice Batches)
        let invoice_batch_engine = Arc::new(InvoiceBatchEngine::new(Arc::new(
        ...
        // Initialize Tax Registration engine (Oracle Fusion: Financials > Tax > Tax Registrations)
        let tax_registration_engine = Arc::new(TaxRegistrationEngine::new(Arc::new(
            PostgresTaxRegistrationRepo::new(db_pool.clone())
        )));

        // Initialize Doubtful Account Allowance engine (Oracle Fusion: Receivables > Collections > Allowance for Doubtful Accounts)
        let doubtful_account_engine = Arc::new(DoubtfulAccountAllowanceEngine::new(Arc::new(
        ...
        // Initialize Distribution Set engine (Oracle Fusion: Financials > Payables > Distribution Sets)
        let distribution_set_engine = Arc::new(DistributionSetEngine::new(Arc::new(
        ...
        // Initialize Third-Party Payment engine (Oracle Fusion: Financials > Payables > Third-Party Payments)
        let third_party_payment_engine = Arc::new(ThirdPartyPaymentEngine::new(Arc::new(
        ...
        // Initialize Cash Receipt Management engine (Oracle Fusion: AR > Receipts)
        let cash_receipt_engine = Arc::new(CashReceiptEngine::new(Arc::new(
            PostgresCashReceiptRepo::new(db_pool.clone())
        )));

        // Initialize Direct Debit Mandate Management engine (Oracle Fusion: AR > Direct Debit Mandates)
