...
async fn create_test_group(
    ...
    if status != StatusCode::CREATED {
        panic!(
            "Expected CREATED for account group but got {}: {}",
            status,
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(format!(
                    "/api/v1/account-monitor/groups/{}/members",
                    group_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(format!(
                    "/api/v1/account-monitor/groups/{}/snapshots",
                    group_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
...
async fn test_capture_snapshot_with_thresholds() {
    ...
    assert!(
        snap["alertStatus"].as_str().unwrap() == "warning"
            || snap["alertStatus"].as_str().unwrap() == "critical"
            || snap["alertStatus"].as_str().unwrap() == "none"
    );
}
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                .method("DELETE")
                .uri(format!(
                    "/api/v1/account-monitor/inquiries/id/{}",
                    inquiry_id
                ))
                .header(&k, &v)
                .body(Body::empty())
