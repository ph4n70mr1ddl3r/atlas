...
async fn create_retirement(
    ...
    assert_eq!(status, StatusCode::CREATED, "Failed to create retirement: {:?}", body_str);
...
async fn test_create_sale_retirement() {
    ...
    assert!(ret["retirementNumber"].as_str().unwrap().starts_with("RET-"));
}
...
async fn test_list_retirements() {
    let (_state, app) = setup_test().await;
    create_retirement(&app, "55555555-5555-5555-5555-555555555555", "sale", "10000", "5000", "6000", "0").await;
    create_retirement(&app, "66666666-6666-6666-6666-666666666666", "scrap", "8000", "8000", "0", "0").await;
...
async fn test_list_retirements_filter_by_status() {
    let (_state, app) = setup_test().await;
    create_retirement(&app, "77777777-7777-7777-7777-777777777771", "sale", "10000", "5000", "6000", "0").await;
...
async fn test_list_retirements_filter_by_type() {
    let (_state, app) = setup_test().await;
    create_retirement(&app, "77777777-7777-7777-7777-777777777772", "scrap", "5000", "3000", "0", "0").await;
...
async fn test_approve_retirement() {
    let (_state, app) = setup_test().await;
    let ret = create_retirement(&app, "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa", "sale", "10000", "5000", "6000", "0").await;
    let ret_id = ret["id"].as_str().unwrap();
...
async fn test_approve_non_pending_fails() {
    let (_state, app) = setup_test().await;
    let ret = create_retirement(&app, "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb", "sale", "10000", "5000", "6000", "0").await;
    let ret_id = ret["id"].as_str().unwrap();
...
async fn test_complete_retirement() {
    let (_state, app) = setup_test().await;
    let ret = create_retirement(&app, "cccccccc-cccc-cccc-cccc-cccccccccccc", "sale", "20000", "12000", "10000", "200").await;
    let ret_id = ret["id"].as_str().unwrap();
...
async fn test_complete_not_approved_fails() {
    let (_state, app) = setup_test().await;
    let ret = create_retirement(&app, "eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee", "sale", "10000", "5000", "6000", "0").await;
    let ret_id = ret["id"].as_str().unwrap();
...
async fn test_reverse_retirement() {
    let (_state, app) = setup_test().await;
    let ret = create_retirement(&app, "ffffffff-ffff-ffff-ffff-ffffffffffff", "sale", "10000", "5000", "6000", "0").await;
    let ret_id = ret["id"].as_str().unwrap();
...
async fn test_cancel_retirement() {
    let (_state, app) = setup_test().await;
    let ret = create_retirement(&app, "10101010-1010-1010-1010-101010101010", "sale", "10000", "5000", "6000", "0").await;
    let ret_id = ret["id"].as_str().unwrap();
...
async fn test_cancel_approved_retirement_fails() {
    let (_state, app) = setup_test().await;
    let ret = create_retirement(&app, "11111111-2222-3333-4444-555555555555", "sale", "10000", "5000", "6000", "0").await;
    let ret_id = ret["id"].as_str().unwrap();
...
async fn test_retirement_dashboard() {
    let (_state, app) = setup_test().await;
    create_retirement(&app, "aaaa0000-0000-0000-0000-000000000001", "sale", "50000", "30000", "25000", "1000").await;
...
async fn test_gain_on_sale() {
    ...
    let ret = create_retirement(&app, "b0000000-0000-0000-0000-000000000001", "sale", "50000", "30000", "25000", "1000").await;
...
async fn test_loss_on_scrap() {
    ...
    let ret = create_retirement(&app, "b0000000-0000-0000-0000-000000000002", "scrap", "10000", "6000", "0", "500").await;
...
async fn test_no_gain_loss_fully_depreciated() {
    ...
    // Gain/Loss = 0 - 0 - 0 = 0
    let ret = create_retirement(&app, "b0000000-0000-0000-0000-000000000003", "donation", "10000", "10000", "0", "0").await;
...
async fn test_end_to_end_retirement_flow() {
    ...
    let ret = create_retirement(&app,
        "e2e00000-0000-0000-0000-000000000001",
        "sale", "80000", "50000", "35000", "2000"
    ).await;
    let ret_id = ret["id"].as_str().unwrap();
    ...
    assert_eq!(r.status(), StatusCode::OK);
    let fetched: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX).await
        .map(|b| serde_json::from_slice(&b).unwrap()).unwrap();
    ...
    assert_eq!(r.status(), StatusCode::OK);
    let completed: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX).await
        .map(|b| serde_json::from_slice(&b).unwrap()).unwrap();
    ...
    assert_eq!(r.status(), StatusCode::OK);
    let dash: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX).await
        .map(|b| serde_json::from_slice(&b).unwrap()).unwrap();
    ...
    assert_eq!(r.status(), StatusCode::OK);
    let filtered: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX).await
        .map(|b| serde_json::from_slice(&b).unwrap()).unwrap();
    let completed_items: Vec<_> = filtered["data"].as_array().unwrap()
        .iter().filter(|i| i["id"] == ret_id).collect();
    assert_eq!(completed_items.len(), 1);

    // 8. Now create another retirement and cancel it
    let ret2 = create_retirement(&app,
    ...
    assert_eq!(r.status(), StatusCode::OK);
    let final_dash: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX).await
        .map(|b| serde_json::from_slice(&b).unwrap()).unwrap();
