...
async fn setup_test() -> (std::sync::Arc<atlas_gateway::AppState>, axum::Router) {
    ...
    sqlx::query("DELETE FROM _atlas.auto_offset_activities")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM _atlas.auto_offset_lines")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM _atlas.auto_offset_generations")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM _atlas.auto_offset_template_lines")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM _atlas.auto_offset_templates")
        .execute(&state.db_pool)
        .await
        .ok();
    ...
    sqlx::raw_sql(include_str!(
        "../../../../migrations/140_automatic_offsets.sql"
    ))
    .execute(&state.db_pool)
    .await
...
async fn create_template(app: &axum::Router, code: &str, name: &str) -> serde_json::Value {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/auto-offsets/templates/{}/lines",
                    template_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/auto-offsets/generations/{}/reverse",
                    gen_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/auto-offsets/generations/{}/cancel",
                    gen_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_list_offset_lines() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .uri(&format!(
                    "/api/v1/auto-offsets/generations/{}/lines",
                    gen_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_list_activities() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .uri(&format!(
                    "/api/v1/auto-offsets/generations/{}/activities",
                    gen_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    assert!(
        activities.len() >= 2,
        "Should have at least 2 activities (generated + posted)"
    );

    // Check activity types
    let types: Vec<&str> = activities
        .iter()
