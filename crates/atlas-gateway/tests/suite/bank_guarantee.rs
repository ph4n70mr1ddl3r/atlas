...
async fn setup_test() -> (std::sync::Arc<atlas_gateway::AppState>, axum::Router) {
    ...
    sqlx::query(include_str!(
        "../../../../migrations/121_bank_guarantee_management.sql"
    ))
    .execute(&state.db_pool)
    .await
    .ok();
    // Clean up bank guarantee test data
    sqlx::query("DELETE FROM fin_bank_guarantee_amendments")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM fin_bank_guarantees")
        .execute(&state.db_pool)
        .await
        .ok();
...
async fn test_create_guarantee_calculates_margin() {
    let (_state, app) = setup_test().await;
    let guarantee = create_guarantee(&app, "BG-MARGIN", "bid_bond", "200000", "Client A").await;
    ...
    let commission: f64 = guarantee["commissionAmount"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    ...
    app.clone()
        .oneshot(
            Request::builder()
                ...
                .header("Content-Type", "application/json")
                .header(&k, &v)
                .body(Body::from(r#"{"issueDate":"2025-01-01"}"#))
                .unwrap(),
        )
        .await
        .unwrap();
...
async fn test_create_amendment() {
    ...
    assert!(body["amendmentNumber"]
        .as_str()
        .unwrap()
        .starts_with("AMD-BG-AMD-"));
    assert_eq!(body["status"], "pending_approval");
}
...
async fn test_list_amendments() {
    let (_state, app) = setup_test().await;
    let guarantee = create_guarantee(&app, "BG-AMD-LIST", "bid_bond", "50000", "List Corp").await;
    let id = guarantee["id"].as_str().unwrap();
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/bank-guarantees/amendments/{}/approve",
                    amendment_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/bank-guarantees/amendments/{}/reject",
                    amendment_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_submit_non_draft_fails() {
    let (_state, app) = setup_test().await;
    let guarantee = create_guarantee(&app, "BG-DBL-SUB", "bid_bond", "10000", "Test").await;
    let id = guarantee["id"].as_str().unwrap();
...
async fn test_delete_non_draft_fails() {
    let (_state, app) = setup_test().await;
    let guarantee = create_guarantee(&app, "BG-DEL-ND", "bid_bond", "10000", "Test").await;
    let id = guarantee["id"].as_str().unwrap();
...
async fn test_approve_from_draft_fails() {
    let (_state, app) = setup_test().await;
    let guarantee = create_guarantee(&app, "BG-APPR-DRAFT", "bid_bond", "10000", "Test").await;
    let id = guarantee["id"].as_str().unwrap();
...
async fn test_create_amendment_on_draft_fails() {
    let (_state, app) = setup_test().await;
    let guarantee = create_guarantee(&app, "BG-AMD-DRAFT", "bid_bond", "10000", "Test").await;
    let id = guarantee["id"].as_str().unwrap();
