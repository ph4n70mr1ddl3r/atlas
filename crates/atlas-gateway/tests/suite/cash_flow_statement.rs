...
async fn setup_test() -> (std::sync::Arc<atlas_gateway::AppState>, axum::Router) {
    ...
    sqlx::query("DELETE FROM financials.cash_flow_statement_lines").execute(&state.db_pool).await.ok();
    sqlx::query("DELETE FROM financials.cash_flow_statements").execute(&state.db_pool).await.ok();
    sqlx::query("CREATE SCHEMA IF NOT EXISTS financials")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query(include_str!("../../../../migrations/138_cash_flow_statement.sql"))
        .execute(&state.db_pool)
        .await
        .ok();
...
async fn add_line(
    ...
    let r = app.clone().oneshot(Request::builder().method("POST")
        .uri(&format!("/api/v1/cash-flow-statements/{}/lines", statement_id))
        .header("Content-Type", "application/json")
        .header(&k, &v)
        .body(Body::from(serde_json::to_string(&payload).unwrap()))
        .unwrap()
...
async fn test_create_cash_flow_statement_indirect() {
    ...
    let stmt = create_statement(&app, "CFS-2026-Q1", "indirect", "quarterly", "2026-01-01", "2026-03-31").await;
...
async fn test_create_cash_flow_statement_direct() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-DIR-01", "direct", "monthly", "2026-01-01", "2026-01-31").await;
...
async fn test_create_yearly_statement() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-2026", "indirect", "yearly", "2026-01-01", "2026-12-31").await;
...
async fn test_create_statement_duplicate_number_fails() {
    let (_state, app) = setup_test().await;
    create_statement(&app, "CFS-DUP", "direct", "monthly", "2026-01-01", "2026-01-31").await;
...
async fn test_get_cash_flow_statement() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-GET", "indirect", "quarterly", "2026-01-01", "2026-03-31").await;
...
async fn test_list_cash_flow_statements() {
    let (_state, app) = setup_test().await;
    create_statement(&app, "CFS-LIST-A", "direct", "monthly", "2026-01-01", "2026-01-31").await;
    create_statement(&app, "CFS-LIST-B", "indirect", "yearly", "2026-01-01", "2026-12-31").await;
...
async fn test_list_statements_filter_by_method() {
    let (_state, app) = setup_test().await;
    create_statement(&app, "CFS-FLT-D", "direct", "monthly", "2026-01-01", "2026-01-31").await;
    create_statement(&app, "CFS-FLT-I", "indirect", "monthly", "2026-01-01", "2026-01-31").await;
...
async fn test_list_statements_filter_by_status() {
    let (_state, app) = setup_test().await;
    create_statement(&app, "CFS-STS-D", "direct", "monthly", "2026-01-01", "2026-01-31").await;
...
async fn test_add_statement_lines() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-LINE", "indirect", "yearly", "2026-01-01", "2026-12-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
...
async fn test_add_line_with_account_ranges() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-ACCT", "direct", "monthly", "2026-01-01", "2026-01-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
...
async fn test_add_non_cash_line() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-NC", "indirect", "yearly", "2026-01-01", "2026-12-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
...
async fn test_add_line_invalid_category_fails() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-BAD-CAT", "direct", "monthly", "2026-01-01", "2026-01-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
...
async fn test_add_line_invalid_line_type_fails() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-BAD-LT", "direct", "monthly", "2026-01-01", "2026-01-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
...
async fn test_list_statement_lines() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-LL", "indirect", "yearly", "2026-01-01", "2026-12-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
...
async fn test_remove_statement_line() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-RL", "indirect", "yearly", "2026-01-01", "2026-12-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
...
async fn test_calculate_statement() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-CALC", "indirect", "yearly", "2026-01-01", "2026-12-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
    ...
    let (k, v) = auth_header(&admin_claims());
    let r = app.clone().oneshot(Request::builder().method("POST")
        .uri(&format!("/api/v1/cash-flow-statements/{}/calculate", stmt_id))
        .header(&k, &v)
        .body(Body::empty()).unwrap()
    ).await.unwrap();
    assert_eq!(r.status(), StatusCode::OK);
    ...
    let closing: f64 = body["closingCashBalance"].as_str().unwrap().parse().unwrap();
...
async fn test_calculate_skips_non_detail_lines() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-HDR", "indirect", "yearly", "2026-01-01", "2026-12-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
...
async fn test_full_lifecycle_draft_to_archived() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-LC", "indirect", "yearly", "2026-01-01", "2026-12-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
    assert_eq!(stmt["status"], "draft");
...
async fn test_cannot_review_draft_statement() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-REV-D", "direct", "monthly", "2026-01-01", "2026-01-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
...
async fn test_cannot_publish_without_review() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-PUB-NR", "direct", "monthly", "2026-01-01", "2026-01-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
...
async fn test_cannot_archive_unpublished() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-ARC-UP", "direct", "monthly", "2026-01-01", "2026-01-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
...
async fn test_cannot_calculate_twice() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-CALC2", "indirect", "yearly", "2026-01-01", "2026-12-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
...
async fn test_cannot_add_lines_to_calculated_statement() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-ADD-CALC", "indirect", "yearly", "2026-01-01", "2026-12-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
...
async fn test_cash_flow_statement_dashboard() {
    let (_state, app) = setup_test().await;
    create_statement(&app, "CFS-DASH-1", "direct", "monthly", "2026-01-01", "2026-01-31").await;
    create_statement(&app, "CFS-DASH-2", "indirect", "yearly", "2026-01-01", "2026-12-31").await;
    ...
    assert!(total >= 2, "Expected at least 2 total statements, got {}", total);
}
...
async fn test_all_line_types() {
    let (_state, app) = setup_test().await;
    let stmt = create_statement(&app, "CFS-TYPES", "indirect", "yearly", "2026-01-01", "2026-12-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
...
async fn test_full_cash_flow_statement_integration() {
    ...
    let stmt = create_statement(&app, "CFS-INT-2026", "indirect", "yearly", "2026-01-01", "2026-12-31").await;
    let stmt_id: Uuid = stmt["id"].as_str().unwrap().parse().unwrap();
    assert_eq!(stmt["status"], "draft");
    ...
    assert_eq!(r.status(), StatusCode::OK);
    let calculated: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX).await
        .map(|b| serde_json::from_slice(&b).unwrap()).unwrap();
    assert_eq!(calculated["status"], "calculated");

    // Verify calculation: operating=260000, investing=-70000, financing=30000
    let op: f64 = calculated["operatingCashFlow"].as_str().unwrap().parse().unwrap();
    let inv: f64 = calculated["investingCashFlow"].as_str().unwrap().parse().unwrap();
    let fin: f64 = calculated["financingCashFlow"].as_str().unwrap().parse().unwrap();
    let net: f64 = calculated["netChangeInCash"].as_str().unwrap().parse().unwrap();
    ...
    let final_stmt: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX).await
        .map(|b| serde_json::from_slice(&b).unwrap()).unwrap();
    ...
    assert_eq!(r.status(), StatusCode::OK);
    let archived: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX).await
        .map(|b| serde_json::from_slice(&b).unwrap()).unwrap();
