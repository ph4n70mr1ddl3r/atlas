...
async fn test_create_batch() {
    ...
    let r = app.clone().oneshot(Request::builder().method("POST").uri("/api/v1/cash-receipts/batches")
...
async fn test_get_batch_not_found() {
    ...
    let (k, v) = auth_header(&admin_claims());
    let r = app.clone().oneshot(Request::builder().method("GET")
        .uri(&format!("/api/v1/cash-receipts/batches/{}", uuid::Uuid::new_v4()))
        .header(&k, &v).body(Body::empty()).unwrap())
    .await.unwrap();
...
async fn test_create_receipt() {
    let (_state, app) = setup_test().await;
    let (k, v) = auth_header(&admin_claims());
    let r = app.clone().oneshot(Request::builder().method("POST").uri("/api/v1/cash-receipts")
        .header("Content-Type", "application/json").header(&k, &v)
        .body(Body::from(serde_json::to_string(&json!({
...
async fn test_full_workflow() {
    ...
    let r = app.clone().oneshot(Request::builder().method("POST").uri("/api/v1/cash-receipts/applications")
        .header("Content-Type", "application/json").header(&k, &v)
        .body(Body::from(serde_json::to_string(&json!({
    ...
    let r = app.clone().oneshot(Request::builder().method("POST")
        .uri(&format!("/api/v1/cash-receipts/applications/{}/unapply", app_id))
        .header(&k, &v).body(Body::empty()).unwrap())
    .await.unwrap();
    assert_eq!(r.status(), StatusCode::OK);
    let b = axum::body::to_bytes(r.into_body(), usize::MAX).await.unwrap();
...
async fn test_list_applications() {
    ...
    let (k, v) = auth_header(&admin_claims());
    let r = app.clone().oneshot(Request::builder().method("GET")
        .uri(&format!("/api/v1/cash-receipts/{}/applications", uuid::Uuid::new_v4()))
        .header(&k, &v).body(Body::empty()).unwrap())
    .await.unwrap();
    assert_eq!(r.status(), StatusCode::OK);
}
