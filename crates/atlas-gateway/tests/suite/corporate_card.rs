...
async fn create_test_card(
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/v1/corporate-cards/cards")
                .header("Content-Type", "application/json")
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({
...
async fn import_test_transaction(
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/v1/corporate-cards/transactions")
                .header("Content-Type", "application/json")
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({
    ...
    if status != StatusCode::CREATED {
        panic!(
            "import_test_transaction failed: {:?} - {}",
            status,
            String::from_utf8_lossy(&b)
        );
    }
...
async fn test_get_program() {
    let (_state, app) = setup_corporate_card_test().await;
    let _program = create_test_program(
        &app,
        "MC-01",
        "Mastercard Program",
        "Citi",
...
async fn test_issue_card() {
    let (_state, app) = setup_corporate_card_test().await;
    let program =
        create_test_program(&app, "V-02", "Visa Program", "Chase", "Visa", "corporate").await;
...
async fn test_issue_card_works_for_active_program() {
    let (_state, app) = setup_corporate_card_test().await;
    let program =
        create_test_program(&app, "V-03", "Visa Program 3", "Chase", "Visa", "corporate").await;
    let program_id = program["id"].as_str().unwrap();
...
async fn test_suspend_and_reactivate_card() {
    let (_state, app) = setup_corporate_card_test().await;
    let program =
        create_test_program(&app, "V-04", "Visa Program 4", "Wells", "Visa", "corporate").await;
    let card = create_test_card(
        &app,
        program["id"].as_str().unwrap(),
        "****-1111",
        "Test User",
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/corporate-cards/cards/{}/suspend",
                    card_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/corporate-cards/cards/{}/reactivate",
                    card_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_cancel_card() {
    let (_state, app) = setup_corporate_card_test().await;
    let program =
        create_test_program(&app, "V-05", "Visa Program 5", "BOA", "Visa", "corporate").await;
    let card = create_test_card(
        &app,
        program["id"].as_str().unwrap(),
        "****-2222",
...
async fn test_report_stolen_card() {
    let (_state, app) = setup_corporate_card_test().await;
    let program =
        create_test_program(&app, "V-07", "Visa Program 7", "HSBC", "Visa", "travel").await;
    let card = create_test_card(
        &app,
        program["id"].as_str().unwrap(),
        "****-4444",
...
async fn test_list_cards_by_status() {
    let (_state, app) = setup_corporate_card_test().await;
    let program =
        create_test_program(&app, "V-08", "Visa Program 8", "Chase", "Visa", "corporate").await;
    create_test_card(
        &app,
        program["id"].as_str().unwrap(),
        "****-5555",
...
async fn test_import_charge_transaction() {
    let (_state, app) = setup_corporate_card_test().await;
    let program =
        create_test_program(&app, "V-10", "Visa Prog TXN", "Chase", "Visa", "corporate").await;
    let card = create_test_card(
        &app,
        program["id"].as_str().unwrap(),
        "****-7777",
...
async fn test_import_multiple_transactions() {
    let (_state, app) = setup_corporate_card_test().await;
    let program =
        create_test_program(&app, "V-11", "Visa Prog Multi", "Citi", "Visa", "corporate").await;
    let card = create_test_card(
        &app,
        program["id"].as_str().unwrap(),
        "****-8888",
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(&format!(
                    "/api/v1/corporate-cards/transactions?card_id={}",
                    card_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_match_transaction() {
    ...
    let txn = import_test_transaction(
        &app,
        card_id,
        "TXN-M01",
        "Delta Airlines",
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/corporate-cards/transactions/{}/match",
                    txn_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/corporate-cards/transactions/{}/unmatch",
                    txn_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/corporate-cards/transactions/{}/dispute",
                    txn_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/corporate-cards/transactions/{}/resolve-dispute",
                    txn_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
...
async fn test_import_statement() {
    let (_state, app) = setup_corporate_card_test().await;
    let program =
        create_test_program(&app, "V-15", "Visa Prog Stmt", "Chase", "Visa", "corporate").await;
    let program_id = program["id"].as_str().unwrap();
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/v1/corporate-cards/statements")
                .header("Content-Type", "application/json")
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({
...
async fn test_reconcile_and_pay_statement() {
    ...
    let program =
        create_test_program(&app, "V-16", "Visa Prog Rec", "Chase", "Visa", "corporate").await;
    let program_id = program["id"].as_str().unwrap();
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/corporate-cards/statements/{}/reconcile",
                    stmt_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/corporate-cards/statements/{}/pay",
                    stmt_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
...
async fn test_request_and_approve_limit_override() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/v1/corporate-cards/limit-overrides")
                .header("Content-Type", "application/json")
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/corporate-cards/limit-overrides/{}/approve",
                    ovr_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_reject_limit_override() {
    let (_state, app) = setup_corporate_card_test().await;
    let program =
        create_test_program(&app, "V-18", "Visa Prog Rej", "Chase", "Visa", "corporate").await;
    let card = create_test_card(
        &app,
        program["id"].as_str().unwrap(),
        "****-1818",
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/corporate-cards/limit-overrides/{}/reject",
                    ovr_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_suspend_already_suspended_card_fails() {
    let (_state, app) = setup_corporate_card_test().await;
    let program =
        create_test_program(&app, "V-ERR", "Error Program", "Chase", "Visa", "corporate").await;
    let card = create_test_card(
        &app,
        program["id"].as_str().unwrap(),
        "****-E01",
