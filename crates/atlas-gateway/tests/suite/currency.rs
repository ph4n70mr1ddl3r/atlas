...
async fn test_list_exchange_rates() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .uri("/api/v1/exchange-rates?limit=10")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_triangulation() {
    ...
    let to_amount: f64 = result["to_amount"].as_str().unwrap().parse().unwrap();
    assert!(
        to_amount > 800.0 && to_amount < 900.0,
        "Expected ~859 GBP, got {}",
        to_amount
    );
    cleanup_test_db(&state.db_pool).await;
}
...
async fn test_unrealized_gain_loss() {
    ...
    // Gain: 400 USD
    let gain_amount: f64 = result["gain_loss_amount"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    assert!(
        (gain_amount - 400.0).abs() < 1.0,
        "Expected ~400 gain, got {}",
        gain_amount
    );
    cleanup_test_db(&state.db_pool).await;
}
...
async fn test_unrealized_loss() {
    ...
    // Loss: 500 USD
    let loss_amount: f64 = result["gain_loss_amount"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    assert!(
        (loss_amount - 500.0).abs() < 1.0,
        "Expected ~500 loss, got {}",
        loss_amount
    );
    cleanup_test_db(&state.db_pool).await;
}
