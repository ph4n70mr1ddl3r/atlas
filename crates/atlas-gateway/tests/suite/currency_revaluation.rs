...
async fn test_activate_deactivate_definition() {
    ...
    let deact_resp = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/currency-revaluation/definitions/{}/deactivate",
                    def_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let body = axum::body::to_bytes(deact_resp.into_body(), usize::MAX)
        .await
        .unwrap();
    let def: serde_json::Value = serde_json::from_slice(&body).unwrap();
    ...
    let act_resp = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/currency-revaluation/definitions/{}/activate",
                    def_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let body = axum::body::to_bytes(act_resp.into_body(), usize::MAX)
        .await
        .unwrap();
    let def: serde_json::Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(def["isActive"], true);
    ...
    let del_resp = app
        ...
        .oneshot(
            Request::builder()
                .method("DELETE")
                .uri(&format!(
                    "/api/v1/currency-revaluation/accounts/{}",
                    acct_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_post_and_reverse_revaluation_run() {
    ...
    let post_resp = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/currency-revaluation/runs/{}/post",
                    run_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let body = axum::body::to_bytes(post_resp.into_body(), usize::MAX)
        .await
        .unwrap();
    ...
    // Reverse
    let reverse_resp = app
        .clone()
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/currency-revaluation/runs/{}/reverse",
                    run_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let body = axum::body::to_bytes(reverse_resp.into_body(), usize::MAX)
        .await
        .unwrap();
...
async fn test_cancel_revaluation_run() {
    ...
    let cancel_resp = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/currency-revaluation/runs/{}/cancel",
                    run_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let body = axum::body::to_bytes(cancel_resp.into_body(), usize::MAX)
        .await
        .unwrap();
...
async fn test_list_revaluation_runs_with_status_filter() {
    ...
    let result: serde_json::Value = serde_json::from_slice(&body).unwrap();
    assert!(
        result["data"].as_array().unwrap().len() >= 1
            || result["data"].as_array().unwrap().is_empty()
    );
}
