...
async fn setup_test() -> (std::sync::Arc<atlas_gateway::AppState>, axum::Router) {
    ...
    // Clean test data
    sqlx::query("DELETE FROM _atlas.customer_statement_lines")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM _atlas.customer_statements")
        .execute(&state.db_pool)
        .await
        .ok();
    ...
    sqlx::query(include_str!(
        "../../../../migrations/126_customer_statement.sql"
    ))
    .execute(&state.db_pool)
    .await
    .ok();
...
async fn test_create_statement() {
    ...
    assert!(
        (closing - 9500.0).abs() < 1.0,
        "Expected closing 9500, got {}",
        closing
    );
...
async fn test_create_statement_with_negative_balance() {
    ...
    assert!(
        (due - 0.0).abs() < 0.01,
        "Expected amount due 0 for credit balance, got {}",
        due
    );
}
...
async fn test_list_statements_filter_by_customer() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .uri(&format!(
                    "/api/v1/customer-statements?customer_id={}",
                    cust1
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    app.clone()
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/customer-statements/{}/generate",
                    stmt1_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({"reason": "Customer requested cancellation"}))
                        .unwrap(),
                ))
                .unwrap(),
        )
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("DELETE")
                .uri(&format!(
                    "/api/v1/customer-statements/{}/lines/{}",
                    stmt_id, line_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_create_statement_quarterly_cycle() {
    ...
    let r = app
        ...
        .unwrap();
    assert_eq!(r.status(), StatusCode::CREATED);
    let stmt: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
