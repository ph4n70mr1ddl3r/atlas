...
async fn test_delete_method() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("DELETE")
                .uri("/api/v1/demand/methods-by-code/DEL-M")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_list_schedules() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/api/v1/demand/schedules?status=draft")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/demand/schedules/{}/activate",
                    schedule_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_consume_forecast() {
    ...
    assert!(consumption["consumedQuantity"]
        .as_str()
        .unwrap()
        .starts_with("400"));
    ...
    assert!(updated_line["consumedQuantity"]
        .as_str()
        .unwrap()
        .starts_with("400"));
    assert!(updated_line["remainingQuantity"]
        .as_str()
        .unwrap()
        .starts_with("600"));
}
...
async fn test_measure_accuracy() {
    ...
    assert!(accuracy["absoluteError"]
        .as_str()
        .unwrap()
        .starts_with("100"));
    assert!(accuracy["bias"].as_str().unwrap().starts_with("100"));
    let mape: f64 = accuracy["absolutePctError"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(&format!(
                    "/api/v1/demand/schedules/{}/accuracy",
                    schedule_id
                ))
                .header(&k, &v)
                .body(Body::empty())
