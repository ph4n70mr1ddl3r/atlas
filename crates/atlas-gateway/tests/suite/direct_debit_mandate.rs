...
async fn test_create_mandate() {
    ...
    let r = app.clone().oneshot(Request::builder().method("POST").uri("/api/v1/direct-debit-mandates")
...
async fn test_get_mandate_not_found() {
    ...
    let (k, v) = auth_header(&admin_claims());
    let r = app.clone().oneshot(Request::builder().method("GET")
        .uri(&format!("/api/v1/direct-debit-mandates/{}", uuid::Uuid::new_v4()))
        .header(&k, &v).body(Body::empty()).unwrap())
    .await.unwrap();
...
async fn test_full_workflow() {
    ...
    // 2. Activate mandate
    let r = app.clone().oneshot(Request::builder().method("POST")
        .uri(&format!("/api/v1/direct-debit-mandates/{}/activate", mandate_id))
        .header(&k, &v).body(Body::empty()).unwrap())
    .await.unwrap();
    assert_eq!(r.status(), StatusCode::OK);
    let b = axum::body::to_bytes(r.into_body(), usize::MAX).await.unwrap();
    ...
    let r = app.clone().oneshot(Request::builder().method("POST").uri("/api/v1/direct-debit-collections")
        .header("Content-Type", "application/json").header(&k, &v)
        .body(Body::from(serde_json::to_string(&json!({
    ...
    // 4. Submit collection
    let r = app.clone().oneshot(Request::builder().method("POST")
        .uri(&format!("/api/v1/direct-debit-collections/{}/submit", coll_id))
        .header(&k, &v).body(Body::empty()).unwrap())
    .await.unwrap();
    assert_eq!(r.status(), StatusCode::OK);
    let b = axum::body::to_bytes(r.into_body(), usize::MAX).await.unwrap();
    ...
    let r = app.clone().oneshot(Request::builder().method("POST")
        .uri(&format!("/api/v1/direct-debit-collections/{}/complete", coll_id))
        .header("Content-Type", "application/json").header(&k, &v)
        .body(Body::from(serde_json::to_string(&json!({
    ...
    app.clone().oneshot(Request::builder().method("POST")
        .uri(&format!("/api/v1/direct-debit-collections/{}/submit", coll2_id))
        .header(&k, &v).body(Body::empty()).unwrap()).await.unwrap();

    let r = app.clone().oneshot(Request::builder().method("POST")
        .uri(&format!("/api/v1/direct-debit-collections/{}/complete", coll2_id))
        .header("Content-Type", "application/json").header(&k, &v)
        .body(Body::from(serde_json::to_string(&json!({
    ...
    let r = app.clone().oneshot(Request::builder().method("POST")
        .uri(&format!("/api/v1/direct-debit-collections/{}/return", coll2_id))
        .header("Content-Type", "application/json").header(&k, &v)
        .body(Body::from(serde_json::to_string(&json!({
            "reason_code": "R01",
    ...
    // 10. Revoke mandate
    let r = app.clone().oneshot(Request::builder().method("POST")
        .uri(&format!("/api/v1/direct-debit-mandates/{}/revoke", mandate_id))
        .header("Content-Type", "application/json").header(&k, &v)
        .body(Body::from(serde_json::to_string(&json!({
...
async fn test_fail_collection() {
    ...
    // Fail collection
    let r = app.clone().oneshot(Request::builder().method("POST")
        .uri(&format!("/api/v1/direct-debit-collections/{}/fail", coll_id))
        .header("Content-Type", "application/json").header(&k, &v)
        .body(Body::from(serde_json::to_string(&json!({
            "reason_code": "R04",
...
async fn test_list_collections() {
    ...
    let (k, v) = auth_header(&admin_claims());
    let r = app.clone().oneshot(Request::builder().method("GET")
        .uri(&format!("/api/v1/direct-debit-mandates/{}/collections", uuid::Uuid::new_v4()))
        .header(&k, &v).body(Body::empty()).unwrap())
    .await.unwrap();
    assert_eq!(r.status(), StatusCode::OK);
}
