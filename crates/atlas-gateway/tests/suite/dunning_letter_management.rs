...
async fn test_create_letter_set() {
    ...
    let r = app.clone().oneshot(Request::builder().method("POST").uri("/api/v1/dunning/letter-sets")
...
async fn test_get_letter_set_not_found() {
    ...
    let (k, v) = auth_header(&admin_claims());
    let r = app.clone().oneshot(Request::builder().method("GET")
        .uri(&format!("/api/v1/dunning/letter-sets/{}", uuid::Uuid::new_v4()))
        .header(&k, &v).body(Body::empty()).unwrap())
    .await.unwrap();
...
async fn test_create_profile() {
    let (_state, app) = setup_test().await;
    let (k, v) = auth_header(&admin_claims());
    let r = app.clone().oneshot(Request::builder().method("POST").uri("/api/v1/dunning/profiles")
        .header("Content-Type", "application/json").header(&k, &v)
        .body(Body::from(serde_json::to_string(&json!({
...
async fn test_create_run() {
    let (_state, app) = setup_test().await;
    let (k, v) = auth_header(&admin_claims());
    let r = app.clone().oneshot(Request::builder().method("POST").uri("/api/v1/dunning/runs")
        .header("Content-Type", "application/json").header(&k, &v)
        .body(Body::from(serde_json::to_string(&json!({
...
async fn test_full_workflow() {
    ...
    // 2. Add lines
    for (level, name, min_days) in [(1, "Friendly Reminder", 15), (2, "First Notice", 45), (3, "Final Demand", 90)] {
        let r = app.clone().oneshot(Request::builder().method("POST")
            .uri(&format!("/api/v1/dunning/letter-sets/{}/lines", set_id))
            .header("Content-Type", "application/json").header(&k, &v)
            .body(Body::from(serde_json::to_string(&json!({
    ...
    let r = app.clone().oneshot(Request::builder().method("POST")
        .uri(&format!("/api/v1/dunning/letter-sets/{}/deactivate", set_id))
        .header(&k, &v).body(Body::empty()).unwrap())
    .await.unwrap();
    assert_eq!(r.status(), StatusCode::OK);
    let b = axum::body::to_bytes(r.into_body(), usize::MAX).await.unwrap();
