...
async fn create_test_entry(
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/v1/encumbrance/entries")
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/encumbrance/entries/{}/activate",
                    entry_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
...
async fn test_full_entry_lifecycle() {
    ...
    assert_eq!(
        entry["original_amount"].as_str().unwrap().replace("\"", ""),
        "50000.00"
    );
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/v1/encumbrance/liquidations")
                .header("Content-Type", "application/json")
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/encumbrance/liquidations/{}/reverse",
                    liq_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
