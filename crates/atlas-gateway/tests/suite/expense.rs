...
async fn test_create_expense_category() {
    ...
    let category =
        create_test_category(&app, "TRAVEL", "Travel Expenses", true, Some("75.00")).await;
...
async fn test_get_expense_category() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/api/v1/expense/categories/LODGING")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_delete_expense_category() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("DELETE")
                .uri("/api/v1/expense/categories/MISC")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_expense_report_full_lifecycle() {
    ...
    let total: f64 = updated_report["total_amount"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
