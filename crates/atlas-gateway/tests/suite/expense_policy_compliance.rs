...
async fn setup_test() -> (std::sync::Arc<atlas_gateway::AppState>, axum::Router) {
    ...
    sqlx::query(include_str!(
        "../../../../migrations/120_expense_policy_compliance.sql"
    ))
    .execute(&state.db_pool)
    .await
    .ok();
    // Clean up expense compliance test data
    sqlx::query("DELETE FROM fin_expense_compliance_violations")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM fin_expense_compliance_audits")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM fin_expense_policy_rules")
        .execute(&state.db_pool)
        .await
        .ok();
...
async fn create_audit(app: &axum::Router, report_id: &str, trigger: &str) -> serde_json::Value {
    let (k, v) = auth_header(&admin_claims());
    let payload = json!({
        "report_id": report_id,
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/expense-compliance/rules/{}/deactivate",
                    rule_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/expense-compliance/rules/{}/activate",
                    rule_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_evaluate_compliance_no_rules() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/expense-compliance/audits/{}/evaluate",
                    audit_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    assert_eq!(
        body["complianceScore"].as_str().unwrap_or_default(),
        "100.00"
    );
...
async fn test_evaluate_compliance_with_violations() {
    ...
    assert!(
        score < 100.0,
        "Score should be < 100 when violations found, got {}",
        score
    );
}
...
async fn test_evaluate_compliance_creates_violations() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .uri(&format!(
                    "/api/v1/expense-compliance/audits/{}/violations",
                    audit_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    assert!(
        !violations.is_empty(),
        "Should have violations when expense exceeds threshold"
    );
}
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/expense-compliance/audits/{}/complete",
                    audit_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/expense-compliance/audits/{}/escalate",
                    audit_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
...
async fn test_resolve_violation() {
    ...
    let r = app
        ...
        .await
        .unwrap();
    let violations: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    let violation_id = violations["data"].as_array().unwrap()[0]["id"]
        .as_str()
        .unwrap();
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/expense-compliance/violations/{}/resolve",
                    violation_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    app.clone()
        .oneshot(
            Request::builder()
                ...
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({"review_notes": "Done"})).unwrap(),
                ))
                .unwrap(),
        )
        .await
