...
async fn create_test_template(
    ...
    if status != StatusCode::CREATED {
        panic!(
            "Expected CREATED for template but got {}: {}",
            status,
            String::from_utf8_lossy(&b)
        );
    }
...
async fn test_create_library_template() {
    let (_state, app) = setup_test().await;
    let tmpl =
        create_test_template(&app, "REV_GROWTH", "Revenue Growth Target", "organization").await;
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({"status": "closed"})).unwrap(),
                ))
                .unwrap(),
        )
        .await
    ...
    let _ = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(format!(
                    "/api/v1/goal-management/plans/id/{}/status",
                    plan_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                .method("DELETE")
                .uri(format!(
                    "/api/v1/goal-management/alignments/id/{}",
                    align_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let _ = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(format!(
                    "/api/v1/goal-management/goals/id/{}/progress",
                    g1["id"].as_str().unwrap()
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
...
async fn test_goal_management_full_lifecycle() {
    ...
    let org_goal = create_test_goal(
        &app,
        "Company Goal: $10M Revenue",
        "organization",
        Some(plan_id),
    )
    .await;
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(format!(
                    "/api/v1/goal-management/goals/{}/notes",
                    ind_goal_id
                ))
                .header("Content-Type", "application/json")
                .header(&k5, &v5)
