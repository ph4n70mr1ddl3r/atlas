...
async fn setup_inventory_test() -> (std::sync::Arc<atlas_gateway::AppState>, axum::Router) {
    ...
    sqlx::query(include_str!(
        "../../../../migrations/030_inventory_management.sql"
    ))
...
async fn create_test_inv_org(
    ...
    assert_eq!(
        r.status(),
        StatusCode::CREATED,
        "Failed to create inventory org"
    );
...
async fn test_receive_and_issue_full_lifecycle() {
    ...
    let r = app
        ...
        .unwrap();
    assert_eq!(r.status(), StatusCode::OK);
    let balances: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    ...
    let issue_txn: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    ...
    let balances: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        ...
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    let remaining: f64 = balances["data"][0]["quantity"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    assert!(
        (remaining - 70.0).abs() < 0.01,
        "Expected 70, got {}",
        remaining
    );
...
async fn test_transfer_between_subinventories() {
    ...
    let r = app
        ...
        .unwrap();
    assert_eq!(r.status(), StatusCode::CREATED);
    let transfer: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    ...
    assert_eq!(
        entries.len(),
        2,
        "Expected 2 balance entries, got {}",
        entries.len()
    );
}
...
async fn test_quantity_adjustment() {
    ...
    let r = app
        ...
        .unwrap();
    assert_eq!(r.status(), StatusCode::CREATED);
    let adj: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    ...
    let balances: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        ...
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    let qty: f64 = balances["data"][0]["quantity"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
...
async fn test_transaction_history() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .uri(&format!(
                    "/api/v1/inventory/transactions?item_id={}",
                    item_id
                ))
                .header(&k, &v)
                .body(Body::empty())
        ...
        .unwrap();
    assert_eq!(r.status(), StatusCode::OK);
    let txns: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .uri(&format!(
                    "/api/v1/inventory/transactions?item_id={}&transaction_action=receive",
                    item_id
                ))
                .header(&k, &v)
                .body(Body::empty())
        ...
        .unwrap();
    assert_eq!(r.status(), StatusCode::OK);
    let receive_txns: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
