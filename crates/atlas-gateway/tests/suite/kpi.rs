...
async fn create_test_kpi(
    ...
    if status != StatusCode::CREATED {
        panic!(
            "Expected CREATED for KPI but got {}: {}",
            status,
...
async fn create_test_dashboard(app: &axum::Router, code: &str, name: &str) -> serde_json::Value {
    let (k, v) = auth_header(&admin_claims());
    let r = app
        .clone()
        .oneshot(
    ...
    if status != StatusCode::CREATED {
        panic!(
            "Expected CREATED for dashboard but got {}: {}",
            status,
            String::from_utf8_lossy(&b)
        );
    }
...
async fn test_get_latest_data_point() {
    ...
    let _ = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({"value": "50"})).unwrap(),
                ))
                .unwrap(),
        )
        .await
    ...
    let _ = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({"value": "75"})).unwrap(),
                ))
                .unwrap(),
        )
        .await
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                .uri(format!(
                    "/api/v1/kpi/definitions/{}/data-points/latest",
                    kpi_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_list_data_points() {
        ...
        let _ = app
            ...
            .oneshot(
                Request::builder()
                    ...
                    .header(&k, &v)
                    .body(Body::from(
                        serde_json::to_string(&json!({"value": val})).unwrap(),
                    ))
                    .unwrap(),
            )
            .await
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                .uri(format!(
                    "/api/v1/kpi/definitions/{}/data-points?limit=10",
                    kpi_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({"value": "99"})).unwrap(),
                ))
                .unwrap(),
        )
        .await
...
async fn test_list_widgets() {
    ...
    let (k, v) = auth_header(&admin_claims());
    // Add two widgets
    for (i, (wt, title)) in [("kpi_card", "Card 1"), ("trend", "Trend 1")]
        .iter()
        .enumerate()
    {
        let _ = app
    ...
    let _ = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(format!(
                    "/api/v1/kpi/definitions/{}/data-points",
                    kpi1["id"].as_str().unwrap()
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({"value": "110"})).unwrap(),
                ))
                .unwrap(),
        )
        .await
