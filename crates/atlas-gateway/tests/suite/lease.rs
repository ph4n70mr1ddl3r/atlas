...
async fn test_get_lease() {
    ...
    let lease =
        create_test_lease(&app, "Warehouse Lease", "operating", "96000.00", 48, "0.06").await;
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/lease/contracts/{}/modifications",
                    lease_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(&format!(
                    "/api/v1/lease/contracts/{}/terminations",
                    lease_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_lease_full_lifecycle() {
    ...
    let payments_made: f64 = updated["total_payments_made"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    ...
    let _ = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/lease/contracts/{}/activate",
                    lease1["id"].as_str().unwrap()
                ))
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
