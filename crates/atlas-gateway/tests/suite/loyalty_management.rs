...
async fn create_test_program(app: &axum::Router, number: &str, name: &str) -> serde_json::Value {
...
async fn test_list_transactions() {
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                .uri(format!(
                    "/api/v1/loyalty/members/{}/transactions",
                    member_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_redeem_reward() {
    ...
    let reward_resp = app
        ...
        .await
        .unwrap();
    let reward_body = axum::body::to_bytes(reward_resp.into_body(), usize::MAX)
        .await
        .unwrap();
    let rewards: serde_json::Value = serde_json::from_slice(&reward_body).unwrap();
    let reward_id = rewards["data"].as_array().unwrap()[0]["id"]
        .as_str()
        .unwrap();
...
async fn test_loyalty_full_lifecycle() {
    ...
    .await;
    create_test_tier(
        &app, program_id, "PREMIUM", "Premium", 2000.0, None, 30.0, false,
    )
    .await;
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(format!(
                    "/api/v1/loyalty/redemptions/{}/fulfill",
                    redemption_id
                ))
                .header(&k, &v)
                .body(Body::empty())
