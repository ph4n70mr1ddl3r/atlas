...
async fn setup_marketing_test() -> (std::sync::Arc<atlas_gateway::AppState>, axum::Router) {
    ...
    sqlx::query("DELETE FROM _atlas.campaign_responses")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM _atlas.campaign_members")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM _atlas.marketing_campaigns")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM _atlas.campaign_types")
        .execute(&state.db_pool)
        .await
        .ok();
...
async fn create_test_campaign_type(
    app: &axum::Router,
    code: &str,
    channel: &str,
) -> serde_json::Value {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/v1/marketing/campaign-types")
    ...
    assert_eq!(
        r.status(),
        StatusCode::CREATED,
        "Expected 201 for campaign type, got {}",
        r.status()
    );
...
async fn create_test_campaign(
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/v1/marketing/campaigns")
                .header("Content-Type", "application/json")
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({
...
async fn test_delete_campaign_type() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("DELETE")
                .uri(&format!("/api/v1/marketing/campaign-types/{}", code))
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_create_campaign() {
    ...
    assert!(campaign["campaignNumber"]
        .as_str()
        .unwrap()
        .starts_with("MKT-001"));
...
async fn test_list_campaigns() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/api/v1/marketing/campaigns?status=draft")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_campaign_lifecycle_activate_pause_complete() {
    ...
    assert_eq!(
        r.status(),
        StatusCode::OK,
        "Expected OK on resume from paused, got {}",
        r.status()
    );
    let b = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/marketing/campaigns/{}/members",
                    campaign_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    app.clone()
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/marketing/campaigns/{}/activate",
                    campaign_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/marketing/campaigns/{}/responses",
                    campaign_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
...
async fn test_marketing_dashboard() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/api/v1/marketing/dashboard")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
