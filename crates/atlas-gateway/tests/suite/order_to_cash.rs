...
async fn seed_customer(app: &axum::Router, admin: &Claims) -> String {
    let customer = create_record(
        app,
        "crm_customers",
        json!({
...
async fn seed_o2c_products(app: &axum::Router, admin: &Claims) -> (String, String) {
...
async fn test_o2c_full_happy_path() {
    ...
    let (so_id, line_ids) = create_so_with_lines(
        &app,
        &admin,
        &customer_id,
        vec![(product_id.clone(), 50.0, 99.99)],
    )
    .await;
    ...
    // Deliver shipment
    let result =
        execute_workflow_action(&app, "scm_shipments", &shipment_id, "deliver", &wh_clerk).await;
    assert_eq!(result["success"], true);
    ...
    // Mark SO as invoiced
    let result =
        execute_workflow_action(&app, "scm_sales_orders", &so_id, "invoice", &fin_mgr).await;
    assert_eq!(result["success"], true);
    ...
    let result =
        execute_workflow_action(&app, "scm_sales_orders", &so_id, "complete", &sales_mgr).await;
    assert_eq!(result["success"], true);
...
async fn test_o2c_lead_disqualification() {
    ...
    let result =
        execute_workflow_action(&app, "crm_leads", &lead_id, "disqualify", &sales_mgr).await;
    assert_eq!(result["success"], true);
...
async fn test_o2c_cancel_order_at_draft() {
    ...
    let (so_id, _) =
        create_so_with_lines(&app, &admin, &customer_id, vec![(product_id, 10.0, 50.0)]).await;
...
async fn test_o2c_cancel_confirmed_order_requires_manager() {
    ...
    let (product_id, _) = seed_o2c_products(&app, &admin).await;
    let (so_id, _) =
        create_so_with_lines(&app, &admin, &customer_id, vec![(product_id, 5.0, 100.0)]).await;

    execute_workflow_action(&app, "scm_sales_orders", &so_id, "confirm", &sales_mgr).await;

    // Regular user cannot cancel
    let result =
        execute_workflow_action(&app, "scm_sales_orders", &so_id, "cancel", &wh_clerk).await;
    assert_eq!(result["success"], false);

    // Sales manager can cancel
    let result =
        execute_workflow_action(&app, "scm_sales_orders", &so_id, "cancel", &sales_mgr).await;
    assert_eq!(result["success"], true);
    assert_eq!(result["to_state"], "cancelled");
...
async fn test_o2c_partial_shipment() {
    ...
    let (so_id, line_ids) = create_so_with_lines(
        ...
        &admin,
        &customer_id,
        vec![(product_id.clone(), 100.0, 99.99)],
    )
    .await;
...
async fn test_o2c_customer_multiple_orders() {
    ...
    // Create 3 orders
    let (so1, _) = create_so_with_lines(
        &app,
        &admin,
        &customer_id,
        vec![(product_id.clone(), 10.0, 99.99)],
    )
    .await;
    let (so2, _) = create_so_with_lines(
        &app,
        &admin,
        &customer_id,
        vec![(product_id.clone(), 20.0, 99.99)],
    )
    .await;
    let (so3, _) = create_so_with_lines(
        &app,
        &admin,
        &customer_id,
        vec![(product_id.clone(), 30.0, 99.99)],
    )
    .await;
    ...
    let customer_orders: Vec<_> = list["data"]
        .as_array()
        .unwrap()
        .iter()
...
async fn test_o2c_contact_management() {
    ...
    let customer_contacts: Vec<_> = list["data"]
        .as_array()
        .unwrap()
        .iter()
...
async fn test_o2c_shipment_tracking() {
    ...
    let (product_id, wh_id) = seed_o2c_products(&app, &admin).await;
    let (so_id, _line_ids) =
        create_so_with_lines(&app, &admin, &customer_id, vec![(product_id, 10.0, 99.99)]).await;
...
async fn test_o2c_customer_crud() {
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                .uri(format!("/api/v1/crm_customers/{}", id))
                .header(k, v)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_o2c_customer_payment() {
    ...
    // Mark invoice paid
    let result =
        execute_workflow_action(&app, "fin_invoices", &inv_id, "mark_paid", &fin_mgr).await;
    assert_eq!(result["success"], true);
    assert_eq!(result["to_state"], "paid");
...
async fn test_o2c_multiple_invoices_same_customer() {
    ...
    let customer_invoices: Vec<_> = list["data"]
        .as_array()
        .unwrap()
        .iter()
...
async fn test_o2c_shipment_lifecycle() {
    ...
    let result =
        execute_workflow_action(&app, "scm_shipments", &ship_id, "deliver", &wh_clerk).await;
    assert_eq!(result["success"], true);
    assert_eq!(result["to_state"], "delivered");
...
async fn test_o2c_warehouse_cannot_confirm_order() {
    ...
    let result =
        execute_workflow_action(&app, "scm_sales_orders", &so_id, "confirm", &wh_clerk).await;
    assert_eq!(result["success"], false);
