...
async fn setup_test() -> (std::sync::Arc<atlas_gateway::AppState>, axum::Router) {
    ...
    sqlx::query("DELETE FROM _atlas.settlement_activities")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM _atlas.settlement_lines")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM _atlas.settlement_batches")
        .execute(&state.db_pool)
        .await
        .ok();
    ...
    sqlx::raw_sql(include_str!(
        "../../../../migrations/131_payment_settlement.sql"
    ))
    .execute(&state.db_pool)
    .await
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({"reason": "Payment error"})).unwrap(),
                ))
                .unwrap(),
        )
        .await
...
async fn test_settle_marks_lines_settled() {
    ...
    assert!(
        lines.iter().all(|l| l["status"] == "settled"),
        "All lines should be settled"
    );
