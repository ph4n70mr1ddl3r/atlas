...
async fn setup_pricing_test() -> (std::sync::Arc<atlas_gateway::AppState>, axum::Router) {
    ...
    sqlx::query(include_str!(
        "../../../../migrations/032_advanced_pricing.sql"
    ))
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/pricing/price-lists/{}/lines",
                    price_list_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
...
async fn create_test_charge(
    ...
    assert_eq!(
        r.status(),
        StatusCode::CREATED,
        "Failed to create charge definition"
    );
    let b = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .unwrap();
...
async fn test_price_list_crud() {
    ...
    // Create
    let pl =
        create_test_price_list(&app, "STD-SALE", "Standard Sale Price List", "sale", "USD").await;
...
async fn test_discount_rule_crud() {
    ...
    // Create
    let rule =
        create_test_discount_rule(&app, "SUMMER-10", "Summer 10% Off", "percentage", "10").await;
...
async fn test_pricing_strategy() {
    ...
    let r = app
        ...
        .unwrap();
    assert_eq!(r.status(), StatusCode::CREATED);
    let strategy: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
...
async fn test_price_calculation_with_discount_and_charge() {
    ...
    let r = app
        ...
        .unwrap();
    assert_eq!(r.status(), StatusCode::OK);
    let result: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    ...
    let selling: f64 = result["unit_selling_price"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    ...
    assert!(
        (list_price - 100.0).abs() < 0.01,
        "Expected list_price 100, got {}",
        list_price
    );
    assert!(
        (discount - 10.0).abs() < 0.01,
        "Expected discount 10, got {}",
        discount
    );
    assert!(
        (selling - 90.0).abs() < 0.01,
        "Expected unit_selling 90, got {}",
        selling
    );
    assert!(
        (extended - 275.0).abs() < 0.01,
        "Expected extended 275, got {}",
        extended
    );
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .uri(&format!(
                    "/api/v1/pricing/calculation-logs?entity_type=sales_order&entity_id={}",
                    entity_id
                ))
                .header(&k, &v)
                .body(Body::empty())
        ...
        .unwrap();
    assert_eq!(r.status(), StatusCode::OK);
    let logs: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
...
async fn test_list_price_lists_by_type() {
    ...
    let r = app
        ...
        .await
        .unwrap();
    let sale_only: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
