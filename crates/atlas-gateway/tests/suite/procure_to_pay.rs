...
async fn seed_master_data(app: &axum::Router, admin: &Claims) -> (String, String, String) {
    let supplier = create_record(
        app,
        "scm_suppliers",
        json!({
...
async fn test_p2p_full_happy_path() {
    ...
    let (po_id, line_ids) = create_po_with_lines(
        &app,
        &admin,
        &supplier_id,
        vec![(product_id.clone(), 100.0, 29.99)],
    )
    .await;
    ...
    let result =
        execute_workflow_action(&app, "fin_invoices", &invoice_id, "approve", &fin_mgr).await;
    assert_eq!(result["success"], true);
    assert_eq!(result["to_state"], "approved");
    ...
    let result =
        execute_workflow_action(&app, "fin_invoices", &invoice_id, "mark_paid", &fin_mgr).await;
    assert_eq!(result["success"], true);
...
async fn test_p2p_po_rejection_and_resubmit() {
    ...
    let (po_id, _) =
        create_po_with_lines(&app, &admin, &supplier_id, vec![(product_id, 50.0, 10.0)]).await;
    ...
    // Reject
    let result =
        execute_workflow_action(&app, "scm_purchase_orders", &po_id, "reject", &buyer).await;
    assert_eq!(result["success"], true);
    ...
    let actions: Vec<&str> = transitions["transitions"]
        .as_array()
        .unwrap()
        .iter()
    ...
    let result =
        execute_workflow_action(&app, "scm_purchase_orders", &po_id, "resubmit", &admin).await;
    assert_eq!(result["success"], true);
...
async fn test_p2p_cancel_po_at_draft() {
    ...
    let (supplier_id, product_id, _) = seed_master_data(&app, &admin).await;
    let (po_id, _) =
        create_po_with_lines(&app, &admin, &supplier_id, vec![(product_id, 25.0, 50.0)]).await;

    // Cancel from draft
    let result =
        execute_workflow_action(&app, "scm_purchase_orders", &po_id, "cancel", &admin).await;
    assert_eq!(result["success"], true);
...
async fn test_p2p_cannot_cancel_submitted_po() {
    ...
    let (supplier_id, product_id, _) = seed_master_data(&app, &admin).await;
    let (po_id, _) =
        create_po_with_lines(&app, &admin, &supplier_id, vec![(product_id, 10.0, 20.0)]).await;
...
async fn test_p2p_role_check_warehouse_cannot_approve_po() {
    ...
    let (supplier_id, product_id, _) = seed_master_data(&app, &admin).await;
    let (po_id, _) =
        create_po_with_lines(&app, &admin, &supplier_id, vec![(product_id, 100.0, 5.0)]).await;

    execute_workflow_action(&app, "scm_purchase_orders", &po_id, "submit", &admin).await;

    // Warehouse clerk tries to approve - should fail
    let result =
        execute_workflow_action(&app, "scm_purchase_orders", &po_id, "approve", &wh_clerk).await;
...
async fn test_p2p_inventory_updated_on_receipt() {
    ...
    let (po_id, line_ids) = create_po_with_lines(
        ...
        &admin,
        &supplier_id,
        vec![(product_id.clone(), 200.0, 15.0)],
    )
    .await;
    execute_workflow_action(&app, "scm_purchase_orders", &po_id, "submit", &admin).await;
    execute_workflow_action(&app, "scm_purchase_orders", &po_id, "approve", &buyer).await;
...
async fn test_p2p_partial_goods_receipt() {
    ...
    let (po_id, line_ids) = create_po_with_lines(
        ...
        &admin,
        &supplier_id,
        vec![(product_id.clone(), 100.0, 25.0)],
    )
    .await;
...
async fn test_p2p_duplicate_po_number() {
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/v1/scm_purchase_orders")
...
async fn test_p2p_gl_journal_entry_for_invoice() {
    ...
    let expense_account = create_record(
        &app,
        "fin_chart_of_accounts",
        json!({
            "account_number": "5000",
    ...
    let total_debit: f64 = je_lines
        .iter()
        .map(|l| l["debit_amount"].as_f64().unwrap_or(0.0))
        .sum();
    let total_credit: f64 = je_lines
        .iter()
        .map(|l| l["credit_amount"].as_f64().unwrap_or(0.0))
        .sum();
...
async fn test_p2p_audit_trail() {
    ...
    assert!(
        history2["history"].as_array().unwrap().len()
...
async fn test_p2p_payment_reconciliation() {
    ...
    // Reconcile
    let result =
        execute_workflow_action(&app, "fin_payments", &payment_id, "reconcile", &fin_mgr).await;
    assert_eq!(result["success"], true);
...
async fn test_p2p_no_transition_from_closed() {
    ...
    let (supplier_id, product_id, _) = seed_master_data(&app, &admin).await;
    let (po_id, _) =
        create_po_with_lines(&app, &admin, &supplier_id, vec![(product_id, 10.0, 10.0)]).await;
    execute_workflow_action(&app, "scm_purchase_orders", &po_id, "submit", &admin).await;
    execute_workflow_action(&app, "scm_purchase_orders", &po_id, "approve", &buyer).await;
    execute_workflow_action(&app, "scm_purchase_orders", &po_id, "receive", &wh_clerk).await;
    execute_workflow_action(&app, "scm_purchase_orders", &po_id, "invoice", &fin_mgr).await;
...
async fn test_p2p_supplier_crud() {
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                .uri(format!("/api/v1/scm_suppliers/{}", id))
                .header(k, v)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_p2p_regular_user_cannot_approve_invoice() {
    ...
    let result =
        execute_workflow_action(&app, "fin_invoices", &inv_id, "approve", &regular_user).await;
    assert_eq!(result["success"], false);
...
async fn test_p2p_multiple_pos_same_supplier() {
    ...
    let (po1, _) = create_po_with_lines(
        &app,
        &admin,
        &supplier_id,
        vec![(product_id.clone(), 10.0, 25.0)],
    )
    .await;
    let (po2, _) = create_po_with_lines(
        &app,
        &admin,
        &supplier_id,
        vec![(product_id.clone(), 20.0, 25.0)],
    )
    .await;
    let (po3, _) = create_po_with_lines(
        &app,
        &admin,
        &supplier_id,
        vec![(product_id.clone(), 30.0, 25.0)],
    )
    .await;
...
async fn test_p2p_update_po_line_quantity() {
    ...
    let (supplier_id, product_id, _) = seed_master_data(&app, &admin).await;
    let (po_id, line_ids) =
        create_po_with_lines(&app, &admin, &supplier_id, vec![(product_id, 50.0, 20.0)]).await;
...
async fn test_p2p_payment_confirm_requires_role() {
    ...
    let result =
        execute_workflow_action(&app, "fin_payments", &pay_id, "confirm", &regular_user).await;
    assert_eq!(result["success"], false);
...
async fn test_p2p_supplier_with_no_orders() {
    ...
    let list = list_records(&app, "scm_purchase_orders", &admin).await;
    let supplier_pos: Vec<_> = list["data"]
        .as_array()
        .unwrap()
        .iter()
...
async fn test_p2p_direct_goods_receipt_no_po() {
    ...
    // Close it
    let result =
        execute_workflow_action(&app, "scm_goods_receipts", &gr_id, "close", &wh_clerk).await;
    assert_eq!(result["success"], true);
    assert_eq!(result["to_state"], "closed");
