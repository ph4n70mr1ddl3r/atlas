...
async fn setup_procurement_contracts_test(
) -> (std::sync::Arc<atlas_gateway::AppState>, axum::Router) {
    let state = build_test_state().await;
    cleanup_test_db(&state.db_pool).await;
    setup_test_db(&state.db_pool).await;
    // Run migration for procurement contracts tables
    sqlx::query(include_str!(
        "../../../../migrations/029_procurement_contracts.sql"
    ))
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/procurement-contracts/{}/lines",
                    contract_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/procurement-contracts/{}/milestones",
                    contract_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
...
async fn test_contract_full_lifecycle() {
    ...
    // Step 3: Add lines
    let line1 =
        add_test_contract_line(&app, &contract_id, "Laptop Computers", "10", "1200.00").await;
    ...
    let line2 =
        add_test_contract_line(&app, &contract_id, "Desktop Monitors", "20", "350.00").await;
    ...
    let r = app
        ...
        .await
        .unwrap();
    let updated: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    // total_committed_amount should reflect line amounts (12000 + 7000 = 19000)
    let committed: f64 = updated["total_committed_amount"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    assert!(
        (committed - 19000.0).abs() < 0.01,
        "Expected 19000, got {}",
        committed
    );
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/procurement-contracts/{}/submit",
                    contract_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/procurement-contracts/{}/approve",
                    contract_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/procurement-contracts/{}/spend",
                    contract_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    let r = app
        ...
        .unwrap();
    assert_eq!(r.status(), StatusCode::OK);
    let ms: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/procurement-contracts/{}/close",
                    contract_id
                ))
                .header(&k, &v)
                .body(Body::empty())
        ...
        .unwrap();
    assert_eq!(r.status(), StatusCode::OK);
    let closed: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
...
async fn test_contract_rejection() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/procurement-contracts/{}/reject",
                    contract_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
        ...
        .unwrap();
    assert_eq!(r.status(), StatusCode::OK);
    let rejected: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    ...
    assert_eq!(
        ...
        "Budget constraints - try next quarter"
    );
...
async fn test_contract_termination() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/procurement-contracts/{}/terminate",
                    contract_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    assert_eq!(
        ...
        "Supplier failed to meet SLA requirements"
    );
}
...
async fn test_contract_renewal() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/procurement-contracts/{}/renewals",
                    contract_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
        ...
        .unwrap();
    assert_eq!(r.status(), StatusCode::CREATED);
    let renewal: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    ...
    let r = app
        ...
        .unwrap();
    assert_eq!(r.status(), StatusCode::OK);
    let renewals: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
...
async fn test_spend_tracking() {
    ...
    let updated: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        ...
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    let released: f64 = updated["total_released_amount"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    assert!(
        (released - 30000.0).abs() < 0.01,
        "Expected 30000, got {}",
        released
    );
    ...
    let r = app
        ...
        .unwrap();
    assert_eq!(r.status(), StatusCode::OK);
    let spend: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
...
async fn test_auto_approve_without_approval_requirement() {
    ...
    let r = app
        ...
        .await
        .unwrap();
    let contract: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    let contract_id = contract["id"].as_str().unwrap();
