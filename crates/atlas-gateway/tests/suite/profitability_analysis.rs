...
async fn setup_test() -> (std::sync::Arc<atlas_gateway::AppState>, axum::Router) {
    ...
    sqlx::raw_sql(include_str!(
        "../../../../migrations/129_profitability_analysis.sql"
    ))
    .execute(&state.db_pool)
    .await
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({"status": "calculated"})).unwrap(),
                ))
                .unwrap(),
        )
        .await
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({"status": "reviewed"})).unwrap(),
                ))
                .unwrap(),
        )
        .await
...
async fn test_add_lines_and_margins() {
    ...
    assert!(
        (gm - 4000.0).abs() < 0.01,
        "Expected gross margin 4000, got {}",
        gm
    );
    let gm_pct: f64 = line["grossMarginPct"].as_f64().unwrap();
    assert!(
        (gm_pct - 40.0).abs() < 0.01,
        "Expected gross margin % 40, got {}",
        gm_pct
    );
    ...
    assert!(
        (om - 2500.0).abs() < 0.01,
        "Expected operating margin 2500, got {}",
        om
    );
...
async fn test_run_totals_recalculated() {
    ...
    assert!(
        (total_rev - 15000.0).abs() < 0.01,
        "Expected total revenue 15000, got {}",
        total_rev
    );

    let total_gm: f64 = body["totalGrossMargin"].as_f64().unwrap();
    assert!(
        (total_gm - 7000.0).abs() < 0.01,
        "Expected total gross margin 7000, got {}",
        total_gm
    );

    let total_nm: f64 = body["totalNetMargin"].as_f64().unwrap();
    assert!(
        (total_nm - 4700.0).abs() < 0.01,
        "Expected total net margin 4700, got {}",
        total_nm
    );

    let gm_pct: f64 = body["grossMarginPct"].as_f64().unwrap();
    assert!(
        (gm_pct - 46.666666666666664).abs() < 0.01,
        "Expected GM% ~46.67, got {}",
        gm_pct
    );
}
...
async fn test_remove_line_and_recalc() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("DELETE")
                .uri(&format!(
                    "/api/v1/profitability/runs/{}/lines/{}",
                    run_id, line1_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let total_rev: f64 = body["totalRevenue"].as_f64().unwrap();
    assert!(
        (total_rev - 5000.0).abs() < 0.01,
        "Expected total revenue 5000, got {}",
        total_rev
    );
}
...
async fn test_contribution_percentages() {
    ...
    assert!(
        rev_contrib > 59.0 && rev_contrib < 61.0,
        "Expected ~60% contribution, got {}",
        rev_contrib
    );
}
