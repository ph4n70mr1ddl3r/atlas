...
async fn create_test_location(app: &axum::Router, code: &str, name: &str) -> serde_json::Value {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/v1/receiving/locations")
...
async fn test_delete_location() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("DELETE")
                .uri("/api/v1/receiving/locations/WH-DEL")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_list_receipts() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/api/v1/receiving/receipts?status=draft")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_create_and_complete_inspection() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/receiving/receipts/{}/inspections",
                    rcpt_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    assert!(inspection["inspectionNumber"]
        .as_str()
        .unwrap()
        .starts_with("INS-"));
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/receiving/inspections/{}/complete",
                    insp_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    assert!(
        (qs.parse::<f64>().unwrap() - 90.5).abs() < 0.01,
        "quality_score was {}",
        qs
    );
...
async fn test_inspection_accepted_rejected_must_equal_inspected() {
    ...
    let r = app
        ...
        .await
        .unwrap();
    let insp: serde_json::Value = serde_json::from_slice(
        &axum::body::to_bytes(r.into_body(), usize::MAX)
            .await
            .unwrap(),
    )
        ...
        let r = app
            ...
            .oneshot(
                Request::builder()
                    .method("POST")
                    .uri(&format!(
                        "/api/v1/receiving/inspections/{}/details",
                        insp_id
                    ))
                    .header("Content-Type", "application/json")
                    .header(&k, &v)
...
async fn test_create_delivery() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/receiving/receipts/{}/deliveries",
                    rcpt_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    assert!(delivery["deliveryNumber"]
        .as_str()
        .unwrap()
        .starts_with("DEL-"));
}
...
async fn test_cancel_return() {
    ...
    let r = app
        ...
        .await
        .unwrap();
    let ret: serde_json::Value = serde_json::from_slice(
        &axum::body::to_bytes(r.into_body(), usize::MAX)
            .await
            .unwrap(),
    )
...
async fn test_list_returns() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/api/v1/receiving/returns?status=draft")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_receiving_dashboard() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/api/v1/receiving/dashboard")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
