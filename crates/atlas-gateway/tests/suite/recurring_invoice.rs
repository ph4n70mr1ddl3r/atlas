...
async fn setup_test() -> (std::sync::Arc<atlas_gateway::AppState>, axum::Router) {
    ...
    sqlx::raw_sql(include_str!(
        "../../../../migrations/130_recurring_invoice_management.sql"
    ))
    .execute(&state.db_pool)
    .await
    ...
    app.clone()
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/recurring-invoices/{}/transition",
                    tmpl2_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("DELETE")
                .uri(&format!(
                    "/api/v1/recurring-invoices/{}/lines/{}",
                    tmpl_id, line1_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_generate_invoice() {
    ...
    let _: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    ...
    assert!(
        (inv_amt - 5000.0).abs() < 0.01,
        "Expected invoice amount 5000, got {}",
        inv_amt
    );

    let tax_amt: f64 = body["taxAmount"].as_f64().unwrap();
    assert!(
        (tax_amt - 450.0).abs() < 0.01,
        "Expected tax amount 450, got {}",
        tax_amt
    );

    let total: f64 = body["totalAmount"].as_f64().unwrap();
    assert!(
        (total - 5450.0).abs() < 0.01,
        "Expected total 5450, got {}",
        total
    );
...
async fn test_generate_updates_template() {
    ...
    assert!(
        ...
        "Expected total generated 3000, got {}",
        total
    );
