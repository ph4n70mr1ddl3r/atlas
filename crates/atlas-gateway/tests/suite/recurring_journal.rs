...
async fn setup_test() -> (std::sync::Arc<atlas_gateway::AppState>, axum::Router) {
    ...
    sqlx::query(include_str!(
        "../../../../migrations/040_recurring_journals.sql"
    ))
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/recurring-journals/schedules/{}/lines",
                    schedule_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/recurring-journals/schedules/{}/activate",
                    schedule_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/recurring-journals/schedules/{}/deactivate",
                    schedule_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_standard_generation_workflow() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/recurring-journals/schedules/{}/generate",
                    schedule_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    assert_eq!(r.status(), StatusCode::CREATED);
    let gen: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .uri(&format!(
                    "/api/v1/recurring-journals/generations/{}/lines",
                    gen_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/recurring-journals/generations/{}/post",
                    gen_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/recurring-journals/generations/{}/reverse",
                    gen_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/recurring-journals/generations/{}/cancel",
                    gen_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_list_generations() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .uri(&format!(
                    "/api/v1/recurring-journals/schedules/{}/generations",
                    schedule_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_incremental_generation() {
    ...
    let r = app
        ...
        .await
        .unwrap();
    let schedule: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    let schedule_id: Uuid = schedule["id"].as_str().unwrap().parse().unwrap();
    ...
    let r = app
        ...
        .await
        .unwrap();
    let gen1: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    ...
    let r = app
        ...
        .await
        .unwrap();
    let _lines1: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    ...
    let r = app
        ...
        .await
        .unwrap();
    let gen2: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();
    ...
    let r = app
        ...
        .await
        .unwrap();
    let lines2: serde_json::Value = axum::body::to_bytes(r.into_body(), usize::MAX)
        .await
        .map(|b| serde_json::from_slice(&b).unwrap())
        .unwrap();

    // Verify the second generation amount is higher than first (1100 vs 1000)
    let gen2_debits: f64 = lines2["data"]
        .as_array()
        .unwrap()
        .iter()
    ...
    assert!(
        gen2_debits > 1000.0,
        "Second generation should have incremented amount, got {}",
        gen2_debits
    );
}
