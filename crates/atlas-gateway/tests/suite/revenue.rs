...
async fn setup_revenue_test() -> (std::sync::Arc<atlas_gateway::AppState>, axum::Router) {
    ...
    sqlx::query("DELETE FROM _atlas.revenue_schedule_lines")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM _atlas.revenue_modifications")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM _atlas.performance_obligations")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM _atlas.revenue_contracts")
        .execute(&state.db_pool)
        .await
        .ok();
    sqlx::query("DELETE FROM _atlas.revenue_policies")
        .execute(&state.db_pool)
        .await
        .ok();
...
async fn test_get_revenue_policy() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/api/v1/revenue/policies/POL-GET")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_delete_revenue_policy() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("DELETE")
                .uri("/api/v1/revenue/policies/POL-DEL")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_list_revenue_contracts() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/api/v1/revenue/contracts?status=draft")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/revenue/contracts/{}/obligations",
                    contract_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
...
async fn test_allocate_transaction_price() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/revenue/contracts/{}/allocate",
                    contract_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let resp: serde_json::Value = serde_json::from_slice(&b).unwrap_or_else(|_| {
        panic!(
            "Allocate response not JSON: {}",
            String::from_utf8_lossy(&b)
        );
    });
    let obligations = resp["data"]
        .as_array()
        .unwrap_or_else(|| panic!("Expected data array: {:?}", resp));
    assert_eq!(obligations.len(), 2, "allocate response: {:?}", resp);

    // Verify proportional allocation: 60k/100k * 100k = 60000, 40k/100k * 100k = 40000
    let lic = obligations
        .iter()
        .find(|o| o["description"] == "License")
        .unwrap();
    let sup = obligations
        .iter()
        .find(|o| o["description"] == "Support")
        .unwrap();
    // Check allocated prices are roughly correct (may have rounding)
    let lic_alloc: f64 = lic["allocated_transaction_price"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    let sup_alloc: f64 = sup["allocated_transaction_price"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/revenue/obligations/{}/schedule/straight-line",
                    obligation_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/revenue/obligations/{}/schedule/point-in-time",
                    obligation_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/revenue/schedule-lines/{}/recognize",
                    line_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/revenue/schedule-lines/{}/reverse",
                    line_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(&format!(
                    "/api/v1/revenue/obligations/{}/schedule-lines",
                    obligation_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(&format!(
                    "/api/v1/revenue/contracts/{}/schedule-lines",
                    contract_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    app.clone()
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/revenue/contracts/{}/activate",
                    contract_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(&format!(
                    "/api/v1/revenue/contracts/{}/modifications",
                    contract_id
                ))
                .header("Content-Type", "application/json")
                .header(&k, &v)
...
async fn test_full_asc606_five_step_flow() {
    ...
    let platform = obligations
        .iter()
        .find(|o| o["description"] == "Platform License")
        .unwrap();
    let impl_svc = obligations
        .iter()
        .find(|o| o["description"] == "Implementation Services")
        .unwrap();
    let platform_alloc: f64 = platform["allocated_transaction_price"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    let impl_alloc: f64 = impl_svc["allocated_transaction_price"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
