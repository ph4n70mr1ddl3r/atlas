    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({"status": "pending_customer"})).unwrap(),
                ))
                .unwrap(),
        )
        .await
...
async fn test_list_assignments() {
    ...
    assert!(
        assignments.len() >= 1,
        "Expected at least 1 assignment, got {}",
        assignments.len()
    );
    // Verify we have at least one with escalation type
    let escalation_count = assignments
        .iter()
        .filter(|a| a["assignmentType"] == "escalation")
        .count();
    assert!(
        escalation_count >= 1,
        "Expected at least 1 escalation assignment"
    );
}
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(&format!(
                    "/api/v1/service/requests/{}/updates?include_internal=false",
                    id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(&format!(
                    "/api/v1/service/requests/{}/updates?include_internal=true",
                    id
                ))
                .header(&k, &v)
                .body(Body::empty())
