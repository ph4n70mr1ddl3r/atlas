...
async fn test_create_accounting_method() {
    ...
    let method = state
        ...
        .create_accounting_method(
            ...
            "AP_STD",
            "AP Standard Method",
            Some("Standard payables accounting"),
            "payables",
            "invoice",
            None,
            Some(true),
...
async fn test_create_method_invalid_application() {
    ...
    assert!(
        matches!(err, atlas_shared::AtlasError::ValidationFailed(msg) if msg.contains("Invalid application"))
    );
}
...
async fn test_get_accounting_method() {
    ...
    let fetched = state
        .financials
        .sla_engine
        .get_accounting_method(org_id(), "GET_TEST")
        .await
        .unwrap()
        .unwrap();
...
async fn test_get_nonexistent_method() {
    ...
    let result = state
        .financials
        .sla_engine
        .get_accounting_method(org_id(), "NO_SUCH")
        .await
        .unwrap();
...
async fn test_list_accounting_methods() {
    ...
    let methods = state
        .financials
        .sla_engine
        .list_accounting_methods(org_id(), None)
        .await
        .unwrap();
...
async fn test_list_methods_by_application() {
    ...
    let payables = state
        .financials
        .sla_engine
        .list_accounting_methods(org_id(), Some("payables"))
        .await
        .unwrap();
...
async fn test_delete_accounting_method() {
    ...
    state
        .financials
        .sla_engine
        .delete_accounting_method(org_id(), "DEL_ME")
        .await
        .unwrap();
    let result = state
        .financials
        .sla_engine
        .get_accounting_method(org_id(), "DEL_ME")
        .await
        .unwrap();
    assert!(result.is_none());
}
...
async fn test_delete_nonexistent_method() {
    ...
    let result = state
        .financials
        .sla_engine
        .delete_accounting_method(org_id(), "NO_SUCH")
        .await;
    assert!(result.is_err());
}
...
async fn test_create_lookup_derivation_rule() {
    ...
    let rule = state
        ...
        .create_derivation_rule(
            ...
            method.id,
            "EXP_DEBIT_CAT",
            "Expense Debit by Category",
            None,
            "debit",
            10,
            serde_json::json!({}),
            Some("expense_category"),
            "lookup",
            None,
            lookup,
            None,
...
async fn test_constant_rule_requires_fixed_account() {
    ...
    let err = result.unwrap_err();
    assert!(
        matches!(err, atlas_shared::AtlasError::ValidationFailed(msg) if msg.contains("fixed_account_code"))
    );
}
...
async fn test_lookup_rule_requires_source_field() {
    ...
    let err = result.unwrap_err();
    assert!(
        matches!(err, atlas_shared::AtlasError::ValidationFailed(msg) if msg.contains("source_field"))
    );
}
...
async fn test_list_derivation_rules() {
    ...
    let rules = state
        .financials
        .sla_engine
        .list_derivation_rules(org_id(), method.id)
        .await
        .unwrap();
    assert!(rules.len() >= 2);
}
...
async fn test_delete_derivation_rule() {
    ...
    state
        .financials
        .sla_engine
        .delete_derivation_rule(org_id(), method.id, "DEL_RULE")
        .await
        .unwrap();
}
...
async fn test_list_entries_by_status() {
    ...
    let drafts = state
        .financials
        .sla_engine
        .list_journal_entries(org_id(), Some("draft"), None, None, None, None)
        .await
        .unwrap();
    assert!(drafts.len() >= 1);
    for e in &drafts {
...
async fn test_list_entries_invalid_status() {
    ...
    let result = state
        .financials
        .sla_engine
        .list_journal_entries(org_id(), Some("invalid_status"), None, None, None, None)
        .await;
    assert!(result.is_err());
}
...
async fn create_balanced_entry(
    state: &Arc<atlas_gateway::AppState>,
    amount: &str,
) -> atlas_shared::SubledgerJournalEntry {
    let entry = state
        .financials
        .sla_engine
        .create_journal_entry(
...
async fn test_account_unbalanced_entry_rejected() {
    ...
    let result = state
        .financials
        .sla_engine
        .account_entry(entry.id, Some(user_id()))
        .await;
    assert!(result.is_err());
}
...
async fn test_full_entry_lifecycle() {
    ...
    let posted = state
        .financials
        .sla_engine
        .post_entry(entry.id, Some(user_id()))
        .await
        .unwrap();
...
async fn test_reverse_posted_entry() {
    ...
    let reversed = state
        .financials
        .sla_engine
        .reverse_entry(entry.id, "Incorrect posting", Some(user_id()))
        .await
        .unwrap();
...
async fn test_reverse_requires_reason() {
    ...
    let result = state
        .financials
        .sla_engine
        .reverse_entry(entry.id, "", Some(user_id()))
        .await;
    assert!(result.is_err());
}
...
async fn test_cannot_reverse_draft_entry() {
    ...
    let result = state
        .financials
        .sla_engine
        .reverse_entry(entry.id, "Should fail", Some(user_id()))
        .await;
    assert!(result.is_err());
}
...
async fn test_transfer_to_gl() {
    ...
    let transfer = state
        .financials
        .sla_engine
        .transfer_to_gl(org_id(), None, None, Some(user_id()))
        .await
        .unwrap();
...
async fn test_list_transfer_logs() {
    ...
    let _logs = state
        .financials
        .sla_engine
        .list_transfer_logs(org_id(), None)
        .await
        .unwrap();
...
async fn test_auto_generate_lines() {
    ...
    let lines = state
        ...
        .generate_journal_lines(
            org_id(),
            entry.id,
            &serde_json::json!({"amount": 2500.0, "expense_category": "Travel"}),
        )
        .await
        .unwrap();
    assert_eq!(
        lines.len(),
        2,
        "Expected 2 auto-generated lines (debit + credit)"
    );
    // Verify entry is balanced
    let updated = state
        .financials
...
async fn test_list_sla_events() {
    ...
    let _events = state
        .financials
        .sla_engine
        .list_sla_events(org_id(), None, None)
        .await
        .unwrap();
    // Just verify no error — the call succeeded
}
...
async fn test_resolve_account_code_constant() {
    ...
    let rules = state
        .financials
        .sla_engine
        .list_active_derivation_rules(org_id(), method.id, "debit")
        .await
        .unwrap();
    let result = state.financials.sla_engine.resolve_account_code(
        &rules,
        "debit",
...
async fn test_resolve_wrong_line_type_returns_none() {
    ...
    let result =
        state
            .financials
            .sla_engine
            .resolve_account_code(&rules, "credit", &serde_json::json!({}));
    assert!(result.is_none());
}
