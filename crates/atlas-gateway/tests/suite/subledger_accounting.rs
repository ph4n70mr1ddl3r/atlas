...
async fn test_create_accounting_method() {
    ...
    let method = state.financials.sla_engine.create_accounting_method(
...
async fn test_create_method_invalid_application() {
    let state = setup_sla_test().await;
    let result = state.financials.sla_engine.create_accounting_method(
        org_id(), "BAD", "Bad", None,
...
async fn test_get_accounting_method() {
    let state = setup_sla_test().await;
    state.financials.sla_engine.create_accounting_method(
        org_id(), "GET_TEST", "Get Test", None,
        "receivables", "receipt", None,
        None, None, None, None, None, None, None, None, None, None,
    ).await.unwrap();
    let fetched = state.financials.sla_engine.get_accounting_method(org_id(), "GET_TEST").await.unwrap().unwrap();
...
async fn test_get_nonexistent_method() {
    let state = setup_sla_test().await;
    let result = state.financials.sla_engine.get_accounting_method(org_id(), "NO_SUCH").await.unwrap();
...
async fn test_list_accounting_methods() {
    ...
    let methods = state.financials.sla_engine.list_accounting_methods(org_id(), None).await.unwrap();
...
async fn test_list_methods_by_application() {
    ...
    let payables = state.financials.sla_engine.list_accounting_methods(org_id(), Some("payables")).await.unwrap();
...
async fn test_delete_accounting_method() {
    ...
    state.financials.sla_engine.delete_accounting_method(org_id(), "DEL_ME").await.unwrap();
    let result = state.financials.sla_engine.get_accounting_method(org_id(), "DEL_ME").await.unwrap();
    assert!(result.is_none());
}
...
async fn test_delete_nonexistent_method() {
    let state = setup_sla_test().await;
    let result = state.financials.sla_engine.delete_accounting_method(org_id(), "NO_SUCH").await;
    assert!(result.is_err());
}
...
async fn test_create_constant_derivation_rule() {
    ...
    let rule = state.financials.sla_engine.create_derivation_rule(
...
async fn test_constant_rule_requires_fixed_account() {
    ...
    let result = state.financials.sla_engine.create_derivation_rule(
...
async fn test_list_derivation_rules() {
    ...
    state.financials.sla_engine.create_derivation_rule(
    ...
    let rules = state.financials.sla_engine.list_derivation_rules(org_id(), method.id).await.unwrap();
    assert!(rules.len() >= 2);
}
...
async fn test_delete_derivation_rule() {
    ...
    state.financials.sla_engine.delete_derivation_rule(org_id(), method.id, "DEL_RULE").await.unwrap();
}
...
async fn test_create_journal_entry() {
    let state = setup_sla_test().await;
    let entry = state.financials.sla_engine.create_journal_entry(
...
async fn test_get_journal_entry() {
    ...
    let fetched = state.financials.sla_engine.get_journal_entry(entry.id).await.unwrap().unwrap();
...
async fn test_list_journal_entries() {
    let state = setup_sla_test().await;
    state.financials.sla_engine.create_journal_entry(
        org_id(), "payables", "invoice",
        Uuid::new_v4(), None, None,
        None, None,
        NaiveDate::from_ymd_opt(2026, 4, 1).unwrap(), None,
    ...
    let entries = state.financials.sla_engine.list_journal_entries(org_id(), None, None, None, None, None).await.unwrap();
...
async fn test_list_entries_by_status() {
    ...
    let drafts = state.financials.sla_engine.list_journal_entries(org_id(), Some("draft"), None, None, None, None).await.unwrap();
    assert!(drafts.len() >= 1);
...
async fn test_list_entries_invalid_status() {
    let state = setup_sla_test().await;
    let result = state.financials.sla_engine.list_journal_entries(org_id(), Some("invalid_status"), None, None, None, None).await;
    assert!(result.is_err());
}
...
async fn test_add_debit_and_credit_lines() {
    ...
    // Add debit
    let debit = state.financials.sla_engine.add_journal_line(
    ...
    // Add credit
    let credit = state.financials.sla_engine.add_journal_line(
    ...
    let updated = state.financials.sla_engine.get_journal_entry(entry.id).await.unwrap().unwrap();
...
async fn test_unbalanced_entry() {
    ...
    state.financials.sla_engine.add_journal_line(
...
async fn test_add_line_invalid_type_rejected() {
    ...
    let result = state.financials.sla_engine.add_journal_line(
...
async fn test_list_journal_lines() {
    ...
    let lines = state.financials.sla_engine.list_journal_lines(entry.id).await.unwrap();
...
async fn create_balanced_entry(state: &Arc<atlas_gateway::AppState>, amount: &str) -> atlas_shared::SubledgerJournalEntry {
    ...
    state.financials.sla_engine.get_journal_entry(entry.id).await.unwrap().unwrap()
}
...
async fn test_account_unbalanced_entry_rejected() {
    ...
    let result = state.financials.sla_engine.account_entry(entry.id, Some(user_id())).await;
    assert!(result.is_err());
}
...
async fn test_full_entry_lifecycle() {
    ...
    let accounted = state.financials.sla_engine.account_entry(entry.id, Some(user_id())).await.unwrap();
    assert_eq!(accounted.status, "accounted");
    // Post (accounted → posted)
    let posted = state.financials.sla_engine.post_entry(entry.id, Some(user_id())).await.unwrap();
...
async fn test_cannot_post_draft_entry() {
    ...
    let result = state.financials.sla_engine.post_entry(entry.id, Some(user_id())).await;
    assert!(result.is_err());
}
...
async fn test_cannot_account_already_accounted() {
    let state = setup_sla_test().await;
    let entry = create_balanced_entry(&state, "500.00").await;
    state.financials.sla_engine.account_entry(entry.id, Some(user_id())).await.unwrap();
    let result = state.financials.sla_engine.account_entry(entry.id, Some(user_id())).await;
    assert!(result.is_err());
}
...
async fn test_reverse_posted_entry() {
    ...
    state.financials.sla_engine.account_entry(entry.id, Some(user_id())).await.unwrap();
    state.financials.sla_engine.post_entry(entry.id, Some(user_id())).await.unwrap();
    let reversed = state.financials.sla_engine.reverse_entry(entry.id, "Incorrect posting", Some(user_id())).await.unwrap();
...
async fn test_reverse_requires_reason() {
    ...
    state.financials.sla_engine.account_entry(entry.id, Some(user_id())).await.unwrap();
    state.financials.sla_engine.post_entry(entry.id, Some(user_id())).await.unwrap();
    let result = state.financials.sla_engine.reverse_entry(entry.id, "", Some(user_id())).await;
    assert!(result.is_err());
}
...
async fn test_cannot_reverse_draft_entry() {
    let state = setup_sla_test().await;
    let entry = create_balanced_entry(&state, "100.00").await;
    let result = state.financials.sla_engine.reverse_entry(entry.id, "Should fail", Some(user_id())).await;
    assert!(result.is_err());
}
...
async fn test_transfer_to_gl() {
    ...
    state.financials.sla_engine.account_entry(entry.id, Some(user_id())).await.unwrap();
    state.financials.sla_engine.post_entry(entry.id, Some(user_id())).await.unwrap();
    let transfer = state.financials.sla_engine.transfer_to_gl(
...
async fn test_transfer_with_no_posted_entries() {
    let state = setup_sla_test().await;
    let result = state.financials.sla_engine.transfer_to_gl(org_id(), None, None, Some(user_id())).await;
    assert!(result.is_err());
}
...
async fn test_get_transfer_log() {
    ...
    let log = state.financials.sla_engine.get_transfer_log(transfer.id).await.unwrap().unwrap();
...
async fn test_list_transfer_logs() {
    let state = setup_sla_test().await;
    let _logs = state.financials.sla_engine.list_transfer_logs(org_id(), None).await.unwrap();
...
async fn test_auto_generate_lines() {
    ...
    let lines = state.financials.sla_engine.generate_journal_lines(
...
async fn test_list_sla_events() {
    let state = setup_sla_test().await;
    let _events = state.financials.sla_engine.list_sla_events(org_id(), None, None).await.unwrap();
    // Just verify no error — the call succeeded
}
...
async fn test_sla_dashboard() {
    ...
    let dashboard = state.financials.sla_engine.get_dashboard_summary(org_id()).await.unwrap();
...
async fn test_resolve_account_code_constant() {
    ...
    let rules = state.financials.sla_engine.list_active_derivation_rules(org_id(), method.id, "debit").await.unwrap();
    let result = state.financials.sla_engine.resolve_account_code(&rules, "debit", &serde_json::json!({"category": "Travel"}));
...
async fn test_resolve_account_code_lookup() {
    ...
    let rules = state.financials.sla_engine.list_active_derivation_rules(org_id(), method.id, "debit").await.unwrap();
    let result = state.financials.sla_engine.resolve_account_code(&rules, "debit", &serde_json::json!({"expense_category": "Travel"}));
    assert_eq!(result, Some("6100".to_string()));
    let result = state.financials.sla_engine.resolve_account_code(&rules, "debit", &serde_json::json!({"expense_category": "Meals"}));
    assert_eq!(result, Some("6200".to_string()));
    let result = state.financials.sla_engine.resolve_account_code(&rules, "debit", &serde_json::json!({"expense_category": "Unknown"}));
    assert!(result.is_none());
}
...
async fn test_resolve_wrong_line_type_returns_none() {
    ...
    let rules = state.financials.sla_engine.list_active_derivation_rules(org_id(), method.id, "debit").await.unwrap();
    let result = state.financials.sla_engine.resolve_account_code(&rules, "credit", &serde_json::json!({}));
    assert!(result.is_none());
}
