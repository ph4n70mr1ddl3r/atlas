...
async fn test_create_planning_scenario() {
    ...
    assert!(scenario["scenarioNumber"]
        .as_str()
        .unwrap()
        .starts_with("SCP-"));
}
...
async fn test_list_scenarios() {
    ...
    assert!(
        arr.len() >= 2,
        "Expected at least 2 scenarios, got {}",
        arr.len()
    );
}
    ...
    app.clone()
        .oneshot(
            Request::builder()
                ...
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({"name": "Draft One"})).unwrap(),
                ))
                .unwrap(),
        )
        .await
...
async fn test_list_parameters() {
    ...
    .unwrap();
    assert!(
        list.as_array().unwrap().len() >= 3,
        "Expected at least 3 params"
    );
    // Clean up for parallel test safety
    for p in list.as_array().unwrap() {
        if let Some(item_id) = p["itemId"].as_str() {
            sqlx::query("DELETE FROM _atlas.planning_parameters WHERE item_id = $1")
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({"name": "SD Test"})).unwrap(),
                ))
                .unwrap(),
        )
        .await
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(&format!(
                    "/api/v1/scp/scenarios/{}/supply-demand",
                    scenario_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    for s in list.as_array().unwrap() {
        if let Some(id) = s["id"].as_str() {
            ...
            sqlx::query("DELETE FROM _atlas.planning_exceptions WHERE scenario_id = $1")
                ...
                .await
                .ok();
            sqlx::query("DELETE FROM _atlas.supply_demand_entries WHERE scenario_id = $1")
                .bind(uuid::Uuid::parse_str(id).unwrap())
                .execute(&_state.db_pool)
                .await
                .ok();
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({"name": "SD Filter Test"})).unwrap(),
                ))
                .unwrap(),
        )
        .await
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(&format!(
                    "/api/v1/scp/scenarios/{}/supply-demand?entry_type=supply",
                    scenario_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_full_mrp_flow_with_shortage() {
    ...
    let shortage: Vec<&serde_json::Value> = ex_arr
        .iter()
        .filter(|e| e["exceptionType"] == "shortage")
        .collect();
...
async fn test_mrp_with_no_shortage() {
    ...
    let excess: Vec<&serde_json::Value> = exceptions
        .iter()
        .filter(|e| e["exceptionType"] == "excess_supply")
        .collect();
...
async fn test_firm_and_cancel_planned_order() {
    ...
    .unwrap();
    assert_eq!(
        firmed["status"], "firmed",
        "Expected firmed status, got: {:?}",
        firmed
    );
    ...
    assert!(
        r.status() == StatusCode::CONFLICT || r.status() == StatusCode::BAD_REQUEST,
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(&format!(
                    "/api/v1/scp/scenarios/{}/orders?status=unfirm",
                    scenario_id
                ))
                .header(&k, &v)
                .body(Body::empty())
...
async fn test_resolve_exception() {
    ...
    assert_eq!(
        ...
        "Ordered from alternate supplier"
    );
}
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(&format!(
                    "/api/v1/scp/scenarios/{}/exceptions?severity=critical",
                    scenario_id
                ))
                .header(&k, &v)
                .body(Body::empty())
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(&k, &v)
                .body(Body::from(
                    serde_json::to_string(&json!({"name": "Already Done"})).unwrap(),
                ))
                .unwrap(),
        )
        .await
