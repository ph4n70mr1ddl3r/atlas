...
async fn test_list_jurisdictions() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .uri("/api/v1/tax/jurisdictions?regime_code=EU_VAT")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_create_tax_rate() {
    ...
    let rate_str = if rate_pct.is_empty() {
        rate["rate_percentage"].to_string()
    } else {
        rate_pct.to_string()
    };
...
async fn test_calculate_tax_inclusive() {
    ...
    let taxable: f64 = result["total_taxable_amount"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    let tax: f64 = result["total_tax_amount"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    assert!(
        (taxable - 1000.0).abs() < 1.0,
        "Expected taxable ~1000, got {}",
        taxable
    );
...
async fn test_calculate_tax_multiple_lines() {
    ...
    let total_tax: f64 = result["total_tax_amount"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    assert!(
        (total_tax - 225.0).abs() < 1.0,
        "Expected total tax ~225, got {}",
        total_tax
    );
    // Total taxable: 1000 + 500 = 1500
    let total_taxable: f64 = result["total_taxable_amount"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    assert!(
        (total_taxable - 1500.0).abs() < 1.0,
        "Expected total taxable ~1500, got {}",
        total_taxable
    );
    cleanup_test_db(&state.db_pool).await;
}
...
async fn test_calculate_tax_with_recovery() {
    ...
    let recoverable: f64 = line["recoverable_amount"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    let non_recoverable: f64 = line["non_recoverable_amount"]
        .as_str()
        .unwrap()
        .parse()
        .unwrap();
    assert!(
        (recoverable - 190.0).abs() < 1.0,
        "Expected recoverable ~190, got {}",
        recoverable
    );
    assert!(
        (non_recoverable - 0.0).abs() < 0.01,
        "Expected non-recoverable ~0, got {}",
        non_recoverable
    );
    cleanup_test_db(&state.db_pool).await;
}
...
async fn test_list_determination_rules() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .uri("/api/v1/tax/rules/US_SALES_TAX")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_calculate_tax_with_persist() {
    ...
    let b = axum::body::to_bytes(r2.into_body(), usize::MAX)
        .await
        .unwrap();
...
async fn test_list_tax_reports() {
    ...
    let r = app
        ...
        .oneshot(
            Request::builder()
                .uri("/api/v1/tax/reports?regime_code=EU_VAT")
                .header(&k, &v)
                .body(Body::empty())
                .unwrap(),
        )
